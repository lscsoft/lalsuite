...
static void NRSur7dq2_core(
  ...
  (void) pthread_once(&NRSur7dq2_is_initialized, NRSur7dq2_Init_LALDATA);
    ...
    int ell, m, ell_max, m_loop_min;
    ell_max = NRSUR7DQ2_LMAX;
    m_loop_min = 1;
    if (quadrupole_only) {
        // Restrict modes over which we sum such that we only generate the (2, 2) and (2, -2) modes
        ell_max = 2;
        m_loop_min = 2;
    }
 ...
 * Note: This implementation for the lalinference_o2 branch, compared to the master implementation,
 * adds pi/2 to phiRef, evaluates the master implementation, then multiplies the polarizations by
 * -1. This convention was found to be used by SEOBNRv3 as well; using phiRef in master and
 * phiRef - pi/2 in lalinference_o2 as input parameters leads to essentially identical waveforms
...
int XLALSimInspiralNRSur7dq2Polarizations(
    ...
    // lalinference_o2 version: add pi/2 to phiRef
    phiRef += 0.5 * LAL_PI;
     ...
     * by phiRef. Note that we've used the convention that master == lalinference_o2 except for a
     * change in phiRef of pi/2 and a minus sign in the polarizations, which is the convention of
     * SEOBNRv3. */
    ...
    NRSur7dq2_core(&h_inertial_modes, q, chiA0, chiB0, omegaRef_dimless, phiRef, q_ref, quadrupole_only);
    if (!h_inertial_modes) {
        return XLAL_FAILURE;
    }
    ...
    for (ell=2; ell <= ell_max; ell++) {
        for (m=-ell; m<=ell; m++) {
            ...
            // lalinference_o2 difference: minus sign in polarizations
            c_re = -1 * creal(swsh_coef);
            c_im = -1 * cimag(swsh_coef);
...
SphHarmTimeSeries *XLALSimInspiralNRSur7dq2Modes(
    ...
    NRSur7dq2_core(&h_inertial, q, chiA0, chiB0, omegaRef_dimless, phiRef, q_ref, false);
    if (!h_inertial) {
