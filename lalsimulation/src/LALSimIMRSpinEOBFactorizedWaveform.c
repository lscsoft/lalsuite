...
#if UsePrecFunctions
static INT4 XLALSimIMRSpinEOBGetPrecSpinFactorizedWaveform (COMPLEX16 *
							    restrict hlm,
							    REAL8Vector *
							    restrict values,
							    const REAL8 v,
...
UNUSED static int
XLALSimIMREOBCalcPrecNoSpinFacWaveformCoefficients (FacWaveformCoeffs *
						    const coeffs,
						    const REAL8 m1,
						    const REAL8 m2,
						    const REAL8 eta,
...
UNUSED static int
XLALSimIMREOBCalcPrecSpinFacWaveformCoefficients (FacWaveformCoeffs *
						  const coeffs,
						  const REAL8 m1,
						  const REAL8 m2,
						  const REAL8 eta,
...
#if UsePrecFunctions
static INT4
XLALSimIMRSpinEOBGetPrecSpinFactorizedWaveform (COMPLEX16 * restrict hlm,
  ...
  )
{
  ...
  s1dotL =
    spin1[0] * rcrossp[0] + spin1[1] * rcrossp[1] + spin1[2] * rcrossp[2];
  s2dotL =
    spin2[0] * rcrossp[0] + spin2[1] * rcrossp[1] + spin2[2] * rcrossp[2];
  ...
  status =
    XLALSimIMREOBCalcPrecSpinFacWaveformCoefficients (hCoeffs, m1, m2, eta,
						      tplspin, chiS, chiA, 2);
  ...
  switch (l)
	...
	case 2:
	  deltalm = vh3 * (hCoeffs->delta22vh3
			   +
			   vh3 *
			   ((hCoeffs->delta22vh6 + hCoeffs->delta22vh6S) +
			    vh * vh * (hCoeffs->delta22vh9 * vh))) +
	    hCoeffs->delta22v5 * v * v2 * v2 + (hCoeffs->delta22v6 +
						hCoeffs->delta22v6S) * v2 *
	    v2 * v2 + (hCoeffs->delta22v8 +
		       hCoeffs->delta22v8S) * v2 * v2 * v2 * v2;
	  rholm =
			    ...
			    v * ((hCoeffs->rho22v4 + hCoeffs->rho22v4S) +
				 v * ((hCoeffs->rho22v5 + hCoeffs->rho22v5S) +
					...
					((hCoeffs->rho22v8 +
					 ...
					 (hCoeffs->rho22v10 +
					  hCoeffs->rho22v10l * eulerlogxabs) *
					 v2)))))));
	  break;
	case 1:
	  {
	    deltalm = vh3 * (hCoeffs->delta21vh3
			     +
			     vh3 *
			     ((hCoeffs->delta21vh6 + hCoeffs->delta21vh6S) +
			      vh *
			      ((hCoeffs->delta21vh7 + hCoeffs->delta21vh7S) +
	      ...
	      1. + v * (hCoeffs->rho21v1 +
			v * ((hCoeffs->rho21v2 + hCoeffs->rho21v2S) +
			     v * ((hCoeffs->rho21v3 + hCoeffs->rho21v3S) +
				  v *
				  ((hCoeffs->rho21v4 + hCoeffs->rho21v4S) +
				   v *
				   ((hCoeffs->rho21v5 + hCoeffs->rho21v5S) +
				    v *
				    ((hCoeffs->rho21v6 + hCoeffs->rho21v6S) +
				     hCoeffs->rho21v6l * eulerlogxabs +
				     v *
				     ((hCoeffs->rho21v7 + hCoeffs->rho21v7S) +
				      (hCoeffs->rho21v7l +
				       hCoeffs->rho21v7lS) * eulerlogxabs +
				      v * (hCoeffs->rho21v8 +
					   hCoeffs->rho21v8l * eulerlogxabs +
					   (hCoeffs->rho21v10 +
					    hCoeffs->rho21v10l *
					    eulerlogxabs) * v2))))))));
	...
	case 3:
	  deltalm = vh3 * (hCoeffs->delta33vh3
			   +
			   vh3 *
			   ((hCoeffs->delta33vh6 + hCoeffs->delta33vh6S) +
	...
	case 2:
	  deltalm = vh3 * (hCoeffs->delta32vh3
			   +
			   vh *
			   ((hCoeffs->delta32vh4 + hCoeffs->delta32vh4S) +
			    vh * vh *
			    ((hCoeffs->delta32vh6 + hCoeffs->delta32vh6S) +
			     hCoeffs->delta32vh9 * vh3)));
	  rholm =
	    1. + v * ((hCoeffs->rho32v + hCoeffs->rho32vS) +
		      v * ((hCoeffs->rho32v2 + hCoeffs->rho32v2S) +
			   v * ((hCoeffs->rho32v3 + hCoeffs->rho32v3S) +
				v * ((hCoeffs->rho32v4 + hCoeffs->rho32v4S) +
				     v *
				     ((hCoeffs->rho32v5 + hCoeffs->rho32v5S) +
				      v *
				      ((hCoeffs->rho32v6 +
					hCoeffs->rho32v6S) +
	...
	case 1:
	  deltalm = vh3 * (hCoeffs->delta31vh3
			   +
			   vh3 *
			   ((hCoeffs->delta31vh6 + hCoeffs->delta31vh6S) +
			    vh *
			    ((hCoeffs->delta31vh7 + hCoeffs->delta31vh7S) +
	...
	case 3:
	  deltalm = vh3 * (hCoeffs->delta43vh3
			   +
			   vh *
			   ((hCoeffs->delta43vh4 + hCoeffs->delta43vh4S) +
	...
	case 2:
	  deltalm = vh3 * (hCoeffs->delta42vh3
			   + (hCoeffs->delta42vh6 +
			      hCoeffs->delta42vh6S) * vh3);
	  rholm =
	    1. + v2 * (hCoeffs->rho42v2 +
	...
	case 1:
	  deltalm = vh3 * (hCoeffs->delta41vh3
			   +
			   vh *
			   ((hCoeffs->delta41vh4 + hCoeffs->delta41vh4S) +
	...
	case 4:
	  deltalm = vh3 * (hCoeffs->delta54vh3
			   + (hCoeffs->delta54vh4 +
			      hCoeffs->delta54vh4S) * vh);
	  rholm =
	    1. + v2 * (hCoeffs->rho54v2 +
	...
	case 3:
	  ...
	  rholm = 1. + v2 * (hCoeffs->rho53v2
			     + v * ((hCoeffs->rho53v3 + hCoeffs->rho53v3S)
				    +
				    v *
				    ((hCoeffs->rho53v4 + hCoeffs->rho53v4S) +
				     (hCoeffs->rho53v5 +
				      hCoeffs->rho53v5S) * v)));
	  break;
	case 2:
	  deltalm = vh3 * (hCoeffs->delta52vh3
			   + (hCoeffs->delta52vh4 +
			      hCoeffs->delta52vh4S) * vh);
	  rholm =
	    1. + v2 * (hCoeffs->rho52v2 +
	...
	case 1:
	  ...
	  rholm = 1. + v2 * (hCoeffs->rho51v2
			     + v * ((hCoeffs->rho51v3 + hCoeffs->rho51v3S)
				    +
				    v *
				    ((hCoeffs->rho51v4 + hCoeffs->rho51v4S) +
				     (hCoeffs->rho51v5 +
				      hCoeffs->rho51v5S) * v)));
	  break;
	default:
...
UNUSED static int XLALSimIMREOBCalcPrecNoSpinFacWaveformCoefficients (FacWaveformCoeffs * const coeffs,
					    /**< OUTPUT, pre-computed waveform coefficients */
								      const REAL8 m1,
					    /**< mass 1 */
								      const REAL8 m2,
  ...
  )
{
  ...
  if (SpinAlignedEOBversion == 4)
    {
      coeffs->delta22vh6 =
	(428. * LAL_PI) / 105. - 4. / 3. * (dM * chiA + chiS * (1 - 2 * eta));
    }
  ...
  if (dM2)
	...
	default:
      ...
      /*coeffs->rho21v3   = (-567.*chiA*chiA*chiA - 1701.*chiA*chiA*chiS*dM
         ...
         + chiS* dM3 *(4708. - 567.*chiS*chiS
         + 1816.*eta))/(2688.*dM3); */
      coeffs->rho21v4 = -47009. / 56448.
				       /*- (865.*a2)/1792. - (405.*a2*a2)/2048.*/
	- (10993. * eta) / 14112. + (617. * eta2) / 4704.;
  ...
  coeffs->rho44v4 = /*a2/2. + */ (-511573572.
				  + 2338945704. * eta - 313857376. * eta2 -
				  6733146000. * eta3 +
				  1252563795. * eta2 * eta2) / (317116800. *
								m1Plus3eta2);
  ...
  coeffs->rho42v4 =
	       /*a2/2. + */
	       (-114859044. + 295834536. * eta + 1204388696. * eta2 -
		3047981160. * eta3
		- 379526805. * eta2 * eta2) / (317116800. * m1Plus3eta2);
...
UNUSED static int XLALSimIMREOBCalcPrecSpinFacWaveformCoefficients (FacWaveformCoeffs * const coeffs,
					    /**< OUTPUT, pre-computed waveform coefficients */
								    const REAL8 m1,
					    /**< mass 1 */
								    const REAL8 m2,
  ...
  )
{
  ...
  switch (SpinAlignedEOBversion)
    ...
    case 1:
      ...
      break;
    case 2:
      coeffs->rho22v8S = /*-387216563023./160190110080. +*/ (18353. * a2) /
	21168. - a2 * a2 / 8.;
      break;
    }
  //coeffs->rho22v8l  =  9202./2205.;
  ...
  if (dM2)
	...
	(-98635. * a) / 75264. + (2031. * a * a2) / 7168. -
	(1701. * a2 * a3) / 8192.;
      coeffs->rho21v6S = /*7613184941./2607897600.;+ */ (9032393. * a2) /
	1806336. + (3897. * a2 * a2) / 16384. - (15309. * a3 * a3) / 65536.;
      coeffs->rho21v6l = -107. / 105.;
      coeffs->rho21v7S =
	(-3859374457. * a) / 1159065600. - (55169. * a3) / 16384. +
	(18603. * a2 * a3) / 65536. - (72171. * a2 * a2 * a3) / 262144.;
  ...
  coeffs->rho44v6S = /*16600939332793./1098809712000.; + */ (217. * a2) /
    3960.;
  //coeffs->rho44v6l = - 12568./3465.;
