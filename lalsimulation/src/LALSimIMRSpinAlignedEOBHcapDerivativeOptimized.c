...
#ifndef LALSIMIMRSPINALIGNEDEOBHCAPDERIVATIVEOPTIMIZED_C
                  ...
                  )
{
 ...
 /* OPTIMIZATION NOTE:
  * The original function computed derivatives of the Hamiltonian with respect to 6 variables (three coordinate & three momenta).
  * In SEOBNRv2, only three are nonzero! */
  GSLSpinAlignedHamiltonianWrapper_derivs_allatonce( tmpDValues, cartValues, &params );
  ...
  H =  XLALSimIMRSpinEOBHamiltonianOptimized( eta, &rVec, &pVec, s1Vec, s2Vec, sKerr, sStar, params.params->tortoise, params.params->seobCoeffs );
  ...
  /* OPTIMIZATION NOTE:
   *   Why are we multiplying and then dividing by the total mass two lines later? */
  H = H * (mass1 + mass2);
  ...
  flux  = XLALInspiralSpinFactorizedFluxOptimized( &polarDynamics, nqcCoeffs, omega, params.params, H/(mass1+mass2), lMax, SpinAlignedEOBversion );
...
#endif /* LALSIMIMRSPINALIGNEDEOBHCAPDERIVATIVEOPTIMIZED_C */
