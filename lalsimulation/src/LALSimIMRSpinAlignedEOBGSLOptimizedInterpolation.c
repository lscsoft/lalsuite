...
static size_t
optimized_gsl_interp_bsearch (const double x_array[], double x,
...
optimized_gsl_interp_accel_find (gsl_interp_accel * a, const double xa[],
				 size_t len, double x)
{
  size_t x_index = a->cache;

  if (x < xa[x_index])
    {				//OPTIMIZED: This shouldn't occur often, if at all
      a->miss_count++;
	  ...
	  if (x >= xa[x_index + 2])	//OPTIMIZED: Check the next one. It'll probably be there.
	    {
	      a->miss_count++;
	      a->cache =
		optimized_gsl_interp_bsearch (xa, x, x_index, len - 1);
	    }
	  else
	    {
	      a->hit_count++;
...
static inline void
optimized_coeff_calc (const double c_array[], double dy, double dx,
...
static int
optimized_cspline_eval (const void *vstate, const double x_array[],
			const double y_array[], size_t size, double x,
			gsl_interp_accel * a, double *y,
			unsigned int *index_old, double *x_lo_old,
			double *y_lo_old, double *b_i_old, double *c_i_old,
			double *d_i_old)
{
  ...
  if (index == *index_old && (*index_old) > 0)
    {
      ...
      *y =
	*y_lo_old + delx * (*b_i_old + delx * (*c_i_old + delx * *d_i_old));
    }
  else
    {
      /* evaluate */
...
static int
optimized_gsl_spline_eval_e (const gsl_spline * spline, double interptime,
			     gsl_interp_accel * accel, double *output,
			     unsigned int *index_old, double *x_lo_old,
			     double *y_lo_old, double *b_i_old,
			     double *c_i_old, double *d_i_old)
{
  return optimized_cspline_eval (spline->interp->state, spline->x, spline->y,
				 spline->interp->size, interptime, accel,
				 output, index_old, x_lo_old, y_lo_old,
