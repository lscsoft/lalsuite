...
UNUSED static int SEOBNRv2OptimizedInterpolatorNoAmpPhase(REAL8Array *yin, REAL8 tinit, REAL8 deltat, UINT4 num_input_times, REAL8Array ** yout)
{
    int errnum = 0;
    ...
    /* interpolate! */
    for (unsigned int i = 1; i <= dim; i++) { /* only up to dim (4) because we are not interpolating amplitude and phase */
...
UNUSED static int SEOBNRv2OptimizedInterpolatorIncludeAmpPhase(REAL8Array *yin, REAL8 tinit, REAL8 deltat, UINT4 num_input_times, REAL8Array ** yout) // Davids: maybe this function can be removed since it is split into noampphase and onlyampphase
{
    int errnum = 0;
...
UNUSED static int SEOBNRv2OptimizedInterpolatorOnlyAmpPhase(REAL8Array *yin, REAL8 tinit, REAL8 deltat, UINT4 num_input_times, REAL8Array ** yout)
{
    int errnum = 0;
    ...
    /* interpolate! */
    for (unsigned int i = dim+1; i <= dim+2; i++) { /* Original (dim), Optimized (dim+2), since we're also interpolating amp & phase */
     //gsl_spline_init(interp, &yin->data[0], &yin->data[num_input_times * i], num_input_times + 1);
     gsl_spline_init(interp, &yin->data[0], &yin->data[num_input_times * i], num_input_times);
      vector = output->data + outputlen * i;
      unsigned int index_old=0;
