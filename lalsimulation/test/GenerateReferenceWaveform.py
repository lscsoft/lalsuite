...
parameterInfo = [
    [
        ...
        "string",
        "TD",
        "Generate either a time (TD) or " + "frequency (FD) domain waveform" + defstr,
        "DOM",
    ],
    ...
    [
        ...
        "int",
        -1,
        "Twice PN order of phase, [default -1 " + "for highest available order]",
        "ORD",
    ],
    [
        "ampOrder",
        "int",
        -1,
        "Twice PN order of amplitude, [default -1 " + "for highest available order]",
        "ORD",
    ],
    [
        "sampleRate",
        "float",
        16384.0,
        "Sample rate in Hz for TD waveforms. "
    ...
    [
        ...
        "float",
        1.0 / 16384.0,
        "Time steps in seconds for TD "
        + "waveforms. (Will be overwritten if SRATE is defined.)"
        + defstr,
        "DELTAT",
    ],
    [
        ...
        "float",
        1.0 / 8.0,
        "Frequency bin size in Hz for FD waveforms" + defstr,
        "DF",
    ],
    ["m1", "float", 10.0, "Mass of first object in solar masses" + defstr, "M1"],
    ["m2", "float", 1.4, "Mass of second object in solar masses" + defstr, "M2"],
    ["distance", "float", 100.0, "Distance in Mpc" + defstr, "DIST"],
    [
        "inclination",
        "float",
        lal.PI / 3.0,
        "Inclination of binary to " + "line of sight [default: PI/3]",
        "INCL",
    ],
    [
        "spin1x",
        "float",
        0.0,
        "Vector components for spin of mass1 " + "(default all 0)",
        "S1X",
    ],
    [
        "spin1y",
        "float",
        0.0,
        "z-axis=line of sight, L in x-z plane at " + "reference",
        "S1Y",
    ],
    ["spin1z", "float", 0.0, "Kerr limit: S1X^2 + S1Y^2 + S1Z^2 <= 1", "S1Z"],
    [
        "spin2x",
        "float",
        0.0,
        "Vector components for spin of mass2 " + "(default all 0)",
        "S2X",
    ],
    [
        "spin2y",
    ...
    ["spin2z", "float", 0.0, "Kerr limit: S2X^2 + S2Y^2 + S2Z^2 <= 1", "S2Z"],
    [
        "lambda1",
        "float",
        0.0,
        "(tidal deformability of mass 1) / "
    ...
    [
        ...
        "float",
        0.0,
        "(tidal deformability of mass 2) / "
        + "(mass of body 2)^5 Reasonable values (~128-2560 for NS, 0 for BH)"
        + defstr,
        "L2",
    ],
    ["f_min", "float", 40.0, "Lowest GW frequency in Hz" + defstr, "FMIN"],
    [
        "f_max",
        "float",
        0.0,
        "Highest GW frequency in Hz - only used by FD "
        + "waveforms [default 0 (as high as possible)]",
        "FMAX",
    ],
    [
        "f_ref",
        "float",
        0.0,
        "Reference GW frequency in Hz at which phase, "
        + "binary orientation are defined [default 0 (coalescence)]",
        "FREF",
    ],
    ["phiref", "float", 1.2, "Orbital phase at FREF" + defstr, "PHIREF"],
    ["longAscNodes", "float", 0.0, "Longitude of ascending nodes", "LONGASCNODES"],
    ["eccentricity", "float", 0.0, "eccentricity", "ECCENTRICITY"],
    ["meanPerAno", "float", 0.0, "Mean periastron anomaly", "MEANPERANO"],
    ["eps", "float", 1.0e-2, "Tolerance check", "TOLERANCE"],
]
...
parameterDicts = [
    {"name": p[0], "type": p[1], "default": p[2], "help": p[3], "metavar": p[4]}
...
parser.add_option(
    ...
    default=None,
    dest="infile",
    help="Specify waveform parameters in FILE; if "
...
group = OptionGroup(
    parser,
    "Approximant details",
    "Options that can be " + "specified in input file under section [approximant]",
)
...
group = OptionGroup(
    parser,
    "Waveform parameters",
    "Options that can be " + "specified in input file under section [parameters]",
)
for p in parameterDicts[2:]:
    group.add_option(
        "--" + p["name"],
...
if (
    opts.sampleRate != defaultDict["sampleRate"]
    and opts.deltaT != defaultDict["deltaT"]
):
    raise ValueError(
        "Specified both --sampleRate and --deltaT. " + "Provide one or the other."
    )
...
class InputFile:
    ...
    def __init__(self, filename):
        ...
        self.dataset = [
            configparser.RawConfigParser(defaults=defaultDict)
...
def updateOptions(config):
    ...
    pardict1 = {
        p["name"]: typeget[p["type"]]("approximant", p["name"])
        for p in parameterDicts[:2]
    }
    pardict2 = {
        p["name"]: typeget[p["type"]]("parameters", p["name"])
        for p in parameterDicts[2:]
    }
...
def generateWfparameters(opts):
    ...
    inputpar["distance"] *= 1.0e6 * lal.PC_SI
    inputpar["approximant"] = lalsim.GetApproximantFromString(inputpar["approximant"])

    if inputpar["sampleRate"] != defSampleRate:
        inputpar["deltaT"] = 1.0 / inputpar["sampleRate"]
...
def writeData(filename, polarisations, optsDict):
    ...
    outputpars = set(paramnames[optsDict["domain"]]).intersection(set(optsDict.keys()))
