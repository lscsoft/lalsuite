...
#####################################################################
pars_dict_td = {
    "mass1": 20.0 * u.solMass,
    "mass2": 30.0 * u.solMass,
...
def gen_td_waveform():
    ...
    s1y = 0.0 * u.dimensionless_unscaled
    s1z = 0.0 * u.dimensionless_unscaled
    s2x = 0.0 * u.dimensionless_unscaled
    s2y = 0.0 * u.dimensionless_unscaled
    s2z = 0.0 * u.dimensionless_unscaled
    deltaT = 1.0 / 1024.0 * u.s
    f_min = 20.0 * u.Hz
    f_ref = 20.0 * u.Hz
    distance = 1.0 * u.Mpc / (lal.PC_SI * 1e6)
...
def get_amp_phase_sums(hp, hc, hlm):
    ...
    hp_mode, hc_mode = hlm(np.pi / 3 * u.rad, 0.0 * u.rad)
    ...
    if all(
        isinstance(i, u.Quantity)
        for i in [amp_sum, phase_sum, amp_sum_mode, phase_sum_mode]
    ):
...
def test_gwsignal():
    ...
    expected_time_domain_results = np.array(
        [
            2840580.6974479025,
            -169050.53344446962,
            3878983.3547071503,
    ...
    expected_freq_domain_results = np.array(
        [155285.95927165466, 2322376.861281745, 758571.2935111821, 4117645.756563821]
    )
    ...
    np.testing.assert_allclose(
        expected_freq_domain_results,
        expected_freq_domain_results,
        rtol=1e-6,
        err_msg="GWSignal IMRPhenomXPHM Test Failed",
    )
