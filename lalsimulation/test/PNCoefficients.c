...
/* The TaylorT5 waveform is defined by
 * dw/dt = -1/ trunc[(dE/dw(v))/flux(v) ]
 * where trunc[ ] means truncate the expression at the desired PN order.
 * There's a recursive expression for dw/dt at each order. Write
 * dw/dt   = W_0/( 1 + \sum_{i=1}^n W_i v^i )
 * e(v)    = E_0 ( 1 + \sum_{i=1}^n E_i v^i )
 ...
 * Then
 *
 * W_i = -F_i + (1+i/2)E_i - \sum_{j=1}^{i-1} F_j T_{i-j}
 *
 * This doesn't handle terms involving log(v), so we'll treat them separately.
 * Also, because spin-spin PN terms are only known to leading order, we have
 * to be careful with spin terms. We first add in the spin-orbit terms, but
...
static void T5wdot_from_energy_flux(
    PNPhasingSeries *wdotm1,
    const REAL8 m1M,
    const REAL8 chi1,
    const REAL8 chi2,
    const REAL8 qm_def1,
    ...
    )
{
    ...
    memset(wdotm1, 0, sizeof(PNPhasingSeries));
    wdotm1->v[0] = -XLALSimInspiralPNFlux_0PNCoeff(eta)/(2./3.*XLALSimInspiralPNEnergy_0PNCoeff(eta));
    wdotm1->v[1] = 0.; /* there's no 0.5 PN term */
    for (int i = 2; i <8; i++)
    {
        wdotm1->v[i] = (1.+0.5*i)*energy[i] -flux[i] - sumF(flux, wdotm1->v, i);
    }
    wdotm1->vlogv[6]= -XLALSimInspiralPNFlux_6PNLogCoeff(eta);
    ...
    wdotm1->v[4] += 3.*energy_ss4 - flux_ss4;

    //need to subtract the Spin^2 terms from (1.5PN)^2, not included in the
    //wdot coefficients, in 3PN term
    wdotm1->v[6]-= (-2.5*energy_so3+flux_so3)*flux_so3;

    return;
}

static void T5wdot_from_pncoefficients(
    PNPhasingSeries *wdotm1,
    const REAL8 m1M,
    const REAL8 chi1,
    const REAL8 chi2,
    ...
    )
{
    ...
    memset(wdotm1, 0, sizeof(PNPhasingSeries));
    wdotm1->v[0] = XLALSimInspiralTaylorT4wdot_0PNCoeff(eta);
    wdotm1->v[1] = 0.;
    wdotm1->v[2] = XLALSimInspiralTaylorT5wdot_2PNCoeff(eta);
    wdotm1->v[3] = XLALSimInspiralTaylorT5wdot_3PNCoeff(eta);
    wdotm1->v[4] = XLALSimInspiralTaylorT5wdot_4PNCoeff(eta);
    wdotm1->v[5] = XLALSimInspiralTaylorT5wdot_5PNCoeff(eta);
    wdotm1->v[6] = XLALSimInspiralTaylorT5wdot_6PNCoeff(eta);
    wdotm1->vlogv[6] = XLALSimInspiralTaylorT5wdot_6PNLogCoeff(eta);
    wdotm1->v[7] = XLALSimInspiralTaylorT5wdot_7PNCoeff(eta);
    wdotm1->v[8] = 0.;

    wdotm1->v[3] += XLALSimInspiralTaylorT5wdot_3PNSOCoeff(m1M)*S1L + XLALSimInspiralTaylorT5wdot_3PNSOCoeff(m2M)*S2L;
    wdotm1->v[5] += XLALSimInspiralTaylorT5wdot_5PNSOCoeff(m1M)*S1L + XLALSimInspiralTaylorT5wdot_5PNSOCoeff(m2M)*S2L;
    wdotm1->v[6] += XLALSimInspiralTaylorT5wdot_6PNSOCoeff(m1M)*S1L + XLALSimInspiralTaylorT5wdot_6PNSOCoeff(m2M)*S2L;
    wdotm1->v[7] += XLALSimInspiralTaylorT5wdot_7PNSOCoeff(m1M)*S1L + XLALSimInspiralTaylorT5wdot_7PNSOCoeff(m2M)*S2L;

    wdotm1->v[4] += XLALSimInspiralTaylorT5wdot_4PNS1S2Coeff(eta)*S1L*S2L;
    wdotm1->v[4] += XLALSimInspiralTaylorT5wdot_4PNS1S2OCoeff(eta)*S1L*S2L;
    wdotm1->v[4] += XLALSimInspiralTaylorT5wdot_4PNSelf2SCoeff(m1M)*S1L*S1L;
    wdotm1->v[4] += XLALSimInspiralTaylorT5wdot_4PNSelf2SOCoeff(m1M)*S1L*S1L;
    wdotm1->v[4] += XLALSimInspiralTaylorT5wdot_4PNSelf2SCoeff(m2M)*S2L*S2L;
    wdotm1->v[4] += XLALSimInspiralTaylorT5wdot_4PNSelf2SOCoeff(m2M)*S2L*S2L;
    wdotm1->v[4] += qm_def1*XLALSimInspiralTaylorT5wdot_4PNQM2SCoeff(m1M)*S1L*S1L;
    wdotm1->v[4] += qm_def1*XLALSimInspiralTaylorT5wdot_4PNQM2SOCoeff(m1M)*S1L*S1L;
    wdotm1->v[4] += qm_def2*XLALSimInspiralTaylorT5wdot_4PNQM2SCoeff(m2M)*S2L*S2L;
    wdotm1->v[4] += qm_def2*XLALSimInspiralTaylorT5wdot_4PNQM2SOCoeff(m2M)*S2L*S2L;
    ...
    )
{
    ...
    fprintf(stdout, "\n=== Testing T5  eta=%.4f, chi1=%.4f, chi2=%.4f, qm1=%.4f, qm2=%.4f ===\n", eta, chi1, chi2, qm_def1, qm_def2);

    PNPhasingSeries wdotm1_ef;
    T5wdot_from_energy_flux(&wdotm1_ef, m1M, chi1, chi2, qm_def1, qm_def2);

    PNPhasingSeries wdotm1_pn;
    T5wdot_from_pncoefficients(&wdotm1_pn, m1M, chi1, chi2, qm_def1, qm_def2);

    fprintf(stdout, "Testing wdot consistency with energy and flux.\n");
    ret += compare_pnseries(&wdotm1_pn, &wdotm1_ef);
...
int main (int argc, char **argv)
{
    ...
    ret += test_consistency_T5(0.5, 0., 0., 1., 4.5);
    ret += test_consistency_T5(0.9, 0., 0., 1., 4.5);
    ret += test_consistency_T5(0.01, 0., 0., 1., 4.5);

    ret += test_consistency_T5(0.5, 1., 0., 1., 4.5);
    ret += test_consistency_T5(0.9, 1., 0., 1., 4.5);
    ret += test_consistency_T5(0.01, 1., 0., 1., 4.5);

    ret += test_consistency_T5(0.5, 0., -0.4, 1., 4.5);
    ret += test_consistency_T5(0.9, 0., -0.4, 1., 4.5);
    ret += test_consistency_T5(0.01, 0., -0.4, 1., 4.5);

    ret += test_consistency_T5(0.5, 0.9, 0.9, 0., 0.);
    ret += test_consistency_T5(0.9, 0.9, -0.9, 3., 2.);
    ret += test_consistency_T5(0.01, 0.9, 0.9, 4., 4.);
