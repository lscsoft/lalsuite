...
def check_masses(m1, m2):
    if m1 <= 0.0 or m2 <= 0.0:
        raise ValueError(
            "The input masses must both be positive, while they are m1, m2 = %e, %e kg"
            % (m1, m2)
        )
    ...
    if m1 < 0.09 * MSUN_SI or m2 < 0.09 * MSUN_SI:
        warn(
            ...
            "The masses must be input in kg." % (m1, m1 / MSUN_SI, m2, m2 / MSUN_SI),
            ValueWarning,
        )
...
def check_spin_mags(chi1, chi2):
    if chi1 < 0.0 or chi1 > 1.0 or chi2 < 0.0 or chi2 > 1.0:
        raise ValueError(
            "The magnitudes of the spins must both be between 0 and 1, while they are chi1, chi2 = %f, %f"
            % (chi1, chi2)
        )


def check_tilts(tilt1, tilt2):
    if tilt1 < 0.0 or tilt1 > np.pi or tilt2 < 0.0 or tilt2 > np.pi:
        raise ValueError(
            "The tilt angles must both be between 0 and pi, while they are tilt1, tilt2 = %f, %f"
            % (tilt1, tilt2)
        )
...
def check_fref(fref, m1, m2, evol_type):
    ...
    f_ISCO = 6.0 ** (-1.5) / (np.pi * (m1 + m2) * kg_to_s)
    ...
    if fref > f_ISCO:
        warn(
            ...
            "with the binary's total mass of %f Hz" % (evol_type, fref, f_ISCO),
            ValueWarning,
        )
...
def evolution_error_handling(
    ...
    if failure_mode == "Error":
        ...
        if hybrid_evol:
            hybrid_output_tilts = package_tilts(
                failure_output, failure_output, Lf, swap
            )
