...
def L_vec_from_masses_spins_and_f(
    m1,
    m2,
    f,
    PNorder=2,
    ...
    if f <= 0.0:
        raise ValueError("The frequency must be positive, while it is f = %f" % f)
    ...
    if PNorder >= 1:
        L += (1.5 + eta / 6.0) * v2 * zhat
    if PNorder >= 1.5:
        L += (
            -0.25 * ((1.0 + 3.0 / X1) * S1_vec + (1.0 + 3.0 / X2) * S2_vec)
            - ((1.0 + 27.0 / X1) * S1z_vec + (1.0 + 27.0 / X2) * S2z_vec) / 12.0
        ) * v3
    if PNorder >= 2:
        L += (3.375 - 2.375 * eta + eta * eta / 24.0) * v2 * v2 * zhat
    if PNorder >= 2.5:
        L += (
            (
                (-71.0 + 21.0 / X1 - 13.0 * X1 - 9.0 * X1sq) * S1_vec
                + (-71.0 + 21.0 / X2 - 13.0 * X2 - 9.0 * X2sq) * S2_vec
                + (1.0 - 147.0 / X1 - 367.0 * X1 / 3.0 + 13.0 * X1sq / 3.0) * S1z_vec
                + (1.0 - 147.0 / X2 - 367.0 * X2 / 3.0 + 13.0 * X2sq / 3.0) * S2z_vec
            )
            * v3
            * v2
            / 48.0
...
def L_from_masses_a_and_ecc(m1, m2, a, e):
    ...
    if a <= 0.0:
        raise ValueError("The semimajor axis must be positive, while it is a = %e" % a)
    ...
    return eta * (a * (1.0 - e * e) / (M * kg_to_m)) ** 0.5
...
def E_ratio_term(m):
    ...
    return (
        (3.0 * (1.0 + 3.0 * (4.0 - m) / (omm * fpm)) / (16.0 * omm**1.5) - 0.5)
        * m
        / fpm
    )
...
def eq_coeffs(u, kappaxiq, q, S1, S2, xi, S0sq):
    ...
    Upsilon = opq * (2.0 * q * Sigma + q2 * S1sq + S2sq)
    ...
    Btransf = (
        0.25 * opq * eps * eps
        + q * eps * (xi - 2.0 * opq * kappaxiq) * u
        + Upsilon * u2
    )

    Ctransf = (
        eps * (opq * (Sigma + q * kappaxiq2) - q * kappaxiq * xi)
        + 2.0 * (zeta - Upsilon * kappaxiq) * u
    )

    Dtransf = (
        opq * (Sigma * Sigma - S1sq * S2sq)
        + q2 * S1sq * xi * xi / opq
        - 2.0 * zeta * kappaxiq
        + Upsilon * kappaxiq2
    )
...
def solve_cubic(
    ...
    if use_mpmath:
        ...
        roots, err = mp.polyroots(
            [vareps, Btransf, Ctransf, Dtransf],
            error=True,
            maxsteps=polyroots_maxsteps,
            extraprec=polyroots_extraprec,
        )
        ...
        if err > root_tol:
            warn(
                "polyroots error of %e greater than root_tol = %e" % (err, root_tol),
                RuntimeWarning,
            )
    else:
    ...
    if Ssq3.real == Ssqm.real:
        if Ssq3.imag != 0.0 or Ssqm.imag != 0.0:
            warn(
                r"\bar{S}_3^2 and \bar{S}_-^2 have the same real parts, with values of: %e, %e"
                % (Ssq3, Ssqm),
                RuntimeWarning,
            )
        elif Ssq3 == 0.0 and Ssqm == 0.0:
            raise NonprecessingError(
                r"The system is nonprecessing: \bar{S}^2_3 = \bar(S}^2_- = 0"
            )  # This system is nonprecessing to numerical accuracy

    if Ssqm.real == Ssqp.real:
        if Ssqm.imag != 0.0 or Ssqp.imag != 0.0:
            warn(
                r"\bar{S}_-^2 and \bar{S}_+^2 have the same real parts, with values of: %e, %e"
                % (Ssqm, Ssqp),
                RuntimeWarning,
            )
        elif Ssqm == 0.0 and Ssqp == 0.0:
            raise NonprecessingError(
...
def dkappaxiq_du(
    ...
    if u > 0.0:
        ...
        vareps, Btransf, Ctransf, Dtransf = eq_coeffs(u, kappaxiq, q, S1, S2, xi, S0sq)
        ...
        if (
            min(Btransf, Ctransf, Dtransf) > 0
            and 4.0 * Ctransf * vareps < Btransf * Btransf
        ):
            z_eps_abs = abs(
                (-Btransf + (Btransf * Btransf - 4.0 * Ctransf * vareps) ** 0.5)
                / (2.0 * Ctransf)
            )
            ...
            sum_err_bound = (
                z_eps_abs
                * (
                    abs(Ctransf * Ctransf - Btransf * Dtransf)
                    + Ctransf * Dtransf * z_eps_abs
                )
            ...
            if z_eps_abs * (Ctransf / Btransf + sum_err_bound) < 1:
                m_bound = (
                    z_eps_abs
                    * (Ctransf / Btransf + sum_err_bound)
                    / (1.0 - z_eps_abs * (Ctransf / Btransf + sum_err_bound))
                )
                ...
                if m_bound < 1.0:
                    if (
                        E_ratio_term(m_bound)
                        <= 0.5
                        * lin_tol
                        * max(1.0, 1.0 / (Ctransf / Btransf + sum_err_bound))
        ...
        if only_quadratic:
            ...
            Ssq_pr_unbarred = (1.0 - q) * Ssq_pr + S0sq

            if Ssq_pr_unbarred < 0.0:
                raise RuntimeError(
                    "The unbarred S_+^2 + S_-^2 value (which was all that was necessary to compute to this accuracy) is negative, "
                    "indicating a problem with the evolution, with a value of %e"
                    % (2.0 * Ssq_pr_unbarred)
                )
        else:
            ...
            Ssqp_unbarred = (1.0 - q) * Ssqp + S0sq
            Ssqm_unbarred = (1.0 - q) * Ssqm + S0sq

            if Ssqp_unbarred < 0.0 or Ssqm_unbarred < 0.0:
                raise RuntimeError(
            ...
            mm = (Ssqp - Ssqm) / (
                Ssqp - Ssq3
            )  # Eq. (25) in Chatziioannou, Klein, Yunes, and Cornish PRD 95, 104004 (2017)
            ...
            def cubic_deriv(Ssq):
                if use_mpmath:
                    ...
                    return (3.0 * vareps * Ssq + 2.0 * Btransf) * Ssq + Ctransf
            ...
            if Ssqp - Ssqm < 1.1 * max(0.5 * abs(Ssqp + Ssqm), 1.0) * root_tol:
                root_tol_pm = 0.5 * (Ssqp - Ssqm) / max(0.5 * abs(Ssqp + Ssqm), 1.0)
            else:
                root_tol_pm = root_tol

            if Ssqm - Ssq3 < 1.1 * max(0.5 * abs(Ssqm + Ssq3), 1.0) * root_tol:
                root_tol_m3 = 0.5 * (Ssqm - Ssq3) / max(0.5 * abs(Ssqm + Ssq3), 1.0)
            else:
            ...
            Ssqp_resp, Ssqp_resm = cubic(Ssqp + max(abs(Ssqp), 1.0) * root_tol), cubic(
                Ssqp - max(abs(Ssqp), 1.0) * root_tol
            )
            Ssqm_resp, Ssqm_resm = cubic(Ssqm + max(abs(Ssqm), 1.0) * root_tol), cubic(
                Ssqm - max(abs(Ssqm), 1.0) * root_tol
            )
            ...
            if Ssqp == Ssqm:
                ...
                # shrink root_tol appropriately
                if Ssqm - Ssq3 < 1.1 * max(0.5 * abs(Ssqm + Ssq3), 1.0) * root_tol_orig:
                    root_tol_m0 = 0.5 * (Ssqm - Ssq3) / max(0.5 * abs(Ssqm + Ssq3), 1.0)
                ...
                Ssqp_resp, Ssqp_resm = cubic_deriv(
                    Ssqp + max(abs(Ssqp), 1.0) * root_tol_m0
                ), cubic_deriv(Ssqp - max(abs(Ssqp), 1.0) * root_tol_m0)
                ...
                # shrink root_tol appropriately
                if Ssqp - Ssqm < 1.1 * max(0.5 * abs(Ssqp + Ssqm), 1.0) * root_tol_orig:
                    root_tol_m1 = 0.5 * (Ssqp - Ssqm) / max(0.5 * abs(Ssqp + Ssqm), 1.0)
                ...
                Ssqm_resp, Ssqm_resm = cubic_deriv(
                    Ssqm + max(abs(Ssqm), 1.0) * root_tol_m1
                ), cubic_deriv(Ssqm - max(abs(Ssqm), 1.0) * root_tol_m1)
            ...
            elif E_ratio_term(mm) <= lin_tol * max(
                1.0, 1.0 / (Ssqp - Ssqm)
            ):  # Linearization check
                if np.sign(Ssqp_resp) == np.sign(Ssqp_resm) or np.sign(
                    Ssqm_resp
                ) == np.sign(Ssqm_resm):
                    root_tol_message = (
                ...
                Ssq3_resp, Ssq3_resm = cubic(
                    Ssq3 + max(abs(Ssq3), 1.0) * root_tol
                ), cubic(Ssq3 - max(abs(Ssq3), 1.0) * root_tol)

                if (
                    np.sign(Ssqp_resp) == np.sign(Ssqp_resm)
                ...
                Ssq_pr = Ssqp + (ellip_ratio - 1.0) * (Ssqp - Ssq3)
    else:
        Ssq_pr = 0.0
...
def kappaxiq_evol(
    ...
    if use_solve_ivp:
        ...
        soln = solve_ivp(
            ...
            method=method,
            t_eval=[uf],
            args=(q, S1, S2, xi, S0sq, imag_tol, root_tol, lin_tol, root_tol_raise_err),
            **solve_ivp_kwargs,
        )
        ...
        soln = mp.odefun(
            lambda ub, kappaxiq: -dkappaxiq_du(
                -ub,
                kappaxiq,
                q,
                S1,
        ...
        solver.set_f_params(
            q, S1, S2, xi, S0sq, imag_tol, root_tol, lin_tol, root_tol_raise_err
        )
        ...
        if abs(solver.t - uf) > 0.75 * du:
            raise RuntimeError(
                "Final u of %e is more than 0.75*du different from uf of %e"
                % (solver.t, uf)
            )
...
def prec_avg_tilt_comp_vec_inputs(
    m1,
    m2,
    chi1_vec,
    chi2_vec,
    ...
    Lf=None,
    du=1.0e-3,
    ode_atol_base=1.0e-8,
    ode_atol_inplane_spin_scaling=True,
    ode_atol_floor=1.0e-13,
    ode_rtol=-1,
    method="lsoda",
    use_solve_ivp=False,
    solve_ivp_lsoda_min_step=0.0,
    use_fallback=True,
    du_fallback=1.0e-5,
    ode_atol_fallback=1.0e-13,
    ode_rtol_fallback=-1,
    method_fallback="lsoda",
    use_mpmath_fallback=True,
    mpmath_dps=30,
    ode_tol_mpmath=1.0e-15,
    odefun_degree=None,
    polyroots_maxsteps=50,
    polyroots_extraprec=50,
    LPNorder=2,
    ...
    if (
        type(chi1_vec) is not np.ndarray
        or type(chi2_vec) is not np.ndarray
        or len(np.atleast_1d(chi1_vec)) != 3
        or len(np.atleast_1d(chi2_vec)) != 3
    ):
        raise ValueError(
            "chi1_vec and chi2_vec must both be numpy arrays of length 3, while they are",
            chi1_vec,
            chi2_vec,
        )
    ...
    if L0_vec is not None:
        if len(L0_vec) != 3:
            raise ValueError(
                "L0_vec must be either None or a numpy array of length 3, while it is",
                L0_vec,
            )
    ...
    if chi1 == 0.0 or chi1 > 1.0 or chi2 == 0.0 or chi2 > 1.0:
        raise ValueError(
    ...
    if (
        method not in np.append(tested_scipy_integrators, "odefun")
        or method_fallback not in tested_scipy_integrators
    ):
        raise ValueError(
    ...
    if method == "odefun" and not mpmath_avail:
        raise ValueError(
            ...
            "which must in this case be one of the tested scipy integrators: %s"
            % tested_scipy_integrator_string
        )
    ...
    if odefun_degree is not None and (
        odefun_degree <= 0 or not isinstance(odefun_degree, int)
    ):
        raise ValueError(
            "The degree for the mpmath.odefun must be a natural number, while it is odefun_degree = %f"
            % odefun_degree
        )

    if (
        polyroots_maxsteps <= 0
        ...
        raise ValueError(
            ...
            "numbers, while they are polyroots_maxsteps, polyroots_extraprec = %f, %f"
    ...
    if fref is not None and L0_vec is not None:
        warn(
            ...
            "evolution and fref is not used.",
            ValueWarning,
        )
    ...
    if ode_atol_inplane_spin_scaling:
        ode_atol = max(min(chi1_inplane, chi2_inplane) * ode_atol_base, ode_atol_floor)
    else:
    ...
    if (
        ode_atol_base <= 0.0
        or ode_atol_floor <= 0.0
        or ode_rtol <= 0.0
        or ode_atol_fallback <= 0.0
        or ode_rtol_fallback <= 0.0
        or ode_tol_mpmath <= 0.0
    ):
        raise ValueError(
    ...
    if imag_tol < 0.0 or root_tol < 0.0 or lin_tol < 0.0 or lin_tol_fallback < 0.0:
        raise ValueError(
            "The tolerance values must all be nonnegative, while they are imag_tol, root_tol, lin_tol, "
            "lin_tol_fallback = %e, %e, %e, %e"
            % (imag_tol, root_tol, lin_tol, lin_tol_fallback)
        )

    # Compute effective spin [Eq. (12) of Gerosa et al.]

    xi = np.dot((1.0 + q) * S1_vec + (1.0 + 1.0 / q) * S2_vec, L0_hat)
    ...
    cos_tilt1 = np.clip(S1L / S1, -1.0, 1.0)
    cos_tilt2 = np.clip(S2L / S2, -1.0, 1.0)
                ...
                try:
                    ...
                    if use_mpmath_fallback and mpmath_avail:
                        warn(
                            ...
                            "polyroots_maxsteps = %d, polyroots_extraprec = %d"
                            % (
                                format_error(re2),
                                mpmath_dps,
                                ode_tol_mpmath,
                        ...
                        try:
                            ...
                            RuntimeError,
                            mp.ctx_base.StandardBaseContext.NoConvergence,
                        ) as err:
                            return evolution_error_handling(
                                failure_mode,
                                failure_message_mpmath(err),
                        ...
                        failure_message = (
                            ...
                            "case may not be possible to evolve accurately with the scipy integration."
    ...
    if Lf is None:
        ...
        cos_tilt2inf = q * (xi / (1.0 + q) - k_evol) / S2

        # Clip to avoid issues with taking the arccos if there are rounding issues

        cos_tilt1inf = np.clip(cos_tilt1inf, -1.0, 1.0)
        cos_tilt2inf = np.clip(cos_tilt2inf, -1.0, 1.0)
        ...
        vareps, Btransf, Ctransf, Dtransf = eq_coeffs(uf, k_evol, q, S1, S2, xi, S0sq)
        ...
        cos_tilt2sep_Sm = q * (xi / (1.0 + q) - S1 * cos_tilt1sep_Sm) / S2

        cos_tilt1sep_Sp = (k_evol - uf * Ssqp) / S1
        cos_tilt2sep_Sp = q * (xi / (1.0 + q) - S1 * cos_tilt1sep_Sp) / S2

        cos_tilt1sep_Savg = (k_evol - uf * Ssqavg) / S1
        cos_tilt2sep_Savg = q * (xi / (1.0 + q) - S1 * cos_tilt1sep_Savg) / S2

        # Clip to avoid issues with taking the arccos if there are rounding issues

        cos_tilt1sep_Sm = np.clip(cos_tilt1sep_Sm, -1.0, 1.0)
        cos_tilt2sep_Sp = np.clip(cos_tilt2sep_Sp, -1.0, 1.0)

        cos_tilt1sep_Sm = np.clip(cos_tilt1sep_Sm, -1.0, 1.0)
        cos_tilt2sep_Sp = np.clip(cos_tilt2sep_Sp, -1.0, 1.0)

        cos_tilt1sep_Savg = np.clip(cos_tilt1sep_Savg, -1.0, 1.0)
        cos_tilt2sep_Savg = np.clip(cos_tilt2sep_Savg, -1.0, 1.0)

        # Return tilt angles at separation

        if swap:
            cos_tilt1sep_Sm, cos_tilt2sep_Sm, cos_tilt1sep_Sp, cos_tilt2sep_Sp = (
                cos_tilt2sep_Sp,
                cos_tilt1sep_Sp,
                cos_tilt2sep_Sm,
                cos_tilt1sep_Sm,
        ...
        return {
            ...
            "tilt2_sep_min": tilt2sepmin,
            "tilt2_sep_max": tilt2sepmax,
            "tilt2_sep_avg": tilt2sepavg,
        }


def prec_avg_tilt_comp(
    m1, m2, chi1, chi2, tilt1, tilt2, phi12, fref, Lf=None, **kwargs
):
    """
    ...
    chi1_vec = chi1 * np.array([np.sin(tilt1), 0.0, np.cos(tilt1)])

    chi2_vec = chi2 * np.array(
        [np.sin(tilt2) * np.cos(phi12), np.sin(tilt2) * np.sin(phi12), np.cos(tilt2)]
    )

    # Pass to prec_avg_tilt_comp_vec_inputs() to do calculation

    return prec_avg_tilt_comp_vec_inputs(
        m1, m2, chi1_vec, chi2_vec, fref, Lf=Lf, **kwargs
    )
