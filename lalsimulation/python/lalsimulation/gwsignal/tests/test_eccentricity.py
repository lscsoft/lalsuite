...
def test_circular_conversion(eccentricity):
    ...
    recomputed_mean = ecc.mean_anomaly_from_true(
        ecc.true_anomaly_from_eccentric(
            ecc.eccentric_anomaly_from_mean(
                mean_anomaly, 
                eccentricity), 
            eccentricity),
        eccentricity)
...
@pytest.mark.xfail(reason=
                   'Measuring at the wrong frequency. WIP. '
...
@pytest.mark.xfail(reason='Waveforms generated change significantly in length for small eccentricity!')
def test_quasicircular_limit(gen, parameters, plot):
    ...
    zeroecc_params["eccentricity"] = 0. * u.dimensionless_unscaled
    ...
    for ecc in eccentricities:
        ...
        if plot:
            ...
            plt.plot(zeroecc_hp.times - zeroecc_hp.times[np.argmax(zeroecc_hp)], zeroecc_hp)
            plt.show()
