...
@pytest.mark.parametrize('scale_factor', [2., 10.])
def test_mass_scaling(parameters, scale_factor, plot, gen):
    
    hp, hc = wfm.GenerateTDWaveform(parameters, gen)
    
    hp2, hc2 = wfm.GenerateTDWaveform(scale_initial_conditions_by_factor(parameters, scale_factor), gen)
...
def test_sample_rate_scaling(parameters, scale_factor, plot, gen):
    ...
    times = hp2.times
    new_hp = interp1d(hp.times, hp, bounds_error=False, fill_value=0.)(times)
    new_hp2 = hp2

    if plot and scale_factor==2.:
        import matplotlib.pyplot as plt
        plt.plot(times, new_hp, label=f'Baseline sample rate ({(1/parameters["deltaT"]).to(u.Hz):.0f}), downsampled')
        plt.plot(times, new_hp2, label=f'New sample rate ({(1/new_parameters["deltaT"]).to(u.Hz):.0f})')
...
def test_distance_scaling(parameters, plot, gen):
    ...
    if plot:
        ...
        plt.plot(distances, baseline_maximum / (distances / baseline_distance), label='Theoretical maxima', c='black')
        plt.xscale('log')
        plt.yscale('log')
...
def test_inclination_dependence(parameters, plot, gen):
    ...
    for inclination in inclinations:
        ...
        sigmas.append(
            sigma(hp_pycbc, 
                low_frequency_cutoff = new_parameters['f22_start'].value
            )
        )
...
@pytest.mark.xfail(reason='Times are strangely defined - is the epoch supposed not to be zero?')
