...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--model", action="append", default=[], help=f"""
        Model to test. 
...
def pytest_generate_tests(metafunc):
    ...
    if 'gen' in metafunc.fixturenames:
        ...
        if len(models) == 0:
            ...
            try:
                models_to_test = {
                    model: ALL_MODELS[model] for model in models
                }
            except KeyError as e:
                raise ValueError('Model not found') from e
        metafunc.parametrize('gen', models_to_test.values(), indirect=True, ids=models_to_test.keys())
