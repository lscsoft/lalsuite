...
def fix_ref_frequency(parameter_dict, generator):
    ...
    if generator.metadata["implementation"] == "LALSimulation":
        ...
        cases_by_approximant = {"NRSurr": parameter_dict["f22_start"]}
...
def generate_conditioned_td_waveform_from_td(parameter_dict, generator):
    ...
    tmerge = lalsim.SimInspiralMergeTimeBound(
        m1, m2
    ) + lalsim.SimInspiralRingdownTimeBound(m1 + m2, s)
    ...
    fstart = lalsim.SimInspiralChirpStartFrequencyBound(
        (1.0 + extra_time_fraction) * tchirp + tmerge + textra, m1, m2
    )
    ...
    hp, hc = cond.time_array_condition_stage1(
        hp,
        hc,
        dt,
        extra_time_fraction * tchirp + textra,
        parameter_dict["f22_start"].value,
    )
...
def generate_conditioned_td_waveform_from_fd(parameter_dict, generator):
    ...
    new_parameters["deltaF"] = 0.0 * u.Hz
    ...
    hpt = cond.high_pass_time_series(hpt, dt, original_f_min, 0.99, 8.0)
    hct = cond.high_pass_time_series(hct, dt, original_f_min, 0.99, 8.0)

    # Resize time series
    fstart = lalsim.SimInspiralChirpStartFrequencyBound(
        (1.0 + extra_time_fraction) * tchirp, m1, m2
    )
...
def generate_conditioned_fd_waveform_from_fd(parameter_dict, generator):
    ...
    elif df > 1.0 / (chirplen * deltaT):
        warnings.warn(
            "Specified frequency interval of %.2f Hz is too large for a chirp of duration %.2f s"
            % (df, chirplen * deltaT)
        )
