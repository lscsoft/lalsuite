...
def transform_time_dependent_vector(quat, vec, inverse=0):
    ...
    return multiply_quats(
        quat, multiply_quats(np.append(np.array([np.zeros(len(vec[0]))]), vec, 0), qInv)
    )[1:]
...
def transform_vector_coorb_to_inertial(vec_coorb, orbPhase, quat_copr):
    ...
    vec = transform_time_dependent_vector(
        np.array([quat_copr]).T, np.array([vec_copr]).T
    ).T[0]
