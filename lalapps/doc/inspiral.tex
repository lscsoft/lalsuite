...
\verb$--ifo-a$ \textsc{IFO} 
\verb$--ifo-b$ \textsc{IFO} \newline
%
...
of the two interferometers must be given. The default behavior outputs
triggers during playground times to the file \textsc{file};  to obtain
...
\item[\texttt{--ifo-a} \textsc{IFO}] Required. This is the name of the
...
\item[\texttt{--ifo-b} \textsc{IFO}] Required. This is the name of the
...
\item[\texttt{--output} \textsc{file}] Required.  Name of the file
to be used for output.  The output format is LIGO lightweight XML with
a \texttt{process} and \texttt{process\_params} table. If any 
coincident events are found, they will be written to a \texttt{sngl\_inspiral}
...
--output L1H1-INCA-734357353-1024.xml \
...
\texttt{currentTrigger[0]}, call \\
\texttt{LALCompareSnglInspiral()} to check if the triggers match as determied
by the options on the command line. If the trigger match, record them for
later output as coincident triggers.

\item If the \texttt{currentTrigger[1]} is not within $\delta t$, of
\texttt{currentTrigger[0]} continue loop over triggers from IFO A.
\end{enumerate}
\end{enumerate}
\end{enumerate}
