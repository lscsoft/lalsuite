...
%
\verb$--gps-start-time$ \textsc{seconds} \quad 
\verb$--gps-end-time$ \textsc{seconds} 
%
\verb$--ifo-a$ \textsc{IFOA} 
[\verb$--ifo-b$ \textsc{IFOB}] \newline
%
[\verb$--user-tag$ \textsc{usertag}]
[\verb$--no-playground$]
[\verb$--playground-only$] \newline
%
[\verb$--triggered-bank$ \textsc{file}
\verb$--minimal-match$  \textsc{M}] 
[\verb$--dm$ \textsc{$\delta m$}]
[\verb$--dpsi0$ \textsc{$\delta \psi_0$}]
[\verb$--dpsi3$ \textsc{$\delta \psi_3$}]
[\verb$--dt$ \textsc{$\delta t$}] \newline
%
[\verb$--epsilon$ \textsc{$\epsilon$}]
[\verb$--kappa$ \textsc{$\kappa$}]
[\verb$--ifo-b-snr-threshold$ \textsc{SNRSTAR}]
[\verb$--ifo-b-range-cut$] \newline 
%
[\verb$--approximant$] \newline
%
[\verb$--silde-time$ \textsc{sec}]
[\verb$--slide-time-ns$ \textsc{ns}] 
...
\end{equation}
where $\hat{\rho}_\mathrm{IFOB} = \frac{\sigma_\mathrm{IFOB}}
...
\item In this case we, the expected signal to noise ratio in \textsc{IFOB} is
above our threshold:
\begin{equation} 
...
coincident if \textsc{$\psi_3$} parameters agree within $\delta \psi_3$. 
 If notsupplied,  then $\delta  \psi_3 = 0$.
...
\item[\texttt{--approximant} APPROX] Required. Set approximant to APPROX.
{TaylorF2|BCV}.
