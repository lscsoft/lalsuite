...
\prog{LSCDataFind} on some systems.  One of \option{--frame-cache},
\option{--frame-dir}, or \option{--gaussian-noise-rms} must be specified.
...
\option{--frame-dir} will be used.  One of \option{--frame-cache},
\option{--frame-dir}, or \option{--gaussian-noise-rms} must be specified.
...
If this parameter is provided instead of \option{--frame-cache} or
\option{--frame-dir}, then Gaussian white noise will be synthesized and
used as the input data.  One of \option{--frame-cache},
\option{--frame-dir}, or \option{--gaussian-noise-rms} must be specified.
...
\begin{equation}
\tilde{s}_{k}
   = \frac{\Delta}{\sigma_{w}} \sum_{j = 0}^{N - 1} w_{j} s_{j} \ee^{-2 \pi
   \aye j k / N}
\end{equation}
where $w_{j}$ is the window function, and $\sigma_{w}$ is  
\begin{equation}
\sigma_{w}^{2}
...
\begin{equation}
\hat{s}_{k}
   = 2 \sqrt{\frac{\Delta f}{P_{k}}} \tilde{s}_{k}
\end{equation}
where $\Delta f = 1 / N \Delta$ is the sampling frequency interval and
$P_{k} = 2 \Delta f \mean{\magnitude{\tilde{s}_{k}^2}}$ is the power
spectral estimate.  This spectrum is estimated using the median power at
each frequency for a number of overlapping segments.  The use of the median
...
construction makes the real and imaginary parts of each frequency component
a unit variance Gaussian random variable. 
...
plane in a pass-band $f_{0} < f < f_{0} + b$.  A naive construction would
set all Fourier components outside of this band to zero and inverse
transform.  A major problem with this approach is that the band-pass
filter,  when viewed in the time-domain, has infinite extent(as shown in
...
\begin{align}
G(t)
   & = \int_{-f_{0} - b}^{-f_{0}} \ee^{2 \pi \aye f t} \diff f  +
   \int_{f_{0}}^{f_{0} + b} \ee^{2 \pi \aye f t} \diff f \\
   & = 2 \int_{f_{0}}^{f_{0} + b} \cos (2 \pi f t) \diff f \\
   & = \frac{\sin[2 \pi (f_{0} + b) t] - \sin[2 \pi f_{0} t]}{\pi t}.
\end{align}
This is just the sum of a sine and cosine at frequency $f_{0}$ modulated at
frequency $b$.  Consequently,  the amplitude modulation has its first zero
at $t = 1 / (2 b)$ and we can use a truncated time-domain filter
\begin{equation}
\label{eq:filter}
\Theta(t - t_0; f_0 , b)
   = \begin{cases}
   \frac{\sin[2 \pi (f_{0} + b) (t - t_{0})] - \sin[2 \pi f_{0} (t -
   t_{0})]}{\pi (t - t_{0})} & \magnitude{t - t_{0}} < 1/b \\
   0 & \text{Otherwise}
   \end{cases}
\end{equation}
which covers a time frequency volume $V = b (1 / b) = 1$ and has its peak
at time $t_{0}$.  Notice that
\begin{equation}
\lim_{t \rightarrow t_{0}} \Theta(t - t_{0}; f_{0} , b)
   = b.
\end{equation}
...
The signal to noise for a filter determined by $f_0$ and $b$ can be written
as a function of the peak time $t_0$ as
\begin{equation}
\rho(t_{0})
   = \frac{z(t_{0})}{\sigma}
\end{equation}
...
\begin{equation}
z(t_{0})
   = \int_{-\infty}^{\infty} \hat{s}(t) \Theta(t - t_{0}; f_{0}, b) \diff t
\end{equation}
and $\sigma^{2} = \mean{ \magnitude{z(t_{0})}^2 }$.  The time series
$z(t_{0})$ is most conveniently computed using Fourier transforms,
therefore
\begin{equation}
z(t_{0})
   = \int_{-\infty}^{\infty} \hat{s}(f) \tilde{\Theta}^{*}(f) \ee^{2 \pi
   \aye f t_{0}} \diff f.
\end{equation}

The Excess Power search then identifies triggers which may be gravitational
wave bursts using the following algorithm.  We construct a single time
frequency plane and then search over all possible tiles of different time
durations and frequency bands.  One first decides the maximum frequency
band and the maximum time duration that are to be tolerated in the search.
The maximum frequency band determines the minimum duration a tile requiring
the time-frequency volume be $1$ and similarly the maximum time duration
determines the minimum frequency band of the tile.  The time frequency
plane is then constructed with a time-frequency resolution finer by at
least a factor of $2$ compared to the minimum resolutions of the tiles.  As
a concrete example,  suppose the maximum allowed time duration of a tile is
$\Delta T = \unit{2^{-(a + 1)}}{\second}$  and the maximum frequency band
is $\Delta F = \unit{2^{(b - 1)}}{\hertz}$.  This implies the minimum
duration of a tile in the search will be  $\Delta t_{\text{tile}} =
\unit{2^{-(b-1)}}{\second}$  and the minimum frequency band will be $\Delta
f_{\text{tile}} = \unit{2^{(a+1)}}{\hertz}$.  The time-frequency plane is
then constructed with a timing resolution of $\delta t =
\unit{2^{-b}}{\second}$ and a frequency resolution of $\delta f =
\unit{2^a}{\hertz}$.

For each whitened segment $\hat{s}_k$, one constructs multiple channels,
each of which is an inverse Fourier tranform of a bandwidth $\delta f$ of
data.  Now suppose the bandwidth over which we want to search is $F$ and
the time duration is $T$. Therefore there will be $n_{\text{chan}} =
\frac{F}{\delta f}$ channels and $n_{\text{time bins}} = \frac{T}{\delta
t}$ time bins in each channel.  $n_{\text{time bins}} \times
n_{\text{chan}}$ number of elements will then represent this time-frequency
plane.

If we consider a channel whose low frequency index is $k_{0}$ (where $k_{0}
= \frac{f_{\text{low}}}{\delta f}$,  $f_{\text{low}}$ being the low
frequency of the channel) and the channel width in number of sample points
is $b = \frac{\delta f}{\Delta f}$ ($\Delta f$ is the frequency resolution
of the fully sampled frequency series), it's elements can be represented
as
\begin{equation}
\label{eq:tfplanecountspoints}
z_{j}(k_{0}, b)
   = \sum_{k = 0}^{N - 1} \hat{s}_{k} \tilde{\Theta}_{k}^{*}(k_{0}, b)
   \ee^{2 \pi \aye j k / N},
\end{equation}
where $j = 0, \ldots, N - 1$.  Note, the timing resolution of $z_{j}(k_{0},
b)$ is equal to that of the sampled detector data $s_{j}$.  The elements of
the time-frequency plane corresponding to this particular channel
$z_{j}(k_{0}, b)$ will then be 
\begin{equation}
Z_{J}(k_{0}, b)
   = z_{J n_{t}}(k_{0}, b),
\end{equation}
where $n_{t} = \frac{\delta t}{\Delta}$ are the number of sample points in
one time-frequency plane time bin ($\Delta$ being the full sampling rate of
the timing series) and $J = 0, \ldots, n_{\text{time bins}} - 1$.

Each channel is normalised so that a sample in the channel is a real
Gaussian random variable with zero mean and unit variance. The
normalisation constant is given by
\begin{equation}
\sigma^{2}(k_{0}, b)
   = \mean{z_{j}^{2}(k_{0}, b)}
   = \sum_{k = 0}^{N - 1} \sum_{l = 0}^{N - 1} \mean{\hat{s}_{k}
   \hat{s}_{l}^{*}} \tilde{\Theta}_{k}^{*}(k_{0}, b)
   \tilde{\Theta}_{l}(k_{0}, b) \ee^{2 \pi \aye j (k - l) / N}.
\end{equation}
If the input time series is stationary, the Fourier modes are independant
so $\mean{\hat{s}_{k} \hat{s}_{l}^{*}} = 2 \delta_{k - l}$, and the
normalisation simplifies to 
\begin{equation}
\sigma^{2}(k_{0}, b)
   = 2 \sum_{k = 0}^{N - 1} \magnitude{\tilde{\Theta}_{k}^{2}(k_{0}, b)}.
\end{equation}

Now suppose we have a channel with the same low frequency $k_{0}$ but twice
the frequency band,  i.e.\ $2 b$ compared to the previous channel.  We
therefore have 
\begin{equation}
z_{j}(k_{0}, 2 b)
   = \sum_{k = 0}^{N - 1} \hat{s}_{k} \tilde{\Theta}_{k}^{*}(k_{0}, 2 b)
   \ee^{2 \pi \aye j k / N},
\end{equation}
where $\tilde{\Theta}_{k}^{*}(k_{0}, 2 b)$ from \eqref{eq:filter} is given
by
\begin{equation}
\tilde{\Theta}(k_{0}, 2 b)
   = \sum_{j = 0}^{N - 1} \frac{\sin[2 \pi / N (k_{0} + 2 b) j] - \sin[2
   \pi / N k_{0} j]}{\pi j \Delta} \ee^{-12 \pi j k / N}
\end{equation}
However we can write
\begin{multline}
\label{eq:sines}
\sin[2 \pi / N (k_{0} + 2 b) j] - \sin[2 \pi / N k_{0} j]
   = \\ \sin[2 \pi / N(k_{0} + b) j] - \sin[2 \pi / N k_{0} j] + \sin[2 \pi
   / N (k_{0} + 2 b) j] - \sin[2 \pi / N (k_{0} + b) j]
\end{multline}
and hence we get
\begin{equation}
\tilde{\Theta}(k_{0}, 2 b)
   = \tilde{\Theta}(k_{0}, b) + \tilde{\Theta}(k_{0} + b, b)
\end{equation}
which implies
\begin{equation}
z_{j}(k_{0}, 2 b)
   = z_{j}(k_{0}, b) + z_{j}(k_{0} + b, b).
\end{equation}
This is shown in Fig~\ref{fig:sumfilters}.  
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{figures/sumoffilters}
\caption{Response to a $\unit{4}{\hertz}$ wide filter compared to the sum
of two $\unit{2}{\hertz}$ wide filters}
...
curve in the Fig~\ref{fig:sumfilters}). The black curve shows the output
when the channel width is $\unit{4}{\hertz}$, while the red curve shows the
sum of the output of two $\unit{2}{\hertz}$ wide channels.  The
corresponding normalisation is
\begin{equation}
\label{eq:norm}
\sigma^{2}(k_{0}, 2 b)
   = \sigma^{2}(k_{0}, b) + \sigma^{2}(k_{0} + b, b) + 2 \sum_{k = 0}^{N -
   1} \tilde{\Theta}_{k}^{*}(k_{0}, b) \tilde{\Theta}_{k}(k_{0} + b, b)
\end{equation}
The filter being orthogonal (almost), the cross term in \eqref{eq:norm}
will be negligible.  

A tile in this plane can be denoted as $\tau = \{J_{1}, \Delta n_{t},
K_{1}, \Delta n_{f}\}$,  where $J_{1}$ and $K_{1}$ represent the indices
corresponding to the start time and start frequency of the tile
respectively while $\Delta n_{t}$ and $\Delta n_{f}$ are respectively the
time and frequency bins.  The degrees of freedom assosciated with $\tau$
will then be $d = 2 n_{t} \Delta t n_{f} \Delta f$.  Requiring that there
are at least two sample points in the unit time-frequency volume tile we
see that the time-frequency sample points have to be picked at every
$\Delta t_{\text{tile}} = 1 / (2 n_{f} \delta f)$,  where $\delta f$ is the
time-frequency plane frequency resolution.  In terms of sample points then
every $\Delta t_{\text{tile}} / \delta t$ sample point has to be considered
while calculating the power of the tile.  Now,  $\Delta t_{\text{tile}} /
\delta t = \frac{1}{(2 n_{f} \delta f) \delta t} = n_{t} / d$ and the power
in the tile will then be
\begin{equation}
\varepsilon_{\tau}
   = \sum_{n = 0}^{d - 1} \frac{\left( \sum_{k = K_{1}}^{\Delta n_{f} - 1}
   Z_{[J_{1} + n \frac{n_{t}}{d}]}(k, b) \right)^{2}}{\sum_{k =
   K_{1}}^{\Delta n_{f} -1} \sigma^{2}(k, b)}.
\end{equation}

In Gaussian noise, the power in a given tile is $\chi^2$-distributed with
$2 V$ degrees of freedom where $V$ is the time-frequency volume of the
tile.  A burst trigger is identified if the probability of obtaining the
excess power in a tile from Gaussian noise is smaller than some threshold
$\alpha$.  For large bursts in the data stream, many tiles can be
identified as triggers with different sizes and aspect ratios.


\subsubsection{$h_{\text{rss}}$ Estimation}

The way we estimate $h_{\text{rss}}$ corresponding to a tile $\tau$ is
described here.  By definition,
\begin{equation}
h_{\text{rss}}^{2}
   = \int h^{2}(t) \diff t
   = \sum h^{2}_{j} \delta t,
...
\tilde{h}_{k}
   = R_{k} \tilde{s}_{k},
\end{equation}  
where $R_k$ is the detector response function with units strain / count
while the unit of $h_{k}$ is strain and of $s_{k}$ is counts.  In terms of
the normalised data:
\begin{equation}
\tilde{h}_k
   = \frac{R_{k} \sqrt{P_{k}}}{2 \sqrt{\Delta f}} \hat{s}_{k}.
\end{equation}
The corresponding time-frequency plane points are 
\begin{align}
H_{j}(k_{0}, b)
   & = \frac{1}{N} \sum_{k = 0}^{N - 1} \tilde{h}_k
   \tilde{\Theta}_{k}^{*}(k_{0}, b) \ee^{\aye 2 \pi j k / N} \\
   &= \frac{1}{N} \sum_{k = 0}^{N - 1} \frac{R_{k}(k_{0}, b)
   \sqrt{P_{k}(k_{0}, b)}}{2 \sqrt{\Delta f}} \hat{s}_{k}
   \tilde{\Theta}_{k}^{*}(k_{0}, b) \ee^{\aye 2 \pi j k / N}.
\end{align}
...
\begin{equation}
F(k_{0}, b)
   = \frac{R_{k}(k_{0}, b) \sqrt{P_{k}(k_{0}, b)}}{2 N \sqrt{\Delta f}}.
\end{equation}
Therefore we get 
\begin{equation}
H_{j}(k_{0}, b)
   = F(k_{0}, b) z_{j}(k_{0}, b), 
\end{equation} 
where $z_{j}$ are the time-frequency sample points obtained by using the
data in counts as given in \eqref{eq:tfplanecountspoints}.  The
$h_{\text{rss}}$ corresponding to the tile $\tau$ will then be 
\begin{equation}
h_{\text{rss}}^2
   = \sum_{n = 0}^{d - 1} \left( \sum_{k = K_{1}}^{\Delta n_{f} - 1} F(k,b)
   Z_{[J_{1} + n \frac{n_{t}}{d}]}(k, b) \right)^{2} \Delta
   t_{\text{tile}}.
\end{equation}
How well can we estimate the $h_{\text{rss}}$ under the above assumptions
are shown by Fig~\ref{fig:hrss}.  We have plotted the injected
$h_{\text{rss}}$ on the x-axis and the estimated $h_{\text{rss}}$ on the
y-axis. Ideally we would expect them to lie on a straight line at 45
degrees with the x-axis(as marked by the red line in the plot). 
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{figures/inj_det_hrss}
\caption{Injected vs Detected $h_{\text{rss}}$}
\label{fig:hrss}
\end{center}
\end{figure}
\clearpage
