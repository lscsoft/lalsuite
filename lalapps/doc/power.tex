...
strategy~\cite{Anderson:2000yy} having knowledge of only of the time duration
...
Then suppose Excess Power processed data at 16384Hz in 1 second segments 
...
\end{center}
\end{figure}
The impulse response in the Figure~\ref{fig:checkbuttertimeseries} shows  
...
This option sets the bandwidth in which the search is to be performed.  The
integer parameter \parm{bandwidth} is related to the physical bandwidth by
formula
\[
\mbox{\parm{bandwidth}}
...
\parm{cache file} specifies the path to a LAL format frame cache file in
which is described the locations of the \texttt{.gwf} frame files needed to
provide the calibration data ($\alpha$ and $\beta$ coefficients) for the
analysis.  Cache files are explained in the ``framedata'' package in LAL.
...
the PSD average length to \parm{count}.
