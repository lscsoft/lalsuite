...
\verb$--stop_time$ \textsc{sec}  
$\backslash$ \newline \hspace*{0.25in}
\verb$--stop_time_ns$ \textsc{nsec}
...
\item[\texttt{--stop\_time} \textsc{sec}] The GPS time
corresponding to the end of the time series read from frames.  

\item[\texttt{--stop\_time\_ns} \textsc{nsec}] The number of
nanoseconds after \textsc{sec} for the time series read from frames.
...
\begin{verbatim}
 ...
 --start_time 729332600    --start_time_ns 0 --stop_time 729332800  \
 --stop_time_ns 0  --srate 16384   --framecache frcache-H-729332032-729333188.out \
 --comment 040511 --verbose  --dbglevel 65 \
 --calcache /ldas_outgoing/calibration/cache_files/H1-CAL-V03-729273600-734367600.cache \
...
\verb$lalapps_bread$ 
[\verb$--help$]
\verb$--input$ \textsc{infile} 
\verb$--outfile$ \textsc{filename} 
...
\item[\texttt{--outfile} \textsc{filename}]
\verb$lalapps_bread$ writes all the triggers out in one single xml file, specified by filename

\item[\texttt{--max-confidence} \textsc{maximum conf}]
Optional. Reads in only the triggers with confidence less than the value specified by maximum conf

\item[\texttt{--noplayground}]
Optional. By default only triggers lying inside the playground are read. If this option is 
specified then all the triggers will be read. 

\item[\texttt{--sort}]
Not properly implemented yet. Now the triggers are always sorted in time before being written
out.

\item[\texttt{--min-duration} \textsc{min dur}]
Optional. Reads in only the triggers with duration more than the duration specified by min dur

\item[\texttt{--min-duration} \textsc{max dur}]
Optional. Reads in only the triggers with duration less than the duration specified by max dur

\item[\texttt{--min-centralfreq} \textsc{min centralfreq}]
Optional. Reads in only the triggers with central freq above the frequency specified by min centralfreq

\item[\texttt{--max-centralfreq} \textsc{max centralfreq}]
Optional. Reads in only the triggers with central freq below the frequency specified by max centralfreq

\item[\texttt{--min-bandwidth} \textsc{min bandwidth}]
Optional. Reads in only the triggers with bandwidth more than the bandwidth specified by min bandwidth.

\item[\texttt{--max-bandwidth} \textsc{max bandwidth}]
Optional. Reads in only the triggers with bandwidth less than the bandwidth specified by max bandwidth.

\item[\texttt{--min-amplitude} \textsc{min amplitude}]
Optional. Reads in only the triggers with amplitude more than the amplitude specified by min amplitude.

\item[\texttt{--max-amplitude} \textsc{max amplitude}]
Optional. Reads in only the triggers with amplitude less than the amplitude specified by max amplitude.

\item[\texttt{--min-snr} \textsc{min snr}]
Optional. Reads in only the triggers with snr more than the snr specified by min snr.

\item[\texttt{--max-snr} \textsc{max snr}]
Optional. Reads in only the triggers with snr less than the snr specified by max snr.
...
\item[Example]
\begin{verbatim}
lalapps_bread --input H1.txt --outfile H1.xml --max-confidence -50 --verbose
\end{verbatim}
