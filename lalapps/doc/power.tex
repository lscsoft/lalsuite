...
A tile in this plane can be denoted as $\tau =\{J_1, \Delta n_t, K_1, \Delta n_f\}$,  where
...
\begin{eqnarray}
H_J(k_0, b) &=& \frac{1}{N}\sum_{k=0}^{N-1}\tilde h_k \tilde \Theta^\ast_k(k_0, b)
e^{i 2 \pi jk/N} \\
&=& \frac{1}{N}\sum_{k=0}^{N-1} \frac{R_k \sqrt P_k}{2\sqrt {\Delta f }}\hat s_k 
\tilde \Theta^\ast_k(k_0, b)e^{i 2 \pi jk/N}
\end{eqnarray}
For the present implementation we have assumed that over the channel width
the response and the average spectrum remains constant and so we can pull them out
of the sum. If,  $F_k = \frac{R_k \sqrt P_k}{2\sqrt {\Delta f }}$ we have 
\begin{equation}
H_J(k_0, b) = F_k / N Z_J(k_0, b) , 
\end{equation} 
where $Z_J$ are the time-frequency sample points obtained by using the data in counts 
