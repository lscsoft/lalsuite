...
my @lengths = qw(10 100);


my $startEpoch = 729273613;

foreach(@lengths){
	print LOG ("Starting test of length $_ at ", localtime(), "\n");
	print "Starting test of length $_ at ", localtime, "\n";
	...
	print LOG ("Completed test of length $_ at ", localtime, "\n");
	print "Completed test of length $_ at ", localtime, "\n";
...
print PLOT_FILE << "PLOT_COMMANDS";
	...
	set data style lp
	plot "$DATA_FILE" using 1:2 t "LALdataFind", using 1:3 t "lalapps_power ($NSEG segments)"
