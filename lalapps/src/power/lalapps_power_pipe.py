...
def parse_command_line():
    ...
    parser.add_option(
        "--full-segments",
        action="store_true",
        help="Analyze all data from segment lists, not just coincident times.",
    )
    parser.add_option(
        "--minimum-gap",
        metavar="seconds",
        type="float",
        default=60.0,
        help="Merge jobs analyzing data from the same instrument if the gap between them is less than this many seconds (default = 60).",
    )
    parser.add_option(
        "--variant",
        metavar="[injections|noninjections|both]",
        default="both",
        help='Select the variant of the pipeline to construct.  "injections" produces a simulations-only version of the pipeline, "noninjections" produces a version with no simulation jobs, and "both" produces a full pipeline with both simulation and non-simulation jobs.',
    )
    parser.add_option(
        ...
        default=[],
        action="append",
        help='Set file from which to obtain the time slide table for use in the background branch of the pipeline (default = "background_time_slides.xml.gz").  Provide this argument multiple times to provide multiple time slide files, each will result in a separate set of lalburst_coinc jobs.',
    )
    parser.add_option(
        "--injection-time-slides",
        metavar="filename",
        help='Set file from which to obtain the time slide table for use in the injection branch of the pipeline (default = "injection_time_slides.xml.gz").',
    )
...
def parse_config_file(options):
    ...
    for ifo in config.get("pipeline", "ifos").split():
        seglistdict[ifo] = segmentsUtils.fromsegwizard(
            open(config.get("pipeline", "seglist_%s" % ifo)), coltype=LIGOTimeGPS
        ).coalesce()
        try:
...
def compute_segment_lists(
    ...
    if not full_segments:
        ...
        for seglist in new.values():
            ...
            psds = [
                power.psds_from_job_length(timing_params, float(abs(seg)))
                for seg in seglist
            ]
...
if options.verbose:
    print(
        "Computing segments for which lalapps_power jobs are required ...",
        file=sys.stderr,
    )
...
if options.do_injections:
    for filename in options.injection_time_slides:
        ...
        injection_time_slides[cache_entry] = timeslides.load_time_slides(
            filename, verbose=options.verbose
        ).values()
        injection_seglistdict |= compute_segment_lists(
            seglistdict,
...
datafinds = power.make_datafind_stage(
    dag, injection_seglistdict | background_seglistdict, verbose=options.verbose
)
...
def make_coinc_branch(
    ...
    trigger_nodes = power.make_single_instrument_stage(
        dag,
        datafinds,
        seglistdict,
        tag,
    ...
    if enable_clustering:
        ...
        trigger_nodes = power.make_bucluster_fragment(
            dag, trigger_nodes, "PRELLADD_%s" % tag, verbose=verbose
        )
    ...
    coinc_nodes = set()
    binj_cache = set(
        [cache_entry for node in binjnodes for cache_entry in node.get_output_cache()]
    )
    ...
    for n, (time_slides_cache_entry, these_time_slides) in enumerate(
        ...
        if enable_clustering:
            ...
            nodes = power.make_bucluster_fragment(
                dag, nodes, "POSTLLADD_%s_%d" % (tag, n), verbose=verbose
            )
        if verbose:
            print("building burca jobs ...", file=sys.stderr)
        coinc_nodes |= power.make_burca_fragment(
            dag, nodes, "%s_%d" % (tag, n), verbose=verbose
        )
        if verbose:
    ...
    if do_injections:
        ...
        coinc_nodes = power.make_binjfind_fragment(
            dag, coinc_nodes, tag, verbose=verbose
        )
...
coinc_nodes = make_coinc_branch(
    dag,
    datafinds,
    background_seglistdict,
    background_time_slides,
    options.timing_params,
    options.psds_per_power,
    options.enable_clustering,
    options.tag,
    do_injections=False,
    verbose=options.verbose,
)
inj_coinc_nodes = make_coinc_branch(
    dag,
    datafinds,
    injection_seglistdict,
    injection_time_slides,
    options.timing_params,
    options.psds_per_injection,
    options.enable_clustering,
    "INJECTIONS_RUN_0_%s" % options.tag,
    do_injections=True,
    verbose=options.verbose,
)
