...
\item[Synopsis]
\prog{lalapps\_binj} [\option{--help}]
\option{--gps-start-time}~\parm{seconds}
\option{--gps-end-time}~\parm{seconds} [\option{--time-step}~\parm{value}]
[\option{--seed}~\parm{seed}] [\option{--waveform}~\parm{name}]
[\option{--population}~\parm{name}] [\option{--freq-dist}~\parm{name}]
[\option{--flow}~\parm{Hertz}] [\option{--fhigh}~\parm{Hertz}]
[\option{--deltaf}~\parm{Hertz}] [\option{--quality}~\parm{value}]
[\option{--strain-dist}~\parm{name}]
[\option{--strain-scale-min}~\parm{value}]
[\option{--strain-scale-max}~\parm{value}]
[\option{--max-distance}~\parm{value}]
[\option{--min-distance}~\parm{value}]
[\option{--simwaveform-duration}~\parm{seconds}]
[\option{--waveform\_number-min}~\parm{value}]
[\option{--waveform\_number-max}~\parm{value}]
[\option{--usertag}~\parm{string}]
...
\item[Options]\leavevmode
\begin{entry}
\item[\option{--deltaf} \parm{Hertz}]
Optional.  The linear spacing between frequencies used to make
injections.  Default value is \(\unit{0}{\hertz}\).

\item[\option{--fhigh} \parm{Hertz}]
Optional.  Only generate injections with frequencies below \parm{fhigh}.
Default value is \(\unit{2000}{\hertz}\).

\item[\option{--flow} \parm{Hertz}]
Optional.  The code can generate injections at multiple frequencies.  This
option sets the first frequency used in that case.  Default value is
\(\unit{70}{\hertz}\).
...
\item[\option{--max-distance} \parm{value}]
Optional.  This is used when one wants to inject simulated
Inspiral+Burst+Ringdown waveforms.  This specifies the maximum distance in
Kpc that a source can be placed at.  This should be used with the
\option{--min-distance}.

\item[\option{--min-distance} \parm{value}]
Optional.  This is used when one wants to inject simulated
Inspiral+Burst+Ringdown waveforms.  This specifies the minimum distance in
Kpc that a source can be placed at.  This should be used with the
\option{--max-distance}.
...
\item[\option{--quality} \parm{value}]
Optional.  The quality factor for sine-Gaussian, Gaussian, ring-down and
ring-up waveforms.

\item[\option{--seed} \parm{value}]
Optional. Seed the random number generator with the integer \parm{value}.
Defaults to 1.

\item[\option{--simwaveform-duration} \parm{seconds}]
Optional.  This specifies the duration in seconds of the frames containing
...
Optional.  Sets the interval between injections.  The injections will
occour at \parm{value}$/\pi$ second intervals.  Defaults to 210.
...
\item[\option{--waveform} \parm{name}]
...
package are:
\begin{description}
\item[\texttt{SineGaussian}]  Inject a sine-Gaussian waveform.

\end{description}

\item[\option{--waveform-number-max} \parm{value}] 
Optional.  This specifies the maximum number of the simulated waveforms to
be injected.  

\item[\option{--waveform-number-min} \parm{value}] 
Optional.  This specifies the minimum number of the simulated waveforms to
be injected.
...
--quality 8.89 \
--strain-dist consthrss
--strain-scale-min 6.0e-21 \
--seed 45
\end{verbatim}
