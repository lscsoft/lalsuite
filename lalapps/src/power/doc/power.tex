...
\begin{entry}
\item[Name]
\prog{ligolw\_bucluster} --- cluster the burst events in LIGO light weight
XML files.

\item[Synopsis]
\prog{ligolw\_bucluster} (\option{-c}~\parm{name},
\option{--cluster-algorithm}~\parm{name}) [\option{--comment}~\parm{text}]
[\option{--help}] [\option{-i}~\parm{filename},
\option{--input-cache}~\parm{filename}] [\option{-p}~\parm{name},
\option{--program}~\parm{name}] [\option{--verbose}] [\option{--version}]
[filename~...]

\item[Description]
The program \prog{ligolw\_bucluster} applies a clustering alogrithm to the
burst events stored in the sngl\_burst tables of one or more LIGO light
weight XML files.  At this time only one clustering algorithm is
implemented, the clustering algorithm used by the excess power burst search
pipeline.  The files to be processed can be listed on the command line
and/or in one or more LAL cache files.  If the command line does not
specify any files to process, then input is read from stdin and written to
stdout.  Any file whose name ends in ``.gz'' is assumed to be
gzip-compressed.

\item[Options]\leavevmode
\begin{entry}
\item[\option{-c}~\parm{name}, \option{--cluster-algorithm}~\parm{name}]
Set the clustering method (required).  Allowed values are
\begin{entry}
\item[excesspower]  Select the clustering algorithm used by the excess
power burst search pipeline.  This algorithm is associative, meaning the
final trigger list is independent of the order in which the consituent
triggers are clustered.  This allows the algorithm to be safely applied
repeatedly to the same triggers (the clustering algorithm is an identity
transform on triggers that have already been processed).

\end{entry}
(yes, only one clustering algorithm is implemented at this time, and yes,
it's still a required option).

\item[\option{--comment}~\parm{text}] Set the comment string to be
recorded in the process table (default = ``'').

\item[\option{--help}] Print a help message.

\item[\option{-i}~\parm{filename}, \option{--input-cache}~\parm{filename}]
Process the LIGO light-weight XML files listed in this LAL cache.  If this
option is provided more than once, then all files from all input caches are
processed.

\item[\option{-p}~\parm{name}, \option{--program}~\parm{name}]  Set the
name of the program that generated the events as it appears in the process
table (default = ``\prog{lalapps\_power}'').  At this time, this
information is only used to extract live time information from the search
summary table in order for this program to record an appropriate entry for
itself in the search summary table.  In the future, the program name might
be used to restrict the triggers that are processed.

\item[\option{--verbose}]  Be verbose.

\item[\option{--version}]  Print the version of this program and exit.

\end{entry}

\item[Example]
\begin{verbatim}
$ ligolw_bucluster --comment TEST --cluster-algorithm excesspower --verbose \
input1.xml.gz input2.xml.gz
\end{verbatim}
...
\begin{entry}
\item[Name]
\prog{ligolw\_burca} --- perform a coincidence analysis on LIGO
light-weight XML files containing burst events.

\item[Synopsis]
\prog{ligolw\_burca} [\option{--version}] [\option{-h}, \option{--help}]
[\option{-c}~\parm{text}, \option{--comment}~\parm{text}] [\option{-f},
\option{--force}] (\option{-p}~\parm{name}, \option{--program}~\parm{name})
(\option{-a}~\parm{name}, \option{--coincidence-algorithm}~\parm{name})
[\option{--likelihood-data}~\parm{filename}]
[\option{--likelihood-data-cache}~\parm{filename}]
[\option{-t}~\parm{inst1,inst2=[threshold,...]},
\option{--thresholds}~\parm{inst1,inst2=[threshold,...]}]
[\option{--tmp-space}~\parm{path}]
[\option{-s}~\parm{\(\kappa\),\(\epsilon\)},
\option{--stringcusp-parms}~\parm{\(\kappa\),\(\epsilon\)}] [\option{-v},
\option{--verbose}] [filename~...]
...
applications.  The first function formed by this program is a
...
unnecessary comparisons --- these optimizations form the bulk of the
internal complexity of this program --- but users should not be surprised
to find that this program can take a long time to run.
...
In this mode of operation, \prog{ligolw\_burca} assigns likelihood ratios
to the coincident \(n\)-tuples it has previously identified using parameter
distribution density data collected from populations of time-slide
(``noise'') and injection (``gravitational wave'') \(n\)-tuples.  The
procedure goes as follows.  First, \prog{ligolw\_burca} is used to identify
coincident \(n\)-tuples in time-slide and injection trigger lists.  Another
program, \prog{ligolw\_burca\_tailor} (see Section \ref{section1}), scans
these \(n\)-tuples and collects summary statistics about their properties.
