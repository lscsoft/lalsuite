...
cache files."""
	...
	cache = [CacheEntry(l, coltype = lal.LIGOTimeGPS) for name in cache_names for l in file(name)]
...
def element_filter(name, attrs):
	return (name == ligolw.Table.tagName) and (table.Table.TableName(attrs["Name"]) in map(table.Table.TableName, [lsctables.SnglBurstTable.tableName, lsctables.SearchSummaryTable.tableName]))
