...
def parse_command_line():
    ...
    parser.add_option(
        ...
        "--single-instrument",
        action="store_true",
        help="Select single instrument mode.  In this mode, after grouping the files as usual, for each group a separate LAL cache file is written for each instrument, rather than a single cache listing all input files.",
    )
    parser.add_option(
        "-t",
        "--time-slides",
        metavar="filename",
        help="Read the time slide table from this file.",
    )
    ...
    parser.add_option(
        ...
        metavar="base",
        default="cafe_",
        help='Set base for output caches (default = "cafe_").',
    )
...
seglists, outputcaches = cafe.ligolw_cafe(
    ...
    lsctables.TimeSlideTable.get_table(
        ligolw_utils.load_filename(options.time_slides, verbose=options.verbose)
    )
    .as_dict()
    .values(),
    options.verbose,
...
if options.single_instrument:
    cafe.write_single_instrument_caches(
        options.base, outputcaches, instruments, options.verbose
    )
else:
