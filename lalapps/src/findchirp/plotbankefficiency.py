...
def comparisonRelated(results, results2):
  ...
  try:
    ...
    xlabel('total mass')
    ylabel('SNR in file 1 /  SNR in file 2')
  ...
  for ecc in ['ecc_sim','ecc_sim_fl']:
    for this_param,label in zip(['totalmass_sim','chirpmass_sim'],\
      ...
      try:
        ...
        plotting.contourf(results[ecc], results[this_param], snr, \
            xbin=nbin,ybin=nbin, xmin=0, xmax=0.4, vmin=0.5,vmax=1)
        xlabel(r'Eccentricity')
        ylabel(label)
        mysavefig(opts,name +'.png')
        
        name = __name__ + '_contour_detectability_versus_'+this_param+'_'+ecc
        plotting.contourf(results[ecc], results[this_param], (snr)**(3.),\
                      xbin=nbin, ybin=nbin, vmin=0.5,vmax=1,colormap='cube')
        xlabel(r'Eccentricity')
        ylabel(label)
        mysavefig(opts,name +'.png')        
      except: 
...
# --------------------------------------------------- specific to  Eccentricity
if opts.skip_eccentricity is False:
  if signal=='Eccentricity':
    try: eccentricityRelated(results,results2,nbin=opts.nbin)
