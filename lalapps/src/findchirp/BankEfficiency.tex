...
Here, we want to inject EOB waveforms (\textbf{{-}{-}signal EOB}), and filter them through a template bank that is made of EOB waveforms (\textbf{{-}{-}template EOB}). This command will return a bunch of numbers, among which the \textbf{overlaps} betweem the injected waveform and the template bank. 

The next step is to know what BankEfficiency returns. From the output on the screen this is cumbersome. Luckily, at the end of the previous command, one can add \textbf{--xml-output}:
...
What you want to do to test that a template bank is well designed is large number of simulations. Imagine your template bank is made of about xx templates. Let us say that performing 10 times more injections is reasonable. This number of simulations can be specified with \textbf{--n 10000}. 

Then, to be realistic, the minimal match of the bank is set to \textbf{-- mm 95\%}.
