...
#define FMIN   20.0     /* Lowest frequency filled with calibration */
...
static int mylocate(REAL4 *farray, REAL4 target, INT4 n)
{
  INT4 i=0;
  INT4 jmin;
  REAL4 dummy;
...
int main(int argc, char **argv)
{
  ...
  while ( arg < argc ) {
	...
	LALCCreateVector(&stat, &(calrec.transfer->data), npoints);
      ...
      arg++;
      /* Note the factor of two here to account for light travel path */
  ...
  for (i=0 ; i<npoints ; i++){
    ...
    if ( freq >= FMIN )
    {
      ...
      if (location > (countsamples-intParams.n)) location = (countsamples-intParams.n);
      else location-=(intParams.n/2);
      intParams.x = &farray[location];

      /* 
      *
      * interpolate the AMPLITUDE and fold in the actuator response and
      ...
      *
      ...
      /*
       *
       * Below FMIN,  we fill the response function array with unity 
       *
       */
  ...
  if ( outfile ) {
    ...
    for(i=0;i<npoints;i++){
      ...
      fprintf(fp,"%e %e %e %e\n",fmin+ calrec.transfer->deltaF * i, re, im, re*re+im*im);
    }
    fclose( fp );
  }
