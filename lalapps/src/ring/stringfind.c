...
int main( int argc, char *argv[] )
{
  ...
  static StringSearchData    data;
  static StringSearchInput   input;
  static StringSearchParams *params;
  static StringEventList    *events;
  ...
  /* initialize string search */
  LALStringSearchInit( &status, &params, fltrargv, fltrargc );
  TESTSTATUS(&status);
  ...
  /* condition data */
  LAL_CALL( LALStringSearchConditionData( &status, params, &data ), &status );
  if ( save )
  {
    UINT4 seg;
  ...
  /*create bank */
  bankin.freqPower = params->freqPower;
  bankin.minFrequency = params->minFrequency;
  bankin.maxFrequency = params->maxFrequency;
  bankin.maxMismatch = params->maxMismatch;
  bankin.invSpectrum = params->invSpectrum;
  LAL_CALL( LALCreateStringTemplateBank(&status, &params->templateBank, &bankin ),&status);
  
  /* filter entire template bank */
  bmin = bmin < (int)params->templateBank->numTmplt ? bmin :
    (int)params->templateBank->numTmplt;
  bmin = bmin > 0 ? bmin : 0;
  bmax = bmax < (int)params->templateBank->numTmplt ? bmax :
    (int)params->templateBank->numTmplt;
  bmax = bmax >= bmin ? bmax : (int)params->templateBank->numTmplt;
  input.startTemplate = bmin;
  input.templatesToDo = bmax - bmin;
  params->keepResults = keep;
  LAL_CALL( LALStringSearch( &status, &events, &input, params ),&status);
    
  /* output events */
  fp = outfile ? fopen( outfile, "w" ) : stdout;
  if ( events )
  {
    fprintf( fp, "# gps start time\tsignal/noise\tamplitude\tfrequency\n" );
  }
  while ( events )
  {
    StringEventList *next = events->next;
    fprintf( fp, "%9d.%09d\t%e\t%e\t%e\n",
        (int)( events->startTimeNS / 1000000000 ),
        (int)( events->startTimeNS % 1000000000 ),
        events->snr, events->amplitude, events->frequency );
    LALFree( events );
    events = next;
  }
  outfile ? fclose( fp ) : 0;
  ...
  /* finalize strring search */
  LAL_CALL( LALStringSearchFini( &status, &params ), &status );
