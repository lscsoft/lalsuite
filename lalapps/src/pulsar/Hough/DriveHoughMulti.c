...
int main(int argc, char *argv[]){
  ...
  /* line 375  insert*/
  LAL_CALL( LALRegisterINTUserVar(   &status, "PatchToplist",      0,  UVAR_OPTIONAL, "Create a Toplist per Patch", &uvar_EnablePatchToplist), &status);
	...
	while ( (fBinSearch <= fLastBin) && (fBinSearch < fBinSearchMax) ) 
	  {
		...
		for(j = 0; j < histTotal->length; j++){ 
	      ...
	      /* select candidates from hough maps */
            /* line 995 insert*/
            if ( uvar_EnablePatchToplist != 0) {
                LAL_CALL( GetToplistFromHoughmap( &status, toplist[skyCounter], &ht, &patch, &parDem, meanN, sigmaN), &status);
            }
            else  {
  ...
  if (uvar_EnableChi2){
    ...
    if ( uvar_EnablePatchToplist != 0) {
        for (skyCounter = 0;
             < nSkyPatches; skyCounter++)
        {LAL_CALL(ComputeandPrintChi2(&status, toplist[skyCounter], timeDiffV, &velV, skyCounter, uvar_chiSqBins, alphaPeak, mdetStates, weightsNoise, &upgV), &status);
        }
    }
    else {
         LAL_CALL(ComputeandPrintChi2(&status, toplist, timeDiffV, &velV, 0, uvar_chiSqBins, alphaPeak, mdetStates, weightsNoise, &upgV), &status);
    }
			   ...
			   UCHARPeakGramVector      *upgV /* Expanded (UCHAR) peakgrams */)
{
    ...
    fpChi2 = fopen("hough_top_" << skyCounter << ".dat", "w");
