...
# choose start of freq range to be analyzed 108.8571-0.05
startfreq=108.8071
freqband=0.1
...
./DriveHoughMulti -d 0 --f0=$freq --fSearchBand=$freqband --timeStampsFile=./ts-inject-S4_Multi.txt  --skyfile=./skypulsar.3 --weighAM=1 --weighNoise=1 --printMaps=1 --printStats=1 --earthEphemeris=./earth05-09.dat --sunEphemeris=./sun05-09.dat --sftDir=$sftdir --dirnameOut=${tempoutdir}/Multi.P3 --fbasenameOut=freq_${freq}_  --linefiles=./S4lines_H1_xavi.txt.v2,./S4lines_L1_xavi.txt.v2,./S4lines_H2.txt --printLog

echo finished running driver

# now copy the results to the home directory
mkdir -p ${workdir}/Multi.P3
rsync -a ${tempoutdir}/Multi.P3 $workdir
