...
get_psd () {
    ...
    cmdline="${psd_code} ${IFObit} --inputData=${outSFTbname}* --outputPSD=$psdfile --blocksRngMed=$blocksRngMed --outputNormSFT=1 --startTime=$3 --endTime=$4 --PSDmthopSFTs=$1 --PSDmthopIFOs=$1"
    echo $cmdline;
    if ! eval $cmdline; then
        echo "Error.. something failed when running '$psd_code' ..."
        exit 1
...
for mthop in 0 1 2 3 4 5 6 7 8; do
    ...
    psdH1L1awk=$(echo $psdH1 $psdL1 | awk "${mthops_awk[$mthop]}")
    psd_reldev=$(echo $psdH1L1 $psdH1L1awk | awk "$awk_reldev")
