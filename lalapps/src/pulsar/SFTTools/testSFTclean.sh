...
awk_is_much_lesser='{if($1<10*$2) {print "1"}}'
...
test_cleaning "50.005 0.0 1 0.0 0.0 /*single_bin_line*/" "1" "check"
...
checked=$(echo $abs1 $abs2 | awk "$awk_is_much_lesser")
if [ "$checked" ]; then
    echo "error: Strong line at $freq1 Hz should have been cleaned, but SFT data is $abs2>=10*$abs1."
    exit 1
else
