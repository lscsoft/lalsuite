...
for ifo in 0 1; do
    ...
    dump_orig="dump_${original_SFTs}.txt"
    cmdline="${dump_code} --dataOnly --SFTfiles=${original_SFTs} > $dump_orig"
    echo $cmdline
    if ! eval $cmdline; then
...
test_cleaning () {
    ...
    cmdline="${clean_code} --sftDir=*${original_SFT_pattern} --outDir=. --fMin $fmin --fMax $fmax --linefiles=$linefile"
    ...
    for ifo in 0 1; do
        ...
        if [ "${IFOs[$ifo]}" == "L1" ]; then
            ...
            if [ $ndiff == $nexpected ]; then
                echo "OK: $dump_orig and $dump_clean differ by exactly NSFTs*$2=$NSFTs*$2=$nexpected line(s), as expected."
            else
                echo "error: $dump_orig and $dump_clean should differ by exactly NSFTs*$2=$NSFTs*$2=$nexpected line(s)."
                exit 1
            fi
        else
            diff_cmd="diff -s $dump_orig $dump_clean"
            echo $diff_cmd
            if ! eval $diff_cmd; then
                echo "error: $dump_orig and $dump_clean should be equal as long as no matching --linefiles entry for ${IFOs[$ifo]} is given."
                exit 1
            fi
        fi
    done
