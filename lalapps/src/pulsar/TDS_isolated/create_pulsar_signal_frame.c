...
'%s'\n", fn, inputs.ephemYr );
  ...
  CHAR parname[256];
  BinaryPulsarParams pulparams[numpulsars]; /*Change for number of pulsars in ER1*/
  ...
  CHAR OUTFILE[256];
  /*  sprintf(OUTFILE, "/home/pankow/ER1/inj_only/pulsar_inj/%s",out_file);*/
  ...
  while(1){
    ...
    switch(c){
      ...
      case 'i': /* interferometer/detector */
        snprintf(inputParams->det, sizeof(inputParams->det), "%s", optarg);
        break;
      case 'c': /* channel name */
      ...
      case 'o': /* output directory */
        snprintf(inputParams->outDir, sizeof(inputParams->outDir), "%s",
                 optarg);
        break;
      case 's': /* output name string */
        snprintf(inputParams->outStr, sizeof(inputParams->outStr), "%s",
                 optarg);
        break;
      ...
      case 'y': /* ephemeris file year */
        snprintf(inputParams->ephemYr, sizeof(inputParams->ephemYr), "%s",
                 optarg);
        break;
      case '?':
