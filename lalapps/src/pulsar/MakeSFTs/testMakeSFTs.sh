...
cmdline="$injectdir/lalapps_Makefakedata_v5 --IFOs H1 --sqrtSX 1e-24 --startTime ${tstart} --duration ${Tsft} --fmin 0 --Band ${Band} --injectionSources '{Alpha=0.1; Delta=0.4; Freq=50; f1dot=1e-10; h0=1e-24; cosi=0.7; refTime=${tstart}}' --outFrameDir $testdir/ --outSFTdir $testdir/"
if ! eval "$cmdline"; then
    echo "ERROR: something failed when running '$cmdline'"
    exit 1
fi
MFDv5gwf="$testdir/H-H1_mfdv5-${tstart}-${Tsft}.gwf"
MFDv5sft="$testdir/H-1_H1_${Tsft}SFT_mfdv5-${tstart}-${Tsft}.sft"
...
cmdline="$builddir/lalapps_MakeSFTs -f 0 -t ${Tsft} -p $testdir -C $framecache -s ${tstart} -e ${tend} -N H1:mfdv5 -v 2 -i H1 -u PROC_REAL8 -w 0 -F 0 -B ${Band2} -D 4 -X MSFT"
if ! eval "$cmdline"; then
    echo "ERROR: something failed when running '$cmdline'"
    exit 1
fi
MSFTsft="$testdir/H-1_H1_${Tsft}SFT_MSFT-1257/H-1_H1_${Tsft}SFT_MSFT-${tstart}-${Tsft}.sft"
...
tol=1e-10
cmdline="$sftdir/lalapps_compareSFTs -V -e $tol -1 $MFDv5sft -2 $MSFTsft"
