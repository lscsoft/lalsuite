...
main(int argc, char *argv[])
{
  ...
  { /* build up full comment-string to be added to SFTs: 'generated by LISAmakeSFTs + RCSID + GitID + cmdline + user extraComment' */
    UINT4 len;
    CHAR *logstr = NULL;
    CHAR *cvsstr = NULL;
    CHAR *gitstr = NULL;
    LAL_CALL ( LALUserVarGetLog ( &status, &logstr,  UVAR_LOGFMT_CMDLINE ), &status );
    if ( ( cvsstr = LALCalloc ( 1, 512 ) ) == NULL )
      return LISAMAKESFTS_EMEM;
    /* Should use LAL version of sprintf? */
    sprintf ( cvsstr, "Generated by $Id$:\n" );
    if ( ( gitstr = LALCalloc ( 1, 1024 ) ) == NULL )
      return LISAMAKESFTS_EMEM;
    sprintf
      ( gitstr,
    ...
    len = 16 + strlen( cvsstr ) + strlen( gitstr ) + strlen ( logstr );
    if ( ( add_comment = LALCalloc ( 1, len ) ) == NULL )
      return LISAMAKESFTS_EMEM;
    sprintf ( add_comment, "%s%s%s", cvsstr, gitstr, logstr );
    LALFree ( logstr );
