...
/*---------- log string formats ----------*/
#define CVS_LOG_STR "Generated by $Id$:\n"
#define GIT_LOG_FMT "SC-version of executable:\n  lalCommitID = %s\n  lalappsCommitID = %s\n"
...
main(int argc, char *argv[])
{
  ...
  { /* build up full comment-string to be added to SFTs: 'generated by LISAmakeSFTs + RCSID + GitID + cmdline + user extraComment' */
    ...
    LALSnprintf ( cvsstr, sizeof(cvsstr), CVS_LOG_STR );
    if ( ( gitstr = LALCalloc ( 1, 1024 ) ) == NULL )
      return LISAMAKESFTS_EMEM;
    LALSnprintf ( gitstr, sizeof(gitstr), GIT_LOG_FMT,
		  lalGitID, lalappsGitID );
    len = 16 + strlen( cvsstr ) + strlen( gitstr ) + strlen ( logstr );
    if ( ( add_comment = LALCalloc ( 1, len ) ) == NULL )
      return LISAMAKESFTS_EMEM;
    LALSnprintf ( sizeof(add_comment), add_comment, "%s%s%s",
		  cvsstr, gitstr, logstr );
    LALFree ( logstr );
