 ...
 * Copyright (C) 2010, 2014 Chris Messenger
 *
                                   ...
                                   )
{
  ...
  while ( !flag ) {
    ...
    if ( ( nu >= n1 ) && ( nu <= n2 ) && ( a >= a1 ) && ( a <= a2 ) && ( tasc >= t1 ) && ( tasc <= t2 ) && ( Om >= O1 ) && ( Om <= O2 ) ) {
      flag = 1;
    }
                                                 ...
                                                 )
{
  ...
  for ( i = 0; i < dsdata->length; i++ ) {
      ...
      for ( j = 0; j < fs->data->length; j++ ) {
        ...
        if ( fp != NULL ) {
          if ( ( *power )->segment[i]->data->data[idx] > maxpower ) {
            ...
            for ( k = 0; k < spintemp->ndim; k++ ) {
              maxspintemp[k + 1] = spintemp->x[k];
            }
          }
        }
        idx++;
    ...
    if ( fp != NULL ) {
      ...
      for ( j = 0; j < maxdim; j++ ) {
        fprintf( fp, "%.12f\t", maxspintemp[j] );
      }
      for ( j = maxdim; j < NFREQMAX; j++ ) {
        fprintf( fp, "0.0\t" );
      }
                             ...
                             )
{
  ...
  for ( i = 0; i < 2; i++ ) { /* nu */
    if ( i == 0 ) {
      bintemp.x[0] = space->data[0].min;
    } else {
      bintemp.x[0] = space->data[0].max;
    }

    for ( j = 0; j < 2; j++ ) { /* a */
      if ( j == 0 ) {
        bintemp.x[1] = space->data[1].min;
      } else {
        bintemp.x[1] = space->data[1].max;
      }
      ...
      for ( k = 0; k < ngrid; k++ ) { /* tasc */
        ...
        for ( l = 0; l < ngrid; l++ ) { /* omega */
          ...
          for ( n = 0; n < NFREQMAX; n++ ) {
            if ( fdots.x[n] < fnmin[n] ) {
              fnmin[n] = fdots.x[n];
            }
            if ( fdots.x[n] > fnmax[n] ) {
              fnmax[n] = fdots.x[n];
            }
          }
  ...
  ( *gridparams )->max = 1;
  for ( k = 0; k < ( *gridparams )->ndim; k++ ) {
    ( *gridparams )->max *= ( *gridparams )->grid[k].length;
  }

  ( *gridparams )->prod[0] = 1;
  for ( k = 1; k < ( *gridparams )->ndim; k++ ) {
    ( *gridparams )->prod[k] = ( *gridparams )->prod[k - 1] * ( *gridparams )->grid[k - 1].length;
  }
                               ...
                               )
{
  ...
  for ( k = 0; k < ( INT4 )( *binarygridparams )->ndim; k++ ) {
    ( *binarygridparams )->max *= ( *binarygridparams )->grid[k].length;
  }

  ( *binarygridparams )->prod[0] = 1;
  for ( k = 1; k < ( INT4 )( *binarygridparams )->ndim; k++ ) {
    ( *binarygridparams )->prod[k] = ( *binarygridparams )->prod[k - 1] * ( *binarygridparams )->grid[k - 1].length;
  }
