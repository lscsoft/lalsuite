...
lalapps_Makefakedata_v5 --randSeed=2345 --fmin=45.0 --Band=1.0 \
...
set -x
lalapps_Weave --output-file=WeaveOutRecalc.fits \
    ...
    --alpha=2.3/0.9 --delta=-1.2/2.3 --freq=45.5~0.005 --f1dot=-1e-9,0 \
    --semi-max-mismatch=7 --interpolation=no
lalapps_fits_overview WeaveOutRecalc.fits
set +x
echo

echo "=== Check recalc'ed results against original ==="
set -x
lalapps_fits_table_list -n "WeaveOutRecalc.fits[mean2F_toplist][col coh2F;     coh2F_H1;     coh2F_L1; ]"   > stage0.dat
lalapps_fits_table_list -n "WeaveOutRecalc.fits[mean2F_toplist][col coh2F_rec; coh2F_H1_rec; coh2F_L1_rec]" > stage1.dat
...
set -x
nsegments=`lalapps_fits_header_getval "WeaveOut.fits[0]" 'NSEGMENT' | tr '\n\r' '  ' | awk 'NF == 1 {printf "%d", $1}'`
coh_ntmpl=`lalapps_fits_header_getval "WeaveOut.fits[0]" 'NCOHTPL' | tr '\n\r' '  ' | awk 'NF == 1 {printf "%d", $1}'`
...
#   octapps_run WeaveFstatMismatch --setup-file=WeaveSetup.fits --spindowns=1 --semi-max-mismatch=7 --coh-max-mismatch=0 --printarg=meanOfHist
mean_mu=0.53991
