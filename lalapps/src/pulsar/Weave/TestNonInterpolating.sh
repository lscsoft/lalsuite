...
${builddir}/lalapps_Weave --output-file=WeaveOut.fits --output-max-size=5000 --output-per-detector --output-per-segment --output-details --setup-file=WeaveSetup.fits \
    --sft-files='*.sft' --alpha=1.2/0.05 --delta=0.3/0.05 --freq=50.5/1e-6 --f1dot=-1e-8,0 --semi-max-mismatch=0.6 --interpolation=no --Fstat-method=DemodBest
set +x
echo
...
for seg in 1 2 3; do
    ...
    set -x
    ${fitsdir}/lalapps_fits_table_list "WeaveOut.fits[toplist_mean_twoF][col c1=freq; c2=alpha; c3=delta; c4=f1dot; c5=0; c6=0; c7=seg${seg}_twoF]" > WeaveSeg${seg}SemiBankCohFstats.txt
    ${fitsdir}/lalapps_fits_table_list "WeaveOut.fits[toplist_mean_twoF][col c1=freq; c2=alpha; c3=delta; c4=f1dot; c5=0; c6=0; c7=seg${seg}_twoF_H1]" > WeaveSeg${seg}SemiBankCohFstatsH1.txt
    ${fitsdir}/lalapps_fits_table_list "WeaveOut.fits[toplist_mean_twoF][col c1=freq; c2=alpha; c3=delta; c4=f1dot; c5=0; c6=0; c7=seg${seg}_twoF_L1]" > WeaveSeg${seg}SemiBankCohFstatsL1.txt
    set +x
    echo
