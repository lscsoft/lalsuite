...
/* sets up a HoughFStatCheckpointFile from parameters */
int houghFStat_cpt_file_create (HoughFStatCheckpointFile **cptf,
			   CHAR  *filename,
			   UINT4 bufsize,
			   UINT4 maxsize,
			   toplist_t*list) {
  ...
  if ( (cptf == NULL) ||
       ...
       (filename == NULL) ||
       (strlen (filename) == 0) ) {
    LogPrintf (LOG_CRITICAL, "ERROR: error in input parameters (houghFStat_cpt_file_create)\n");
    return(-1);
  }

  /* allocation */
  *cptf = LALMalloc(sizeof(HoughFStatCheckpointFile));
  if (!(*cptf)) {
    LogPrintf (LOG_CRITICAL, "ERROR: out of memeory (houghFStat_cpt_file_create)\n");
    return(-1);
  }
...
/* destroys a HoughFStatCheckpointFile structure */
int houghFStat_cpt_file_destroy (HoughFStatCheckpointFile **cptf) {
  if (!cptf) {
    LogPrintf (LOG_CRITICAL, "ERROR: HoughFStatCheckpointFile is NULL\n");
    return(-1);
  }
  if((*cptf)->filename)
    LALFree((*cptf)->filename);
...
int houghFStat_cpt_file_open (HoughFStatCheckpointFile *cptf) {
  if (!cptf) {
    LogPrintf (LOG_CRITICAL, "ERROR: HoughFStatCheckpointFile is NULL\n");
    return(-1);
  }
...
int houghFStat_cpt_file_flush (HoughFStatCheckpointFile *cptf) {
  if (!cptf) {
    LogPrintf (LOG_CRITICAL, "ERROR: HoughFStatCheckpointFile is NULL\n");
    return(-1);
  }
...
extern int houghFStat_cpt_file_info (HoughFStatCheckpointFile *cptf,
...
int houghFStat_cpt_file_close(HoughFStatCheckpointFile*cptf) {
  if (!cptf) {
    LogPrintf (LOG_CRITICAL, "ERROR: HoughFStatCheckpointFile is NULL\n");
    return(-1);
  }
 ...
 */
int houghFStat_cpt_file_add (HoughFStatCheckpointFile*cptf, HoughFStatOutputEntry line) {
  int ret, bytes;

  ret = insert_into_toplist(cptf->list, &line);
  if (ret) {
    bytes = write_houghFStat_toplist_item_to_fp(line, cptf->fp, &(cptf->checksum));
    if (bytes < 0) {
...
/* reads a written checkpointed toplist back into a toplist
   ...
   -1 if the file contained a syntax error,
   -2 if given an improper toplist */
int houghFStat_cpt_file_read (HoughFStatCheckpointFile*cptf, UINT4 checksum_should, UINT4 maxbytes) {
  INT4  bytes;
  UINT4 checksum_read;
  if (!cptf) {
    LogPrintf (LOG_CRITICAL, "ERROR: HoughFStatCheckpointFile is NULL\n");
  ...
  bytes = read_houghFStat_toplist_from_fp(cptf->list, cptf->fp, &checksum_read, maxbytes);

  LogPrintf (LOG_DEBUG, "DEBUG: read_houghFStat_toplist_from_fp() returned %d\n", bytes);
...
int houghFStat_cpt_file_compact(HoughFStatCheckpointFile*cptf) {
  INT4  bytes;
  UINT4 checksum;
  ...
  bytes = atomic_write_houghFStat_toplist_to_file(cptf->list, cptf->filename, &checksum);
  if (bytes < 0) {
    return(bytes);
  }

  houghFStat_cpt_file_open(cptf);
