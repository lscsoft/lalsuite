			       ...
			       )
{
  ...
  INITSTATUS( status, "ComputeFStatFreqBand_RS", COMPUTEFSTATC );
  ATTATCHSTATUSPTR (status);
			 ...
			 ) 
{
  ...
  INITSTATUS( status, "ResampleMultiSFTs", COMPUTEFSTATC );
  ATTATCHSTATUSPTR (status);
  ...
  if (Tprime>T) {
    ...
    ABORT ( status->statusPtr, COMPUTEFSTATC_EXLAL, COMPUTEFSTATC_MSGEXLAL );
  }
  ...
  *multitimeseries = (MultiCOMPLEX8TimeSeries *)calloc(1,sizeof(MultiCOMPLEX8TimeSeries));
  ...
  if (((*multitimeseries)->Fat == NULL)||((*multitimeseries)->Fbt == NULL)) {
    ...
    ABORT ( status->statusPtr, COMPUTEFSTATC_EMEM, COMPUTEFSTATC_MSGEMEM );
  }
  for (i=0;i<numDetectors;i++) {
