...
extern int atomic_write_houghFStat_toplist_to_file(toplist_t*list, char*filename, UINT4*checksum);

/** meant for the final writing of the toplist
   - reduces toplist precision
   - sorts the toplist
   - finally calls atomic_write_houghFStat_toplist_to_file() */
extern int final_write_houghFStat_toplist_to_file(toplist_t*list, char*filename, UINT4*checksum);
