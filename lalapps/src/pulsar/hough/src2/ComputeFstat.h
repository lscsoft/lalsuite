...
typedef struct {
  ...
  MultiCOMPLEX8TimeSeries *multiTimeseries;     /**< the buffered vectors of the resmapled Fa(t) and Fb(t) that only need to be recomputed for a new sky position */
} ComputeFBuffer;
...
typedef struct {
  ...
  ComputeFBuffer *buffer;    /**< buffer for storing data that may be used in repeated calls to ComputeFStatFreqBand_RS() */
} ComputeFParams;
