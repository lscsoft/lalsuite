...
./DriveHoughMulti -d 0 --f0=$freq --fSearchBand=$freqband --nfSizeCylinder=45 --timeStampsFile=./ts-inject-S4_Multi.txt  --skyfile=./skypulsar.8 --weighAM=1 --weighNoise=1 --printMaps=1 --printStats=1 --earthEphemeris=./earth05-09.dat --sunEphemeris=./sun05-09.dat --sftDir=$sftdir --dirnameOut=${tempoutdir}/Multi.P8 --fbasenameOut=freq_${freq}_  --linefiles=./S4lines_H1_xavi.txt.v2,./S4lines_L1_xavi.txt.v2,./S4lines_H2.txt --printLog

echo finished running driver

# now copy the results to the home directory
mkdir -p ${workdir}/Multi.P8
rsync -a ${tempoutdir}/Multi.P8 $workdir
