...
/******************************************/
 ...
 for (i = 1; i < argc; i++) {
	...
	switch(i) {
		...
		case 40: params->orientationAngle = (REAL8)atof(argv[i]); break;
  ...
  params->numSUMsTotal = params->numSUMsPerParamSpacePt*params->numParamSpacePts;  /* Total Number of Sums = numSUMsPerParamPt*numParamSpacePts */
   ...
   if ((params->debugOptionFlag & 1) > 0 ) {
    	...
    	fprintf(stdout,"\n");
    	fprintf(stdout,"set weightFlag         %23d; #39 INT2 how to weight STKs. \n", params->weightFlag);
    	...
    	/* fprintf(stdout,"# Note that < 0 and 0 are only options currently supported. \n"); */ /* 10/28/04 gam */
    	fprintf(stdout,"set orientationAngle   %23.16e; #40 REAL8 orientation angle in radians used to find F_+ and F_x when used to weight STKs or if using fixed polarization for a Monte Carlo Simulation. \n", params->orientationAngle);
    	fprintf(stdout,"\n");
...
/*****************************************/
   ...
   /* if (params->finishedSUMs) */ /* 05/26/04 gam */
         ...
         /* Deallocate memory for the SUMStats structure */ /* 02/11/04 gam; comment out */
	 /* LALFree(params->SUMStats); */

         /* Deallocate memory for the SUMData structure */
         /* for(i=0;i<params->numSUMsTotal;i++)
         {
	     LALFree(params->SUMData[i]->data->data);
	     LALFree(params->SUMData[i]->data);
	     LALFree(params->SUMData[i]);
         }
