...
int main( int argc, char **argv ){
  ...
  EmissionTime emit, emit2, emit3, emitS, emitE;
  ...
  /* Set finish time to infinity for comparisons */
  /* fmaxf treats INFTY as NaN, says 0 > INFTY   */
  ...
  while(segcount<numSegs){
    ...
    for (sftnum=0; sftnum<(SFTdat->length); sftnum++){
      ...
      for (h=0; h<numpulsars;h++){
        ...
        INT4 datapoint=0;
        EarthState earth, earth2, earth3;
        REAL8 phaseShift = 0., fnew = 0., f1new = 0., sqrtf1new = 0., f2new = 0.;
        REAL8 tdt = 0., tdt2 = 0.;
        ...
        if( pulparams[h].model!=empty_BinaryPulsarParams.model ){
          EarthState earthS, earthE;
          BinaryPulsarInput binInputS, binInputM, binInputE;
          BinaryPulsarOutput binOutputS, binOutputM, binOutputE;
          BarycenterInput baryInputS, baryInputE;

          baryInputS = baryInputE = baryInput;
          ...
          emit.deltaT = emit.deltaT+binOutputM.deltaT;
          emit.tDot = emit.tDot+(binOutputE.deltaT - binOutputS.deltaT)*deltaf;
          binTdDot = (2*binOutputM.deltaT-binOutputE.deltaT - binOutputS.deltaT)*(deltaf*deltaf)/4;
        }

        /* Calculate relevant time difference to epoch for use in calculations */

        tdt=timestamp - pulparams[h].pepoch + 1./(2.*deltaf) + emit.deltaT;
        ...
        fnew=inputParams.freqfactor*emit.tDot*(
               pulparams[h].f0
               + pulparams[h].f1*tdt
        ...
        /* Work out d^2(emit.deltaT)/dt^2  - the second derivative of barycenter time difference */

        LIGOTimeGPS upper=empty_LIGOTimeGPS, lower = empty_LIGOTimeGPS;

        XLALGPSSetREAL8( &upper, XLALGPSGetREAL8(&baryInput.tgps) +  1./(2.*deltaf) );
        XLALGPSSetREAL8( &lower, XLALGPSGetREAL8(&baryInput.tgps) -  1./(2.*deltaf) );

        XLALBarycenterEarthNew(&earth2, &upper, edat, tdat, ttype);
        XLALBarycenterEarthNew(&earth3, &lower, edat, tdat, ttype);

        XLALBarycenter(&emit2, &baryInput, &earth2);
        XLALBarycenter(&emit3, &baryInput, &earth3);

        tDdot=(emit3.tDot-emit2.tDot)*deltaf + binTdDot;
        if (inputParams.baryFlag){
          tDdot=binTdDot;
        }


        /* calculate f2 at the centre of the SFT, so it can be ignored if over threshold */
        f2new=inputParams.freqfactor*(
               emit.tDot*emit.tDot*emit.tDot*(pulparams[h].f2
               + pulparams[h].f3*tdt
               + (1./2.)*pulparams[h].f4*tdt2
               + (1./6.)*pulparams[h].f5*tdt2*tdt
               + (1./24.)*pulparams[h].f6*tdt2*tdt2
               + (1./120.)*pulparams[h].f7*tdt2*tdt2*tdt
               + (1./720.)*pulparams[h].f8*tdt2*tdt2*tdt2
               + (1./5040.)*pulparams[h].f9*tdt2*tdt2*tdt2*tdt)
               + 6*tDdot*emit.tDot*(pulparams[h].f1
               + pulparams[h].f2*tdt
               + (1./2.)*pulparams[h].f3*tdt2
               + (1./6.)*pulparams[h].f4*tdt*tdt2
               + (1./24.)*pulparams[h].f5*tdt2*tdt2
               + (1./120.)*pulparams[h].f6*tdt2*tdt2*tdt
               + (1./720.)*pulparams[h].f7*tdt2*tdt2*tdt2
               + (1./5040.)*pulparams[h].f8*tdt2*tdt2*tdt2*tdt
               + (1./40320.)*pulparams[h].f9*tdt2*tdt2*tdt2*tdt2));

        if(fabs(f2new)/(deltaf*deltaf*deltaf)>0.1){
          fprintf(stderr,"Pulsar %s has frequency second derivative %.4e outside of the permissible range of +/- %.4e at time %.0f\n",
              psrname, f2new, 0.1*(deltaf*deltaf*deltaf) , timestamp);
          continue;
        }
        /* calculate difference in phase between beginning of SFT and the epoch */
        /* fmod means that phaseShift is always between 0 and 2*LAL_PI          */
        ...
        f1new=inputParams.freqfactor*(
               emit.tDot*emit.tDot*(pulparams[h].f1
               + pulparams[h].f2*tdt
               + (1./2.)*pulparams[h].f3*tdt2
               + (1./6.)*pulparams[h].f4*tdt2*tdt
               + (1./24.)*pulparams[h].f5*tdt2*tdt2
               + (1./120.)*pulparams[h].f6*tdt2*tdt2*tdt
               + (1./720.)*pulparams[h].f7*tdt2*tdt2*tdt2
               + (1./5040.)*pulparams[h].f8*tdt2*tdt2*tdt2*tdt
               + (1./40320.)*pulparams[h].f9*tdt2*tdt2*tdt2*tdt2)
               + tDdot*(pulparams[h].f0
               + pulparams[h].f1*tdt
               + (1./2.)*pulparams[h].f2*tdt2
               + (1./6.)*pulparams[h].f3*tdt*tdt2
               + (1./24.)*pulparams[h].f4*tdt2*tdt2
        ...
        sqrtf1new = sqrt(fabs(f1new)); /* Calculate now instead of repeatedly within the datapoint loop */
        REAL8 signf1new = f1new/fabs(f1new);

        UINT4 dataLength = 0, dpNum = 0;

        /* Load data straight away */
        ...
        /* While doing this, add the square of reach in order to obtain the std deviation of the data */
        ...
        /* Data has now been loaded into ReDp/ImDp and dpUsed set to initial values, with initial outliers removed */
        ...
        /* Data loaded, Model Set and first outlier removal performed - now calculate complex amplitude */
        /* and from that, perform residual outlier removal */
        ...
        UINT4 numReduced = 1; /* i.e. the number of datapoints has been reduced */
...
pulsar spin frequency */
  ...
  inputParams->minSegLength = 5400; /* Default minimum segment length is 5400s */
  ...
  inputParams->cacheDir=0.;
  inputParams->cacheFile=0.;
...
/* Function to compute the Fresnel Integrals, taken from Numerical Recipes in C (1992)*/
