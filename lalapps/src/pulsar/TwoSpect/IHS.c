...
ihsMaximaStruct * new_ihsMaxima(INT4 fbins, INT4 rows)
{
   ...
   //INT4 ii;
   //INT4 numberofmaxima = 0;
   //for (ii=2; ii<=rows; ii++) numberofmaxima += fbins - (ii-1);
...
void runIHS(ihsMaximaStruct *output, ffdataStruct *input, inputParamsStruct *params, INT4 rows, REAL4Vector *FbinMean)
{
   ...
   REAL4Vector *ihsvector = XLALCreateREAL4Vector((INT4)floor(0.2*numfprbins)-5);
   ...
   if (row==NULL) {
      ...
      fprintf(stderr,"%s: XLALCreateREAL4Vector(%d) failed.\n", fn, (INT4)floor(0.2*numfprbins)-5);
      XLAL_ERROR_VOID(fn, XLAL_EFUNC);
   } else if (locs==NULL) {
      fprintf(stderr,"%s: XLALCreateINT4Vector(%d) failed.\n", fn, numfbins);
      XLAL_ERROR_VOID(fn, XLAL_EFUNC);
   ...
   for (ii=0; ii<(INT4)ihss->length; ii++) {
      ...
      for (jj=0; jj<(INT4)row->length; jj++) {
         if (fabs(params->Tobs/(24.0*3600.0)-jj)<=1.0 || fabs(params->Tobs/(12.0*3600.0)-jj)<=1.0 || fabs(params->Tobs/(8.0*3600.0)-jj)<=1.0 || fabs(params->Tobs/(6.0*3600.0)-jj)<=1.0) row->data[jj] = 0.0;
      }
      
      //Run the IHS algorithm on the row
      incHarmSumVector(ihsvector, row);
      if (xlalErrno!=0) {
...
void incHarmSum(ihsVals *output, REAL4Vector *input)
{
   ...
   REAL4Vector *tempvect = XLALCreateREAL4Vector((INT4)floor(0.2*input->length)-5);
   if (tempvect==NULL) {
      fprintf(stderr, "%s: XLALCreateREAL4Vector(%d) failed.\n", fn, (INT4)floor(0.2*input->length)-5);
      XLAL_ERROR_VOID(fn, XLAL_EFUNC);
   }
   incHarmSumVector(tempvect, input);
   if (xlalErrno!=0) {
      fprintf(stderr, "%s: incHarmSumVector() failed.\n", fn);
      XLAL_ERROR_VOID(fn, XLAL_EFUNC);
   }
   for (ii=0; ii<(INT4)tempvect->length; ii++) {
      ...
      if (output->ihs<tempvect->data[ii]) {
...
void incHarmSumVector(REAL4Vector *output, REAL4Vector *input)
{
   ...
   for (ii=5; ii<(INT4)floor(0.2*input->length); ii++) output->data[ii-5] = (REAL4)(input->data[ii] + input->data[ii*2] + input->data[ii*3] + input->data[ii*4] + input->data[ii*5]);
   if (5*(ii-1)>(INT4)input->length-1) {
...
void genIhsFar(ihsfarStruct *output, inputParamsStruct *params, INT4 rows, REAL4Vector *aveNoise)
{
   ...
   INT4 trials = (INT4)round(10000*0.0001/params->ihsfar);    //Number of trials to determine FAR value
   if (params->ihsfomfar!=0.0 && trials<(INT4)round(10000*0.0001/params->ihsfomfar)) {
      trials = (INT4)round(10000*0.0001/params->ihsfomfar);
   }
   trials += rows;
   ...
   REAL4Vector *ihsvector = XLALCreateREAL4Vector((INT4)floor(0.2*aveNoise->length)-5);
   ...
   if (noise==NULL) {
      ...
      fprintf(stderr, "%s: XLALCreateREAL4Vector(%d) failed.\n", fn, (INT4)floor(0.2*aveNoise->length)-5);
      XLAL_ERROR_VOID(fn, XLAL_EFUNC);
   ...
   for (ii=0; ii<trials; ii++) {
      ...
      incHarmSumVector(ihsvector, noise);
      if (xlalErrno!=0) {
         fprintf(stderr, "%s: incHarmSumVector() failed.\n", fn);
         XLAL_ERROR_VOID(fn, XLAL_EFUNC);
      }
...
void ihsSums2_withFAR(ihsMaximaStruct *output, ihsfarStruct *outputfar, REAL4VectorSequence *ihsvectorsequence, REAL4Vector *ihss, INT4Vector *locs, REAL4Vector *aveNoise, INT4 rows, REAL4Vector *FbinMean, INT4 locationnormfactor, inputParamsStruct *params, INT4 calcPInvVals)
{
   ...
   if (calcPInvVals!=0) {
      ...
      for (ii=0; ii<(INT4)aveNoise->length; ii++) {
         if (fabs(params->Tobs/(24.0*3600.0)-ii)<=1.0 || fabs(params->Tobs/(12.0*3600.0)-ii)<=1.0 || fabs(params->Tobs/(8.0*3600.0)-ii)<=1.0 || fabs(params->Tobs/(6.0*3600.0)-ii)<=1.0) tempaveNoise->data[ii] = 0.0;
         else tempaveNoise->data[ii] = aveNoise->data[ii];
      }
      incHarmSumVector(ihsvalsfromaveNoise, tempaveNoise);
      if (xlalErrno!=0) {
         fprintf(stderr, "%s: incHarmSumVector() failed.\n", fn);
         XLAL_ERROR_VOID(fn, XLAL_EFUNC);
      }
   ...
   for (ii=1; ii<=rows; ii++) {
      if (ii==1) {
         ...
         if (calcPInvVals!=0) {
            ...
            REAL4Vector *sampledtempihsvals = sampleREAL4Vector(tempihsvals, 10000);
         ...
         for (jj=0; jj<(INT4)ihsvectorsequence->length-(ii-1); jj++) {
            ...
            memcpy(singlerow->data, &(tworows->data[jj*ihsvectorsequence->vectorLength]), sizeof(REAL4)*ihsvectorsequence->vectorLength);
            output->locations->data[(ii-2)*ihss->length-((ii-1)*(ii-1)-(ii-1))/2+jj] = max_index(singlerow) + 5;
            output->maxima->data[(ii-2)*ihss->length-((ii-1)*(ii-1)-(ii-1))/2+jj] = singlerow->data[output->locations->data[(ii-2)*ihss->length-((ii-1)*(ii-1)-(ii-1))/2+jj]-5];
            
            if (calcPInvVals!=0) for (kk=0; kk<(INT4)ihsvalsfromaveNoise->length; kk++) tworows2->data[jj*ihsvalsfromaveNoise->length + kk] = tworows2->data[jj*ihsvalsfromaveNoise->length + kk] + ihsvalsfromaveNoise->data[kk]*randvals->data[ii-1+jj];
            
            memcpy(rowsequencemaxima->data, &(ihss->data[jj]), sizeof(REAL4)*ii);
            memcpy(fbinmeanvals->data, &(FbinMean->data[jj]), sizeof(REAL4)*ii);
            memcpy(rowsequencelocs->data, &(locs->data[jj]), sizeof(INT4)*ii);
            output->foms->data[(ii-2)*ihss->length-((ii-1)*(ii-1)-(ii-1))/2+jj] = ihsFOM(rowsequencemaxima, rowsequencelocs, fbinmeanvals, locationnormfactor);
         } /* for jj< ihsvectorsequence->length - (ii-1) */
         ...
         if (calcPInvVals!=0) {
            ...
            memcpy(tempfomvals->data, &(output->foms->data[(ii-2)*ihss->length-((ii-1)*(ii-1)-(ii-1))/2]), sizeof(REAL4)*tempfomvals->length);
            outputfar->ihsfomdistMean->data[ii-2] = calcMean(tempfomvals);
            outputfar->ihsfomdistSigma->data[ii-2] = calcStddev(tempfomvals);
            if (params->ihsfomfar!=1.0 && params->ihsfom==0.0) {
               REAL4Vector *smallestfomvals = XLALCreateREAL4Vector((UINT4)roundf((ihss->length-ii+1)*params->ihsfomfar)+1);
      ...
      for (jj=0; jj<(INT4)numfbins-(ii-1); jj++) {
         ...
         if (ihsmaxima->maxima->data[locationinmaximastruct] > ihsfarstruct->ihsfar->data[ii-2]*meanNoise) {
            ...
            if (ihsfarstruct->fomfarthresh->data[ii-2]==-1.0 || ihsmaxima->foms->data[locationinmaximastruct]<=ihsfarstruct->fomfarthresh->data[ii-2]) {
               /* memcpy(ihss->data, &(ihsmaxima->maximaForEachFbin->data[jj]), sizeof(REAL4)*ii);
               memcpy(locs->data, &(ihsmaxima->locations->data[jj]), sizeof(INT4)*ii);
...
REAL8 ihs2h0(REAL8 ihsval, INT4 location, INT4 lowestfrequencybin, INT4 rows, inputParamsStruct *params, REAL4Vector *aveNoise, REAL4Vector *fbinavgs)
{
   ...
   INT4 ii;
   for (ii=1; ii<=5; ii++) if (fabs(params->Tobs/(24.0*3600.0)-ii*location)<=1.0 || fabs(params->Tobs/(12.0*3600.0)-ii*location)<=1.0 || fabs(params->Tobs/(8.0*3600.0)-ii*location)<=1.0 || fabs(params->Tobs/(6.0*3600.0)-ii*location)<=1.0) noise -= aveNoise->data[location*ii];
   REAL8 totalnoise = 0.0;
   for (ii=0; ii<rows; ii++) totalnoise += noise*fbinavgs->data[lowestfrequencybin+ii];
   //REAL8 h0 = 4.25*pow((ihsval-totalnoise)/(params->Tcoh*params->Tobs),0.25);
