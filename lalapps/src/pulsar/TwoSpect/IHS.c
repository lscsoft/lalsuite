      ...
      if (ii==2) {
         ...
         if ((ihsvectorsequence->length-(ii-1))*ihsvectorsequence->vectorLength>10000) {
            ...
            for (jj=0; jj<(INT4)sampledtempihsvals->length; jj++) {
               averageval += 1.0;
               if (params->ihsfar != 1.0 && !params->fastchisqinv) farave += gsl_cdf_chisq_Qinv(params->ihsfar, 0.5*sampledtempihsvals->data[jj]) + 0.5*sampledtempihsvals->data[jj];
               if (params->ihsfar != 1.0 && params->fastchisqinv) farave += cdf_chisq_Qinv(params->ihsfar, 0.5*sampledtempihsvals->data[jj]) + 0.5*sampledtempihsvals->data[jj];
               if (xlalErrno!=0) {
...
void sseSSVectorSequenceSum(REAL4VectorSequence *output, REAL4VectorSequence *input1, REAL4VectorSequence *input2, INT4 vectorpos1, INT4 vectorpos2, INT4 outputvectorpos, INT4 numvectors)
{
   ...
   for (ii=0; ii<numvectors; ii++) {
      ...
      // currently unused: INT4 input1vecaligned = 0;
      INT4 outputvecaligned = 0;
      // currently unused: INT4 input2vecaligned = 0;
      __m128 *arr1, *arr2, *result;
      if ( &(input1->data[vec1])==(void*)(((UINT8)&(input1->data[vec1])+15) & ~15) ) {
        // currently unused: input1vecaligned = 1;
      ...
      if ( &(input2->data[vec2])==(void*)(((UINT8)&(input2->data[vec2])+15) & ~15) ) {
        // currently unused: input2vecaligned = 1;
