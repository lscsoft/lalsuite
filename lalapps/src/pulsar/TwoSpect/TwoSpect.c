...
int main(int argc, char *argv[])
{
   ...
   for (ii=0; ii<numffts; ii++) {
      for (jj=0; jj<numfbins+2*inputParams->maxbinshift; jj++) usableTFdata->data[ii*(numfbins+2*inputParams->maxbinshift) + jj] = tfdata->data[ii*(tempnumfbins+2*inputParams->maxbinshift) + jj + (INT4)round(0.5*(inputParams->blksize-1))];
   }
...
ffdataStruct * new_ffdata(inputParamsStruct *param)
{
   ...
   UINT4 numfbins = (UINT4)(round(param->fspan*param->Tcoh)+1);
...
void ffPlaneNoise(REAL8Vector *aveNoise, inputParamsStruct *param, REAL8Vector *backgrnd, REAL8Vector *antweights, REAL8 *normalization)
{
   ...
   for (ii=0; ii<1000; ii++) {
      ...
      for (jj=0; jj<(INT4)aveNoise->length; jj++) aveNoise->data[jj] += psd->data[jj];
   }
   
   //Average
   for (ii=0; ii<(INT4)aveNoise->length; ii++) aveNoise->data[ii] *= 1.0e-3*psdfactor;
