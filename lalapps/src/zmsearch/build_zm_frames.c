...
int main(int argc, char **argv)
{
  ...
  float n[80];
  float dt=1000.0/16384.0;            /*time diff in ms.=0.2441ms*/
  float *time, *amp;
  char filename[10], outputfile[30];   
  float t[80][4000],a[80][4000], h[80][4000];  
  float x; 
  float h_intpolat[80][7000], t_intpolat[80][7000];
  ...
  zmseries.deltaT = dt/1000.0;
  printf("%e\n",zmseries.deltaT);
  zmseries.epoch.gpsSeconds = 0;
  zmseries.epoch.gpsNanoSeconds = 0;
  snprintf( zmseries.name, LALNameLength * sizeof(CHAR), "ZMSIM" );
   ...
   ************************************************************/
  for(i=0;i<78;i++){
    snprintf(outputfile,30,"ZM_WAVEFORM_%0d",i);
