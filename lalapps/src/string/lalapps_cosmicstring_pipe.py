...
def parse_command_line():
    ...
    parser.add_option(
        ...
        metavar="filename",
        action="append",
        help="Set the name of the file from which to obtain the time slide table for use in the background branch of the pipeline (required).  This option can be given multiple times to parallelize the background analysis across time slides.  You will want to make sure the time slide files have distinct vectors to not repeat the same analysis multiple times, and in particular you'll want to make sure only one of them has a zero-lag vector in it.",
    )
    parser.add_option(
        "--injection-time-slides",
        metavar="filename",
        help="Set the name of the file from which to obtain the time slide table for use in the injection branch of the pipeline (required).",
    )
    parser.add_option(
        "--segments-file",
        metavar="filename",
        help="Set the name of the LIGO Light-Weight XML file from which to obtain segment lists (required).  See ligolw_segments and ligolw_segment_query for more information on constructing an XML-format segments file.  See also --segments-name.",
    )
    parser.add_option(
        "--segments-name",
        metavar="name",
        default="segments",
        help='Set the name of the segment lists to retrieve from the segments file (default = "segments").  See also --segments-file.',
    )
    parser.add_option(
        "--vetoes-file",
        metavar="filename",
        help="Set the name of the LIGO Light-Weight XML file from which to obtain veto segment lists (optional).  See ligolw_segments and ligolw_segment_query for more information on constructing an XML-format segments file.  See also --vetos-name.",
    )
    parser.add_option(
        "--vetoes-name",
        metavar="name",
        default="vetoes",
        help='Set the name of the segment lists to retrieve from the veto segments file (default = "vetoes").  See also --vetoes-file.',
    )
    ...
    if missing_options:
        raise ValueError(
            ...
            % ", ".join(
                sorted("--%s" % option.replace("_", "-") for option in missing_options)
            )
        )
...
power.init_job_types(
    config_parser,
    job_types=("datafind", "binj", "lladd", "binjfind", "burca", "sqlite"),
)
cosmicstring.init_job_types(
    config_parser,
    job_types=("string", "meas_likelihood", "calc_likelihood", "runsqlite"),
)
...
short_segment_duration = config_parser.getint(
    "lalapps_StringSearch", "short-segment-duration"
)
pad = config_parser.getint("lalapps_StringSearch", "pad")
min_segment_length = config_parser.getint(
    "pipeline", "segment-length"
)  # not including pad at each end
...
seglists = ligolw_segments.segmenttable_get_by_name(
    ligolw_utils.load_filename(options.segments_file, verbose=options.verbose),
...
for instrument in set(seglists) - instruments:
    if options.verbose:
        print(
            "warning: ignoring segments for '%s' found in '%s'"
...
if options.verbose:
    print(
        "Computing segments for which lalapps_StringSearch jobs are required ...",
        file=sys.stderr,
    )
...
for filename in options.background_time_slides:
    cache_entry = CacheEntry(
        None, "BG", None, "file://localhost" + os.path.abspath(filename)
    )
    ...
    for i in range(len(background_time_slides[cache_entry])):
        background_time_slides[cache_entry][i] = offsetvector.offsetvector(
            (instrument, LIGOTimeGPS(offset))
            for instrument, offset in background_time_slides[cache_entry][i].items()
        )
    background_seglists |= cosmicstring.compute_segment_lists(
        seglists,
        offsetvector.component_offsetvectors(background_time_slides[cache_entry], 2),
        min_segment_length,
        pad,
    )
...
for filename in options.injection_time_slides:
    cache_entry = CacheEntry(
        None, "INJ", None, "file://localhost" + os.path.abspath(filename)
    )

    injection_time_slides[cache_entry] = (
        lsctables.TimeSlideTable.get_table(
            ligolw_utils.load_filename(filename, verbose=options.verbose)
        )
        .as_dict()
    ...
    for i in range(len(injection_time_slides[cache_entry])):
        injection_time_slides[cache_entry][i] = offsetvector.offsetvector(
            (instrument, LIGOTimeGPS(offset))
            for instrument, offset in injection_time_slides[cache_entry][i].items()
        )
    injection_seglists |= cosmicstring.compute_segment_lists(
        seglists,
        offsetvector.component_offsetvectors(injection_time_slides[cache_entry], 2),
        min_segment_length,
        pad,
    )
...
def check_for_reused_offsetvectors(background_time_slides, injection_time_slides):
    ...
    for background_cache_entry, background_offsetvector in [
        (cache_entry, offsetvector)
        for cache_entry, offsetvectors in background_time_slides.items()
        for offsetvector in offsetvectors
    ]:
        for injection_cache_entry, injection_offsetvector in [
            (cache_entry, offsetvector)
            for cache_entry, offsetvectors in injection_time_slides.items()
            for offsetvector in offsetvectors
        ]:
            if background_offsetvector.deltas == injection_offsetvector.deltas:
                raise ValueError(
                    "injections offset vector %s from %s is the same as non-injections offset vector %s from %s.  to avoid a self-selection bias, injections must not be performed at the same relative time shifts as a non-injection run"
                    % (
                        str(injection_offsetvector),
                        injection_cache_entry.url,
                        str(background_offsetvector),
                        background_cache_entry.url,
                    )
                )
...
datafinds = power.make_datafind_stage(
    dag, injection_seglists | background_seglists, verbose=options.verbose
)
...
def make_coinc_branch(
    ...
    if do_injections:
        ...
        maxoffset = max(
            abs(offset)
            for offsetvectorlist in time_slides.values()
        ...
        for seg in (
            seglists.union(seglists)
            .protract(power.binjjob.time_step + maxoffset)
            .coalesce()
            .contract(power.binjjob.time_step + maxoffset)
        ):
    ...
    coinc_nodes = []
    for n, (time_slides_cache_entry, these_time_slides) in enumerate(
        time_slides.items()
    ):
        if verbose:
            print(
        ...
        for segnum, (seg, parents, cache, clipseg) in enumerate(
            power.group_coinc_parents(
                trigger_nodes,
                these_time_slides,
                extentlimit=150000000.0 / (len(these_time_slides) or 1),
                verbose=verbose,
            )
        ):
            binj_cache = set(
            ...
            these_lladd_nodes = power.make_lladd_fragment(
                ...
                preserve_cache=binj_cache | segments_cache | tisi_cache | vetoes_cache,
            )
        ...
        coinc_nodes[-1] |= power.make_burca_fragment(
            dag, lladd_nodes, "%s_%d" % (tag, n), verbose=verbose
        )
        if verbose:
    ...
    if do_injections:
        ...
        coinc_nodes = [
            power.make_binjfind_fragment(
                dag, these_coinc_nodes, "%s_%d" % (tag, n), verbose=verbose
            )
    ...
    coinc_nodes = [
        power.make_sqlite_fragment(
            dag, these_coinc_nodes, "%s_%d" % (tag, n), verbose=verbose
        )
        for n, these_coinc_nodes in enumerate(coinc_nodes)
    ]
    coinc_nodes = [
        cosmicstring.make_run_sqlite_fragment(
            dag, these_coinc_nodes, "%s_%d" % (tag, n), clipsegments_sql_filename
        )
        for n, these_coinc_nodes in enumerate(coinc_nodes)
    ]
    ...
    likelihood_nodes = [
        cosmicstring.make_meas_likelihood_fragment(
            dag, these_coinc_nodes, "%s_%d" % (tag, n)
        )
        for n, these_coinc_nodes in enumerate(coinc_nodes)
    ]
    ...
    for n, (these_coinc_nodes, these_likelihood_nodes) in enumerate(
        ...
        power.write_output_cache(
            these_coinc_nodes | these_likelihood_nodes,
            "%s_%s_output.cache"
...
for i, folder in enumerate(injection_folders):
    ...
    [these_coinc_nodes], [these_likelihood_nodes] = make_coinc_branch(
        dag,
        datafinds,
        injection_seglists,
        injection_time_slides,
        ...
        short_segment_duration,
        "%s_INJ_%d" % (user_tag, i),
        vetoes_cache=options.vetoes_cache,
        do_injections=True,
        injections_offset=float(i) / len(injection_folders),
...
for i, folder in enumerate(noninjection_folders):
    ...
    cwd = os.getcwd()
    os.chdir(folder)
    background_coinc_nodes, background_likelihood_nodes = make_coinc_branch(
        dag,
        datafinds,
        background_seglists,
        background_time_slides,
...
def round_robin_and_flatten(
    injection_coinc_node_groups, injection_likelihood_node_groups
):
    # round-robin the node lists
    A = list(itertools.combinations(injection_coinc_node_groups, 1))
    B = list(
        itertools.combinations(
            injection_likelihood_node_groups, len(injection_likelihood_node_groups) - 1
        )
    )
    B.reverse()
...
coinc_nodes = set()
for n, (these_inj_coinc_nodes, these_inj_likelihood_nodes) in enumerate(
    round_robin_and_flatten(injection_coinc_nodes, injection_likelihood_nodes)
):
    these_inj_coinc_nodes = cosmicstring.make_calc_likelihood_fragment(
        dag,
        these_inj_coinc_nodes,
        these_inj_likelihood_nodes | all_background_likelihood_nodes,
        "%s_INJ_%d" % (user_tag, n),
        verbose=options.verbose,
    )
...
coinc_nodes |= cosmicstring.make_calc_likelihood_fragment(
    dag,
    flatten_node_groups(background_coinc_nodes),
    all_injection_likelihood_nodes | all_background_likelihood_nodes,
    user_tag,
