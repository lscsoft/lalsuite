...
copyreg.pickle(
    lal.LIGOTimeGPS, lambda gps: (lal.LIGOTimeGPS, (gps.gpsSeconds, gps.gpsNanoSeconds))
)
...
def parse_command_line():
    ...
    parser.add_option(
        ...
        metavar="fraction",
        type="float",
        help="Set the fractional uncertainty in amplitude due to calibration uncertainty (eg. 0.08).  This option is required, use 0 to disable calibration uncertainty.",
    )
    parser.add_option(
        "--injections-bin-size",
        metavar="bins",
        type="float",
        default=16.7,
        help="Set bin width for injection efficiency curves (default = 16.7).",
    )
    parser.add_option(
        "-c",
        "--input-cache",
        metavar="filename",
        action="append",
        help="Also process the files named in this LAL cache.  See lalapps_path2cache for information on how to produce a LAL cache file.  This option can be given multiple times.",
    )
    parser.add_option(
        "--import-dump",
        metavar="filename",
        action="append",
        help="Import additional rate vs. threshold or efficiency data from this dump file.  Dump files are one of the data products produced by this program.  Whether the file provides rate vs. threshold data or efficiency data will be determined automatically.  This option can be given multiple times",
    )
    parser.add_option(
        "--image-formats",
        metavar="ext[,ext,...]",
        default="png,pdf",
        help='Set list of graphics formats to produce by providing a comma-delimited list of the filename extensions (default = "png,pdf").',
    )
    parser.add_option(
        "-p",
        "--live-time-program",
    ...
    parser.add_option(
        "--vetoes-name",
        metavar="name",
        help="Set the name of the segment lists to use as vetoes (default = do not apply vetoes).",
    )
    parser.add_option(
        "--detection-threshold",
        metavar="log likelihood ratio",
        type="float",
        help="Override the detection threshold.  Only injection files will be processed, and the efficiency curve measured.",
    )
    parser.add_option(
        "--record-background",
        metavar="N",
        type="int",
        default=10000000,
        help="Set the number of background log likelihood ratios to hold in memory for producing the rate vs. threshold plot (default = 10000000).",
    )
    parser.add_option(
        "--record-candidates",
        metavar="N",
        type="int",
        default=100,
        help="Set the number of highest-ranked zero-lag candidates to dump to the candidate file (default = 100).",
    )
    parser.add_option(
        "--threads",
        metavar="N",
        type="int",
        default=1,
        help="Set the maximum number of parallel threads to use for processing files (default = 1).  Contention for the global Python interpreter lock will throttle the true number that can run.  The number of threads will be automatically adjusted downwards if the number requested exceeds the number of input files.",
    )
    ...
    if options.input_cache:
        filenames += [
            CacheEntry(line).path
            for input_cache in options.input_cache
            for line in file(input_cache)
        ]
...
def ratevsthresh_bounds(background_x, background_y, zero_lag_x, zero_lag_y):
    if len(zero_lag_x):
        if len(background_x):
            minX, maxX = min(min(zero_lag_x), min(background_x)), max(
                max(zero_lag_x), max(background_x)
            )
            minY, maxY = min(min(zero_lag_y), min(background_y)), max(
                max(zero_lag_y), max(background_y)
            )
        else:
    ...
    if len(zero_lag_x):
        if len(background_x):
            maxY = max(
                zero_lag_y[bisect.bisect_left(zero_lag_x, minX)],
                background_y[bisect.bisect_left(background_x, minX)],
            )
        else:
...
class RateVsThreshold(object):
    ...
    def __iadd__(self, other):
        ...
        self.background[:] = heapq.nlargest(
            self.record_background, itertools.chain(self.background, other.background)
        )
        self.most_significant_background[:] = heapq.nlargest(
            self.record_candidates,
            itertools.chain(
                self.most_significant_background, other.most_significant_background
            ),
        )
        self.candidates[:] = heapq.nlargest(
            self.record_candidates, itertools.chain(self.candidates, other.candidates)
        )
    ...
    def add_contents(self, contents, verbose=False):
        ...
        self.zero_lag_time += stringutils.time_slides_livetime(
            seglists,
            zero_lag_time_slides.values(),
            2,
            clip=contents.coincidence_segments,
        )
        self.background_time += stringutils.time_slides_livetime(
            seglists,
            background_time_slides.values(),
            2,
            clip=contents.coincidence_segments,
        )
        if set(("H1", "H2")).issubset(set(contents.seglists)):
            ...
            self.zero_lag_time -= stringutils.time_slides_livetime_for_instrument_combo(
                seglists,
                zero_lag_time_slides.values(),
                ("H1", "H2"),
                clip=contents.coincidence_segments,
            ...
            self.background_time -= (
                stringutils.time_slides_livetime_for_instrument_combo(
                    seglists,
                    background_time_slides.values(),
                    ("H1", "H2"),
                    clip=contents.coincidence_segments,
...
WHERE
	coinc_event.coinc_def_id == ?
		""",
            ...
            record = (
                ...
                contents.filename,
                coinc_event_id,
                dbtables.lsctables.instrumentsproperty.get(instruments),
                peak_time,
            )
    ...
    def finish(self):
        ...
        for (
            ...
            instruments,
            peak_time,
        ) in self.most_significant_background:
            print(file=f)
        ...
        # standard deviation
        background_y = (
            numpy.arange(len(background_x), 0.0, -1.0, dtype="double")
            / self.background_time
            * self.zero_lag_time
        )
        ...
        if self.open_box:
            minX, maxX, minY, maxY = ratevsthresh_bounds(
                background_x, background_y, self.zero_lag, zero_lag_y
            )
        else:
            minX, maxX, minY, maxY = ratevsthresh_bounds(
        ...
        background_x, background_y, background_yerr = compress_ratevsthresh_curve(
            background_x, background_y, background_yerr
        )
        ...
        numpy.savetxt(
            "string_rate_background.txt",
            numpy.transpose((background_x, background_y, background_yerr)),
        )
        ...
        ratefig, axes = SnglBurstUtils.make_burst_plot(
        ...
        poly_y = numpy.concatenate(
            (
                background_y + 1 * background_yerr,
                (background_y - 1 * background_yerr)[::-1],
            )
        )
        axes.add_patch(
            patches.Polygon(
                zip(poly_x, numpy.clip(poly_y, minY, maxY)),
                edgecolor="k",
                facecolor="k",
                alpha=0.3,
            )
        ...
        (line1,) = axes.semilogy(
            background_x.repeat(2)[:-1],
            background_y.repeat(2)[1:],
            color="k",
            linestyle="--",
        )
        if self.open_box:
            (line2,) = axes.semilogy(
                self.zero_lag.repeat(2)[:-1],
                zero_lag_y.repeat(2)[1:],
                color="k",
                linestyle="-",
                linewidth=2,
            )
            axes.legend(
                (line1, line2), (r"Expected background", r"Zero-lag"), loc="lower left"
            )
        else:
        ...
        residualfig, axes = SnglBurstUtils.make_burst_plot(
            r"Ranking Statistic Threshold, $\log \Lambda$",
            r"Event Count Residual / $\sqrt{N}$",
            width=108.0,
        )
        axes.set_position([0.125, 0.15, 0.83, 0.75])
        axes.xaxis.grid(True, which="major,minor")
        ...
        if self.open_box:
            (line1,) = axes.plot(
                self.zero_lag, zero_lag_residual, color="k", linestyle="-", linewidth=2
            )
...
def render_data_from_bins(
    ...
    yerr = numpy.sqrt(eff * (1.0 - eff) / efficiency_den.array)
    xerr = numpy.array([filter_width / 2.0] * len(yerr))
    ...
    net_yerr = numpy.sqrt(
        (xerr * dydx) ** 2.0
        + yerr**2.0
        + (cal_uncertainty / x_factor_per_sample * dydx) ** 2.0
    )
    ...
    patch = patches.Polygon(
        zip(
            ...
            numpy.concatenate(
                (
                    eff + upper_err(eff, yerr, filter_width / 2.0),
                    (eff - lower_err(eff, yerr, filter_width / 2.0))[::-1],
                )
            ),
        ),
        edgecolor=colour,
    ...
    print(
        "Bins were %g samples wide, ideal would have been %g"
        % (
            filter_width,
            (num_samples / num_injections / interpolate.interp1d(x, dydx)(A50) ** 2.0)
            ** (1.0 / 3.0),
        ),
        file=sys.stderr,
    )
    print(
        "Average number of injections in each bin = %g" % efficiency_den.array.mean(),
        file=sys.stderr,
    )
...
class Efficiency(object):
    def __init__(self, detection_threshold, cal_uncertainty, filter_width, open_box):
        ...
        self.found = []
        self.n_diagnostics = (
            100  # keep 100 loudest missed and quietest found injections
        )
        self.loudest_missed = []
        self.quietest_found = []
        self.all = []
    ...
    def __iadd__(self, other):
        ...
        self.loudest_missed[:] = heapq.nlargest(
            self.n_diagnostics,
            itertools.chain(self.loudest_missed, other.loudest_missed),
        )
        self.quietest_found[:] = heapq.nlargest(
            self.n_diagnostics,
            itertools.chain(self.quietest_found, other.quietest_found),
        )
        self.all += other.all
        return self
    ...
    def add_contents(self, contents, verbose=False):
        ...
        if set(("H1", "H2")).issubset(set(seglists)):
            ...
            seglists -= segments.segmentlistdict.fromkeys(
                seglists,
                seglists.intersection(("H1", "H2"))
                - seglists.union(set(seglists) - set(("H1", "H2"))),
            )
        ...
        stringutils.create_recovered_ln_likelihood_ratio_table(
            contents.connection, contents.bb_definer_id
        )
...
FROM
	...
	LEFT JOIN recovered_ln_likelihood_ratio ON (
		...
		"""):
            ...
            if (
                len(
                    SimBurstUtils.on_instruments(
                        sim, seglists, offsetvectors[sim.time_slide_id]
                    )
                )
                >= 2
            ):
                ...
                if found and ln_likelihood_ratio > self.detection_threshold:
                    ...
                    record = (
                        1.0 / sim.amplitude,
                        sim,
                        offsetvectors[sim.time_slide_id],
                        contents.filename,
                        ln_likelihood_ratio,
                    )
                ...
                print(
                    "%s: odd, injection %s was found but not injected ..."
                    % (contents.filename, sim.simulation_id),
                    file=sys.stderr,
                )
    ...
    def finish(self):
        ...
        bins = rate.NDBins(
            (
                rate.LogarithmicBins(
                    min(sim.amplitude for sim in self.all),
                    max(sim.amplitude for sim in self.all),
                    400,
                ),
            )
        )
        ...
        efficiency_den.array[
            (efficiency_num.array == 0) & (efficiency_den.array == 0)
        ] = 1

        line1, A50, A50_err = render_data_from_bins(
            file("string_efficiency.dat", "w"),
            axes,
            efficiency_num,
            efficiency_den,
            self.cal_uncertainty,
            self.filter_width,
            colour="k",
            linestyle="-",
            erroralpha=0.2,
        ...
        axes.legend(
            (line1,),
            (
                r"\noindent Injections recovered with $\log \Lambda > %.2f$"
        ...
        for (
            ...
            if ln_likelihood_ratio is None:
                ...
                print(
                    "Recovered with \\log \\Lambda = %.16g, detection threshold was %.16g"
            ...
            for instrument in self.seglists:
                ...
                print(
                    "\tInjected amplitude:\t%.16g"
                    % SimBurstUtils.string_amplitude_in_instrument(
                        sim, instrument, offsetvector
                    ),
                    file=f,
                )
                print(
                    "\tTime of injection:\t%s s"
            ...
            print(
                "Segments within 60 seconds:\t%s"
                % segmentsUtils.segmentlistdict_to_short_string(
                    self.seglists
            ...
            print(
                ...
                % segmentsUtils.segmentlistdict_to_short_string(
                    ...
                    & segments.segmentlistdict(
                        ...
                        for instrument in self.vetoseglists
                    )
                ),
                file=f,
            )
...
def process_file(
    ...
    working_filename = dbtables.get_connection_filename(
        filename, tmp_path=tmp_path, verbose=verbose
    )
    ...
    contents.coincidence_segments = ligolwprocess.get_process_params(
        contents.xmldoc, "lalburst_coinc", "--coincidence-segments"
    )
    if contents.coincidence_segments:
        ...
        contents.coincidence_segments = segments.segmentlistdict.fromkeys(
            contents.seglists,
            segmentsUtils.from_range_strings(
                contents.coincidence_segments.split(","),
                boundtype=dbtables.lsctables.LIGOTimeGPS,
            ).coalesce(),
        )
    else:
...
injection_filenames, noninjection_filenames = group_files(
    filenames, verbose=options.verbose
)
...
if options.import_dump:
    ...
    rate_vs_threshold, efficiency = import_dumps(
        options.import_dump, verbose=options.verbose
    )
...
if options.detection_threshold is None:
    ...
    for filenames in pack_files(
        noninjection_filenames,
        min(options.threads, len(noninjection_filenames)),
        ...
        if pid == 0:
            ...
            try:
                ...
                rate_vs_threshold = RateVsThreshold(
                    options.open_box,
                    record_background=options.record_background,
                    record_candidates=options.record_candidates,
                )
                process_files(
                    filenames,
                    [rate_vs_threshold],
    ...
    print(
        "Total time in zero-lag segments: %s s" % str(rate_vs_threshold.zero_lag_time),
        file=sys.stderr,
    )
    print("Time-slide events: %d" % rate_vs_threshold.n_background, file=sys.stderr)
    print(
        "Total time in time-slide segments: %s s"
        % str(rate_vs_threshold.background_time),
        file=sys.stderr,
    )
    if options.open_box:
        detection_threshold = rate_vs_threshold.zero_lag[-1]
        print(
            "Likelihood ratio for highest-ranked zero-lag survivor: %.9g"
            % detection_threshold,
            file=sys.stderr,
        )
    else:
        ...
        detection_threshold = sorted(rate_vs_threshold.background)[
            -int(
                round(
                    rate_vs_threshold.background_time / rate_vs_threshold.zero_lag_time
                )
            )
        ]
        print(
            "Simulated \\log \\Lambda for highest-ranked zero-lag survivor: %.9g"
            % detection_threshold,
            file=sys.stderr,
        )
else:
    ...
    print(
        "Likelihood ratio for highest-ranked zero-lag survivor from command line: %.9g"
        % detection_threshold,
        file=sys.stderr,
    )
...
for filenames in pack_files(
    injection_filenames,
    min(options.threads, len(injection_filenames)),
    verbose=options.verbose,
):
    # shuffle file names to avoid copying all the big files
    # into the scratch space at the same time
    ...
    if pid == 0:
        ...
        try:
            ...
            efficiency = Efficiency(
                detection_threshold,
                options.cal_uncertainty,
                options.injections_bin_size,
                options.open_box,
            )
            process_files(
                filenames,
...
if efficiency is not None:
    ...
    if options.verbose:
        print(
            "no injection data available, not writing efficiency data products.",
            file=sys.stderr,
        )
