...
def parse_command_line():
    ...
    parser.add_option(
        ...
        metavar="filename",
        action="append",
        help="Set the name of the likelihood ratio data file to use.  Can be given more than once.",
    )
    parser.add_option(
        "--likelihood-cache",
        metavar="filename",
        help="Also load the likelihood ratio data files listsed in this LAL cache.  See lalapps_path2cache for information on how to produce a LAL cache file.",
    )
    parser.add_option(
        "-t",
        "--tmp-space",
...
for n, filename in enumerate(filenames):
    ...
    calc_likelihood.assign_likelihood_ratios(
        ...
        events_func=lambda cursor, coinc_event_id: calc_likelihood.sngl_burst_events_func(
            cursor, coinc_event_id, contents.sngl_burst_table.row_from_cols
        ),
