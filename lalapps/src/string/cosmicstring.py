...
class MeasLikelihoodJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self,
            "vanilla",
            power.get_executable(config_parser, "lalapps_string_meas_likelihood"),
        )
...
class MeasLikelihoodNode(pipeline.CondorDAGNode):
    ...
    def set_output(self, description):
        ...
        filename = os.path.join(
            self.output_dir,
            "%s-STRING_LIKELIHOOD_%s-%d-%d.xml.gz"
            % (
                cache_entry.observatory,
                cache_entry.description,
                int(cache_entry.segment[0]),
                int(abs(cache_entry.segment)),
            ),
        )
...
class CalcLikelihoodJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self,
            "vanilla",
            power.get_executable(config_parser, "lalapps_string_calc_likelihood"),
        )
...
class CalcLikelihoodNode(pipeline.CondorDAGNode):
    ...
    def set_name(self, *args):
        ...
        self.cache_name = os.path.join(self.cache_dir, "%s.cache" % self.get_name())
        self.add_var_opt("input-cache", self.cache_name)
        self.likelihood_cache_name = os.path.join(
            self.cache_dir, "%s_likelihood.cache" % self.get_name()
        )
...
class StringJob(pipeline.CondorDAGJob, pipeline.AnalysisJob):
    ...
    def __init__(self, config_parser):
        ...
        pipeline.CondorDAGJob.__init__(
            self,
            power.get_universe(config_parser),
...
class StringNode(pipeline.CondorDAGNode, pipeline.AnalysisNode):
    ...
    def set_ifo(self, instrument):
        ...
        for optvalue in self.job()._AnalysisJob__cp.items(
        ...
        if not self.output_cache:
            self.output_cache = [
                CacheEntry(
                    ...
                    self.__usertag,
                    segments.segment(
                        LIGOTimeGPS(self.get_start()), LIGOTimeGPS(self.get_end())
                    ),
...
class RunSqliteJob(pipeline.CondorDAGJob):
    ...
    def __init__(self, config_parser):
        ...
        """
        pipeline.CondorDAGJob.__init__(
            self, "vanilla", power.get_executable(config_parser, "lalapps_run_sqlite")
        )
...
def remove_too_short_segments(seglists, min_segment_length, pad):
    ...
    for seglist in seglists.values():
        iterutils.inplace_filter(
            lambda seg: segment_ok(seg, min_segment_length, pad), seglist
        )
...
def compute_segment_lists(seglists, offset_vectors, min_segment_length, pad):
    ...
    for seglist in new.values():
        for i in range(len(seglist)):
            seglist[i] = segments.segment(
                int(math.floor(seglist[i][0])), int(math.ceil(seglist[i][1]))
            )
...
def make_string_fragment(dag, parents, instrument, seg, tag, framecache, injargs={}):
    ...
    node.set_name(
        "lalapps_StringSearch_%s_%s_%d_%d"
        % (tag, instrument, int(seg[0]), int(abs(seg)))
    )
...
def split_segment(
    seg, min_segment_length, pad, overlap, short_segment_duration, max_job_length
):
    # avoid infinite loop
    ...
    while abs(seg) >= min_segment_length + 2 * pad:
        ...
        if abs(seg) >= max_job_length:
            ...
            seglist.append(
                segments.segment(
                    seg[0],
                    seg[0] + clip_segment_length(abs(seg), pad, short_segment_duration),
                )
            )
        assert abs(seglist[-1]) != 0  # safety-check for no-op
        # bounds must be integers
        if (
            abs((int(seglist[-1][0]) - seglist[-1][0]) / seglist[-1][0]) > 1e-14
...
def make_string_segment_fragment(
    dag,
    datafindnodes,
    instrument,
    seg,
...
def make_single_instrument_stage(
    dag,
    datafinds,
    seglistdict,
    tag,
    ...
    for instrument, seglist in seglistdict.items():
        for seg in seglist:
            if verbose:
                print(
                    "generating %s fragment %s" % (instrument, str(seg)),
                    file=sys.stderr,
                )
            ...
            dfnodes = set(
                [
                    ...
                    and (seg in segments.segment(node.get_start(), node.get_end()))
                ]
            )
            if len(dfnodes) != 1:
                raise ValueError(
                    "error, not exactly 1 datafind is suitable for trigger generator job at %s in %s"
                    % (str(seg), instrument)
                )

            # trigger generator jobs
            nodes |= make_string_segment_fragment(
                dag,
                dfnodes,
                instrument,
                seg,
...
def make_run_sqlite_fragment(dag, parents, tag, sql_file, files_per_run_sqlite=None):
    ...
    while input_cache:
        ...
        node.add_input_cache(
            [cache_entry for cache_entry, parent in input_cache[:files_per_run_sqlite]]
        )
        for parent in set(
            parent for cache_entry, parent in input_cache[:files_per_run_sqlite]
        ):
...
def make_meas_likelihood_fragment(dag, parents, tag, files_per_meas_likelihood=None):
    ...
    while input_cache:
        ...
        for parent in set(
            parent for cache_entry, parent in input_cache[:files_per_meas_likelihood]
        ):
            node.add_parent(parent)
        del input_cache[:files_per_meas_likelihood]
        seg = power.cache_span(node.get_input_cache())
        node.set_name(
            "lalapps_string_meas_likelihood_%s_%d_%d"
...
def make_calc_likelihood_fragment(
    dag, parents, likelihood_parents, tag, files_per_calc_likelihood=None, verbose=False
):
    ...
    while input_cache:
        ...
        for parent in set(
            parent for cache_entry, parent in input_cache[:files_per_calc_likelihood]
        ):
            node.add_parent(parent)
        del input_cache[:files_per_calc_likelihood]
        seg = power.cache_span(node.get_input_cache())
        node.set_name(
            "lalapps_string_calc_likelihood_%s_%d_%d"
            % (tag, int(seg[0]), int(abs(seg)))
        )
