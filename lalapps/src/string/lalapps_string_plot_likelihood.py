...
def parse_command_line():
    ...
    parser.add_option(
        ...
        action="append",
        default=[],
        help='Set the image output format by setting the filename extension (default = "png").  Can be given multiple times to generate plots in multiple formats.',
    )
    parser.add_option(
        "--no-filter",
        action="store_true",
        help="Do not apply smoothing/normalization filters to data, plot raw bin values.",
    )
...
def clip_binned_array_2d(binnedarray, xlim, ylim):
    ...
    return (
        xcoords[imin:imax],
        ycoords[jmin:jmax],
        binnedarray.at_centres()[imin:imax, jmin:jmax],
    )
...
def snr2_chi2_plot(
    ...
    FigureCanvas(fig)
    axes = fig.add_axes((0.15, 0.15, 0.95 - 0.15, 0.90 - 0.15))
    axes.loglog()
    ...
    if numpy.any(denominator_data) or numpy.any(numerator_data):
        ...
        contours = numpy.arange(hi - 30, hi, 1.0, dtype="double")
        numerator_cset = axes.contour(
            numerator_xcoords, numerator_ycoords, numerator_data, contours, cmap=cm.Reds
        )
        denominator_cset = axes.contour(
            denominator_xcoords,
            denominator_ycoords,
            denominator_data,
            contours,
...
def dA_plot(
    key, denominator_coords, denominator_data, numerator_coords, numerator_data
):
    fig = figure.Figure(figsize=(4, 4 / golden_ratio))
    FigureCanvas(fig)
    axes = fig.add_axes((0.14, 0.15, 0.98 - 0.14, 0.90 - 0.15))
    ...
    axes.set_xlabel(
        r"$\log_{10} \left|A_{\mathrm{%s}} / A_{\mathrm{%s}}\right|$"
...
def instrumentgroup_plot(key, xcoords, denominator_data, numerator_data):
    ...
    axes.plot(xcoords, numerator_data, "ro-", label="Injections")
    axes.set_ticklabels(
        [",".join(sorted(stringutils.category_to_instruments(i))) for i in xcoords]
    )
...
def instrumentgroup_timingresidual_plot(
    key,
    denominator_xcoords,
    denominator_ycoords,
    denominator_data,
    ...
    ymin = min(
        max(denominator_data[i, :].max(), numerator_data[i, :].max())
        for i in range(len(denominator_xcoords))
    )
...
coincparamsdistributions, seglists = stringutils.load_likelihood_data(
...
for (denominator_name, denominator_pdf), (numerator_name, numerator_pdf) in zip(
    sorted(coincparamsdistributions.denominator.densities.items()),
    sorted(coincparamsdistributions.numerator.densities.items()),
):
    assert numerator_name == denominator_name
    name = denominator_name
    instruments = set(name.split("_")) & set(
        stringutils.StringCoincParamsDistributions.instrument_categories
    )
    if name.endswith("_snr2_chi2"):
        if options.verbose:
            print("generating plots for %s ..." % name, file=sys.stderr)
        denominator_xcoords, denominator_ycoords, denominator_data = (
            clip_binned_array_2d(denominator_pdf, [10, 1e6], [0.01, 1e4])
        )
        numerator_xcoords, numerator_ycoords, numerator_data = clip_binned_array_2d(
            numerator_pdf, [10, 1e6], [0.01, 1e4]
        )
        fig = snr2_chi2_plot(
            "%s" % name.replace("_", "-"),
            denominator_xcoords,
        ...
        if options.verbose:
            print("generating plots for %s ..." % name, file=sys.stderr)
        dt = 0.010 + snglcoinc.light_travel_time(*instruments)
        denominator_coords, denominator_data = clip_binned_array_1d(
            denominator_pdf, (-dt, +dt)
        )
        numerator_coords, numerator_data = clip_binned_array_1d(
            numerator_pdf, (-dt, +dt)
        )
        fig = dt_plot(
            "%s" % name.replace("_", "-"),
        ...
        if options.verbose:
            print("generating plots for %s ..." % name, file=sys.stderr)
        denominator_coords, denominator_data = clip_binned_array_1d(
            denominator_pdf, (-0.6, +0.6)
        )
        numerator_coords, numerator_data = clip_binned_array_1d(
            numerator_pdf, (-0.6, +0.6)
        )
        fig = df_plot(
            "%s" % name.replace("_", "-"),
            denominator_coords,
        ...
        fig = nevents_plot(
            "%s" % name.replace("_", "-"),
            denominator_pdf.centres()[0],
            denominator_pdf.array,
            numerator_pdf.array,
        )
        for extension in options.format:
            outname = "%s.%s" % (name, extension)
        ...
        if options.verbose:
            print("generating plots for %s ..." % name, file=sys.stderr)
        denominator_xcoords, denominator_ycoords, denominator_data = (
            clip_binned_array_2d(
                denominator_pdf, [1, denominator_pdf.array.shape[0] + 0.5], [0, 0.02]
            )
        )
        numerator_xcoords, numerator_ycoords, numerator_data = clip_binned_array_2d(
            numerator_pdf, [1, denominator_pdf.array.shape[0] + 0.5], [0, 0.02]
        )
        fig = instrumentgroup_timingresidual_plot(
