...
def parse_command_line():
    ...
    parser.add_option(
        ...
        "--base",
        metavar="base",
        default="string_plotbinj_",
        help='Set the prefix for output filenames (default = "string_plotbinj_").',
    )
    parser.add_option(
        "-f",
        "--format",
        ...
        action="append",
        default=[],
        help='Set the output image format (default = "png").  Option can be given multiple times to generate plots in multiple formats.',
    )
    parser.add_option(
        "--amplitude-func",
        metavar="det|wave",
        default="det",
        help='Select the amplitude to show on the plots.  "det" = the amplitude expected in the detector, "wave" = the amplitude of the wave (default = "det").',
    )
    parser.add_option(
        "--input-cache",
        metavar="filename",
        action="append",
        default=[],
        help="Get list of files from this LAL cache.",
    )
    parser.add_option(
        "-l",
        "--live-time-program",
        ...
        help='Set the name of the program, as it appears in the process table, whose search summary entries define the search live time (default = "StringSearch").',
    )
    parser.add_option(
        "--plot",
        metavar="number",
        action="append",
        default=None,
        help="Generate the given plot number (default = make all plots).",
    )
    parser.add_option(
        "-t",
        "--tmp-space",
...
FROM
	...
	JOIN sngl_burst ON (
		...
		"""):
            ...
            data.x.append(
                abs(
                    self.amplitude_func(
                        sim, instrument, offsetvectors[sim.time_slide_id]
                    )
                )
            )
    ...
    def finish(self):
        for instrument, data in sorted(self.data.items()):
            fig, axes = SnglBurstUtils.make_burst_plot(
                "Injected %s" % self.amplitude_lbl, r"Recovered $A$"
            )
            axes.loglog()
            ...
            axes.set_title(
                r"Recovered $A$ vs.\ Injected %s in %s (%d Recovered Injections)"
                % (self.amplitude_lbl, instrument, data.found)
            )
            yield fig
...
FROM
	...
	JOIN sngl_burst ON (
		...
		"""):
            ...
            sim_peak = sim.time_at_instrument(
                instrument, offsetvectors[sim.time_slide_id]
            )
...
plots = [
    ...
    (
        1,
        True,
        RecoveredVsInjectedAmplitude(options.amplitude_func, options.amplitude_lbl),
    ),
...
for n, filename in enumerate(
    ligolw_utils.sort_files_by_size(filenames, options.verbose, reverse=True)
):
    if options.verbose:
