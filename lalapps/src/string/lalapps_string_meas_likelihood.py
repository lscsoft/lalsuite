...
def parse_command_line():
    ...
    parser.add_option(
        ...
        metavar="filename",
        default=None,
        help="Set the name of the likelihood data file to write (default = stdout).",
    )
    parser.add_option(
        "-c",
        "--input-cache",
        metavar="filename",
        help="Also process the files named in this LAL cache.  See lalapps_path2cache for information on how to produce a LAL cache file.",
    )
    parser.add_option(
        "-t",
        "--tmp-space",
    ...
    parser.add_option(
        ...
        metavar="description",
        default=None,
        help='Write the output to a file whose name is compatible with the file name format described in LIGO-T010150-00-E, "Naming Convention for Frame Files which are to be Processed by LDAS".  The description string will be used to form the second field in the file name.',
    )
    parser.add_option(
        "--injection-reweight",
        metavar="off|astrophysical",
        default="off",
        help='Set the weight function to be applied to the injections (default = "off").  When "off", the injections are all given equal weight and so the injection population is whatever was injected.  When set to "astrophysical", the injections are reweighted to simulate an amplitude^{-4} distribution.',
    )
    parser.add_option(
        "--injection-reweight-cutoff",
        metavar="amplitude",
        default=1e-20,
        type="float",
        help="When using the astrophysical injection reweighting, do not allow the weight assigned to arbitrarily low-amplitude injections to grow without bound, instead clip the weight assigned to injections to the weight given to injections with this amplitude (default = 1e-20, 0 = disabled).  This option is ignored when astrophysical reweighting is not being performed.",
    )
    parser.add_option(
        "--vetoes-name",
        metavar="name",
    ...
    if options.injection_reweight not in ("off", "astrophysical"):
        raise ValueError(
            '--injection-reweight "%s" not recognized' % options.injections_reweight
        )
...
def get_injection_weight_func(contents, reweight_type, amplitude_cutoff):
    if reweight_type == "off":
        ...
        (population,) = ligolw_process.get_process_params(
            contents.xmldoc, "lalapps_binj", "--population"
        )
...
process = ligolw_process.register_to_xmldoc(
    xmldoc,
    program="lalapps_string_meas_likelihood",
    paramdict=paramdict,
    version=__version__,
...
for n, filename in enumerate(filenames):
    ...
    if options.tmp_space is not None:
        dbtables.set_temp_store_directory(
            connection, options.tmp_space, verbose=options.verbose
        )
    ...
    if contents.sim_burst_table is None:
        ...
        for is_background, events, offsetvector in contents.get_noninjections():
            params = distributions.coinc_params(
                [
                    event
                    for event in events
        ...
        weight_func = get_injection_weight_func(
            contents, options.injection_reweight, options.injection_reweight_cutoff
        )
...
ligolw_search_summary.append_search_summary(
    xmldoc, process, ifos=segs.keys(), inseg=segs.extent_all(), outseg=segs.extent_all()
)
xmldoc.childNodes[-1].appendChild(distributions.to_xml("string_cusp_likelihood"))
...
if options.T010150:
    filename = "%s.xml.gz" % T010150_basename(
        segs.keys(), options.T010150, segs.extent_all()
    )
else:
