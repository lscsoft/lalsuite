...
int main(int argc, char *argv[]) {
  ...
  if(argc == 2) {
    ...
    file = argv[2];
    toctarget = argv[1];
    if(regcomp(rexp,toctarget,REG_NOSUB)) {
  ...
  if(!strcmp(file+strlen(file)-3,".gz")) {
    ...
    sprintf(tmpfil,"./BurstProcessTmpFileXXXXXX");
    mkstemp(tmpfil);
  ...
  if(!strcmp(file+strlen(file)-4,".bin")) {
	...
	if(!toctarget) {
	  sprintf(fname,"%s.%i",file,i);
	} else {
	  sprintf(fname,"%s.%i.%i.%i",file,t0s,t0ns,i);
	  
	  if(strrchr(fname,'/')) {
	    char fbuf[1024];
	    strcpy(fbuf,1+strrchr(fname,'/'));
	    strcpy(fname,fbuf);
	  }
	}
	...
	fwrite(bbuf + strlen(bbuf) + 1, 1, ui - strlen(bbuf) - 1, out);

	fclose(out);

	printf("%s %s %u\n",bbuf,fname,(ui - strlen(bbuf) - 1)/size1);
