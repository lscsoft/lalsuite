...
for sampler in samps:
  ...
  for app in apps:
    ...
    for roq in roq_lengths:
      ...
      if not opts.dax and not (single_sampler is False or single_approx is False or use_roq is True):
        print 'Now run condor_submit_dag %s\n'%(fulldagpath)

      if opts.condor_submit and not (single_sampler is False or single_approx is False or use_roq is True):
        import subprocess
      ...
      if opts.pegasus_submit and not (single_sampler is False or single_approx is False or use_roq is True):
        import subprocess
        from subprocess import Popen
