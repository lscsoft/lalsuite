/*
 *  Copyright (C) 2010 Christian Roever
 *
...
typedef struct {
  ...
  char   *logfilename;       /* name of (general) log file ("*.log")                              */
  char   *csvfilename;       /* name of MCMC output CSV file ("*chainXYZ.csv")                    */
  char   *swpfilename;       /* name of parallel tempering swap log file ("*chainXYZ.csv")        */
  long   iterations;         /* number of MCMC iterations to run                                  */
  double *temperature;       /* vector temperatures associated with parallel chains               */
...
void importanceresample(DataFramework *DF, int coherentN, McmcFramework *MF,
                        vector *parameter, 
                        long samplesize, long subsamplesize, int chain);
...
void printhelpmessage()
{
  ...
  printf(" |   --outfilename                          output (text) file name   \n");
...
/* according to supplied command-line options.                       */
{
  ...
  char CLtemplate[512], CLoutfilename[512], CLiterations[512], CLrandomseed[512], 
  ...
  while (1) {
    ...
    switch (optparse) {
      ...
      case 'l': {strcpy(CLoutfilename, optarg); break;}         /* --outfilename                 */
      ...
      case 'E': {strcpy(CLtemperature, optarg); break;}         /* --temperature                 */
  ...
  MF->csvfilename = (char*) malloc(sizeof(char)*256);
  MF->swpfilename = (char*) malloc(sizeof(char)*256);
  if (CLoutfilename[0] != '\0') {
    sprintf(MF->logfilename, "%s.log", CLoutfilename);
    sprintf(MF->csvfilename, "%s", CLoutfilename); /* (name stored w/out extension!) */
    sprintf(MF->swpfilename, "%s.swp", CLoutfilename);
    /* start the log file (header line): */
    if (MPIrank == 0) {
      logfile = fopen(MF->logfilename, "w");
      fprintf(logfile, "\"name\",\"value\"\n");
      /* log file's first entry are the command line arguments: */
      fprintf(logfile, "\"command line\",\"%s", argv[0]);
      for (i=1; i<argc; ++i)
        fprintf(logfile, " %s", argv[i]);
      fprintf(logfile, "\"\n");
      fclose(logfile);
      /* next log file entries: */
      logtoLOGfile(MF, "init() start", datetime);
      sprintf(logstring, "%d", MPIsize);
    ...
    /* once parameter names &c are figured out...)                 */
  }
  else printf(" : ERROR: \"--outfilename\" argument missing!\n");
  ...
  if (MPIrank == 0) {
    sprintf(logstring, "%d", *coherentN);
    logtoLOGfile(MF, "interferometers", logstring); 
  }
  ...
  if (CLrandomseed[0] == '\0') printf(" : WARNING: no random seed specified!\n");
    ...
    if (noVectorStrg(CLrandomseed)) {
      seed1 = seed2 = (unsigned long int) atoi(CLrandomseed);
    }
    else {
      parseParameterOptionString(CLrandomseed, &argNames, &argValues, &argN);
      if (argN==1) {
        seed1 = seed2 = (unsigned long int) argValues[0];
      }
      else if (argN==2) {
        seed1 = (unsigned long int) argValues[0];
        seed2 = (unsigned long int) argValues[1];
      }
  ...
  if (CLsimulateData[0]=='1') { /* generate fake data w/ given noise PSD:             */
    ...
    for (i=0; i<(*coherentN); ++i) {
      if (InitialisationOK) {
        if (MPIrank == 0) {
          sprintf(logstring, "readData(%s) start", DF[i].ifo->name);
          DateTimeString(datetime);
          logtoLOGfile(MF, logstring, datetime); 
        }
        InitialisationOK = readData(&DF[i]);
        if (MPIrank == 0) {
          DateTimeString(datetime);
          sprintf(logstring, "readData(%s) finish", DF[i].ifo->name);
          logtoLOGfile(MF, logstring, datetime); 
        }
      }
    }
  ...
  /* ifo names (etc) have been figured out (just now or earlier already); log to file: */
  if (MPIrank == 0) {
    sprintf(logstring, "%s", DF[0].ifo->name);
    i = 1;
    while (i < *coherentN) {
      sprintf(logstring, "%s, %s", logstring, DF[i].ifo->name);
      ++i;
    }
    logtoLOGfile(MF, "interferometer names", logstring); 
  }
  ...
  for (i=0; i<(*coherentN); ++i) {
    ...
    if (DF[i].simulateData) {
      if (DF[i].simulatePsd) {
        if (MPIrank == 0) {
          sprintf(logstring, "simulatePsdEstimation(%s) start", DF[i].ifo->name);
          DateTimeString(datetime);
          logtoLOGfile(MF, logstring, datetime); 
	}
        ...
        if (MPIrank == 0) {
          DateTimeString(datetime);
          sprintf(logstring, "simulatePsdEstimation(%s) finish", DF[i].ifo->name);
          logtoLOGfile(MF, logstring, datetime); 
	}
      }
      else {
      ...
      if (InitialisationOK) {
        ...
        if (MPIrank == 0) {
          sprintf(logstring, "estimatePSD(%s) start", DF[i].ifo->name);
          DateTimeString(datetime);
          logtoLOGfile(MF, logstring, datetime); 
	}
  ...
  gsl_rng_set(GSLrandom, seed2 + MPIrank*1000);  /* from here on differing random seeds for each process */
  ...
  MF->temperature = (double*) malloc(sizeof(double) * MPIsize);
  if (noVectorStrg(CLtemperature)) {
    tmpDbl = atof(CLtemperature);
    for (i=0; i<MPIsize; ++i) MF->temperature[i] = pow(tmpDbl, (double) i);
  }
  else {
    parseParameterOptionString(CLtemperature, &argNames, &argValues, &argN);
    if ((argN != MPIsize) && (argN != 1))
      printf(" : ERROR: incompatible number of \"--temperature\" arguments (%d)!\n",argN);
    if (argN == 1) {
      tmpDbl = atof(CLtemperature);
      for (i=0; i<MPIsize; ++i) MF->temperature[i] = pow(argValues[0], (double) i);
    }
    else {
      for (i=0; i<MPIsize; ++i) MF->temperature[i] = argValues[i];
    }
    for (i=0; i<argN; ++i) free(argNames[i]);
    free(argNames); free(argValues);
  }
  if (MPIrank == 0) {
    /* little reasonability check: */
    tmpInt = 1;
    for (i=1; i<MPIsize; ++i) tmpInt &= (MF->temperature[i] > MF->temperature[i-1]);
    if (!tmpInt)
      printf(" : WARNING: strange \"--temperature\" arguments: temperature ladder is non-increasing!!\n");
    tmpInt = 0;
    for (i=0; i<MPIsize; ++i) tmpInt |= (MF->temperature[i] == 1.0);
    if (!tmpInt)
      printf(" : WARNING: strange \"--temperature\" arguments: temperature ladder does not include unity!!\n");
    /* log temperatures to file */
    sprintf(logstring, "%.4f", MF->temperature[0]);
    i=1;
    while (i<MPIsize) {
      sprintf(logstring, "%s, %.4f", logstring, MF->temperature[i]);
      ++i;
    }
    logtoLOGfile(MF, "temperature ladder", logstring);
  }
  ...
  if ((MF->template == iR2PN)
    ...
    /*setstdev(MF, parindex(MF,"latitude"),     0.2 * pi/180.0);
    setstdev(MF, parindex(MF,"polarisation"), 5.0 * pi/180.0);
    setstdev(MF, parindex(MF,"inclination"),  0.5 * pi/180.0);
    setstdev(MF, parindex(MF,"phase"),        10.0 * pi/180.0);
    setstdev(MF, parindex(MF,"time"),         0.0002);
    setstdev(MF, parindex(MF,"logdistance"),  0.01);
    setstdev(MF, parindex(MF,"chirpmass"),    0.005);
    setstdev(MF, parindex(MF,"massratio"),    0.0001);*/
  ...
  if (CLinjectpar[0] != '\0') {
    ...
    for (i=0; i<argN; ++i) {
      if (vectorIsElement(&injectpar, argNames[i])) {
        if (strcmp(argNames[i],"polarisation")==0) {
          if (argValues[i] > pi) {
            printf(" : WARNING: too large \"polarisation\" injection parameter (%.3f) is forced into [0,pi].\n", argValues[i]);
            while (argValues[i] > pi)
              argValues[i] -= pi;
            printf(" :          New polarisation value: %.3f.\n", argValues[i]);
          }
        }
    ...
    /* (injection parameters figured out.) */

    /*  Before performing injection, compute SNR and "actualized SNR",  */
    /*  log to file, etc.:                                              */
    if (MPIrank == 0) { 
      indivSNR = (double*) malloc(sizeof(double) * (*coherentN));
      netSNR = signaltonoiseratio(DF, *coherentN, injecttemplate, &injectpar, indivSNR);
      /* assemble human-readable SNR string: */
      sprintf(logstring, "%.3f (%.3f", netSNR, indivSNR[0]);
      if ((*coherentN)>1) for (i=1; i<(*coherentN); ++i) sprintf(logstring, "%s, %.3f", logstring, indivSNR[i]);
      sprintf(logstring, "%s)", logstring);
      if (verbose) {
        printf(" | injected '#%d' waveform signal.\n", injecttemplate);
        printf(" | SNR: %s\n", logstring);
      }
      logtoLOGfile(MF, "injected SNR", logstring);

      sprintf(logstring, "%.3f", (netSNR*netSNR) / 2.0);
      logtoLOGfile(MF, "expected injection logLR", logstring);

      nulllikeli = loglikelihood(DF, *coherentN, MF->template, NULL);
      injlikeli = loglikelihood(DF, *coherentN, injecttemplate, &injectpar);
      sprintf(logstring, "%.3f", netSNR*netSNR + (injlikeli - nulllikeli));
      logtoLOGfile(MF, "actualized injection logLR", logstring);

      /* in case of differing injection & recovery templates, also check recovery SNR: */
      if (injecttemplate != MF->template) {
        netSNR = signaltonoiseratio(DF, *coherentN, MF->template, &injectpar, indivSNR);
        sprintf(logstring, "%.3f (%.3f", netSNR, indivSNR[0]);
        if ((*coherentN)>1) for (i=1; i<(*coherentN); ++i) sprintf(logstring, "%s, %.3f", logstring, indivSNR[i]);
        sprintf(logstring, "%s)", logstring);
        if (verbose) {
          printf(" | (recovery template (#%d) SNR: %s)\n", MF->template, logstring);
        }
        logtoLOGfile(MF, "recovery SNR", logstring);

        sprintf(logstring, "%.3f", (netSNR*netSNR) / 2.0);
        logtoLOGfile(MF, "expected recovery logLR", logstring);

        injlikeli = loglikelihood(DF, *coherentN, injecttemplate, &injectpar);
        sprintf(logstring, "%.3f", netSNR*netSNR + (injlikeli - nulllikeli));
        logtoLOGfile(MF, "actualized recovery logLR", logstring);
      }
      free(indivSNR);

      tmpDbl = logprior(MF, &injectpar);
      sprintf(logstring, "%.6f", tmpDbl);
      logtoLOGfile(MF, "log prior at injection", logstring);

      if ( ! (tmpDbl > -HUGE_VAL) ) {
        printf(" : WARNING: Injection has zero prior probability !!\n");
        if (!verbose)
  ...
  if (MPIrank == 0) logtoLOGfile(MF, "log-likelihood degrees-of-freedom", logstring);

  if (importancedraws > 0)
    importanceresample(DF, *coherentN, MF, &MF->startvalue, importancedraws, 1, MPIrank+1);
...
/* in corresponding elements of "DF".                           */
{
  ...
  iFile = FrFileINew(DF->datafilenames);
  if (iFile == NULL) {
    printf(" : ERROR opening data frame file(s)!\n");
    return 0;
  }
  ...
  dataVector = FrFileIGetVectF(iFile, DF->frameChannel, from, DF->rawDataRange);
  if (dataVector == NULL) {
    printf(" : ERROR reading data frame file(s)!\n");
    printf(" : (channel=\"%s\", start=%.3f, duration=%.3f)\n", DF->frameChannel, from, DF->rawDataRange);
    return 0;
  }
...
/* and frequency range settings defined in the "DF" structure. */
{
  ...
  FourierTemplate = (COMPLEX16*) malloc(sizeof(COMPLEX16)*DF->FTSize);
  /* compute Fourier-domain template: */
...

{
  ...
  if ((MF->parameterset == InspiralNoSpin) | (MF->parameterset == InspiralAlignedSpin))
    ...
    printf(" : ERROR: referring to undefined parameterset in 'logpstar()'!\n");
  return(result);
}
...
/****************************************************************/
{
  ...
  double lambda1 = F1 * pow(mt, -8.0/3.0) * ((mt*mt*mt) / (m1*m2*MsunSec*MsunSec));
  double lambda2 = F2 * pow(mt, -5.0/3.0) * ((mt*mt*mt) / (m1*m2*MsunSec*MsunSec));
  double C1 = (lambda2*F1) / (lambda1*F2);
  double C2 = (lambda2/F2) * pow(F1/lambda1, 4.0);
  double d  = -(F1*F2*pow(C2,1.0/3.0)*sqrt(C1*C1-4.0*pow(C2,1.0/3.0))) / (pow(F1*lambda2,2.0)-4.0*pow(F2*lambda1,2.0)*pow(C2,1.0/3.0));
  /*printf("\n : m1=%f  m2=%f\n", m1, m2);
  printf(" : F1=%e  \n", F1);
  printf(" : F2=%e  \n", F2);
  printf(" : l1=%e  \n", lambda1);
...
{
  double result = HUGE_VAL;
  double m1 = mc2mass1(vectorGetValue(parameter,"chirpmass"), vectorGetValue(parameter,"massratio"));
  double m2 = mc2mass2(vectorGetValue(parameter,"chirpmass"), vectorGetValue(parameter,"massratio"));
  result = log(determinant(m1, m2)) - log(sin(vectorGetValue(parameter,"inclination"))) - log(cos(vectorGetValue(parameter,"latitude")));
  /*printf(" :   logh=%f\n",result);*/
  return(result);
}
...
/* entries, the first columns contains entry names, the second column contains   */
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
{
  ...
  /*sprintf(chainIndicator, "chain%3d", chain);
    for (i=0; i<32; ++i) if (chainIndicator[i]==' ') chainIndicator[i]='0';
  sprintf(filename, "%s.log", MF->logfilename);*/
...
/* The actual Metropolis (MCMC) sampler. */
{
  ...
  snrvec = (double*) malloc(sizeof(double)*coherentN);
  if (MPIrank==0) {
    DateTimeString(logstring);
    ...
    if (MPIsize > 1) {
      swapLogFile = fopen(MF->swpfilename, "w");
      fprintf(swapLogFile, "\"iteration\",\"seconds\"");
      for (i=0; i<(MPIsize-1); ++i){
        sprintf(logstring, "P%3d", i+1);
        for (j=0; logstring[j]!='\0'; ++j)
          if (logstring[j] == ' ') logstring[j] = '0';
        fprintf(swapLogFile, ",\"%s\"", logstring);
      }
      for (i=0; i<(MPIsize-1); ++i){
        sprintf(logstring, "A%3d", i+1);
        for (j=0; logstring[j]!='\0'; ++j)
          if (logstring[j] == ' ') logstring[j] = '0';
        fprintf(swapLogFile, ",\"%s\"", logstring);
      }
    ...
    /* set up SNR logfile: */
    sprintf(logstring, "\"iteration\",\"SNRnetwork\"");
    for (i=0; i<coherentN; ++i)
      sprintf(logstring, "%s,\"SNR%d\"", logstring, i+1);   
    swapLogFile = fopen(snrfilename, "w");
    fprintf(swapLogFile, "%s\n", logstring);
    fclose(swapLogFile);    
  }
  ...
  for (i=1; i<=MF->iterations; ++i) {
    ...
    if (lpriorP > -HUGE_VAL) { /* (only compute likelihood if prior wasn't already zero) */
      ...
      lpstarP  = logpstar(MF, &proposal);
      /*logalpha = (lpriorP-lprior) + (llikeliP-llikeli) + logMHcoef;*/
      logalpha  = (1.0 - 1.0/MF->temperature[ownTempIndex]) * (lpstarP - lpstar);
      logalpha += (1.0/MF->temperature[ownTempIndex]) * ((lpriorP-lprior) + (llikeliP-llikeli));
    }
    ...
    if ((MPIrank == 0) && (MPIsize > 1)) {
      ...
      for (swapattempt=0; swapattempt<50; ++swapattempt) {
        ...
        j = gsl_rng_uniform_int(GSLrandom, MPIsize-1);  /* temperature index: j & j+1 are tried to swap.*/
        ++swapProposalCount[j];
        swap1 = 0;
        ...
        logalpha  = (priorVec[swap2] - priorVec[swap1]) + (likeliVec[swap2] - likeliVec[swap1]);
        logalpha += (pstarVec[swap1] - pstarVec[swap2]);
        logalpha *= (1.0/MF->temperature[tempIndVec[swap1]] - 1.0/MF->temperature[tempIndVec[swap2]]);
        accept = ((logalpha>=0.0) || (log(gsl_rng_uniform(GSLrandom))<logalpha));
        if (accept) { /* swap! */
          ++swapAcceptCount[j];
          /* swap temperature assignments: */
          j = tempIndVec[swap1];
    ...
    MPI_Scatter(tempIndVec, 1, MPI_INT,
                ...
                0, MPI_COMM_WORLD);
    MPI_Scatter(MHacceptCount, 1, MPI_LONG,
                &MHaccept, 1, MPI_LONG,
                0, MPI_COMM_WORLD);

    if (logIter) {          /* log every 100th iteration                              */
      logtoCSVfile(MF, &state, i, MHaccept, lprior, llikeli, lprior+llikeli, lpstar, ownTempIndex+1);
      if ((MPIrank == 0) && (i % 1000 == 0) && (MPIsize > 1)) { /* update the inter-chain-swap log file: */
        time(&endtime);
        seconds = difftime(endtime, starttime);
...
int main(int argc, char *argv[])
{
  ...
  if (verbose) {
    printf(" +----[ lalapps_followupMcmcPT *** parallel tempering (MPI) version ]----------\n");
    printf(" | (This is process #%d out of %d total.)\n", rank, size);
  }
  ...
  if (argc==1) {
    ...
    initOK = init(&DatFW, &McmcFW, &coherentN, argc, argv, ifodata, ifoN, size, rank);
