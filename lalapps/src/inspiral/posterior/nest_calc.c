...
REAL8 nestZ(UINT4 Nruns, UINT4 Nlive, LALMCMCParameter **Live, LALMCMCInput *MCMCinput)
{
	...
	while(((REAL8)i)<=((REAL8)Nlive) || logadd(logZ,logLmax+logw+log((REAL8)Nlive))-logZ > 0.1 )
	{
		minpos=0;
		...
		if(MCMCinput->verbose) fprintf(stderr,"%i: (%2.1lf%%) accpt: %1.3f H: %3.3lf nats (%3.3lf b) logL:%lf ->%lf dZ: %lf logZ: %lf Zratio: %lf db\n",
									   i,100.0*((REAL8)i)/(((REAL8) Nlive)*H*infosafe),accept/MCMCfail,H,H/log(2.0),logLmin,Live[minpos]->logLikelihood,logadd(logZ,logLmax+logw+log((REAL8)Nlive))-logZ,logZ,10.0*log10(exp(1.0))*(logZ-logZnoise));
