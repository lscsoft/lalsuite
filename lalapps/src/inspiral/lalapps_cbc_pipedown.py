...
parser.add_option( "", "--run-mvsc", action="store_true",
  default = False,
  help =
    "Turn on if you want to run the multivariate statistical analysis (MVSC)"
  )
...
# for two stage runs, ihope sticks the instrument time in after the program name,
# e.g., THINCA_SECONCD_H1L1_USER_TAG whereas for one stage pipelines it's
# THINCA_FIRST_USER_TAG.
if 'SECOND' in coinc_zero_lag_tag:
  skip_tags = 3
else:
  skip_tags = 2
...
# Note: the ringdown pipeline actually uses rinca_to_coinc, but that's ok b/c
# both programs have the same options. The difference is handled in the condor
...
t2c_job.add_condor_cmd("request_memory", "1000")
t2c_sim_job.add_condor_cmd("request_memory", "1000")
...
for tag in user_tags:
  ...
  for on_instruments in distinct_instrument_sets:
    ...
    for entry_num, entry in enumerate(instrument_cache):
      ...
      if not simulation:
        slide_entry = lal.CacheEntry(re.sub(coinc_zero_lag_tag, coinc_slide_tag, str(entry)))
      ...
      if (entry_num + 1) % 20 == 0 or (entry_num + 1) == len(instrument_cache):
        ...
        t2c_output += this_t2c_node.get_output_from_cache( coinc_zero_lag_tag )
