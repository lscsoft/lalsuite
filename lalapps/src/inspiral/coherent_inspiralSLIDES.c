...
/*CHECK: input time-slide parameters */
...
int main( int argc, char *argv[] )
{
  ...
  /*CHECK: variables for initializing tempTime to account for time-slides */
  ...
  for( i=0; i<numTmplts; i++)
    {
      ...
      /* CHECK: Parse eventID to get the slide number */
	      ...
	      for( j=0; j<6; j++ )
		{
		  if( caseID[j] )
		      ...
		      for( w=0; w<chanNumArray[j]+numChannels/2; w++)
			{
		      ...
		      /*CHECK: changed "=" tp "+=" below to account for time-slide*/
 ...
 /*this will change the coordinate frame from the georaphic to the equatorial, need to take care of memory allocations for converParams and tempSky*/
	       while( thisEvent )
		    {
		      convertParams.system=COORDINATESYSTEM_EQUATORIAL;
		      convertParams.zenith= NULL;
		      convertParams.gpsTime->gpsSeconds=thisEvent->end_time.gpsSeconds; /*need to check if this should be in ns*/
		      /*The next few lines gets the parameters ready for 
			conversion. Need to check to make sure that 
			thisEvent->ligo_axis_dec is in proper notation, i.e. */

		      tempSky.system=COORDINATESYSTEM_GEOGRAPHIC;
		      tempSky.longitude=thisEvent->ligo_axis_dec;
		      tempSky.latitude=thisEvent->ligo_axis_ra;
		      
		      LALConvertSkyCoordinates(&status,&tempSky, &tempSky,&convertParams);
		      /*This is now ready to be substituted back into this event to reflect the equatorial coordinates*/
		      thisEvent->ligo_axis_dec=tempSky.longitude;
		      thisEvent->ligo_axis_ra=tempSky.latitude;
...
int arg_parse_check( int argc, char *argv[], MetadataTable procparams )
{
   ...
   while (1)
	 ...
	 case 'u':
	   ...
	   /*CHECK: Read in time-slide steps */
