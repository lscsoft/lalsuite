  ...
  for data in all_data playground_only exclude_play; do
    ...
    # write corse jobs for H1L1V1 time
      for combo in H1L1V1 H1V1 H1L1 L1V1; do
        zerofile="second_coire_files/${mass}/${combo}-SECOND_COIRE_${cat}_H1L1V1-${month_gps_time}-${month_duration}.xml.gz"
        slidefile="second_coire_files/${mass}/${combo}-SECOND_COIRE_SLIDE_${cat}_H1L1V1-${month_gps_time}-${month_duration}.xml.gz"
        outfile="corse_all_data_files/${data}/H1L1V1_${combo}-CORSE_`echo ${data} | tr '[a-z]' '[A-Z]'`_${mass}_${cat}-${month_gps_time}-${month_duration}.xml.gz"
        summfile=`echo ${outfile} | sed s/.xml.gz/.txt/g`
	...
	# write corse jobs for H2L1V1 time
      for combo in H2L1V1 L1V1 H2V1 H2L1; do
        zerofile="second_coire_files/${mass}/${combo}-SECOND_COIRE_${cat}_H2L1V1-${month_gps_time}-${month_duration}.xml.gz"
        slidefile="second_coire_files/${mass}/${combo}-SECOND_COIRE_SLIDE_${cat}_H2L1V1-${month_gps_time}-${month_duration}.xml.gz"
        outfile="corse_all_data_files/${data}/H2L1V1_${combo}-CORSE_`echo ${data} | tr '[a-z]' '[A-Z]'`_${mass}_${cat}-${month_gps_time}-${month_duration}.xml.gz"
        summfile=`echo ${outfile} | sed s/.xml.gz/.txt/g`
    ...
    # write corse jobs for H1H2V1 time
      for combo in H1H2V1 H1H2 H1V1 H2V1; do
        zerofile="second_coire_files/${mass}/${combo}-SECOND_COIRE_${cat}_H1H2V1-${month_gps_time}-${month_duration}.xml.gz"
        slidefile="second_coire_files/${mass}/${combo}-SECOND_COIRE_SLIDE_${cat}_H1H2V1-${month_gps_time}-${month_duration}.xml.gz"
        outfile="corse_all_data_files/${data}/H1H2V1_${combo}-CORSE_`echo ${data} | tr '[a-z]' '[A-Z]'`_${mass}_${cat}-${month_gps_time}-${month_duration}.xml.gz"
        summfile=`echo ${outfile} | sed s/.xml.gz/.txt/g`
	...
	# write corse jobs for H1H2L1V1 time
      for combo in H1H2L1V1 H1H2L1 H1L1V1 H2L1V1 H1H2V1 H1H2 H1L1 H2L1 H1V1 H2V1 L1V1; do
        zerofile="second_coire_files/${mass}/${combo}-SECOND_COIRE_${cat}_H1H2L1V1-${month_gps_time}-${month_duration}.xml.gz"
        slidefile="second_coire_files/${mass}/${combo}-SECOND_COIRE_SLIDE_${cat}_H1H2L1V1-${month_gps_time}-${month_duration}.xml.gz"
        outfile="corse_all_data_files/${data}/H1H2L1V1_${combo}-CORSE_`echo ${data} | tr '[a-z]' '[A-Z]'`_${mass}_${cat}-${month_gps_time}-${month_duration}.xml.gz"
        summfile=`echo ${outfile} | sed s/.xml.gz/.txt/g`
