...
AC_DEFUN([LALSUITE_CHECK_LIB],[
  ...
  # prepend to CFLAGS, CPPFLAGS, LDFLAGS, LIBS, LAL_BIN_PATH, LAL_DATA_PATH, LAL_OCTAVE_PATH, LAL_PYTHON_PATH
    ...
    for arg in ${lalsuite_libs}; do
      AS_CASE([${arg}],
        [lalsupport],[:],[
          ...
          DOXYGEN_TAGFILES="${DOXYGEN_TAGFILES} \$(abs_top_builddir)/../${arg}/doxygen/out/${arg}.tag=\$(doxygen_lalpkg_reldir)/${arg}/doxygen/out"
          DOXYGEN_INSTALL_DIRMAP="${DOXYGEN_INSTALL_DIRMAP} \$(doxygen_lalpkg_reldir)/${arg}/doxygen/out:\$(datarootdir)/doc/lalsuite/${arg}"
        ]
      )
    done
