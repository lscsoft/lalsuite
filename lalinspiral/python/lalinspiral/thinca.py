...
class InspiralCoincTables(snglcoinc.CoincTables):
    ...
    def coinc_rows(self, process_id, time_slide_id, events, seglists):
        coinc, coincmaps = super(InspiralCoincTables, self).coinc_rows(
            process_id, time_slide_id, events, "sngl_inspiral"
        )
        ...
        coinc_inspiral = self.coinc_inspiral_table.RowType(
            ...
            snr=math.sqrt(sum(event.snr**2.0 for event in events)),
        ...
        coinc.insts = (
            set(
                instrument
                for instrument, segs in seglists.items()
                if float(end - offsetvector[instrument]) in segs
            )
    ...
    def append_coinc(self, coinc_event, coinc_event_maps, coinc_inspiral):
        coinc_event = super(InspiralCoincTables, self).append_coinc(
            coinc_event, coinc_event_maps
        )
...
def ligolw_thinca(
    ...
    if veto_segments is not None:
        sngl_inspiral_table = (
            event
            for event in sngl_inspiral_table
    ...
    if not incremental:
        ...
        for instrument, events in itertools.groupby(
            sorted(sngl_inspiral_table, key=lambda row: row.ifo),
            lambda event: event.ifo,
        ):
            time_slide_graph.push(
                instrument, tuple(events), snglcoinc.segments.PosInfinity
            )
    else:
        ...
        for instrument, events in itertools.groupby(
            sorted(sngl_inspiral_table, key=lambda row: (row.end, row.ifo)),
            lambda event: event.ifo,
        ):
            events = tuple(events)
            if time_slide_graph.push(
                instrument, events, max(event.end for event in events)
            ):
                for node, events in time_slide_graph.pull(
                    coinc_sieve=ntuple_comparefunc
                ):
                    coinc_tables.append_coinc(
                        *coinc_tables.coinc_rows(
                            process_id, node.time_slide_id, events, seglists=seglists
                        )
                    )
    ...
    for node, events in time_slide_graph.pull(
        coinc_sieve=ntuple_comparefunc, flush=True
    ):
        coinc_tables.append_coinc(
            *coinc_tables.coinc_rows(
                process_id, node.time_slide_id, events, seglists=seglists
...
class sngl_inspiral_coincs(object):
    ...
    def __init__(self, xmldoc):
        ...
        self.sngl_inspiral_index = dict(
            (row.event_id, row) for row in self.sngl_inspiral_table
        )
        self.time_slide_index = {}
        for row in self.time_slide_table:
            self.time_slide_index.setdefault(row.time_slide_id, []).append(row)
        self.zero_lag_time_slide_ids = frozenset(
            time_slide_id
            for time_slide_id, offset_vector in self.time_slide_table.as_dict().items()
        ...
        (self.coinc_def,) = (
            row
            for row in self.coinc_def_table
            if row.search == InspiralCoincDef.search
            and row.search_coinc_type == InspiralCoincDef.search_coinc_type
        )
        coinc_event_map_ids = frozenset(
            row.coinc_event_id for row in self.coinc_event_map_table
        )
        self.coinc_event_index = dict(
            (row.coinc_event_id, row)
            for row in self.coinc_event_table
            if row.coinc_def_id == self.coinc_def.coinc_def_id
            and row.coinc_event_id in coinc_event_map_ids
        )
        self.coinc_inspiral_index = dict(
            (row.coinc_event_id, row)
            for row in self.coinc_inspiral_table
            if row.coinc_event_id in coinc_event_map_ids
        )
        assert frozenset(self.coinc_event_index) == frozenset(self.coinc_inspiral_index)
        self.coinc_event_map_index = dict(
            (coinc_event_id, []) for coinc_event_id in self.coinc_event_index
        )
    ...
    def sngl_inspirals(self, coinc_event_id):
        ...
        return [
            ...
            for row in self.coinc_event_map_index[coinc_event_id]
        ]
    ...
    def single_sngl_inspirals(self):
        ...
        for coinc_event_id, coinc_event in self.coinc_event_index.items():
            if (
                coinc_event.time_slide_id in self.zero_lag_time_slide_ids
                and coinc_event.nevents < 2
            ):
    ...
    def offset_vector(self, time_slide_id):
        ...
        return offsetvector.offsetvector(
            (row.instrument, row.offset) for row in self.time_slide_index[time_slide_id]
        )
    ...
    def __getitem__(self, coinc_event_id):
        ...
        new_process_table = ligolw_elem.appendChild(
            lsctables.ProcessTable.new(self.process_table.columnnames)
        )
        new_process_params_table = ligolw_elem.appendChild(
            lsctables.ProcessParamsTable.new(self.process_params_table.columnnames)
        )
        new_sngl_inspiral_table = ligolw_elem.appendChild(
            lsctables.SnglInspiralTable.new(self.sngl_inspiral_table.columnnames)
        )
        new_coinc_def_table = ligolw_elem.appendChild(
            lsctables.CoincDefTable.new(self.coinc_def_table.columnnames)
        )
        new_coinc_event_table = ligolw_elem.appendChild(
            lsctables.CoincTable.new(self.coinc_event_table.columnnames)
        )
        new_coinc_inspiral_table = ligolw_elem.appendChild(
            lsctables.CoincInspiralTable.new(self.coinc_inspiral_table.columnnames)
        )
        new_coinc_event_map_table = ligolw_elem.appendChild(
            lsctables.CoincMapTable.new(self.coinc_event_map_table.columnnames)
        )
        new_time_slide_table = ligolw_elem.appendChild(
            lsctables.TimeSlideTable.new(self.time_slide_table.columnnames)
        )
        ...
        map(
            new_coinc_event_map_table.append, self.coinc_event_map_index[coinc_event_id]
        )
        map(
            new_time_slide_table.append,
        ...
        for process_id in (
            set(new_sngl_inspiral_table.getColumnByName("process_id"))
            | set(new_coinc_event_table.getColumnByName("process_id"))
            | set(new_time_slide_table.getColumnByName("process_id"))
        ):
            # process row is required
            new_process_table.append(self.process_index[process_id])
            try:
                map(
                    new_process_params_table.append,
