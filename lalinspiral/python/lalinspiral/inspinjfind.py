...
InspiralSICoincDef = lsctables.CoincDef(
    search="inspiral",
    search_coinc_type=1,
    description="sim_inspiral<-->sngl_inspiral coincidences",
)
InspiralSCNearCoincDef = lsctables.CoincDef(
    search="inspiral",
    search_coinc_type=2,
...
InspiralSTCoincDef = lsctables.CoincDef(
    search="inspiral",
    search_coinc_type=4,
    description="sim_inspiral<-->sngl_inspiral template coincidences",
)
...
class DocContents(object):
    ...
    def __init__(
        self, xmldoc, bbdef, sbdef, scedef, scndef, process, end_time_bisect_window
    ):
        #
        ...
        self.tisi_id = ligolw_time_slide.get_time_slide_id(
            xmldoc,
            {}.fromkeys(self.snglinspiraltable.getColumnByName("ifo"), 0.0),
            create_new=process,
            superset_ok=True,
            nonunique_ok=True,
        )
        ...
        try:
            ii_coinc_def_id = ligolw_coincs.get_coinc_def_id(
                xmldoc, bbdef.search, bbdef.search_coinc_type, create_new=False
            )
        except KeyError:
    ...
    def inspirals_near_endtime(self, t):
        ...
        """
        return self.snglinspiraltable[
            bisect.bisect_left(
                self.snglinspiraltable, t - self.end_time_bisect_window
            ) : bisect.bisect_right(
                self.snglinspiraltable, t + self.end_time_bisect_window
            )
        ]
    ...
    def coincs_near_endtime(self, t):
        ...
        for event in self.inspirals_near_endtime(t):
            try:
                ...
                pass
        return [
            (coinc_event_id, self.sngls[coinc_event_id])
...
def find_sngl_inspiral_matches(contents, sim, comparefunc):
    ...
    return [
        inspiral
        for inspiral in contents.inspirals_near_endtime(sim.time_geocent)
...
def find_exact_coinc_matches(coincs, sim, comparefunc):
    ...
    return set(
        coinc_event_id
        for coinc_event_id, inspirals in coincs
        if not any(comparefunc(sim, inspiral) for inspiral in inspirals)
    )
...
def find_near_coinc_matches(coincs, sim, comparefunc):
    ...
    return set(
        coinc_event_id
        for coinc_event_id, inspirals in coincs
        if not all(comparefunc(sim, inspiral) for inspiral in inspirals)
    )
...
def ligolw_inspinjfind(
    ...
    for sim in tqdm.tqdm(
        contents.siminspiraltable, desc=sbdef.description, disable=not verbose
    ):
...
def revert(xmldoc, program, verbose=False):
    ...
    iterutils.inplace_filter(
        (lambda row: row.process_id not in process_ids), process_table
    )
    iterutils.inplace_filter(
        (lambda row: row.process_id not in process_ids),
        lsctables.ProcessParamsTable.get_table(xmldoc),
    )
    ...
    # IDs of things to delete
    coinc_ids = frozenset(
        row.coinc_event_id for row in coinc_event_table if row.process_id in process_ids
    )
    iterutils.inplace_filter(
        (lambda row: row.coinc_event_id not in coinc_ids), coinc_event_table
    )
    iterutils.inplace_filter(
        (lambda row: row.coinc_event_id not in coinc_ids),
        lsctables.CoincMapTable.get_table(xmldoc),
    )
    ...
    coinc_defs = frozenset(
        (row.search, row.search_coinc_type)
        for row in (InspiralSICoincDef, InspiralSCNearCoincDef, InspiralSCExactCoincDef)
    )
    iterutils.inplace_filter(
        (
            lambda row: row.process_id not in process_ids
            or row.time_slide_id in time_slide_ids
        ),
        lsctables.TimeSlideTable.get_table(xmldoc),
    )
    iterutils.inplace_filter(
        (
            lambda row: (row.search, row.search_coinc_type) not in coinc_defs
            or row.coinc_def_id in coinc_def_ids
        ),
        lsctables.CoincDefTable.get_table(xmldoc),
    )
