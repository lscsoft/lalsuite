...

{
  ...
  while (j<nmodes) {
    switch (j) {
      ...
      case 1:
        for (i = 0; i < nmodes; i++) {
          gsl_matrix_set(coef, 2*j, i, modefreqs->data[i].im*modefreqs->data[i].im-crealf(modefreqs->data[i])*crealf(modefreqs->data[i]));
          gsl_matrix_set(coef, 2*j, i+nmodes, -2.*modefreqs->data[i].im*crealf(modefreqs->data[i]));
          gsl_matrix_set(coef, 2*j+1, i, -modefreqs->data[i].im*modefreqs->data[i].im*modefreqs->data[i].im+3.*modefreqs->data[i].im*crealf(modefreqs->data[i])*crealf(modefreqs->data[i]));
          gsl_matrix_set(coef, 2*j+1, i+nmodes, -crealf(modefreqs->data[i])*crealf(modefreqs->data[i])*crealf(modefreqs->data[i])+3.*crealf(modefreqs->data[i])*modefreqs->data[i].im*modefreqs->data[i].im);
        }
        break;
      case 2:
        for (i = 0; i < nmodes; i++) {
          gsl_matrix_set(coef, 2*j, i, pow(modefreqs->data[i].im,4.)+pow(crealf(modefreqs->data[i]),4.)-6.*pow(crealf(modefreqs->data[i])*modefreqs->data[i].im,2.));
          gsl_matrix_set(coef, 2*j, i+nmodes, -4.*pow(modefreqs->data[i].im,3.)*crealf(modefreqs->data[i])+4.*pow(crealf(modefreqs->data[i]),3.)*modefreqs->data[i].im);
          gsl_matrix_set(coef, 2*j+1, i, -pow(modefreqs->data[i].im,5.)+10.*pow(modefreqs->data[i].im,3.)*pow(crealf(modefreqs->data[i]),2.)-5.*modefreqs->data[i].im*pow(crealf(modefreqs->data[i]),4.));
          gsl_matrix_set(coef, 2*j+1, i+nmodes, 5.*pow(modefreqs->data[i].im,4.)*crealf(modefreqs->data[i])-10.*pow(modefreqs->data[i].im,2.)*pow(crealf(modefreqs->data[i]),3.)+pow(crealf(modefreqs->data[i]),5.));
        }
        break;
      default:
...

{
  ...
  if ((l==2)&&(abs(m)==2)) {
    for (i = 0; i < nmodes; i++)
      {
	modefreqs->data[i].realf_FIXME = BCW22re[i][0] + BCW22re[i][1] * pow(1.- finalSpin, BCW22re[i][2]);
	modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
    ...
    if ((l==2)&&(m==0)) {
      for (i = 0; i < nmodes; i++)
	{
	  modefreqs->data[i].realf_FIXME = BCW20re[i][0] + BCW20re[i][1] * pow(1.- finalSpin, BCW20re[i][2]);
	  modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
	    / (BCW20im[i][0] + BCW20im[i][1] * pow(1.- finalSpin, BCW20im[i][2]));
	  modefreqs->data[i].realf_FIXME /= finalMass * totalMass * LAL_MTSUN_SI;
      ...
      if ((l==2)&&(abs(m)==1)) {
	for (i = 0; i < nmodes; i++) {
	  modefreqs->data[i].realf_FIXME = BCW21re[i][0] + BCW21re[i][1] * pow(1.- finalSpin, BCW21re[i][2]);
	  modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
	...
	if ((l==3)&&(abs(m)==3)) {
	  for (i = 0; i < nmodes; i++) {
	    modefreqs->data[i].realf_FIXME = BCW33re[i][0] + BCW33re[i][1] * pow(1.- finalSpin, BCW33re[i][2]);
	    modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
	  ...
	  if ((l==3)&&(abs(m)==2)) {
	    for (i = 0; i < nmodes; i++) {
	      modefreqs->data[i].realf_FIXME = BCW32re[i][0] + BCW32re[i][1] * pow(1.- finalSpin, BCW32re[i][2]);
	      modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
	    ...
	    if ((l==3)&&(abs(m)==1)) {
	      for (i = 0; i < nmodes; i++) {
		modefreqs->data[i].realf_FIXME = BCW31re[i][0] + BCW31re[i][1] * pow(1.- finalSpin, BCW31re[i][2]);
		modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
		...
		modefreqs->data[i].im /= finalMass * totalMass * LAL_MTSUN_SI;
	      ...
	      if ((l==3)&&(m==0)) {
		for (i = 0; i < nmodes; i++) {
		  modefreqs->data[i].realf_FIXME = BCW30re[i][0] + BCW30re[i][1] * pow(1.- finalSpin, BCW30re[i][2]);
		  modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
		...
		if ((l==4)&&(abs(m)==4)) {
		  for (i = 0; i < nmodes; i++) {
		    modefreqs->data[i].realf_FIXME = BCW44re[i][0] + BCW44re[i][1] * pow(1.- finalSpin, BCW44re[i][2]);
		    modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
		  ...
		  if ((l==4)&&(abs(m)==3)) {
		    for (i = 0; i < nmodes; i++) {
		      modefreqs->data[i].realf_FIXME = BCW43re[i][0] + BCW43re[i][1] * pow(1.- finalSpin, BCW43re[i][2]);
		      modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
		    ...
		    if ((l==4)&&(abs(m)==2)) {
		      for (i = 0; i < nmodes; i++) {
			modefreqs->data[i].realf_FIXME = BCW42re[i][0] + BCW42re[i][1] * pow(1.- finalSpin, BCW42re[i][2]);
			modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
			...
			modefreqs->data[i].im /= finalMass * totalMass * LAL_MTSUN_SI;
		      ...
		      if ((l==4)&&(abs(m)==1)) {
			for (i = 0; i < nmodes; i++) {
			  modefreqs->data[i].realf_FIXME = BCW41re[i][0] + BCW41re[i][1] * pow(1.- finalSpin, BCW41re[i][2]);
			  modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
			...
			if ((l==4)&&(m==0)) {
			  for (i = 0; i < nmodes; i++) {
			    modefreqs->data[i].realf_FIXME = BCW40re[i][0] + BCW40re[i][1] * pow(1.- finalSpin, BCW40re[i][2]);
			    modefreqs->data[i].im = crealf(modefreqs->data[i]) / 2.
