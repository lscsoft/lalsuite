   ...
   )
{
  ...
  /* generate +,x waveforms */
  ret = XLALSimInspiralChooseTDWaveform(hplus, hcross, phi0, deltaT, m1, m2, S1x, S1y, S1z, S2x, S2y, S2z, f_min, f_ref, r, i, lambda1, lambda2, waveFlags, nonGRparams, amplitudeO, order, approximant, NULL);
  XLALSimInspiralDestroyWaveformFlags(waveFlags);
  XLALSimInspiralDestroyTestGRParam(nonGRparams);
  if( ret == XLAL_FAILURE)
    XLAL_ERROR(XLAL_EFUNC);
