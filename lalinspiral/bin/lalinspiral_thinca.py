...
def parse_command_line():
    ...
    parser.add_option(
        ...
        metavar="float",
        type="float",
        help="Set the coincidence window in seconds, not including light travel time (required).  The value entered here will be dilated by the light travel time between detectors to define the coincidence window for each detector pair.",
    )
    parser.add_option(
        "--min-instruments",
        metavar="number",
        default="2",
        type="int",
        help="Set the minimum number of instruments that must participate in a coincidence (default = 2).  The value must be greater than 0.",
    )
    ...
    parser.add_option(
        ...
        help='From the input document, use the segment list with this name to define when each detector was operating (default = "whitehtsegments").',
    )
    parser.add_option(
        "--vetoes-segments-name",
        metavar="string",
        help="From the input document, use the segment list with this name to veto single-detector triggers before considering them for coincidence (default = do not apply vetoes.",
    )
    parser.add_option(
        "--coinc-end-time-segment",
        metavar="seg",
        help='The segment of time to retain coincident triggers from.  Uses segmentUtils.from_range_strings() format "START:END" for an interval of the form [START,END), "START:" for an interval of the form [START,INF), and ":END" for an interval of the form (-INF,END) (default = retain all coincidences)',
    )
    ...
    if options.coinc_end_time_segment is not None:
        ...
        options.coinc_end_time_segs = segmentsUtils.from_range_strings(
            [options.coinc_end_time_segment], boundtype=lal.LIGOTimeGPS
        ).coalesce()
    else:
