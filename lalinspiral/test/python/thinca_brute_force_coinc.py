...
def parse_command_line():
    ...
    parser.add_option(
        ...
        type=float,
        default=0.005,
        help="Coincidence window (default = 0.005 s).",
    )
    parser.add_option(
        "--min-instruments",
        metavar="count",
...
for filename in filenames:
    ...
    process = ligolw_process.register_to_xmldoc(
        xmldoc, "brute_force_coinc", process_params
    )
    ...
    # care about them.
    instruments = list(
        set(row.ifo for row in lsctables.SnglInspiralTable.get_table(xmldoc))
    )
    dt = dict(
        (frozenset(pair), options.delta_t + snglcoinc.light_travel_time(*pair))
        for pair in itertools.combinations(instruments, 2)
    )

    sngls = dict.fromkeys(
        row.template_id for row in lsctables.SnglInspiralTable.get_table(xmldoc)
    )
        ...
        for time_slide_id, offsetvector in offsetvectors.items():
            ...
            for n in range(len(instruments), options.min_instruments - 1, -1):
                for select_instruments in itertools.combinations(instruments, n):
                    ...
                    for sngls in itertools.product(
                        *map(sngls_by_instrument.__getitem__, select_instruments)
                        ...
                        if any(
                            ...
                            for (instrument_a, t_a), (
                                instrument_b,
                                t_b,
                            ) in itertools.combinations(ends, 2)
                        ):
                            continue
                        ...
                        exclude |= {
                            frozenset(x)
                            for i in range(n - 1, options.min_instruments - 1, -1)
