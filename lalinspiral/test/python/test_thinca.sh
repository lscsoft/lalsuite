...
for i in 1 2 ; do
	...
	${SRCDIR}/thinca_brute_force_coinc --verbose --delta-t 0.005 --min-instruments ${i} brute_min${i}.xml.gz
	...
	${SRCDIR}/thinca_compare_coinc_lists --verbose thinca_min${i}.xml.gz ${SRCDIR}/test_thinca_correct_min${i}.xml.gz
	${SRCDIR}/thinca_compare_coinc_lists --verbose brute_min${i}.xml.gz ${SRCDIR}/test_thinca_correct_min${i}.xml.gz
done
