...
class index(object):
    def __init__(self, xmldoc):
        coinc_def_id = lsctables.CoincDefTable.get_table(xmldoc).get_coinc_def_id(
            search=InspiralCoincDef.search,
            search_coinc_type=InspiralCoincDef.search_coinc_type,
            create_new=False,
        )

        self.sngls = dict(
            (row.event_id, row) for row in lsctables.SnglInspiralTable.get_table(xmldoc)
        )
        print(
            "document contains %d duplicate sngl_inspiral rows"
            % (len(lsctables.SnglInspiralTable.get_table(xmldoc)) - len(self.sngls))
        )
        self.coincs = dict(
        ...
        coinc_map = [
            ...
            and row.coinc_event_id in coinc_event_ids
        ]
        coinc_map.sort(key=lambda row: row.coinc_event_id)
        print(
            "document is missing %d sngl_inspiral rows that appear in coincs"
            % (len(set(row.event_id for row in coinc_map) - set(self.sngls)))
        )
        print(
            "document has %d sngl_inspiral rows that do not appear in coincs"
            % (len(set(self.sngls) - set(row.event_id for row in coinc_map)))
        )
        self.coinc_map = {}
    ...
    def min_time(self, time_slide_id, event_ids):
        ...
        return min(
            sngl.end + offsetvector[sngl.ifo] for sngl in self.get_sngls(event_ids)
        )
        ...
        for sngl in sorted(sngls, key=lambda row: row.ifo):
            ...
            print(
                ...
                % (
                    ...
                    sngl.end + offsetvector[sngl.ifo],
                )
            )
...
for time_slide_id in indexes[0].offsetvectors:
    for n, event_ids in enumerate(
        sorted(
            set(indexes[0].coinc_map[time_slide_id])
            - set(indexes[1].coinc_map[time_slide_id]),
            key=lambda event_ids: indexes[0].min_time(time_slide_id, event_ids),
        ),
        start=n,
    ):
        print("%d:" % n)
...
for time_slide_id in indexes[0].offsetvectors:
    for n, event_ids in enumerate(
        sorted(
            set(indexes[1].coinc_map[time_slide_id])
            - set(indexes[0].coinc_map[time_slide_id]),
            key=lambda event_ids: indexes[1].min_time(time_slide_id, event_ids),
        ),
        start=n,
    ):
        print("%d:" % n)
