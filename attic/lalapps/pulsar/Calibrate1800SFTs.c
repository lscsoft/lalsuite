...
int CalibrateSfts(struct CommandLineArgsTag CLA)
{
  FILE *fp,*fpo;
  INT4 i,j,t,k=0,q;
  REAL4 alpha,alpha_beta,a[100],ab[100],err_alpha,err_alpha_beta;
  COMPLEX8 R,C,H;
  size_t errorcode;
...
int ReadCommandLine(int argc,char *argv[],struct CommandLineArgsTag *CLA)
{
  ...
  while (!errflg && ((c = getopt(argc, argv,"hb:D:r:I:C:o:n:"))!=-1))
    switch (c) {
    case 'D':
      /* SFT directory */
    ...
    case 'h':
      ...
      fprintf(stderr,"\t-n\tINTEGER\t(Number of alphas to be used per SFT (eg. for an 1800s SFT n=30)\n");
      fprintf(stderr,"\t-o\tSTRING\t(Ouput directory for calibrated SFTs)\n");
      fprintf(stderr,"(eg: ./CalibrateSFTs -D ../KnownPulsarDemo/data/ -n 30 -C ./ -r S1 -I L1 -o ./) \n");
      exit(0);
      break;
    default:
      /* unrecognized option */
