...
{
    LowLatencyData *data =
        (LowLatencyData *) calloc(1, sizeof(LowLatencyData));
    if (!data)
        return NULL;

    strcpy(data->path, data_path);

    snprintf(data->pattern, FILENAME_MAX, "%s-%s-*-*.gwf", observatory,
        frame_type);
...
static char *XLALLowLatencyDataNextFilename(LowLatencyData * data)
{
    ...
    do {
        ...
        do {
            ...
            data->next =
                (struct inotify_event *)((char *)evt +
                sizeof(struct inotify_event) + evt->len);
    ...
    } while (!filename || (data->last_filename
            && strcmp(filename, data->last_filename) <= 0));
    ...
    do {        /* Loop until candidate filename is a non-empty string. */
        ...
        for (filename[0] = '\0', rewinddir(data->dir);
            errno = 0, entry = readdir(data->dir);) {
            int match;
...
void *XLALLowLatencyDataNextBuffer(LowLatencyData * data, size_t * size)
{
    ...
    {
        int ret =
            snprintf(pathname, sizeof(pathname), "%s/%s", data->path,
            filename);
