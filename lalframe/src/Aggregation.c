...
LIGOTimeGPS *XLALAggregationFrameStart(LIGOTimeGPS * gps)
{
    ...
    start->gpsSeconds =
        (INT4) floor(gps->gpsSeconds / LAL_ONLINE_FRAME_DURATION) *
...
LALCache *XLALAggregationFrameCache(CHAR * ifo,
    LIGOTimeGPS * start, REAL8 duration)
{
    /* declare variables */
    LIGOTimeGPS gps;
    LIGOTimeGPS *frame_start;
    ...
    frame_duration =
        (last_frame_start->gpsSeconds + LAL_ONLINE_FRAME_DURATION) -
    ...
    LIGOTimeGPS * start, REAL8 duration)
{
    ...
    series =
        XLALFrStreamReadREAL8TimeSeries(stream, channel, start, duration, 0);
    if (series == NULL) {
        /* failed to read data */
    ...
    LIGOTimeGPS * start, REAL8 duration)
{
    ...
    series =
        XLALFrStreamReadINT4TimeSeries(stream, channel, start, duration, 0);
    if (series == NULL) {
        /* failed to read data */
        XLALFrStreamClose(stream);
        XLAL_ERROR_NULL(XLAL_EINVAL);
    ...
    LIGOTimeGPS * start, REAL8 duration)
{
    ...
    state =
        XLALFrStreamReadREAL4TimeSeries(stream, channel, start, duration, 0);
    if (state == NULL) {
        /* failed to read data */
        XLALFrStreamClose(stream);
        XLAL_ERROR_NULL(XLAL_EINVAL);
    ...
    series = XLALCreateINT4TimeSeries(state->name, &state->epoch,
        state->f0, state->deltaT, &state->sampleUnits, state->data->length);
...
REAL8TimeSeries *XLALAggregationDQStrainData(CHAR * ifo,
    LIGOTimeGPS * start, REAL8 duration, INT4 dq_bitmask)
{
    /* declare variables */
...
UINT4 XLALAggregationDQGapStart(INT4TimeSeries * series, INT4 dq_bitmask)
{
    /* declare variables */
    UINT4 i;
    UINT4 gap = 0;
...
UINT4 XLALAggregationDQGapEnd(INT4TimeSeries * series, INT4 dq_bitmask)
{
    /* declare variables */
    UINT4 i;
    UINT4 gap = 0;
...
UINT4 XLALAggregationDQGap(INT4TimeSeries * series, INT4 dq_bitmask)
{
    /* declare variables */
    UINT4 gap = 0;
...
REAL8TimeSeries *XLALAggregationStrainDataWait(CHAR * ifo,
    LIGOTimeGPS * start, REAL8 duration, UINT4 max_wait)
{
    /* declare variables */
    LALFrStream *stream;
    REAL8TimeSeries *series;
 ...
 * available */
INT4 XLALAggregationStatFiles(CHAR * ifo, LIGOTimeGPS * start, REAL8 duration)
{
    /* declare variables */
    LIGOTimeGPS time_now;
    LALCache *cache;
