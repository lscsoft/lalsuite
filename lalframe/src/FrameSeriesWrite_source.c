...
int XWFUNC(STYPE * series, int frnum)
{
    ...
    if (isupper(series->name[0])) {
        ...
        if (isdigit(series->name[1]) && series->name[2] == ':') {
            ...
            for (detector = 0; detector < LAL_NUM_DETECTORS; ++detector)
                if (!strcmp
                    (ifo,
                     lalCachedDetectors[detector].frDetector.prefix)) {
    ...
    /* construct file name */
    c = snprintf(fname, sizeof(fname), "%c-%s-%d-%d.gwf", site,
                 description, t0, dt);
    ...
    vect =
        FrVectNew1D(series->name, FRTYPE, series->data->length,
    ...
    frame =
        XLALFrameNew(&series->epoch, duration, ifo, run, frnum,
                     detectorFlag);
    if (!frame) {
        FrVectFree(vect);
        XLAL_ERROR(XLAL_EFUNC);
...
void WFUNC(LALStatus * status, STYPE * series, FrOutPar * params)
{
    TYPE *data;
    CHAR seconds[] = "s";
    ...
    /*** CHECK FSHIFT ***/
        vect =
            makeFrVect1D(frame, params->type, series->name, comment,
                         seconds, units, FRTYPE,
                         series->deltaT ? 1.0 / series->deltaT : 0.0,
...
void
FSWFUNC(LALStatus * status,
        FSTYPE * series, FrOutPar * params, INT4 subtype)
{
    TYPE *data;
    CHAR hertz[] = "Hz";
