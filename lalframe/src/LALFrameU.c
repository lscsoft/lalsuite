...
#	define CASE_FRAMEL(errval, function, ...) \
		case LAL_FRAMEU_FRAMELIB_FRAMEL: return function ## _FrameL_ (__VA_ARGS__)
#	ifndef LAL_FRAMEU_FRAME_LIBRARY_DEFAULT
#		define LAL_FRAMEU_FRAME_LIBRARY_DEFAULT LAL_FRAMEU_FRAME_LIBRARY_FRAMEL
#	endif
#else
#	define CASE_FRAMEL(errval, function, ...) \
		case LAL_FRAMEU_FRAMELIB_FRAMEL: XLAL_ERROR_VAL(errval, XLAL_EERR, "FrameL library unavailable")
#endif

/* enable FrameC support if available */
#if defined HAVE_FRAMECPPC_FRAMEC_H && defined HAVE_LIBFRAMECPPC
#	include "LALFrameUFrameC.h"
#	define CASE_FRAMEC(errval, function, ...) \
		case LAL_FRAMEU_FRAMELIB_FRAMEC: return function ## _FrameC_ (__VA_ARGS__)
#	ifndef LAL_FRAMEU_FRAME_LIBRARY_DEFAULT
#		define LAL_FRAMEU_FRAME_LIBRARY_DEFAULT LAL_FRAMEU_FRAME_LIBRARY_FRAMEC
#	endif
#else
#	define CASE_FRAMEC(errval, function, ...) \
		case LAL_FRAMEU_FRAMELIB_FRAMEC: XLAL_ERROR_VAL(errval, XLAL_EERR, "FrameC library unavailable")
#endif
...
#				else
					XLAL_ERROR_VAL(LAL_FRAMEU_FRAME_LIBRARY_UNAVAILABLE, XLAL_ESYS, "LAL_FRAME_LIBRARY=%s: FrameL frame library not available", env);
#				endif
			} else if (strcmp(env, "FrameC") == 0) {
#				if defined HAVE_FRAMECPPC_FRAMEC_H && defined HAVE_LIBFRAMECPPC
					lalFrameLibrary = LAL_FRAMEU_FRAME_LIBRARY_FRAMEC;
#				else
					XLAL_ERROR_VAL(LAL_FRAMEU_FRAME_LIBRARY_UNAVAILABLE, XLAL_ESYS, "LAL_FRAME_LIBRARY=%s: FrameC frame library not available", env);
#				endif
			}
			else {
				XLAL_PRINT_WARNING("LAL_FRAME_LIBRARY=%s: No valid frame library specified [expect: \"FrameL\" or \"FrameC\"]");
...
{
	FRAME_LIBRARY_SELECT_NULL(XLALFrameUFrProcChanAlloc, type, subtype, dtype, ndata);
}
...
{
	FRAME_LIBRARY_SELECT_NULL(XLALFrameUFrDetectorAlloc, name, prefix, latitude, longitude, elevation, azimuthX, azimuthY, altitudeX, altitudeY, midpointX, midpointY, localTime);
}
