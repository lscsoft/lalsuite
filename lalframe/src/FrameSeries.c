...
static struct FrVect *makeFrVect1D(struct FrameH *frame, int chtype,
                                   char *name, char *comment, char *unitx,
                                   char *unity, int datatype, double rate,
                                   double fshift, double dx,
                                   unsigned int npts)
{
    struct FrVect *vect =
        FrVectNew1D(name, datatype, npts, dx, unitx, unity);
    if (!vect)
        return NULL;
    if (chtype == ProcDataChannel) {
        ...
        frame->procData = proc;
        if (!FrStrCpy(&proc->name, name))
            return NULL;
        if (!FrStrCpy(&proc->comment, comment))
            return NULL;
        ...
        raw->firstAdc = adc;
        if (!FrStrCpy(&adc->name, name))
            return NULL;
        if (!FrStrCpy(&adc->comment, comment))
            return NULL;
        if (!FrStrCpy(&adc->units, unity))
            return NULL;
        ...
        frame->simData = sim;
        if (!FrStrCpy(&sim->name, name))
            return NULL;
        if (!FrStrCpy(&sim->comment, comment))
            return NULL;
    } else {
        FrVectFree(vect);
        return NULL;
                                  ...
                                  LALFrStream * stream)
{
    ...
    if (!stream->file->toc) {
        if (FrTOCReadFull(stream->file) == NULL) {
            XLALPrintError
                ("XLAL Error - %s: could not open frame TOC %s\n",
                       ...
                       FrChanIn * chanin, LALFrStream * stream)
{
    ...
    ASSERT(chanin->name, status, FRAMESTREAMH_ENULL,
...
static int copy_FrVect_to_REAL8(REAL8 * data, struct FrVect *vect,
                                size_t ncpy, size_t offset)
{
    size_t i;
...
#	endif
    ...
    series =
        XLALCreateREAL8TimeSeries(channel, &epoch, 0.0, vect->dx[0],
                                  &lalADCCountUnit, need);
    if (!series)
     ...
     * nearest nanosecond */
    SET_EPOCH(&stream->epoch,
              EPOCH_TO_I8TIME(series->epoch) +
