 ...
 * that are written in chronological order, with irrelevant frames from a
 * different detector added in.
 *
...
static void write_dummy_frame(long gps_start_time, int duration)
{
  ...
  /* Irrelevant frame */
  XLALGPSSet(&epoch, gps_start_time, 0);
  sprintf(filename, "%s/Y-R-%010ld-%d.gwf", tempdir, gps_start_time, duration);
  frame = XLALFrameNew(&epoch, duration, "FOOBAR", 0, 0, 0);
  XLALFrameWrite(frame, filename, 0);
}
