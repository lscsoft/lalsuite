...
def make_input_test_document(
    ...
    process = ligolw_process.register_to_xmldoc(
        xmldoc,
        "snglcoinc_coincs_verify",
        {},
        instruments=segs.keys(),
        comment="artificial data for coincidence engine test suite",
    )

    # add time slide information
    for i in range(n_offset_vectors):
        ligolw_time_slide.get_time_slide_id(
            xmldoc,
            offsetvector.offsetvector(
                (
                    instrument,
                    random.uniform(-28.0 * n_offset_vectors, +28.0 * n_offset_vectors),
                )
    ...
    snglbursttable = xmldoc.childNodes[-1].appendChild(
        lsctables.SnglBurstTable.new(
            ["event_id", "ifo", "peak_time", "peak_time_ns", "process:process_id"]
        )
    )
    ...
    for instrument, segment in segs.items():
        for i in range(
            numpy.random.poisson(mean_rates[instrument] * float(abs(segment)))
        ):
            snglbursttable.append(
                snglbursttable.RowType(
                    event_id=snglbursttable.get_next_id(),
                    ifo=instrument,
                    peak=segment[0] + random.uniform(0.0, float(abs(segment))),
...
def do_brute_force_coinc(xmldoc, delta_t=0.015, min_instruments=1):
    ...
    # care about them.
    instruments = list(
        set(row.ifo for row in lsctables.SnglBurstTable.get_table(xmldoc))
    )
    dt = dict(
        (frozenset(pair), delta_t + snglcoinc.light_travel_time(*pair))
    ...
    for time_slide_id, offsetvector in (
        lsctables.TimeSlideTable.get_table(xmldoc).as_dict().items()
    ):
        ...
        for n in range(len(instruments), min_instruments - 1, -1):
            ...
            for select_instruments in itertools.combinations(instruments, n):
                offsets = tuple(
                    offsetvector[instrument] for instrument in select_instruments
                )
                total = functools.reduce(
                    lambda a, b: a * b,
                    map(
                        lambda instrument: len(sngls_by_instrument[instrument]),
                        select_instruments,
                    ),
                    1,
                )
                ...
                for sngls in tqdm(
                    itertools.product(
                        *map(sngls_by_instrument.__getitem__, select_instruments)
                    ),
                    total=total,
                    desc=", ".join(sorted(select_instruments)),
                ):
                    # if any Delta t fails coincidence,
                    # discard this combination of
                    # triggers
                    if any(
                        abs(t_a - t_b) > dt[frozenset((instrument_a, instrument_b))]
                        for (instrument_a, t_a), (
                            instrument_b,
                            t_b,
                        ) in itertools.combinations(
                            (
                                (instrument, sngl.peak + offset)
                                for instrument, sngl, offset in zip(
                                    select_instruments, sngls, offsets
                                )
                            ),
                            2,
                        )
                    ...
                    if event_ids in exclude:
                        continue
                    # record this coinc
                    coinctables.append_coinc(
                        *coinctables.coinc_rows(
                            process.process_id, time_slide_id, sngls, "sngl_burst"
                        )
                    )
                    exclude |= {
                        frozenset(x)
                        for i in range(n - 1, min_instruments - 1, -1)
                        for x in itertools.combinations(event_ids, i)
                    }
...
class coincgen_doubles(snglcoinc.coincgen_doubles):
    ...
    class get_coincs(object):
        ...
        def __call__(self, event_a, offset_a, coinc_window):
            ...
            return self.events[
                bisect_left(self.times, peak - coinc_window) : bisect_right(
                    self.times, peak + coinc_window
                )
            ]
...
def do_snglcoinc(xmldoc, delta_t=0.015, min_instruments=1):
    ...
    # searches
    with tqdm(
        desc="snglcoinc", total=len(lsctables.SnglBurstTable.get_table(xmldoc))
    ) as progress:
        for instrument, events in itertools.groupby(
            sorted(
                lsctables.SnglBurstTable.get_table(xmldoc),
                key=lambda row: (row.peak, row.ifo),
            ),
            lambda event: event.ifo,
        ):
            events = tuple(events)
            progress.update(len(events))
            if time_slide_graph.push(
                instrument, events, max(event.peak for event in events)
            ):
                for node, events in time_slide_graph.pull():
                    coinctables.append_coinc(
                        *coinctables.coinc_rows(
                            process.process_id, node.time_slide_id, events, "sngl_burst"
                        )
                    )
...
class summary(object):
    def __init__(self, xmldoc, algorithm_name):
        (process_id,) = lsctables.ProcessTable.get_table(xmldoc).get_ids_by_program(
            algorithm_name
        )

        coinc_def_id = lsctables.CoincDefTable.get_table(xmldoc).get_coinc_def_id(
            search=burca.StringCuspBBCoincDef.search,
            search_coinc_type=burca.StringCuspBBCoincDef.search_coinc_type,
            create_new=False,
        )

        self.sngls = dict(
            (row.event_id, row) for row in lsctables.SnglBurstTable.get_table(xmldoc)
        )
        if len(lsctables.SnglBurstTable.get_table(xmldoc)) - len(self.sngls):
            raise ValueError(
                "document contains %d duplicate sngl_burst rows"
                % (len(lsctables.SnglBurstTable.get_table(xmldoc)) - len(self.sngls))
            )
        self.coincs = dict(
            (row.coinc_event_id, row)
            for row in lsctables.CoincTable.get_table(xmldoc)
            if row.coinc_def_id == coinc_def_id and row.process_id == process_id
        )
        ...
        coinc_map = [
            ...
            if row.table_name == "sngl_burst" and row.coinc_event_id in coinc_event_ids
        ]
        coinc_map.sort(key=lambda row: row.coinc_event_id)
        self.coinc_map = {}
        for time_slide_id in self.offsetvectors:
            self.coinc_map[time_slide_id] = dict(
                (frozenset(row.event_id for row in rows), coinc_event_id)
                for coinc_event_id, rows in itertools.groupby(
                    coinc_map, key=lambda row: row.coinc_event_id
                )
                if self.coincs[coinc_event_id].time_slide_id == time_slide_id
            )
    ...
    def min_time(self, time_slide_id, event_ids):
        ...
        return min(
            sngl.peak + offsetvector[sngl.ifo] for sngl in self.get_sngls(event_ids)
        )
    ...
    def print_summary(self, time_slide_id, event_ids):
        ...
        for sngl in sorted(sngls, key=lambda row: row.ifo):
            ...
            print(
                "\tevent ID %d: %s: %s + %g s = %s"
                % (
                    sngl.event_id,
                    sngl.ifo,
                    sngl.peak,
                    offsetvector[sngl.ifo],
                    sngl.peak + offsetvector[sngl.ifo],
                )
            )
...
def parse_command_line():
    ...
    parser.add_option(
        ...
        metavar="mode",
        default="test",
        help='Select operating mode.  Allowed values are "initialize" (generate random test document), "brute-force" (apply brute-force algorithm to test document to construct correct-answer document), "test" (default;  apply snglcoinc algorithm to test document and compare to brute-force result contained in the document).',
    )
    parser.add_option(
        "--delta-t",
        metavar="seconds",
        type=float,
        default=0.015,
        help="Coincidence window (default = 0.015 s).",
    )
    parser.add_option(
        "--min-instruments",
        metavar="count",
        type=int,
        default=1,
        help="Require candidates to have at least this many instruments participating (default = 2).",
    )
    ...
    if not filenames:
        filenames = [
            os.path.join(
                os.environ.get("LAL_TEST_SRCDIR", "."),
...
if options.mode == "initialize":
    ...
    do_brute_force_coinc(
        xmldoc, delta_t=options.delta_t, min_instruments=options.min_instruments
    )
    ligolw_utils.write_filename(xmldoc, filename, verbose=options.verbose)
    ...
    for time_slide_id in summaries[0].offsetvectors:
        for n, event_ids in enumerate(
            sorted(
                set(summaries[0].coinc_map[time_slide_id])
                - set(summaries[1].coinc_map[time_slide_id]),
                key=lambda event_ids: summaries[0].min_time(time_slide_id, event_ids),
            ),
            start=m,
        ):
            print("%d:" % n)
    ...
    for time_slide_id in summaries[0].offsetvectors:
        for n, event_ids in enumerate(
            sorted(
                set(summaries[1].coinc_map[time_slide_id])
                - set(summaries[0].coinc_map[time_slide_id]),
                key=lambda event_ids: summaries[1].min_time(time_slide_id, event_ids),
            ),
            start=m,
        ):
            print("%d:" % n)
