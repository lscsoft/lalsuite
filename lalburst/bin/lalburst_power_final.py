...
def parse_command_line():
    ...
    parser.add_option(
        ...
        default=[],
        action="append",
        help="Shell filename pattern for non-injection files (required).",
    )
    parser.add_option(
        "-i",
        "--injections-glob",
        ...
        default=[],
        action="append",
        help="Shell filename pattern for injection files (required).",
    )
    parser.add_option(
        "-l",
        "--live-time-program",
        ...
        help='Set the name, as it appears in the process table, of the program whose search summary "out" segments define the search live time (default = "lalapps_power")',
    )
    parser.add_option(
        "--open-box",
        action="store_true",
        help="Open the box.  If this is not set, then instead of setting a threshold based on the loudest zero-lag coincidence, the threshold is set at the predicted loudest-event amplitude as derived from the background.",
    )
    parser.add_option(
        "--confidence-contour-slope",
        metavar="slope",
        type="float",
        default=-12.0,
        help="Set the slope of the confidence-likelihood joint threshold contours (default = -12.0).",
    )
    parser.add_option(
        "--dump-scatter-data",
        action="store_true",
        help="Instead of computing upper limit, dump data files for confidence-likelihood scatter diagnostic scatter plot.",
    )
    parser.add_option(
        "--plot-scatter-data",
        action="store_true",
        help="Instead of computing upper limit, load data files for confidence-likelihood scatter diagnostic scatter plot and generate scatter plot.",
    )
    parser.add_option(
        "-t",
        "--tmp-space",
    ...
    parser.add_option(
        ...
        type="float",
        default=90,
        help="Set the confidence in percent of the upper limit to derive (default = 90.0).",
    )
    parser.add_option(
        "--upper-limit-scale",
        metavar="E|hrss",
        default="E",
        help="Select the scale on which to measure the upper limit.  Allowed values are 'E' for an upper limit on rate of bursts by energy, and 'hrss' for an upper limit on rate of bursts by h_{rss}.  (default = 'E')",
    )
    parser.add_option(
        "--zero-lag-survivors",
        metavar="number",
        type="int",
        default=0,
        help='Set the final confidence-likelihood joint threshold so that this many events survive at zero lag.  If n survivors are requested, then the threshold is set at exactly the amplitude of the n+1 loudest event, and events are discarded if their amplitudes are <= the threshold.  A "loudest-event" upper limit is obtained by setting this to 0.  The default is 0.',
    )
...
def dump_confidence_likelihood_scatter_data(
    globs, live_time_program="lalapps_power", tmp_path=None, verbose=False
):
    #
    # Collect file names.
    #
    ...
    for n, filename in enumerate(filenames):
        ...
        if database.sim_burst_table is None:
            # non-injections
            for id, l, c, is_background in bb_id_likelihood_confidence_background(
                database
            ):
...
WHERE
	sim_coinc_def_id == ?
        ...
        dbtables.discard_connection_filename(
            filename, working_filename, verbose=verbose
        )
...
def plot_confidence_likelihood_scatter_data(slope, verbose=False):
    ...
    set1 = read_and_plot(
        "lalburst_power_final_injections_scatter.dat", "r+", verbose=verbose
    )
    set2 = read_and_plot(
        "lalburst_power_final_background_scatter.dat", "k+", verbose=verbose
    )
    ...
    axes.legend(
        (set1, set2), (r"Injections", r"Background (time slides)"), loc="lower right"
    )
...
class RateVsThresholdData(object):
    ...
    def update_from(self, contents, filename="", verbose=False):
        ...
        self.zero_lag_live_time += SnglBurstUtils.time_slides_livetime(
            contents.seglists, zero_lag_time_slides.values(), verbose=verbose
        )
        self.background_live_time += SnglBurstUtils.time_slides_livetime(
            contents.seglists, background_time_slides.values(), verbose=verbose
        )
        ...
        for (
            ...
            confidence,
            is_background,
        ) in bb_id_likelihood_confidence_background(contents):
    ...
    def finish(self, zero_lag_survivors, open_box=False, verbose=False):
        ...
        self.background_rate_y = (
            numpy.arange(1, len(self.background_rate_x) + 1, dtype="double")
        ...
        self.background_rate_dy = (
            numpy.sqrt(self.background_rate_y * self.zero_lag_live_time)
        ...
        if open_box:
            ...
            self.amplitude_threshold = self.background_rate_x[
                -1
                - bisect.bisect(
                    self.background_rate_y[::-1], 1.0 / self.zero_lag_live_time
                )
            ]
        ...
        spline = interpolate.UnivariateSpline(
            self.background_rate_x[lo:hi], numpy.log(self.background_rate_y[lo:hi]), k=3
        )
        self.background_rate_approximant = lambda x: numpy.exp(spline(x))

        self.mu_0 = math.exp(spline(self.amplitude_threshold))
        self.dmu_0 = (
            numpy.sqrt(self.mu_0 * self.background_live_time)
            / self.background_live_time
        )
        ...
        self.zero_lag_rate_x = numpy.array(
            [zero_lag_amplitude[0] for zero_lag_amplitude in self.zero_lag_amplitudes],
            dtype="double",
        )
        self.zero_lag_rate_y = (
            numpy.arange(1, len(self.zero_lag_rate_x) + 1, dtype="double")
            / self.zero_lag_live_time
        )
...
def measure_threshold(
    ...
    for n, filename in enumerate(filenames):
        ...
        database = SnglBurstUtils.CoincDatabase(
            sqlite3.connect(str(working_filename)), live_time_program
        )
        if verbose:
            SnglBurstUtils.summarize_coinc_database(database)
...
def print_rate_vs_threshold_data(rate_vs_threshold_data, confidence_contour_slope):
    ...
    print(file=sys.stderr)
    print(
        "threshold definition:  ln likelihood > %.16g ln confidence + %.16g"
        % (confidence_contour_slope, rate_vs_threshold_data.amplitude_threshold),
        file=sys.stderr,
    )
    print(
        "total live time in background = %.16g s"
        % rate_vs_threshold_data.background_live_time,
        file=sys.stderr,
    )
    print(
        "total live time at zero lag = %.16g s"
        % rate_vs_threshold_data.zero_lag_live_time,
        file=sys.stderr,
    )
    print(
        "number of coincs in background = %d"
        % rate_vs_threshold_data.n_background_amplitudes,
        file=sys.stderr,
    )
    print(
        "average number of background coincs per zero lag live time = %.16g"
        % (
            rate_vs_threshold_data.n_background_amplitudes
            / rate_vs_threshold_data.background_live_time
            * rate_vs_threshold_data.zero_lag_live_time
    ...
    print(
        "number of coincs at zero lag = %d"
        % len(rate_vs_threshold_data.zero_lag_amplitudes),
        file=sys.stderr,
    )
    print(
        "at threshold, \\mu_{0} = %.16g Hz +/- %.16g Hz"
        % (rate_vs_threshold_data.mu_0, rate_vs_threshold_data.dmu_0),
        file=sys.stderr,
    )
    print(
        "at threshold, \\mu_{0}' = %.16g Hz / unit of amplitude"
        % rate_vs_threshold_data.mu_0primed,
        file=sys.stderr,
    )
    print(file=sys.stderr)
...
def plot_rate_vs_threshold(data):
    ...
    fig, axes = SnglBurstUtils.make_burst_plot(
        r"Detection Statistic Threshold", r"Mean Event Rate (Hz)"
    )
    axes.semilogy()
    ...
    poly_y = numpy.concatenate(
        (
            data.background_rate_y + 1 * data.background_rate_dy,
            (data.background_rate_y - 1 * data.background_rate_dy)[::-1],
        )
    )
    axes.add_patch(
        patches.Polygon(
            numpy.column_stack((poly_x, numpy.clip(poly_y, ymin, ymax))),
            edgecolor="k",
            facecolor="k",
            alpha=0.3,
        )
    ...
    interp_y = interpolate.interpolate.interp1d(
        data.background_rate_x,
        data.background_rate_y,
        kind="linear",
        bounds_error=False,
    )
    interp_dy = interpolate.interpolate.interp1d(
        data.background_rate_x,
        data.background_rate_dy,
        kind="linear",
        bounds_error=False,
    )
    ...
    axes.plot(
        data.zero_lag_rate_x,
        (data.zero_lag_rate_y - interp_y(data.zero_lag_rate_x))
        / interp_dy(data.zero_lag_rate_x),
        "k-",
    )
    ...
    print(
        "writing lalburst_power_final_rate_vs_threshold_residual.png ...",
        file=sys.stderr,
    )
    ...
    def add_contents(self, contents, threshold):
        if self.instruments != contents.instruments:
            raise ValueError(
                ...
                % ("+".join(contents.instruments), "+".join(self.instruments))
            )
...
FROM
		...
		""",
            ...
            if (
                SimBurstUtils.on_instruments(
                    sim, contents.seglists, offsetvectors[sim.time_slide_id]
                )
                == self.instruments
            ):
                # injection was made
                ...
                if detection_statistic is not None:
                    ...
                    # all)
                    self.recovered_xyz.append(
                        (sim.frequency, amplitude, detection_statistic)
                    )
            ...
            elif (detection_statistic is not None) and (
                detection_statistic > threshold
            ):
                ...
                print(
                    "odd, injection %s was found but not injected ..."
                    % sim.simulation_id,
                    file=sys.stderr,
                )
    ...
    def finish(self, threshold):
        ...
        amplitude_weight = rate.BinnedArray(
            rate.NDBins((rate.LinearBins(threshold - 100, threshold + 100, 10001),))
        )
        ...
        for x, y in enumerate(
            numpy.max(
                numpy.where(
                    (d > 0) & (numpy.roll(d, -1, axis=1) <= 0),
                    numpy.indices(d.shape)[1],
                    bady,
                ),
                axis=1,
            )
        ...
        diagnostic_plot(
            self.efficiency.numerator.array,
            self.efficiency.denominator.bins,
            r"Efficiency Numerator (Before Averaging)",
            self.amplitude_lbl,
            "lalburst_power_final_efficiency_numerator_before.png",
        )
        diagnostic_plot(
            self.efficiency.denominator.array,
            self.efficiency.denominator.bins,
            r"Efficiency Denominator (Before Averaging)",
            self.amplitude_lbl,
            "lalburst_power_final_efficiency_denominator_before.png",
        )
        diagnostic_plot(
            self.found_density.array,
            self.efficiency.denominator.bins,
            r"Injections Lost / Unit of Threshold (Before Averaging)",
            self.amplitude_lbl,
        ...
        diagnostic_plot(
            self.efficiency.numerator.array,
            self.efficiency.denominator.bins,
            r"Efficiency Numerator (After Averaging)",
            self.amplitude_lbl,
        ...
        diagnostic_plot(
            self.efficiency.denominator.array,
            self.efficiency.denominator.bins,
            r"Efficiency Denominator (After Averaging)",
            self.amplitude_lbl,
        ...
        diagnostic_plot(
            self.found_density.array,
            self.efficiency.denominator.bins,
            r"Injections Lost / Unit of Threshold (After Averaging)",
            self.amplitude_lbl,
        ...
        self.dfound_density = numpy.sqrt(
            p * (1 - p) / self.efficiency.denominator.array
        )
...
def measure_efficiency(
    ...
    if upper_limit_scale == "E":
        efficiency = EfficiencyData(
            ...
            r"Equivalent Isotropic Energy ($M_{\odot} / \mathrm{pc}^{2}$)",
            0.1,
        )
...
def rate_upper_limit(efficiency_data, mu_0primed, zero_lag_live_time, p):
    ...
    e_over_eprimed = (
        efficiency_data.efficiency.numerator.array
        / -efficiency_data.found_density.array
    )
    ...
    diagnostic_plot(
        xi,
        rate_data.rate_array.bins,
        r"Background Correction Factor $\xi$",
        rate_data.amplitude_lbl,
...
def plot_rate_upper_limit(rate_data):
    ...
    axes.set_title(
        r"%g\%% Confidence Rate Upper Limit ($\log_{10} R_{%g} / 1\,\mathrm{Hz}$)"
        % (100 * rate_data.confidence, rate_data.confidence)
    )
    ...
    fig, axes = SnglBurstUtils.make_burst_plot(
        rate_data.amplitude_lbl, r"Rate Upper Limit (Hz)"
    )
    axes.loglog()
    ...
    axes.set_title(
        r"%g\%% Confidence Rate Upper Limit at $110\,\mathrm{Hz}$"
        % (100 * rate_data.confidence)
    )
...
def coinc_detection_statistic(
    likelihood, confidence, m=options.confidence_contour_slope
):
...
if options.dump_scatter_data:
    ...
    dump_confidence_likelihood_scatter_data(
        options.background_glob + options.injections_glob,
        live_time_program=options.live_time_program,
...
if options.plot_scatter_data:
    ...
    plot_confidence_likelihood_scatter_data(
        options.confidence_contour_slope, verbose=options.verbose
    )
    print(file=sys.stderr)
    print("=== Done ===", file=sys.stderr)
    sys.exit(0)
...
if options.verbose:
    print("building file list ...", file=sys.stderr)
filenames = sorted(
    filename for g in options.background_glob for filename in glob.glob(g)
)
if not filenames:
    raise ValueError("no background/zero lag files found")

if True:
    rate_vs_threshold_data = measure_threshold(
        filenames,
        options.zero_lag_survivors,
        live_time_program=options.live_time_program,
        tmp_path=options.tmp_space,
    ...
    print_rate_vs_threshold_data(
        rate_vs_threshold_data, options.confidence_contour_slope
    )
...
if options.verbose:
    print("building file list ...", file=sys.stderr)
filenames = sorted(
    filename for g in options.injections_glob for filename in glob.glob(g)
)
if not filenames:
...
efficiency_data = measure_efficiency(
    filenames,
    rate_vs_threshold_data.amplitude_threshold,
    live_time_program=options.live_time_program,
    upper_limit_scale=options.upper_limit_scale,
    tmp_path=options.tmp_space,
    verbose=options.verbose,
)
...
rate_data = rate_upper_limit(
    efficiency_data,
    rate_vs_threshold_data.mu_0primed,
    rate_vs_threshold_data.zero_lag_live_time,
    options.upper_limit_confidence,
)
