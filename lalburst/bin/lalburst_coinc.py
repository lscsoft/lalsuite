...
def parse_command_line():
    ...
    parser.add_option(
        ...
        type="int",
        default=2,
        help="Set the minimum number of instruments required to form a coincidence (default = 2).",
    )
    parser.add_option(
        "-t",
        "--threshold",
        metavar="threshold",
        type="float",
        help="Set the coincidence algorithm's threshold.  For excesspower this parameter is not used and must not be set.  For stringcusp, this parameter sets the maximum peak time difference in seconds not including light travel time (which will be added internally).",
    )
    ...
    if options.coincidence_algorithm not in ("excesspower", "stringcusp"):
        raise ValueError(
            "unrecognized --coincidence-algorithm %s" % options.coincidence_algorithm
        )
    if options.coincidence_segments is not None:
        options.coincidence_segments = segmentsUtils.from_range_strings(
            options.coincidence_segments.split(","), boundtype=lsctables.LIGOTimeGPS
        ).coalesce()
        if filenames is not None and len(filenames) > 1:
            raise ValueError(
                "refusing to allow use of --coincidence-segments with more than one input file"
            )
    ...
    if options.coincidence_algorithm == "stringcusp" and options.threshold is None:
        raise ValueError(
            "--threshold is required for --coincidence-algorithm stringcusp"
        )
    elif (
        options.coincidence_algorithm == "excesspower" and options.threshold is not None
    ):
        raise ValueError(
            "--threshold is meaningless for --coincidence-algorithm excesspower"
        )
...
if options.coincidence_segments is not None:
    ...
    def coinc_segs_ntuple_comparefunc(
        events,
        offset_vector,
        ...
        return (
            min(event.peak + offset_vector[event.ifo] for event in events)
            not in coinc_segs
        )
...
if options.coincidence_algorithm == "excesspower":
    ...
    ntuple_comparefunc = lambda *args: coinc_segs_ntuple_comparefunc(
        *args
    ) or burca.StringCuspCoincTables.ntuple_comparefunc(*args)
...
for n, filename in enumerate(filenames):
    ...
    if options.coincidence_algorithm == "excesspower":
        delta_t = 2.0 * bruca.ep_coincgen_doubles.get_coincs.max_edge_peak_delta(
