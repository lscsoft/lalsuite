...
def parse_command_line():
    ...
    parser.add_option(
        ...
        default=[],
        action="append",
        help="Also add likelihood data from this XML file.",
    )
    parser.add_option(
        "--add-from-cache",
        metavar="filename",
        help="Also add likelihood data from all XML files listed in this LAL cache.",
    )
    parser.add_option(
        "-o",
        "--output",
        metavar="filename",
        default=None,
        help="Set the name of the likelihood control file to write (default = stdout).",
    )
    parser.add_option(
        "-t",
        "--tmp-space",
    ...
    parser.add_option(
        ...
        help="Program from which to draw the livetime segments. (Necessary in case of giving --T010150.",
    )
    ...
    if options.add_from_cache:
        options.add_from += [
            CacheEntry(line).path for line in file(options.add_from_cache)
        ]
...
if options.add_from:
    c, s = distributions.from_filenames(
        options.add_from, "lalburst_power_meas_likelihood", verbose=options.verbose
    )
    distributions += c
    segs |= s
    del c
    ...
    if database.sim_burst_table is None:
        ...
        for is_background, events, offsetvector in database.get_noninjections():
            params = distributions.coinc_params(
                events, offsetvector, MW_CENTER_J2000_RA_RAD, MW_CENTER_J2000_DEC_RAD
            )
...
def T010150_basename(description, seglists):
    ...
    return "%s-%s-%s-%s" % (
        "+".join(sorted(seglists.keys())),
        description,
        str(int(seg[0])),
        str(int(math.ceil(abs(seg)))),
    )
