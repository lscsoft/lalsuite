...
def parse_command_line():
    ...
    parser.add_option(
        ...
        type="float",
        default=64.0,
        help="Set the low-frequency limit of the tiling in Hertz (default = 64).",
    )
    parser.add_option(
        "-d",
        "--delta-f",
        ...
        type="float",
        default=16.0,
        help="Set the bandwidth of the pixels in Hertz (default = 16).  Must be > 0.  The product of --delta-f and --delta-t must be at least 2/pi.",
    )
    parser.add_option(
        "-t",
        "--delta-t",
        ...
        type="float",
        default=1.0 / 16,
        help="Set the duration of the pixels in seconds (default = 1/16).  Must be > 0.  The product of --delta-f and --delta-t must be at least 2/pi.",
    )
    parser.add_option(
        "-f",
        "--overlap-fraction",
        ...
        type="float",
        default=0.25,
        help="Set the fraction by which adjacent tiles overlap (default = 0.25).  The pixels centres are spaced (1 - --overlap-fraction) * --delta-f apart in the frequency domain.  The value must be in the range [0, 1).",
    )
    parser.add_option(
        "-H",
        "--height",
        ...
        type="int",
        default=64,
        help="Set the number of pixles in the frequency domain (default = 64).  The image will be scaled to this vertical size, and the number of pixels in the time domain (horizontal size) will be fixed by the image aspect ratio.",
    )
    parser.add_option(
        "-o",
        "--output",
    ...
    parser.add_option(
        ...
        type="float",
        default=1e-20,
        help='Set the single-pixel hrss (root-sum-square strain) corresponding to "white" (default = 1e-20).',
    )
    ...
    parser.add_option(
        ...
        "--time-slide-xml",
        metavar="filename",
        help="Associate injections with the first time slide ID in this XML file (required).",
    )
    ...
    if not 0.0 < options.delta_f:
        raise ValueError("--delta-f must be > 0")
    if not 0.0 < options.delta_t:
        raise ValueError("--delta-t must be > 0")
    if options.delta_f * options.delta_t < 2.0 / math.pi:
        raise ValueError("the product of --delta-t and --delta-f must be >= 2/pi")
    if not (0.0 <= options.overlap_fraction < 1.0):
        raise ValueError("--overlap-fraction must be in [0, 1)")
    if (
        options.f_low
        + options.height * (1.0 - options.overlap_fraction) * options.delta_f
        >= options.sample_rate / 2.0
    ):
...
process = ligolw_process.register_to_xmldoc(
    xmldoc, "lalburst_inj_pic", options.options_dict, version=git_version.version
)
...
if options.verbose:
    print(
        "associating injections with time slide (%d) %s"
...
if options.verbose:
    print(
        "time-frequency tiles have %g degrees of freedom"
        % (2.0 * options.delta_t * options.delta_f),
        file=sys.stderr,
    )
...
for n, filename in enumerate(filenames, 1):
    ...
    progress = tqdm(
        desc="computing pixels", total=width * height, disable=not options.verbose
    )
    for i in range(width):
        for j in range(height):
            ...
            row = lsctables.SimBurst(
                ...
                waveform_number=0,
                pol_ellipse_e=0.0,
                pol_ellipse_angle=0.0,
                # time-frequency co-ordinates
                time_geocent_gps=options.gps_start_time
                + i * options.delta_t * (1.0 - options.overlap_fraction),
                frequency=options.f_low
                + j * options.delta_f * (1.0 - options.overlap_fraction),
