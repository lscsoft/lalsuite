...
from lalburst import SnglBurstUtils, burca_tailor, calc_likelihood, git_version
...
def parse_command_line():
    ...
    parser.add_option(
        ...
        "--program",
        metavar="name",
        help="Set the name of the program that generated the events as it appears in the process table (required).  The program name is used to extract live time information from the search summary tables in the input files.",
    )
    parser.add_option(
        "--likelihood-data",
        metavar="filename",
        default=[],
        action="append",
        help="Read likelihood data from this XML file.  (use lalburst_power_meas_likelihood to generate these files)",
    )
    parser.add_option(
...
def load_likelihood_data(filenames, verbose=False):
    distributions, ignored = (
        burca_tailor.EPGalacticCoreCoincParamsDistributions.from_filenames(
            filenames, "lalburst_power_meas_likelihood", verbose=verbose
        )
    )
...
for n, filename in enumerate(filenames):
    ...
    if options.likelihood_data_cache:
        likelihood_files = set(
            c.path
            for c in options.likelihood_data_cache
    ...
    calc_likelihood.assign_likelihood_ratios(
        ...
        events_func=lambda cursor, coinc_event_id: calc_likelihood.sngl_burst_events_func(
