...
def parse_command_line():
    ...
    parser.add_option(
        "--coinc-only",
        action="store_true",
        help="Discard events that are not participating in a coincident event.",
    )
    parser.add_option(
        "--comment",
        metavar="text",
        help="Set the comment string to be recorded in the process table for this job (default = None).",
    )
    parser.add_option(
        "--inj-made-only",
        action="store_true",
        help="Discard injections outside the search summary out segments.",
    )
    parser.add_option(
        "--min-amplitude",
        metavar="value",
        type="float",
        help="Discard events below the given amplitude.",
    )
    parser.add_option(
        "--max-amplitude",
        metavar="value",
        type="float",
        help="Discard events above the given amplitude.",
    )
    parser.add_option(
        "--min-bandwidth",
        metavar="Hz",
        type="float",
        help="Discard events narrower than the given bandwidth.",
    )
    parser.add_option(
        "--max-bandwidth",
        metavar="Hz",
        type="float",
        help="Discard events wider than the given bandwidth.",
    )
    parser.add_option(
        "--min-central-freq",
        metavar="Hz",
        type="float",
        help="Discard events with central frequency lower than that given.",
    )
    parser.add_option(
        "--max-central-freq",
        metavar="Hz",
        type="float",
        help="Discard events with central frequency higher than that given.",
    )
    parser.add_option(
        "--min-confidence",
        metavar="value",
        type="float",
        help="Discard events below the given confidence.",
    )
    parser.add_option(
        "--max-confidence",
        metavar="value",
        type="float",
        help="Discard events above the given confidence.",
    )
    parser.add_option(
        "--min-duration",
        metavar="seconds",
        type="float",
        help="Discard events shorter than the given duration.",
    )
    parser.add_option(
        "--max-duration",
        metavar="seconds",
        type="float",
        help="Discard events longer than the given duration.",
    )
    parser.add_option(
        "--min-fhigh",
        metavar="Hz",
        type="float",
        help="Discard events with highest frequency below the given frequency.",
    )
    parser.add_option(
        "--max-fhigh",
        metavar="Hz",
        type="float",
        help="Discard events with highest frequency above the given frequency.",
    )
    parser.add_option(
        "--min-flow",
        metavar="Hz",
        type="float",
        help="Discard events with lowest frequency below the given frequency.",
    )
    parser.add_option(
        "--max-flow",
        metavar="Hz",
        type="float",
        help="Discard events with loest frequency above the given frequency.",
    )
    parser.add_option(
        "--min-hrss",
        metavar="value",
        type="float",
        help="Discard events with h_rss below the given value.",
    )
    parser.add_option(
        "--max-hrss",
        metavar="value",
        type="float",
        help="Discard events with h_rss above the given value.",
    )
    parser.add_option(
        "--cut-instrument",
        metavar="name",
        action="append",
        default=[],
        help="Discard events from given instrument.",
    )
    parser.add_option(
        "--min-peak-time",
        metavar="seconds",
        help="Discard events with peak time before the given GPS time.",
    )
    parser.add_option(
        "--max-peak-time",
        metavar="seconds",
        help="Discard events with peak time after the given GPS time.",
    )
    parser.add_option(
        "--min-start-time",
        metavar="seconds",
        help="Discard events starting before the given GPS time.",
    )
    parser.add_option(
        "--max-start-time",
        metavar="seconds",
        help="Discard events starting after the given GPS time.",
    )
    parser.add_option(
        "--min-stop-time",
        metavar="seconds",
        help="Discard events ending before the given GPS time.",
    )
    parser.add_option(
        "--max-stop-time",
        metavar="seconds",
        help="Discard events ending after the given GPS time.",
    )
    parser.add_option(
        "--min-snr",
        metavar="value",
        type="float",
        help="Discard events below the given SNR.",
    )
    parser.add_option(
        "--max-snr",
        metavar="value",
        type="float",
        help="Discard events above the given SNR.",
    )
    parser.add_option(
        "--program",
        metavar="name",
        help="Process events generated by the given program.",
    )
    parser.add_option(
        "--veto-file",
        metavar="filename",
        help='Veto events using the veto segment list extracted from this XML file.  The file must contain segment tables, and the veto list will be constructed from the segments named "sngl_burst_veto".',
    )
...
def load_veto_segments(filename, verbose=False, contenthandler=None):
    return ligolw_segments.segmenttable_get_by_name(
        ligolw_utils.load_filename(
            filename, verbose=verbose, contenthandler=contenthandler
        ),
        "sngl_burst_veto",
    ).coalesce()
...
def remove_events_by_segment(contents, veto_segments):
    ...
    for i in xrange(len(contents.snglbursttable) - 1, -1, -1):
        ...
        if (
            burst.process_id in contents.process_ids
            and burst.ifo in veto_segments
            and veto_segments[burst.ifo].intersects_segment(burst.period)
        ):
...
def remove_non_coincidences(contents):
    coinc_burst_ids = set(
        row.event_id
        for row in contents.coincmaptable.getColumnByName("event_id")
    ...
    for i in xrange(len(contents.snglbursttable) - 1, -1, -1):
        ...
        if (
            burst.process_id in contents.process_ids
            and burst.event_id not in coinc_burst_ids
        ):
            del contents.snglbursttable[i]
...
def remove_skipped_injections(contents):
    ...
    for i in xrange(len(contents.simbursttable) - 1, -1, -1):
        ...
        if True not in (
            sim.time_at_instrument(instrument) in seglist
            for instrument, seglist in contents.outsegs.iteritems()
        ):
...
def ligolw_bucut(
    ...
    ligolw_search_summary.append_search_summary(
        xmldoc, process, inseg=seg, outseg=seg, nevents=len(contents.snglbursttable)
    )
...
def make_keep_this_sngl_burst(options):
    ...
    if options.min_amplitude is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.amplitude >= options.min_amplitude
        )
    if options.max_amplitude is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.amplitude <= options.max_amplitude
        )
    if options.min_bandwidth is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.bandwidth >= options.min_bandwidth
        )
    if options.max_bandwidth is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.bandwidth <= options.max_bandwidth
        )
    if options.min_central_freq is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst,
            lambda burst: burst.central_freq >= options.min_central_freq,
        )
    if options.max_central_freq is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst,
            lambda burst: burst.central_freq <= options.max_central_freq,
        )
    if options.min_confidence is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst,
            lambda burst: burst.confidence >= options.min_confidence,
        )
    if options.max_confidence is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst,
            lambda burst: burst.confidence <= options.max_confidence,
        )
    if options.min_duration is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.duration >= options.min_duration
        )
    if options.max_duration is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.duration <= options.max_duration
        )
    if options.min_fhigh is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.fhigh >= options.min_fhigh
        )
    if options.max_fhigh is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.fhigh <= options.max_fhigh
        )
    if options.min_flow is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.flow >= options.min_flow
        )
    if options.max_flow is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.flow <= options.max_flow
        )
    if options.min_hrss is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.hrss >= options.min_hrss
        )
    if options.max_hrss is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.hrss <= options.max_hrss
        )
    if options.min_snr is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.snr >= options.min_snr
        )
    if options.max_snr is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.snr <= options.max_snr
        )
    if options.cut_instrument:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.ifo not in options.cut_instrument
        )
    if options.min_peak_time is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.peak >= options.min_peak_time
        )
    if options.max_peak_time is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.peak <= options.max_peak_time
        )
    if options.min_start_time is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.start >= options.min_start_time
        )
    if options.max_start_time is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.start <= options.max_start_time
        )
    if options.min_stop_time is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.stop >= options.min_stop_time
        )
    if options.max_stop_time is not None:
        keep_this_sngl_burst = add_test(
            keep_this_sngl_burst, lambda burst: burst.stop <= options.max_stop_time
        )
...
for filename in filenames:
    ...
    xmldoc = ligolw_bucut(
        ...
        del_non_coincs=options.coinc_only,
        del_skipped_injections=options.inj_made_only,
