...
def parse_command_line():
    ...
    parser.add_option(
        ...
        "--x-instrument",
        metavar="instrument",
        help="Plot this instrument's offsets along the x axis (required).",
    )
    parser.add_option(
        "-y",
        "--y-instrument",
        metavar="instrument",
        help="Plot this instrument's offsets along the y axis (required).",
    )
    parser.add_option(
        "-n",
        "--require-instruments",
        metavar="instrument[,instrument...]",
        help="Plot only time slides involving exactly these instruments.",
    )
    ...
    if options.require_instruments is not None:
        options.require_instruments = set(
            map(str.strip, options.require_instruments.split(","))
        )
...
class Plot(object):
    def __init__(self, xmldoc, x_instrument, y_instrument, require_instruments=None):
        ...
        for offsets in tisitable.dict.itervalues():
            if require_instruments is None or require_instruments == set(
                offsets.keys()
            ):
        ...
        if require_instruments is not None:
            self.axes.set_title(
                "%d %s Time Slides" % (len(x), "+".join(sorted(require_instruments)))
            )
        else:
...
for n, filename in enumerate(filenames):
    ...
    plot = Plot(
        xmldoc,
        options.x_instrument,
        options.y_instrument,
        require_instruments=options.require_instruments,
    )
