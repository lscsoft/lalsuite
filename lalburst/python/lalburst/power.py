...
class TimingParameters(object):
    ...
    def __init__(self, config_parser):
        ...
        self.max_tile_length = int(
            config_parser.getfloat("lalapps_power", "max-tile-duration")
            * self.resample_rate
        )
        self.tile_stride_fraction = config_parser.getfloat(
            "lalapps_power", "tile-stride-fraction"
        )
        self.filter_corruption = config_parser.getint(
            "lalapps_power", "filter-corruption"
        )
        self.max_tile_bandwidth = config_parser.getfloat(
            "lalapps_power", "max-tile-bandwidth"
        )
        ...
        (
            ...
            self.window_pad,
            self.tiling_length,
        ) = lalburst.EPGetTimingParameters(
            self.window_length,
...
def make_cache_entry(input_cache, description, path):
    ...
    return CacheEntry(
        "+".join(instruments) or None, description, seglists.extent_all(), url
    )
...
class RMJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        ...
        self.set_stdout_file(
            os.path.join(get_out_dir(config_parser), "rm-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(get_out_dir(config_parser), "rm-$(cluster)-$(process).err")
        )
...
class BurstInjJob(pipeline.CondorDAGJob, pipeline.AnalysisJob):
    ...
    def __init__(self, config_parser):
        ...
        pipeline.CondorDAGJob.__init__(
            self,
            get_universe(config_parser),
            get_executable(config_parser, "lalapps_binj"),
        )
        ...
        if not self.output_cache:
            ...
            self.output_cache = [
                CacheEntry(
                    ...
                    self.__usertag,
                    segments.segment(
                        lal.LIGOTimeGPS(self.get_start()),
                        lal.LIGOTimeGPS(self.get_end()),
                    ),
    ...
    def get_output(self):
        if self._AnalysisNode__output is None:
            ...
            seg = segments.segment(
                lal.LIGOTimeGPS(self.get_start()), lal.LIGOTimeGPS(self.get_end())
            )
            self.set_output(
                os.path.join(
                    self.output_dir,
                    "G1+H1+H2+L1+T1+V1-INJECTIONS_%s-%d-%d.xml.gz"
                    % (
                        self.__usertag,
                        int(self.get_start()),
                        int(self.get_end() - self.get_start()),
                    ),
                )
            )
...
class PowerJob(pipeline.CondorDAGJob, pipeline.AnalysisJob):
    ...
    def __init__(self, config_parser):
        ...
        pipeline.CondorDAGJob.__init__(
            self,
            get_universe(config_parser),
            get_executable(config_parser, "lalapps_power"),
        )
        pipeline.AnalysisJob.__init__(self, config_parser)
        self.add_ini_opts(config_parser, "lalapps_power")
        self.set_stdout_file(
            os.path.join(
                get_out_dir(config_parser), "lalapps_power-$(cluster)-$(process).out"
            )
        )
        self.set_stderr_file(
            os.path.join(
                get_out_dir(config_parser), "lalapps_power-$(cluster)-$(process).err"
            )
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd("accounting_group", get_accounting_group(config_parser))
...
class LigolwAddNode(pipeline.LigolwAddNode):
    def __init__(self, job, remove_input, *args):
        ...
        self.output_dir = os.path.join(
            os.getcwd(), "."
        )  # "." == self.job().output_dir except the job class doesn't yet have this info
        self._CondorDAGNode__macros["initialdir"] = os.getcwd()
    ...
    def __update_output_cache(self, observatory=None, segment=None):
        ...
        cache_entry = make_cache_entry(
            self.input_cache, None, self._AnalysisNode__output
        )
...
class BucutJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self, "vanilla", get_executable(config_parser, "lalburst_cut")
        )
        self.set_sub_file("lalburst_cut.sub")
        self.set_stdout_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_cut-$(cluster)-$(process).out"
            )
        )
        self.set_stderr_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_cut-$(cluster)-$(process).err"
            )
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd("accounting_group", get_accounting_group(config_parser))
...
class BuclusterJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self, "vanilla", get_executable(config_parser, "lalburst_cluster")
        )
        self.set_sub_file("lalburst_cluster.sub")
        self.set_stdout_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_cluster-$(cluster)-$(process).out"
            )
        )
        self.set_stderr_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_cluster-$(cluster)-$(process).err"
            )
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd("accounting_group", get_accounting_group(config_parser))
...
class BinjfindJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self, "vanilla", get_executable(config_parser, "lalburst_injfind")
        )
        self.set_sub_file("lalburst_injfind.sub")
        self.set_stdout_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_injfind-$(cluster)-$(process).out"
            )
        )
        self.set_stderr_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_injfind-$(cluster)-$(process).err"
            )
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd("accounting_group", get_accounting_group(config_parser))
...
class BurcaJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self, "vanilla", get_executable(config_parser, "lalburst_coinc")
        )
        self.set_sub_file("lalburst_coinc.sub")
        self.set_stdout_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_coinc-$(cluster)-$(process).out"
            )
        )
        self.set_stderr_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_coinc-$(cluster)-$(process).err"
            )
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd("accounting_group", get_accounting_group(config_parser))
...
class Burca2Job(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        ...
        self.set_stdout_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_coinc2-$(cluster)-$(process).out"
            )
        )
        self.set_stderr_file(
            os.path.join(
                get_out_dir(config_parser), "lalburst_coinc2-$(cluster)-$(process).err"
            )
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd("accounting_group", get_accounting_group(config_parser))
...
class BurcaNode(pipeline.CondorDAGNode):
    ...
    def add_input_cache(self, cache):
        ...
        longest_duration = max(
            abs(cache_entry.segment) for cache_entry in self.input_cache
        )
...
class SQLiteJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self, "vanilla", get_executable(config_parser, "ligolw_sqlite")
        )
        self.set_sub_file("ligolw_sqlite.sub")
        self.set_stdout_file(
            os.path.join(
                get_out_dir(config_parser), "ligolw_sqlite-$(cluster)-$(process).out"
            )
        )
        self.set_stderr_file(
            os.path.join(
                get_out_dir(config_parser), "ligolw_sqlite-$(cluster)-$(process).err"
            )
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd("accounting_group", get_accounting_group(config_parser))
...
class SQLiteNode(pipeline.CondorDAGNode):
    ...
    def get_output_cache(self):
        if not self.output_cache:
            self.output_cache = [
                make_cache_entry(
                    self.input_cache, None, self.get_opts()["macrodatabase"]
                )
            ]
        return self.output_cache
...
class BurcaTailorJob(pipeline.CondorDAGJob):
    def __init__(self, config_parser):
        pipeline.CondorDAGJob.__init__(
            self,
            "vanilla",
            get_executable(config_parser, "lalburst_power_meas_likelihood"),
        )
...
def init_job_types(
    ...
    if "datafind" in job_types:
        datafindjob = pipeline.LSCDataFindJob(
            os.path.join(os.getcwd(), get_cache_dir(config_parser)),
            os.path.join(os.getcwd(), get_out_dir(config_parser)),
            config_parser,
        )
    ...
    if "lladd" in job_types:
        lladdjob = pipeline.LigolwAddJob(
            os.path.join(get_out_dir(config_parser)), config_parser
        )
...
def split_segment(timing_params, segment, psds_per_job):
    ...
    # in samples
    joboverlap = 2 * timing_params.filter_corruption + (
        timing_params.psd_length - timing_params.psd_shift
    )
    # in seconds
    ...
    if extra_psds:
        segs.append(
            segments.segment(t, t + job_length_from_psds(timing_params, extra_psds))
        )
    return segs
...
def make_datafind_fragment(dag, instrument, seg):
    ...
    if node.get_type() is None:
        node.set_type(
            datafindjob.get_config_file().get("datafind", "type_%s" % instrument)
        )
    node.set_retry(3)
    dag.add_node(node)
    return set([node])
...
def make_lladd_fragment(
    ...
    node.set_output(
        os.path.join(
            node.output_dir,
            "%s-%s-%d-%d.xml.gz"
            % (cache_entry.observatory, tag, int(segment[0]), int(abs(segment))),
        ),
        segment=segment,
    )
...
def make_binj_fragment(
    dag, seg, time_slides_cache_entry, tag, offset, flow=None, fhigh=None
):
        ...
        node.add_input_cache(
            [
                ...
                for (cache_entry, parent) in input_cache[
                    : binjfindjob.files_per_binjfind
                ]
            ]
        )
        for parent in set(
...
def make_bucut_fragment(dag, parents, tag, verbose=False):
    ...
    while input_cache:
        ...
        for parent in set(
            parent for cache_entry, parent in input_cache[: bucutjob.files_per_bucut]
        ):
            node.add_parent(parent)
...
def make_burca_fragment(dag, parents, tag, coincidence_segments=None, verbose=False):
    ...
    while input_cache:
        ...
        for parent in set(
            parent for cache_entry, parent in input_cache[: burcajob.files_per_burca]
        ):
            node.add_parent(parent)
...
def make_burca_tailor_fragment(dag, input_cache, seg, tag):
    ...
    while input_cache:
        ...
        node.set_name(
            "lalburst_power_meas_likelihood_%s_%d_%d_%d"
            % (tag, int(seg[0]), int(abs(seg)), len(nodes))
        )
    ...
    if delete_cache:
        rmnode = RMNode(rmjob)
        rmnode.set_name(
            "lalburst_power_meas_likelihood_rm_%s_%d_%d"
...
def make_burca2_fragment(dag, coinc_cache, likelihood_parents, tag):
    ...
    likelihood_data_cache_filename = os.path.join(
        burca2job.cache_dir, "burca2_%s.cache" % tag
    )
    likelihood_data_cache_file = file(likelihood_data_cache_filename, "w")
    for cache_entry in [
        cache_entry
        for node in likelihood_parents
        for cache_entry in node.get_output_cache()
    ]:
...
def make_datafind_stage(dag, seglists, verbose=False):
    ...
    for seg, instrument in segs:
        if verbose:
            print(
                "making datafind job for %s spanning %s" % (instrument, seg),
                file=sys.stderr,
            )
...
def make_power_segment_fragment(
    dag,
    datafindnodes,
    instrument,
    segment,
...
def make_single_instrument_stage(
    ...
    for instrument, seglist in seglistdict.iteritems():
        for seg in seglist:
            ...
            nodes += make_power_segment_fragment(
                dag,
                dfnodes,
                instrument,
                seg,
...
def group_coinc_parents(parents, offset_vectors, extentlimit=None, verbose=False):
    ...
    seglists, bins = cafe.ligolw_cafe(
        [
            cache_entry
            for parent in parents
            for cache_entry in parent.get_output_cache()
    ...
    if verbose and unused:
        ...
        # needed
        print(
            "Notice:  %d jobs (of %d) produce output that will not be used by a coincidence job"
            % (unused, len(parents)),
            file=sys.stderr,
        )
