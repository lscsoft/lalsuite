...
def segmentlistdict_normalize(seglistdict, origin):
    ...
    for seglist in seglistdict.itervalues():
        seglist[:] = (
            segments.segment(float(seg[0] - origin), float(seg[1] - origin))
            for seg in seglist
        )
...
def segmentlistdict_unnormalize(seglistdict, origin):
    ...
    for seglist in seglistdict.itervalues():
        seglist[:] = (
            segments.segment(origin + seg[0], origin + seg[1]) for seg in seglist
        )
...
class CafePacker(packing.Packer):
    ...
    def set_offset_vectors(self, offset_vectors):
        ...
        self.offset_vectors.sort(
            key=lambda offset_vector: sorted(offset_vector.items())
        )
        ...
        min_offset = min(
            min(offset_vector.values()) for offset_vector in offset_vectors
        )
        max_offset = max(
            max(offset_vector.values()) for offset_vector in offset_vectors
        )
...
def split_bins(cafepacker, extentlimit, verbose=False):
    ...
    for idx in range(len(cafepacker.bins) - 1, -1, -1):
        ...
        if verbose:
            print(
                "\tsplitting cache spanning %s at %s"
                % (
                    str(origbin.extent),
                    ", ".join(str(extent) for extent in extents[1:-1]),
                ),
                file=sys.stderr,
            )
        extents = [
            segments.segment(*bounds) for bounds in zip(extents[:-1], extents[1:])
        ]
...
def ligolw_cafe(cache, offset_vectors, verbose=False, extentlimit=None):
    ...
    epoch = min(
        [min(seg[0] for seg in seglist) for seglist in seglists.values() if seglist]
        or [None]
    )
    ...
    if verbose:
        print(
            "packing files (considering %s offset vectors) ..." % len(offset_vectors),
            file=sys.stderr,
        )
    for n, cacheentry in enumerate(cache):
        if verbose and not n % 13:
            print(
                "\t%.1f%%\t(%d files, %d caches)\r"
                % (100.0 * n / len(cache), n + 1, len(outputcaches)),
                end=" ",
                file=sys.stderr,
            )
    ...
    if verbose:
        print(
            "\t100.0%%\t(%d files, %d caches)" % (len(cache), len(outputcaches)),
            file=sys.stderr,
        )
    ...
    if extentlimit is not None:
        if verbose:
            print(
                "splitting caches with extent greater than %g s ..." % extentlimit,
                file=sys.stderr,
            )
        split_bins(packer, extentlimit, verbose=verbose)
        if verbose:
            print(
                "\t\t(%d files, %d caches)" % (len(cache), len(outputcaches)),
                file=sys.stderr,
            )
