...
ExcessPowerBBCoincDef = lsctables.CoincDef(
    search="excesspower",
    search_coinc_type=0,
...
class ExcessPowerCoincTables(snglcoinc.CoincTables):
    def __init__(self, xmldoc):
        ...
        try:
            ...
            self.multibursttable = lsctables.MultiBurstTable.new(
                (
                    "process_id",
                    "duration",
                    "central_freq",
    ...
    def make_multi_burst(self, process_id, coinc_event_id, events, offset_vector):
        ...
        multiburst.duration = (
            sum(event.ms_snr**2.0 * event.duration for event in events)
            / multiburst.snr**2.0
        )
        ...
        multiburst.central_freq = (
            sum(event.ms_snr**2.0 * event.peak_frequency for event in events)
            / multiburst.snr**2.0
        )

        # bandwidth = ms_snr squared weighted average of bandwidths

        multiburst.bandwidth = (
            sum(event.ms_snr**2.0 * event.bandwidth for event in events)
            / multiburst.snr**2.0
        )
        ...
        multiburst.amplitude = max(
            events, key=lambda event: event.ms_confidence
        ).ms_hrss
    ...
    def coinc_rows(self, process_id, time_slide_id, events, table_name):
        coinc, coincmaps = super(ExcessPowerCoincTables, self).coinc_rows(
            process_id, time_slide_id, events, table_name
        )
        coinc.insts = (event.ifo for event in events)
        return (
            coinc,
            coincmaps,
            self.make_multi_burst(
                process_id,
                coinc.coinc_event_id,
                events,
                self.time_slide_index[time_slide_id],
            ),
        )
...
StringCuspBBCoincDef = lsctables.CoincDef(
    search="StringCusp",
    search_coinc_type=0,
    description="sngl_burst<-->sngl_burst coincidences",
)
...
class StringCuspCoincTables(snglcoinc.CoincTables):
    def coinc_rows(self, process_id, time_slide_id, events, table_name):
        coinc, coincmaps = super(StringCuspCoincTables, self).coinc_rows(
            process_id, time_slide_id, events, table_name
        )
        coinc.insts = (event.ifo for event in events)
...
class ep_coincgen_doubles(snglcoinc.coincgen_doubles):
    ...
    class get_coincs(object):
        ...
        def max_edge_peak_delta(events):
            ...
            return max(
                max(
                    float(event.peak - event.start),
                    float(event.start + event.duration - event.peak),
                )
                for event in events
            )
        ...
        def __call__(self, event_a, offset_a, coinc_window):
            ...
            return [
                event_b
                for event_b in self.events[
                    bisect_left(self.times, peak - dt) : bisect_right(
                        self.times, peak + dt
                    )
                ]
                if not self.comparefunc(event_a, offset_a, event_b, coinc_window)
            ]
...
class string_coincgen_doubles(ep_coincgen_doubles):
    class get_coincs(object):
        def __init__(self, events):
            self.events = events
            self.times = tuple(
                map(string_coincgen_doubles.singlesqueue.event_time, events)
            )
...
def burca(
    ...
    if not incremental:
        ...
        for instrument, events in itertools.groupby(
            sorted(sngl_burst_table, key=lambda row: row.ifo), lambda event: event.ifo
        ):
        ...
        for instrument, events in itertools.groupby(
            sorted(sngl_burst_table, key=lambda row: (row.peak, row.ifo)),
                ...
                for node, events in time_slide_graph.pull(
                    ...
                    coinc_tables.append_coinc(
                        *coinc_tables.coinc_rows(
                            process_id, node.time_slide_id, events, "sngl_burst"
                        )
                    )
