...
def triangulators(timing_uncertainties):
    ...
    for n in range(2, len(allinstruments) + 1):
        for instruments in itertools.combinations(allinstruments, n):
            triangulators[instruments] = snglcoinc.TOATriangulator(
                [
                    lal.cached_detector_by_prefix[instrument].location
                    for instrument in instruments
                ],
                [timing_uncertainties[instrument] for instrument in instruments],
            )
...
class InstrumentBins(rate.HashableBins):
    ...
    def __init__(self, names):
        super(InstrumentBins, self).__init__(
            frozenset(combo)
...
class LnLRDensity(snglcoinc.LnLRDensity):
    ...
    def __init__(self, instruments):
        ...
        for instrument in instruments:
            self.densities["%s_snr2_chi2" % instrument] = rate.BinnedLnPDF(
                rate.NDBins(
                    (
                        rate.ATanLogarithmicBins(10, 1e7, 801),
                        rate.ATanLogarithmicBins(0.1, 1e4, 801),
                    )
                )
            )
        for pair in itertools.combinations(sorted(instruments), 2):
            dt = 0.005 + snglcoinc.light_travel_time(*pair)  # seconds
            self.densities["%s_%s_dt" % pair] = rate.BinnedLnPDF(
                rate.NDBins((rate.ATanBins(-dt, +dt, 801),))
            )
            self.densities["%s_%s_dA" % pair] = rate.BinnedLnPDF(
                rate.NDBins((rate.ATanBins(-0.5, +0.5, 801),))
            )
            self.densities["%s_%s_df" % pair] = rate.BinnedLnPDF(
        ...
        self.densities["instrumentgroup,rss_timing_residual"] = rate.BinnedLnPDF(
            rate.NDBins(
    ...
    def __call__(self, **params):
        try:
            ...
            interps = self.interps
        return (
            sum(interps[param](*value) for param, value in params.items())
            if params
            else NegInf
        )
    ...
    def to_xml(self, name):
        ...
        instruments = set(
            key.split("_", 1)[0] for key in self.densities if key.endswith("_snr2_chi2")
        )
        xml.appendChild(
            ligolw.Param.from_pyvalue(
    ...
    def from_xml(cls, xml, name):
        ...
        self = cls(
            lsctables.instrumentsproperty.get(
                ligolw.Param.get_param(xml, "instruments").value
            )
        )
...
class StringCoincParamsDistributions(snglcoinc.LnLikelihoodRatioMixin):
    ...
    def coinc_params(self, events, offsetvector):
        ...
        ignored, ignored, ignored, rss_timing_residual = self.triangulators[
            instruments
        ](tuple(event.peak + offsetvector[event.ifo] for event in events))
        ...
        for event1, event2 in itertools.combinations(events, 2):
            ...
            dt = float(
                (event1.peak + offsetvector[event1.ifo])
    ...
    def from_xml(cls, xml, name):
        ...
        # XML
        instruments = [
            key.replace("_snr2_chi2", "")
            for key in self.numerator.densities.keys()
...
def load_likelihood_data(filenames, verbose=False):
    ...
    for n, filename in enumerate(filenames, 1):
        ...
        this_coinc_params = StringCoincParamsDistributions.from_xml(
            xmldoc, "string_cusp_likelihood"
        )
        this_seglists = (
            lsctables.SearchSummaryTable.get_table(xmldoc)
...
def time_slides_livetime(
    seglists, time_slides, min_instruments, verbose=False, clip=None
):
    """
    ...
    for n, time_slide in enumerate(time_slides):
        ...
        if clip is None:
            ...
            livetime += float(
                abs(segmentsUtils.vote((seglists & clip).values(), min_instruments))
            )
    if verbose:
...
def time_slides_livetime_for_instrument_combo(
    seglists, time_slides, instruments, verbose=False, clip=None
):
    """
    ...
    for n, time_slide in enumerate(time_slides):
        ...
        if clip is None:
            livetime += float(
                abs(
                    onseglists.intersection(onseglists.keys())
                    - offseglists.union(offseglists.keys())
                )
            )
        else:
            livetime += float(
                abs(
                    (onseglists & clip).intersection(onseglists.keys())
                    - offseglists.union(offseglists.keys())
                )
            )
    if verbose:
