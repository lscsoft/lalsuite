...
def weighted_average_seg(seg1, weight1, seg2, weight2):
    ...
    return segments.segment(
        seg1[0] + weight2 * float(seg2[0] - seg1[0]) / (weight1 + weight2),
        seg1[1] + weight2 * float(seg2[1] - seg1[1]) / (weight1 + weight2),
    )
...
def cluster_events(
    events, testfunc, clusterfunc, sortkeyfunc=None, bailoutfunc=None, verbose=False
):
    """
    ...
    while 1:
        with tqdm(
            desc="clustering %d events" % len(events),
            total=len(events),
            disable=not verbose,
        ) as progress:
            ...
            while i < len(events):
                ...
                for j, event_j in enumerate(events[i + 1 :], i + 1):
                    if event_j is not None:
                        ...
                        elif (sortkeyfunc is not None) and bailoutfunc(
                            events[i], event_j
                        ):
                            break
