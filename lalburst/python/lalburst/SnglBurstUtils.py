...
class CoincDatabase(object):
	...
	WHERE
		...
		""",
            ...
            rows = [
                (self.sngl_burst_table.row_from_cols(row), row[-1])
...
def summarize_coinc_database(contents, filename=None):
    ...
    for instrument, seglist in sorted(contents.seglists.items()):
        print(
            ...
            % (
                ...
                * float(
                    abs(
                        instrument in contents.vetoseglists
    ...
    if contents.sngl_burst_table is not None:
        print(
            "\t%sburst events: %d" % (filename, len(contents.sngl_burst_table)),
            file=sys.stderr,
        )
    if contents.sim_burst_table is not None:
        print(
            "\t%sburst injections: %d" % (filename, len(contents.sim_burst_table)),
            file=sys.stderr,
        )
    ...
    if contents.coinc_def_table is not None:
        for description, n in cursor.execute(
...
def get_time_slides(connection):
    ...
    zero_lag_time_slides = dict(
        (time_slide_id, offsetvector)
        for time_slide_id, offsetvector in time_slides.items()
