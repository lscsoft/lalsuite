...
class offsetvector(dict):
    ...
    def contains(self, other):
        ...
        return (
            offsetvector(
                (key, offset) for key, offset in self.items() if key in other
            ).deltas
            == other.deltas
        )
...
def component_offsetvectors(offsetvectors, n):
    ...
    for vect in offsetvectors:
        for instruments in itertools.combinations(sorted(vect), n):
            ...
            # of floating-point round off confusing things.
            delta_sets.setdefault(instruments, set()).add(
                tuple(
                    vect[instrument] - vect[instruments[0]]
    ...
    return [
        ...
        for instruments, delta_set in delta_sets.items()
