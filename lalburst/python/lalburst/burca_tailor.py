...
class LnLRDensity(snglcoinc.LnLRDensity):
    ...
    def mkinterps(self):
        self.interps = dict(
            (key, pdf.mkinterp()) for key, pdf in self.densities.items()
        )
    ...
    def to_xml(self, name):
        ...
        instruments = set(
            key.split("_", 2)[0] for key in self.densities if key.endswith("_dt")
        )
        instruments |= set(
            key.split("_", 2)[1] for key in self.densities if key.endswith("_dt")
        )
        xml.appendChild(
        ...
        for n, filename in enumerate(filenames, 1):
            ...
            if self is None:
                ...
                seglists = (
                    ...
                    .get_out_segmentlistdict(set([self.process_id]))
                    .coalesce()
                )
            else:
                ...
                seglists |= (
                    lsctables.SearchSummaryTable.get_table(xmldoc)
                    .get_out_segmentlistdict(set([other.process_id]))
                    .coalesce()
                )
                del other
            xmldoc.unlink()
...
class EPAllSkyCoincParamsDistributions(BurcaCoincParamsDistributions):
    def ln_lr_from_triggers(self, events, offsetvector):
        ...
        events = tuple(events)
        t = events[0].peak
        t += sum(float(event.peak - t) * event.ms_snr**2.0 for event in events) / sum(
            event.ms_snr**2.0 for event in events
        )
        gmst = lal.GreenwichMeanSiderealTime(t) % (2 * math.pi)

        for event1, event2 in itertools.combinations(
            sorted(events, key=lambda x: x.ifo), 2
        ):
...
def delay_and_amplitude_correct(event, ra, dec):
    ...
    fp, fc = lal.ComputeDetAMResponse(
        detector.response, ra, dec, 0, lal.GreenwichMeanSiderealTime(event.peak)
    )
...
class EPGalacticCoreCoincParamsDistributions(BurcaCoincParamsDistributions):
    def ln_lr_from_triggers(self, events, offsetvector):
        ...
        return EPAllSkyCoincParamsDistributions.coinc_params(
            [
...
def gen_likelihood_control(
    coinc_params_distributions, seglists, name=process_program_name, comment=""
):
