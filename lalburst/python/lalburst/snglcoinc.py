...
def light_travel_time(instrument1, instrument2):
    ...
    """
    dx = (
        lal.cached_detector_by_prefix[instrument1].location
        - lal.cached_detector_by_prefix[instrument2].location
    )
...
class singlesqueue(object):
    ...
    def push(self, events, t_complete):
        ...
        if t_complete < self.t_complete:
            raise ValueError(
                "t_complete has gone backwards:  last was %s, new is %s"
    ...
    def pull(self, t):
        ...
        if t + self.max_coinc_window > self.t_complete:
            raise ValueError(
                "pull to %g fails to precede time to which queue is complete, (%g + %g), by the required margin of %g s"
                % (t + self.offset, self.t_complete, self.offset, self.max_coinc_window)
            )
        ...
        return flushed_events + tuple(
            entry.event
            for entry in self.queue[
                : bisect_left(self.queue, t + self.max_coinc_window)
            ]
        )
...
class coincgen_doubles(object):
    ...
    def __init__(self, offset_vector, coinc_windows):
        ...
        self.queues = dict(
            (instrument, self.singlesqueue(offset, max_coinc_window))
        ...
        self.offset_a = (
            self.queues[self.instrumenta].offset - self.queues[self.instrumentb].offset
        )
    ...
    def pull(self, t, singles_ids):
        ...
        return sorted(
            self.doublesgen(
                self.queues[self.instrumenta].pull(t),
                self.offset_a,
                self.queues[self.instrumentb].pull(t),
                singles_ids,
            )
        )
...
class TimeSlideGraphNode(object):
    def __init__(
        ...
        if len(offset_vector) > 2:
            self.components = tuple(
                TimeSlideGraphNode(
                    coincgen_doubles_type,
                    component_offset_vector,
                    coinc_windows,
                    min_instruments,
                )
                for component_offset_vector in self.component_offsetvectors(
                    offset_vector, len(offset_vector) - 1
                )
            )
            ...
            self.index = ChainMap(
                *(index for node in self.components for index in node.index.maps)
            )
            ...
            self.components = (coincgen_doubles_type.singlesqueue(offset, 0.0),)
            self.index = self.components[0].index
        else:
    ...
    def component_offsetvectors(offset_vector, n):
        ...
        for selected_instruments in itertools.combinations(offset_vector, n):
            yield offsetvector.offsetvector(
                (instrument, offset_vector[instrument])
    ...
    def pull(self, t):
        ...
        if len(self.offset_vector) == 2:
            ...
            return coincs, (
                set((eventid,) for eventid in singles_ids)
        ...
        component_coincs_and_partial_coincs = tuple(
            component.pull(t) for component in self.components
        )
        component_coincs = tuple(
            elem[0] for elem in component_coincs_and_partial_coincs
        )
        ...
        if self.keep_partial:
            ...
            partial_coincs.update(
                coinc
                for coinc, count in Counter(
                    itertools.chain(
                        *(elem[1] for elem in component_coincs_and_partial_coincs)
                    )
                ).items()
                if count >= 2
            )
        ...
        for coinc0 in component_coincs0:
            ...
            coincs1 = component_coincs1[
                bisect_left(component_coincs1, coinc0[:-1]) : bisect_left(
                    component_coincs1, coinc0[:-2] + (coinc0[-2] + 1,)
                )
            ]
            ...
            coincs2 = component_coincs2[
                bisect_left(component_coincs2, coinc0[1:]) : bisect_left(
                    component_coincs2, coinc0[1:-1] + (coinc0[-1] + 1,)
                )
            ]
            ...
            for coinc1 in coincs1:
                ...
                if i < len(coincs2) and coincs2[i] == confirmation:
                    ...
                    partial_coincs.difference_update(
                        itertools.combinations(new_coinc, len(new_coinc) - 1)
                    )
...
class TimeSlideGraph(object):
    def __init__(
        ...
        if (
            min(len(offset_vector) for offset_vector in offset_vector_dict.values())
            < min_instruments
        ):
            ...
            # check.
            raise ValueError(
                "encountered offset vector (%s) smaller than min_instruments (%d)",
                (
                    str(
                        min(
                            offset_vector_dict.values(),
                            key=lambda offset_vector: len(offset_vector),
                        )
                    ),
                    min_instruments,
                ),
        ...
        coinc_windows = dict(
            (frozenset(pair), coinc_window + light_travel_time(*pair))
            for pair in itertools.combinations(
                set(
                    instrument
                    for offset_vector in offset_vector_dict.values()
                    for instrument in offset_vector
                ),
                2,
            )
        )
        if verbose:
            print(
                "coincidence windows:\n\t%s"
                % ",\n\t".join(
                    "%s = %g s" % ("+".join(sorted(pair)), dt)
                    for pair, dt in coinc_windows.items()
                )
            )
        ...
        if verbose:
            print(
                "constructing coincidence assembly graph for %d offset vectors ..."
                % len(offset_vector_dict),
                file=sys.stderr,
            )
        self.head = tuple(
            TimeSlideGraphNode(
        ...
        if verbose:
            ...
            def walk(node):
                ...
                return (
                    ...
                    + sum(walk(node) for node in node.components)
                )

            print(
                "graph contains %d fundamental nodes, %d higher-order nodes"
                % tuple(sum(walk(node) for node in self.head)),
                file=sys.stderr,
            )
    ...
    def pull(
        ...
        for node in self.head:
            ...
            for event_ids in itertools.chain(*node.pull(t)):
                ...
                if (
                    min(
                        event_time(event) + offset_vector[event.ifo] for event in events
                    )
...
class CoincTables(object):
    ...
    def __init__(self, xmldoc, coinc_definer_row):
        ...
        self.coinc_def_id = ligolw_coincs.get_coinc_def_id(
            xmldoc,
    ...
    def coinc_rows(self, process_id, time_slide_id, events, table_name):
        ...
        """
        coincmaps = [
            self.coincmaptable.RowType(
                coinc_event_id=None, table_name=table_name, event_id=event.event_id
            )
            for event in events
        ]
...
class CoincRates(object):
    def __init__(
        self, instruments, delta_t, min_instruments=2, abundance_rel_accuracy=1e-4
    ):
        """
        ...
        # light travel time
        self.tau = dict(
            (frozenset(ab), self.delta_t + light_travel_time(*ab))
            for ab in itertools.combinations(tuple(self.instruments), 2)
        )
        ...
        for instruments in self.all_instrument_combos:
            ...
            if not instruments:
                ...
                for i, instrument in enumerate(instruments):
                    ...
                    i *= 2
                    halfspaces[i, j] = +1.0
                    halfspaces[i + 1, j] = -1.0
                    halfspaces[i, -1] = halfspaces[i + 1, -1] = -self.tau[
                        frozenset((anchor, instrument))
                    ]
                # non-anchor constraints
                for i, ((j1, a), (j2, b)) in enumerate(
                    itertools.combinations(enumerate(instruments), 2), dimensions
                ):
                    i *= 2
                    halfspaces[i, j1] = +1.0
                    halfspaces[i, j2] = -1.0
                    halfspaces[i + 1, j1] = -1.0
                    halfspaces[i + 1, j2] = +1.0
                    halfspaces[i, -1] = halfspaces[i + 1, -1] = -self.tau[
                        frozenset((a, b))
                    ]
                ...
                # compute volume
                self.rate_factors[key] = spatial.ConvexHull(
                    spatial.HalfspaceIntersection(halfspaces, interior).intersections
                ).volume
    ...
    def all_instrument_combos(self):
        ...
        return tuple(
            ...
            for n in range(self.min_instruments, len(all_instruments) + 1)
            for instruments in itertools.combinations(all_instruments, n)
        )
    ...
    def coinc_rates(self, **rates):
        ...
        if set(rates) != self.instruments:
            raise ValueError(
                ...
                % (", ".join(sorted(self.instruments)), ", ".join(sorted(rates)))
            )
        ...
        if self.tau and max(rates.values()) * max(self.tau.values()) >= 1.0:
            raise ValueError(
                "events per coincidence window must be << 1: rates = %s, max window = %g"
                % (rates, max(self.tau.values()))
            )
    ...
    def strict_coinc_rates(self, **rates):
        ...
        # instruments ...
        for instruments in sorted(
            strict_coinc_rates, reverse=True, key=lambda instruments: len(instruments)
        ):
        ...
        assert all(rate >= 0.0 for rate in strict_coinc_rates.values()), (
    ...
    def marginalized_strict_coinc_counts(self, seglists, **rates):
        ...
        if set(seglists) != self.instruments:
            raise ValueError(
                ...
                % (", ".join(sorted(self.instruments)), ", ".join(sorted(seglists)))
            )
        ...
        livetime = dict(
            (
                ...
                float(
                    abs(
                        seglists.intersection(on_instruments)
                        - seglists.union(self.instruments - on_instruments)
                    )
                ),
            )
            for on_instruments in self.all_instrument_combos
        )

        coinc_count = dict.fromkeys(livetime, 0.0)
        for on_instruments, T in livetime.items():
            for coinc_instruments, rate in self.strict_coinc_rates(
                **dict(
                    (instrument, (rate if instrument in on_instruments else 0.0))
    ...
    def lnP_instruments(self, **rates):
        ...
        P_instruments = dict(
            (instruments, rate / total_rate)
            for instruments, rate in strict_coinc_rates.items()
        )
        norm = sum(sorted(P_instruments.values()))
        # safety check:  result should be nearly exactly normalized
        assert abs(1.0 - norm) < 1e-14
        return dict(
            (instruments, (math.log(P / norm) if P else NegInf))
            for instruments, P in P_instruments.items()
        )
    ...
    def random_instruments(self, **rates):
        ...
        results = tuple(
            (instruments, lnP - lnN) for instruments, lnP in lnP_instruments.items()
        )
    ...
    def plausible_toas(self, instruments):
        ...
        if set(instruments) > self.instruments:
            raise ValueError(
                ...
                % ", ".join(sorted(set(instruments) - self.instruments))
            )
        if len(instruments) < self.min_instruments:
            raise ValueError(
                "require at least %d instruments, got %d"
                % (self.min_instruments, len(instruments))
            )
        ...
        windows = tuple(
            (
                instrument,
                -self.tau[frozenset((anchor, instrument))],
                +self.tau[frozenset((anchor, instrument))],
            )
            for instrument in instruments
        )
        ijseq = tuple(
            (i, j, self.tau[frozenset((instruments[i], instruments[j]))])
            for (i, j) in itertools.combinations(range(len(instruments)), 2)
        )
        while 1:
...
class TOATriangulator(object):
    ...
    def __init__(self, rs, sigmas, v=lal.C_SI):
        ...
        rbar = sum(self.rs / self.sigmas[:, numpy.newaxis] ** 2) / sum(
            1 / self.sigmas**2
        )
        ...
        if len(rs) >= 3:
            ...
            # len(rs) == 2
            self.max_dt = (
                numpy.dot(self.rs[1] - self.rs[0], self.rs[1] - self.rs[0]) ** 0.5
                / self.v
            )
    ...
    def __call__(self, ts):
        ...
        if len(self.rs) >= 3:
            if self.singular:
                ...
                toa = sum(
                    (ts - numpy.dot(self.rs, n[0]) / self.v) / self.sigmas**2
                ) / sum(1 / self.sigmas**2)

                # chi^{2}
                chi2 = sum(
                    (numpy.dot(self.R, n[0]) / (self.v * self.sigmas) - tau) ** 2
                )
                ...
                # arrival time at origin
                toa = sum(
                    (ts - numpy.dot(self.rs, n) / self.v) / self.sigmas**2
                ) / sum(1 / self.sigmas**2)
            ...
            # arrival time at origin
            toa = sum((ts - numpy.dot(self.rs, n) / self.v) / self.sigmas**2) / sum(
                1 / self.sigmas**2
            )
...
class LnLikelihoodRatioMixin(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if math.isinf(lnP_noise) and math.isinf(lnP_signal):
            ...
            if lnP_noise < 0.0 and lnP_signal < 0.0:
