...
"""Module provides functions for estimating pdf(s) of ranks for classified events (glitch and clean).
"""
...
## addtogroup pkg_py_laldetchar_idq_pdf_estimation
# @{

# Functions for estimating pdf(s) of event (glitch or clean) ranks.
...
def __Nd_gaussian_kernel(x1, x2, v):
  ...
  # determine the type of input data
    ...
    # 1-D kernel
        ...
        if isinstance(x1, (int, float)) and isinstance(x2, (int,
                float)) and isinstance(v, (int, float)):
            if v == 0:
    ...
    # multi-dimensional kernel
        ...
        if len(x1) != len(x2):
            ...
            return False
        elif not isinstance(v, (int, float)) and (len(x1) != len(v)
                or len(x1) != len(v[0])):
    ...
    # uniform standard deviation across all dimensions
            ...
            return np.exp(-sum([(x1[i] - x2[i]) ** 2. for i in
                          range(len(x1))]) / (2. * v))
        else:
            v = np.matrix(v)
            try:
...
def fixed_bandwidth_gaussian_kde(eval, observ, v=0.01):
  ...
  # we normalize each kernal to unity, and normalize the kde so it's integral is unity
    ...
    for e in eval:
        kde += [sum([__Nd_gaussian_kernel(e, o, v) for o in observ])
                / (len(observ) * (2. * np.pi) ** (n_dims / 2.)
                * np.linalg.det(np.matrix(v)) ** (1 / 2.))]

    return kde


def ballooning_gaussian_kde(
    eval,
    observ,
    pilot_interp='NONE',
    pilot_x='NONE',
  ...
  # set up the pilot pdf
    ...
    for e in eval:
        ...
        if np.isnan(s_optimal):
            ...
            kde += [sum([__Nd_gaussian_kernel(e, o, s_optimal ** 2)
                    for o in observ]) / ((2. * np.pi) ** 0.5
                    * len(observ) * s_optimal)]
        elif s_optimal == 0:
            kde += [0]
        else:
            kde += [sum([__Nd_gaussian_kernel(e, o, s_optimal ** 2)
...
def __ballooning_optimal_s(eval, n_samples, pdf):
  ...
  # pdf is an instance of scipy.interpolate.InterpolatedUnivariateSpline
    ...
    if pdf(eval, nu=2)[0] != 0:
        return (16 * np.sqrt(np.pi) * max([pdf(eval)[0], 0]) / (4
                * n_samples * pdf(eval, nu=2)[0] ** 2.)) ** (1 / 5.)  # the maximum statement forces pdf(eval) to be non-negative
    else:
        print 'ERROR: second derivative of the pilot_interp pdf vanishes. Optimal bandwidth is not defined'
        return False


def point_wise_gaussian_kde(
    eval,
    observ,
    scale=0.1,
    pilot_interp='NONE',
  ...
  # k is an int
    ...
    for e in eval:
        ...
        for o in observ:
            ...
            kernel_sum += __Nd_gaussian_kernel(e, o, s ** 2) / ((2.
                    * np.pi) ** 0.5 * s)
...
def __distance_to_kth_neighbor(o, observ, k):
  ...
  # k is an int
    ...
    if isinstance(o, (int, float)) and isinstance(observ[0], (int,
            float)):
    ...
    # pull out a subset of glitches that are close to o
        ...
        return sorted([np.sqrt(sum([(o[ind] - obs[ind]) ** 2.
                      for ind in range(len(o))])) for obs in observ])[k
                - 1]
    else:
...
def k_th_neighbor(eval, observ, k):
  ...
  # k is an int
    ...
    kde = []
    for e in eval:
        kde += [1. * k / len(observ) / (__Unit_sphere_volume(n_dims)
                * __distance_to_kth_neighbor(e, observ, k) ** n_dims)]
...
def num_smoothed_cdf_to_pdf(
  ...
  # set up min_observ and max_observ
    ...
    spline_cdf = scipy.interpolate.InterpolatedUnivariateSpline(cdf_x,
            num_cdf, k=k)
    if smooth:
