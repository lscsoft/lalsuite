...
if __name__ == '__main__':
    ...
    parser.add_option('', '--gps', default=0, type='float',
    ...
    rank_filenames = []
    fap_filenames = []
    for filename in idq.get_all_files_in_range(opts.input_dir,
            opts.start, opts.end, pad=0, suffix='.npy.gz'):
    ...
    for (t, ts) in zip(r_times, r_timeseries):
        ...
        r_ax.plot(t - opts.start, ts, color='r', alpha=0.75)
    ...
    for (t, ts) in zip(f_times, f_timeseries):
        ...
        if (ts > 0).any():
            f_ax.semilogy(t - opts.start, ts, color='b', alpha=0.75)
        else:
            if opts.verbose:
                print 'No non-zero FAP values between %d and %d' \
                    % (_start, _end)
...
# ....f_ax.set_ylabel("$\mathrm{FAP}_{\mathrm{%s}}$" % opts.classifier)
    ...
    r_ax.set_xlabel('time [seconds after %d]' % opts.start)

    r_ax.set_xlim(xmin=0, xmax=opts.end - opts.start)
    ...
    r_ax.plot((opts.gps - opts.start) * np.ones((2, )),
              r_ax.get_ylim(), ':k', linewidth=2, alpha=0.5)
    r_ax.text(opts.gps - opts.start, 0.8, str(opts.gps), ha='center',
              va='center')
    ...
    print >> summary_file, 'coverage : %f' % (dur / (opts.end
            - opts.start))
