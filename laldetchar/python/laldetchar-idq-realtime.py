...
if initial_training:
    ...
    if options.ignore_science_segments_training:
         training_command += ["--ignore-science-segments"]
...
#           (gstlal_good, gstlal_covered) = ([], [])
    ...
    if mvsc:
        ...
        if len(auxmvc_vectors) > 0: ### run mvsc prediction jobs
            (exit_status, ranked_file) = idq.execute_forest_evaluate(
                auxmvc_pat_file_name,
                trainedforest,
                ranked_file,
                config,
    ...
    if ovl:
        ...
        if exit_status != 0:
            ...
            logger.info('	  --> generated ' + this_output_dir + '/' + ovl_eval_file)
#            logger.info('	  --> ovl made predictions about %d events' % len(gw_predict))
            ...
            logger.info('	  converting %s to xml tables' % (this_output_dir + '/' + ovl_eval_file))
