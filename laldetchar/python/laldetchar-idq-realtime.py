...
for classifier in classifiers:
    ...
    if not initial_calibration:		
        intial_calibration = calibration_cache[classifier].is_empty()
...
if initial_calibration:
    logger.info('At least one calibration cache file was empty or did not exist. Launching initial summary job for classifiers.')
...
summary_template = "%s --config %s -l %s --gps-start %d --gps-stop %d --lookback %d --trending-lookback %d"
...
while t  < opts.endgps:
    ...
    if (2*(t - opts.startgps) > train_stride) and (t_remainder > 0) and (t_remainder <= stride):
        ...
        if opts.offline: ### running in "offline mode"
            ...
            proc = subprocess.Popen( train_command.split() , stdout=open(train_out, 'a'), stderr=open(train_err, 'a') , dir=cwd )
            proc.wait() ### blocks!
        else:
            ### running in realtime, don't wait for training job to complete so we can keep the latency low
            train_pid = subprocess.Popen(train_command, stdout=open(train_out, 'a'), stderr=open(train_err, 'a'), dir=cwd ).pid ### only remember the pid, and let the process float
    ...
    if (2*(t - opts.startgps) > calibration_stride) and (t_remainder > 0) and (t_remainder <= stride):
        ...
        if opts.offline:
            ...
            proc = subprocess.Popen(calibration_command.split(), stdout=open(calibration_out, "a"), stderr=open(calibration_err, "a"), dir=cwd)
            proc.wait() # block!
        else:
            summary_pid = subprocess.Popen(calibration_command.split(), stdout=open(calibration_out, 'a'), stderr=open(calibration_err, 'a'), dir=cwd).pid ### only remember the pid, let the process float
    ...
    if (2*(t - opts.startgps) > summary_stride) and (t_remainder > 0) and (t_remainder <= stride):
        ...
        if opts.offline:
            logger.info("Running in OFFLINE mode, will wait until summary is complete before proceeding with evaluation")
            proc = subprocess.Popen(summary_command.split(), stdout=open(summary_out, "a"), stderr=open(summary_err, "a"), dir=cwd)
            proc.wait() # block!
        else:                                              
            summary_pid = subprocess.Popen(summary_command.split(), stdout=open(summary_out, 'a'), stderr=open(summary_err, 'a'), dir=cwd).pid ### only remember the pid, let the process float
    ...
    for classifier in classifiers:
        ...
        logger.info('  Begin: generating %s rank timeseries.'%classifier)
        ...
        logger.info('  Begin: generating %s FAP time-series'%classifier)
