...
<!--  this is to set the style for row head-->
...
<xsl:template name="handle-table-lines">
	...
	<xsl:choose>
		<xsl:when test="contains($string, '&#xA;')">
                <!--divide if there is a new line entity-->
			<tr>
...
<xsl:if test="$to-be-divided">
	<xsl:choose>
		<xsl:when test="starts-with($to-be-divided,'&#x22;')">
			...
			<xsl:choose>
                                <xsl:when test="starts-with($to-be-divided,'&#x22;http')">
                                  <xsl:variable name="url"><xsl:value-of select="substring-before(substring-after($to-be-divided,'&#x22;'),'&#x22;')"/></xsl:variable>
				...
				<xsl:otherwise>
					...
					<td><xsl:value-of select="substring-after($to-be-divided,'&#x22;')"/></td>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:when>
		<xsl:otherwise>
