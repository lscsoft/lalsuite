...
parser.add_argument(
    "cosmology",
    choices=astropy.cosmology.parameters.available,
    default="WMAP9",
    nargs="?",
...
cosmo = astropy.cosmology.default_cosmology.get_cosmology_from_string(args.cosmology)
...
@np.vectorize
def z_for_DL(DL):
    return astropy.cosmology.z_at_value(cosmo.luminosity_distance, DL * u.Mpc)
...
print("static const double dVC_dVL_dt = {:.15f};".format(np.diff(log_DL)[0]))
print("static const double dVC_dVL_high_z_slope = {:.15f};".format(high_z_slope))
print(
    "static const double dVC_dVL_high_z_intercept = {:.15f};".format(high_z_intercept)
)
