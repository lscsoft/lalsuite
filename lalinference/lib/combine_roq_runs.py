...
parser = argparse.ArgumentParser(
    description="posterior samples from individual ROQ runs"
)
parser.add_argument(
    "-p",
    "--posteriors",
...
def add_h5_link(file_name, group_name, comb_file):
    ...
    nest_meta_data = comb_file[group_name + "/posterior"]["lalinference"][
        "lalinference_nest"
    ].attrs
    ...
    comb_file[group_name].attrs["evidence_scale_factor"] = np.exp(
        log_prior_volume
    ) / np.exp(comb_file.attrs["total_log_prior_volume"])

    comb_file[group_name].attrs["evidence_volume_prior"] = (
        log_prior_volume + log_evidence - comb_file.attrs["total_log_prior_volume"]
    )

    comb_file[group_name].attrs["PDF_weight"] = np.exp(
        comb_file[group_name].attrs["evidence_volume_prior"]
        - comb_file.attrs["evidence_volume_prior_max"]
    )
...
def combine_posterior_files(posterior_files):
    ...
    for i, post_file in enumerate(posterior_files):
        ...
        add_h5_link(
            post_file, group_name, comb_file
        )  # this is where the combination actually gets done
...
def compute_total_prior_vol(posterior_files, comb_file):
    ...
    for post_file in posterior_files:
        ...
        log_prior_volume = file["lalinference"]["lalinference_nest"].attrs[
            "log_prior_volume"
        ]
    ...
    comb_file.attrs["evidence_volume_prior_max"] = (
        evp_max - comb_file.attrs["total_log_prior_volume"]
    )
