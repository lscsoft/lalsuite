...
def email_notify(address, path):
    ...
    TEXT = (
        "Hi "
        + USER
        + ",\nYou have a new parameter estimation result on "
        + HOST
...
class LIGOLWContentHandlerExtractSimInspiralTable(ligolw.LIGOLWContentHandler):
    ...
    def startElement(self, name, attrs):
        if (
            attrs.has_key("Name")
            and table.Table.TableName(attrs["Name"]) == self.tabname
        ):
...
def cbcBayesBurstPostProc(
    ...
    if fm_flag:
        ...
        commonResultsObj = peparser.parse(
            data,
            outdir=outdir,
            deltaLogL=deltaLogL,
            fixedBurnins=fixedBurnins,
            nDownsample=nDownsample,
            oldMassConvention=oldMassConvention,
        )
    ...
    if injfile and eventnum is not None:
        ...
        xmldoc = utils.load_filename(
            injfile, contenthandler=LIGOLWContentHandlerExtractSimBurstTable
        )
        got_burst_table = 1
        try:
    ...
    if got_inspiral_table == 1:
        pos = bppu.Posterior(
            commonResultsObj,
            SimInspiralTableEntry=injection,
            injFref=injFref,
            SnglInspiralList=triggers,
        )
    else:
        pos = bppu.BurstPosterior(
            commonResultsObj,
            SimBurstTableEntry=injection,
            injFref=injFref,
            SnglBurstList=triggers,
        )
    ...
    if eventnum is None and injfile is not None:
        ...
        if len(injections) < 1:
            try:
                print(
                    "Warning: Cannot find injection with end time %f"
                    % (pos["time"].mean)
                )
            except KeyError:
            ...
            try:
                injection = itertools.ifilter(
                    lambda a: abs(float(a.get_end()) - pos["time"].mean) < 0.1,
                    injections,
                ).next()
    ...
    if analyticLikelihood:
        ...
        [
            ...
            if param not in analyticLikelihood.names and param not in dievidence_names
        ]
    ...
    html = bppu.htmlPage(
        ...
        javascript=bppu.__default_javascript_string,
    )
    ...
    if "chain" in pos.names:
        ...
        acceptedChainText = "%i of %i chains accepted: %i" % (
            len(acceptedChains),
            len(data),
            acceptedChains[0],
        )
    ...
    if "cycle" in pos.names:
        SampsStats.p(
            "Longest chain has " + str(pos.longest_chain_cycles()) + " cycles."
        )
    ...
    if bayesfactornoise is not None:
        ...
        html_model.p(
            "log Bayes factor ( coherent vs gaussian noise) = %s, Bayes factor=%f"
            % (BSN, exp(float(BSN)))
        )
        if bayesfactorcoherent is not None:
            html_model.p(
                "log Bayes factor ( coherent vs incoherent OR noise ) = %s, Bayes factor=%f"
                % (BCI, exp(float(BCI)))
            )
    ...
    if dievidence:
        ...
        evout.close()
        print(
            "Computing direct integration evidence = %g (log(Evidence) = %g)"
            % (ev, log_ev)
        )
        html_model.p(
            "Direct integration evidence is %g, or log(Evidence) = %g.  (Boxing parameter = %d.)"
            % (ev, log_ev, boxing)
        )
        if "logl" in pos.names:
            log_ev = pos.harmonic_mean_evidence()
            html_model.p(
                "Compare to harmonic mean evidence of %g (log(Evidence) = %g)."
                % (exp(log_ev), log_ev)
            )
    ...
    if ellevidence:
        try:
            ...
            evout.close()
            print(
                "Computing elliptical region evidence = %g (log(ev) = %g)"
                % (ev, log_ev)
            )
            html_model.p(
                "Elliptical region evidence is %g, or log(Evidence) = %g."
                % (ev, log_ev)
            )
    ...
    tabid = "statstable"
    html_stats = html.add_collapse_section(
    ...
    inj_position = None
    tabid = "skywftable"
    html_wf = html.add_collapse_section(
    ...
    if skyres is not None and ("ra" in pos.names and "dec" in pos.names):
        ...
        if inj_position is not None:
            html_sky.p(
                "Injection found at p = %g"
                % bppu.skymap_inj_pvalue(hpmap, inj_position, nest=True)
            )
    ...
    if not os.path.isdir(wfdir):
        os.makedirs(wfdir)
    try:
        wfpointer = bppu.plot_burst_waveform(
            pos=pos, simburst=injfile, event=eventnum, path=wfdir
        )
    except:
        wfpointer = None
    ...
    tabid = "onedmargtable"
    html_ompdf = html.add_collapse_section(
        "1D marginal posterior PDFs", legend=legend, innertable_id=tabid
    )
    # Table matter
    if not noacf:
        html_ompdf_write = (
            '<table id="%s"><tr><th>Histogram and Kernel Density Estimate</th><th>Samples used</th><th>Autocorrelation</th></tr>'
            % tabid
        )
    else:
        html_ompdf_write = (
            '<table id="%s"><tr><th>Histogram and Kernel Density Estimate</th><th>Samples used</th></tr>'
            % tabid
        )
    # Add section for 1D confidence intervals
    tabid = "onedconftable"
    html_ogci = html.add_collapse_section(
    ...
    if "chain" in pos.names:
        ...
        chainCycles = [
            sort(data[data[:, chain_index] == chain, par_index]) for chain in chains
        ]
    ...
    for par_name in oneDMenu:
        ...
        toppoints, injectionconfidence, reses, injection_area, cl_intervals = (
            bppu.greedy_bin_one_param(pos, binParams, confidence_levels)
        )
        ...
        rbins, plotFig = bppu.plot_one_param_pdf(
            pos, oneDPDFParams, pdf, cdf, plotkde=False
        )
        ...
        if not ("chain" in pos.names):
            ...
            chains = unique(pos["chain"].samples)
            chainData = [
                data[data[:, chain_index] == chain, par_index] for chain in chains
            ]
        ...
        if not (noacf):
            ...
            if not ("chain" in pos.names):
                ...
                try:
                    acls = []
                    Nsamps = 0.0
                    for rng, data, Nskip, Ncycles in zip(
                        chainDataRanges, chainData, chainNskips, chainNcycles
                    ):
                        Neff, acl, acf = bppu.effectiveSampleSize(data, Nskip)
                        acls.append(acl)
                        Nsamps += Neff
            ...
            if savepdfs:
                acffig.savefig(
                    os.path.join(sampsdir, figname.replace(".png", "_acf.pdf"))
                )
            plt.close(acffig)
        ...
        if not noacf:
            if not acfail:
                acfhtml = (
                    '<td width="30%"><img width="100%" src="1Dsamps/'
                    + figname.replace(".png", "_acf.png")
                    + '"/></td>'
                )
            else:
                acfhtml = "<td>ACF generation failed!</td>"
            html_ompdf_write += (
                '<tr><td width="30%"><img width="100%" src="1Dpdf/'
                + figname
                + '"/></td><td width="30%"><img width="100%" src="1Dsamps/'
                + figname.replace(".png", "_samps.png")
                + '"/></td>'
                + acfhtml
                + "</tr>"
            )
    ...
    # results.
    tabid = "2dconftable"
    html_tcig = html.add_collapse_section(
    ...
    for par1_name, par2_name in twoDGreedyMenu:
        ...
        try:
            ...
            print(
                "Bin size is not set for %s, skipping %s/%s binning."
        ...
        try:
            toppoints, injection_cl, reses, injection_area = bppu.greedy_bin_two_param(
                pos, greedy2Params, confidence_levels
            )
        except:
        ...
        try:
            greedy2ContourPlot = bppu.plot_two_param_kde_greedy_levels(
                {"Result": pos}, greedy2Params, [0.67, 0.9, 0.95], {"Result": "k"}
            )
            greedy2contourpath = os.path.join(
                greedytwobinsdir, "%s-%s_greedy2contour.png" % (par1_name, par2_name)
            )
            ...
            greedy2HistFig = bppu.plot_two_param_greedy_bins_hist(
                pos, greedy2Params, confidence_levels
            )
            greedy2histpath = os.path.join(
                greedytwobinsdir, "%s-%s_greedy2.png" % (par1_name, par2_name)
            )
            ...
            pass
        greedyFile = open(
            os.path.join(twobinsdir, "%s_%s_greedy_stats.txt" % (par1_name, par2_name)),
            "w",
        )
        ...
        if [par1_name, par2_name] in twoDplots or [par2_name, par1_name] in twoDplots:
            ...
            if row_count_gb == 0:
                html_tgbh_write += "<tr>"
            html_tgbh_write += (
                '<td width="30%"><img width="100%" src="greedy2Dbins/'
                + figname
                + '"/>[<a href="greedy2Dbins/'
                + figname_c
                + '">contour</a>]</td>'
        ...
        if twodkdeplots_flag is True:
            if [par1_name, par2_name] in twoDplots or [
                par2_name,
                par1_name,
            ] in twoDplots:
                ...
                if row_count == 0:
                    html_tcmp_write += "<tr>"
                html_tcmp_write += (
                    '<td width="30%"><img width="100%" src="2Dkde/'
                    + figname
                    + '"/></td>'
                )
                row_count += 1
    ...
    if RconvergenceTests is True:
        ...
        if convergenceResults is not None:
            tabid = "convtable"
            html_conv_test = html.add_collapse_section(
            ...
            for test, test_data in convergenceResults.items():
                ...
                if test_data:
                    ...
                    html_conv_table = (
                        '<table id="%s"><tr><th>Chain</th>' % tabid
                        + html_conv_table_header
                        + "</tr>"
                    )
    ...
    tabid = "covtable"
    html_stats_cov = html.add_collapse_section(
    ...
    html_footer.p(
        "Produced using cbcBayesPostProc.py at " + strftime("%Y-%m-%d %H:%M:%S") + " ."
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--skyres",
        dest="skyres",
        help="Sky resolution to use to calculate sky box size",
        default=None,
    )
    parser.add_option(
        "--eventnum",
        ...
        action="store",
        default=None,
        help="event number in SimInspiral file of this signal",
        type="int",
        metavar="NUM",
    )
    parser.add_option(
        ...
        action="store",
        default=None,
        help="trigger number in CoincTable",
        type="int",
        metavar="NUM",
    )
    parser.add_option(
        ...
        action="store",
        default=None,
        help="Optional file containing the bayes factor signal against noise",
        type="string",
    )
    parser.add_option(
        "--bci",
        action="store",
        default=None,
        help="Optional file containing the bayes factor coherent signal model against incoherent signal model.",
        type="string",
    )
    parser.add_option(
        "--snr",
        action="store",
        default=None,
        help="Optional file containing the SNRs of the signal in each IFO",
        type="string",
    )
    parser.add_option(
        "--dievidence",
        action="store_true",
        default=False,
        help="Calculate the direct integration evidence for the posterior samples",
    )
    parser.add_option(
        "--boxing",
        action="store",
        default=64,
        help="Boxing parameter for the direct integration evidence calculation",
        type="int",
        dest="boxing",
    )
    parser.add_option(
        ...
        action="store",
        default=1.0,
        help="Overall factor (normalization) to apply to evidence",
        type="float",
        dest="difactor",
        metavar="FACTOR",
    )
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Estimate the evidence by fitting ellipse to highest-posterior points.",
        dest="ellevidence",
    )

    parser.add_option(
        "--no2D", action="store_true", default=False, help="Skip 2-D plotting."
    )
    parser.add_option(
        "--header",
        action="store",
        default=None,
        help="Optional file containing the header line for posterior samples",
        type="string",
    )
    # NS
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="(inspnest) Parse input as if it was output from parallel nested sampling runs.",
    )
    parser.add_option(
        "--Nlive",
        action="store",
        default=None,
        help="(inspnest) Number of live points used in each parallel nested sampling run.",
        type="int",
    )
    parser.add_option(
        "--xflag",
        action="store_true",
        default=False,
        help="(inspnest) Convert x to iota.",
    )
    # SS
    parser.add_option(
        "--ss",
        action="store_true",
        default=False,
        help="(SPINspiral) Parse input as if it was output from SPINspiral.",
    )
    parser.add_option(
        "--spin",
        action="store_true",
        default=False,
        help="(SPINspiral) Specify spin run (15 parameters). ",
    )
    # LALInf
    parser.add_option(
        "--lalinfmcmc",
        action="store_true",
        default=False,
        help="(LALInferenceMCMC) Parse input from LALInferenceMCMC.",
    )
    parser.add_option(
        "--downsample",
        action="store",
        default=None,
        help="(LALInferenceMCMC) approximate number of samples to record in the posterior",
        type="int",
    )
    parser.add_option(
        "--deltaLogL",
        action="store",
        default=None,
        help="(LALInferenceMCMC) Difference in logL to use for convergence test.",
        type="float",
    )
    parser.add_option(
        "--fixedBurnin",
        ...
        action="callback",
        callback=multipleFileCB,
        help="(LALInferenceMCMC) Fixed number of iteration for burnin.",
    )
    parser.add_option(
        "--oldMassConvention",
        action="store_true",
        default=False,
        help="(LALInferenceMCMC) if activated, m2 > m1; otherwise m1 > m2 in PTMCMC.output.*.00",
    )
    # FM
    parser.add_option(
        "--fm",
        action="store_true",
        default=False,
        help="(followupMCMC) Parse input as if it was output from followupMCMC.",
    )
    # ACF plots off?
    parser.add_option("--no-acf", action="store_true", default=False, dest="noacf")
    # Turn on 2D kdes
    parser.add_option(
        "--twodkdeplots", action="store_true", default=False, dest="twodkdeplots"
    )
    # Turn on R convergence tests
    parser.add_option(
        "--RconvergenceTests",
    ...
    parser.add_option(
        "-c",
        "--covarianceMatrix",
        dest="covarianceMatrices",
        action="append",
        default=None,
        help="CSV file containing covariance (must give accompanying mean vector CSV. Can add more than one matrix.",
    )
    parser.add_option(
        "-m",
        "--meanVectors",
        ...
        action="append",
        default=None,
        help="Comma separated list of locations of the multivariate gaussian described by the correlation matrix.  First line must be list of params in the order used for the covariance matrix.  Provide one list per covariance matrix.",
    )
    parser.add_option(
        "--email",
        action="store",
        default=None,
        type="string",
        metavar="user@ligo.org",
        help="Send an e-mail to the given address with a link to the finished page.",
    )
    parser.add_option(
        "--stats_only", action="store_true", default=False, dest="stats_only"
    )
    parser.add_option(
        "--archive",
        action="store",
        ...
        help="Create the given tarball with all results",
    )
    parser.add_option(
        "--psdfiles",
        action="store",
        ...
        type="string",
        metavar="H1,L1,V1",
        help="comma separater list of ASCII files with PSDs, one per IFO",
    )
    ...
    cbcBayesBurstPostProc(
        ...
        eventnum=opts.eventnum,
        trigfile=opts.trigfile,
        trignum=opts.trignum,
        skyres=opts.skyres,
        # direct integration evidence
        dievidence=opts.dievidence,
        boxing=opts.boxing,
        difactor=opts.difactor,
        # Ellipitical evidence
        ellevidence=opts.ellevidence,
        # manual bayes factor entry
        bayesfactornoise=opts.bsn,
        bayesfactorcoherent=opts.bci,
        # manual input for SNR in the IFOs, optional.
        snrfactor=opts.snr,
        # nested sampling options
        ns_flag=opts.ns,
        ...
        # spinspiral/mcmc options
        ss_flag=opts.ss,
        ss_spin_flag=opts.spin,
        # LALInferenceMCMC options
        li_flag=opts.lalinfmcmc,
        deltaLogL=opts.deltaLogL,
        fixedBurnins=fixedBurnins,
        nDownsample=opts.downsample,
        oldMassConvention=opts.oldMassConvention,
        # followupMCMC options
        fm_flag=opts.fm,
        # Turn of ACF?
        noacf=opts.noacf,
