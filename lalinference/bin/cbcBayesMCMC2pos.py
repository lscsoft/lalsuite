...
def downsample_and_evidence(
    data_hdf5, deltaLogP=None, fixedBurnin=None, nDownsample=None, verbose=False
):
    ...
    for i in range(1, int(posterior_samples["nTemps"][0])):
        ...
        if verbose:
            print(
                ...
                + str("%02.f" % i)
                + " at a temperature "
                + str(highTchains[i - 1]["temperature"].mean())
            )

    betas = np.zeros(len(highTchains) + 1)
    logls = np.zeros_like(betas)

    betas[0] = 1.0 / np.median(posterior_samples["temperature"])
    logls[0] = np.mean(posterior_samples["logl"])

    for i in range(len(highTchains)):
        betas[i + 1] = 1.0 / np.median(highTchains[i]["temperature"])
    ...
    posterior_samples["chain_log_bayes_factor"] = (
        posterior_samples["chain_log_evidence"]
        - posterior_samples["chain_log_noise_evidence"]
    )

    if verbose:
        print(
            "logZ = "
            + str(posterior_samples["chain_log_evidence"][0])
            + "+-"
            + str(posterior_samples["chain_delta_log_evidence"][0])
        )
...
def weight_and_combine(
    pos_chains, verbose=False, evidence_weighting=True, combine_only=False
):
    ...
    final_posterior = pos_chains[0][
        np.random.uniform(size=len(pos_chains[0])) < fracs[0]
    ]

    for i in range(1, len(pos_chains)):
        final_posterior = apt.vstack(
            [
                final_posterior,
    ...
    final_log_noise_evidence = reduce(np.logaddexp, log_noise_evs) - np.log(
        len(log_noise_evs)
    )
    ...
    metadata = {}
    metadata[run_level] = {}
    metadata[run_level]["log_bayes_factor"] = (
        final_log_evidence - final_log_noise_evidence
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        type="string",
        default=None,
        help="Output file for posterior samples",
        metavar="posterior.hdf5",
    )
    parser.add_option(
        "-v",
        "--verbose",
    ...
    parser.add_option(
        ...
        action="store",
        default=None,
        help="Approximate number of samples to record in the posterior",
        type="int",
    )
    parser.add_option(
        "-l",
        ...
        action="store",
        default=None,
        help="Difference in logPosterior to use for convergence test.",
        type="float",
    )
    parser.add_option(
        "-b",
        ...
        action="callback",
        callback=multipleFileCB,
        help="Fixed number of iteration for burnin.",
    )
    parser.add_option(
        "--equal-weighting",
        action="store_true",
    ...
    for i in range(len(datafiles)):
        chain_posteriors.append(
            downsample_and_evidence(
                datafiles[i],
                deltaLogP=opts.deltaLogP,
                fixedBurnin=fixedBurnins[i],
                nDownsample=opts.downsample,
                verbose=opts.verbose,
            )
        )

    final_posterior, metadata = weight_and_combine(
        chain_posteriors,
        verbose=opts.verbose,
    ...
    write_samples(final_posterior, opts.pos, path=path_to_samples, metadata=metadata)
