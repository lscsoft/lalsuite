...
class confidence(object):
    ...
    def _set_interp(self):
        ...
        # height from index
        self._height_from_idx = interpolate.interp1d(
            np.arange(self._length),
            self.counts_sorted,
            bounds_error=False,
            fill_value=0.0,
        )
        # index from height
        self._idx_from_height = interpolate.interp1d(
            self.counts_sorted[::-1],
            np.arange(self._length)[::-1],
            bounds_error=False,
            fill_value=self._length,
        )
        # level from index
        self._level_from_idx = interpolate.interp1d(
            np.arange(self._length),
            self.norm_cumsum_counts_sorted,
            bounds_error=False,
            fill_value=1.0,
        )
        # index from level
        self._idx_from_level = interpolate.interp1d(
            self.norm_cumsum_counts_sorted,
            np.arange(self._length),
            bounds_error=False,
            fill_value=self._length,
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--insp-post",
        dest="insp_post",
        help="file containing the posterior samples from the PE inspiral run",
    )
    parser.add_option(
        "-r",
        "--ring-post",
        dest="ring_post",
        help="file containing the posterior samples from the PE ringdown run",
    )
    parser.add_option(
        "-m",
        "--imr-post",
        dest="imr_post",
        help="file containing the posterior samples from the full PE IMR run",
    )
    parser.add_option(
        "-f",
        "--fit-formula",
        dest="fit_formula",
        help="fitting formula to be used for the calculation of final mass/spin [options: 'nospin_Pan2011', 'nonprecspin_Healy2014', 'bbh_average_fits_precessing'",
        default="nonprecspin_Healy2014",
    )
    parser.add_option(
        "-p",
        "--mf-chif-prior",
        dest="prior_Mfchif_file",
        help="pickle file containing the interpolation object of the prior in (Mf, chif) used in the PE runs",
        default=None,
    )
    ...
    parser.add_option(
        "--insp-fhigh",
        dest="insp_fhigh",
        help="Upper cutoff freq for the inspiral analysis",
    )
    parser.add_option(
        "--ring-flow",
        dest="ring_flow",
        help="Lower cutoff freq for the ringdown analysis",
    )
    parser.add_option(
        "--m1-inj",
        dest="m1_inj",
        help="injected value of component mass m1 (if this is an injection)",
    )
    parser.add_option(
        "--m2-inj",
        dest="m2_inj",
        help="injected value of component mass m2 (if this is an injection)",
    )
    parser.add_option(
        "--chi1-inj",
        dest="chi1_inj",
        help="injected value of spin magnitude of mass m1 (if this is an injection)",
    )
    parser.add_option(
        "--chi2-inj",
        dest="chi2_inj",
        help="injected value of spin magnitude of mass m2 (if this is an injection)",
    )
    parser.add_option(
        "--chi1z-inj",
        dest="chi1z_inj",
        help="injected value of z-component of spin of mass m1 (if this is an injection)",
    )
    parser.add_option(
        "--chi2z-inj",
        dest="chi2z_inj",
        help="injected value of z-component of spin of mass m2 (if this is an injection)",
    )
    parser.add_option(
        "--phi12-inj",
        dest="phi12_inj",
        help="injected value of the azimuth angle of (hats2 - hats1) from vecL (if this is an injection)",
        default=0.0,
    )
    parser.add_option(
        "-w", "--waveform", dest="waveform", help="waveform used for recovery"
    )
    ...
    parser.add_option(
        ...
        dest="N_bins",
        default=201,
        help="number of bins (default=201)",
    )
    parser.add_option(
        "--dMfbyMf_lim",
        type="float",
        dest="dMfbyMf_lim",
        default=1.0,
        help="absolute value of limit for range of dMfbyMf_vec, defined as [-dMfbyMf_lim, +dMfbyMf_lim]",
    )
    parser.add_option(
        "--dchifbychif_lim",
        type="float",
        dest="dchifbychif_lim",
        default=1.0,
        help="absolute value of limit for range of dchifbychif_vec, defined as [-dchifbychif_lim, +dchifbychif_lim]",
    )
    parser.add_option(
        "--use_KDE",
        type="int",
        dest="MfafKDE",
        help="use KDE or not after getting samples of Mf, af",
    )
    parser.add_option(
        "-s",
        "--pepostproc",
        dest="pepostproc",
        default="cbcbayespostproc",
        help="package used to generate summary webpage to visualize the output [options: 'cbcbayespostproc', 'pesummary']",
    )
    ...
    N_bins = int(
        options.N_bins
    )  # Number of grid points along either axis (dMfbyMf, dchifbychif) for computation of the posteriors
    ...
    os.system(
        "cp %s %s/"
        % (os.path.join(lalinference_datadir, "imrtgr_webpage_templates/*.*"), out_dir)
    )
    ...
    if (
        ...
        or chi1_inj is None
        or chi2_inj is None
        or chi1z_inj is None
        or chi2z_inj is None
        or phi12_inj is None
    ):
        ...
        q_inj = m1_inj / m2_inj
        eta_inj = q_inj / (1.0 + q_inj) ** 2.0
        Mf_inj, chif_inj = tgr.calc_final_mass_spin(
            m1_inj,
            m2_inj,
            chi1_inj,
            chi2_inj,
    ...
    if pepostproc == "cbcbayespostproc":
        m1_id, m2_id, a1_id, a2_id, a1z_id, a2z_id, phi12_id = (
            "m1",
            "m2",
            "a1",
            "a2",
    ...
    m1_i, m2_i, chi1_i, chi2_i, chi1z_i, chi2z_i = (
        insp_data[m1_id],
        insp_data[m2_id],
        insp_data[a1_id],
        insp_data[a2_id],
    ...
    Mf_i, chif_i = tgr.calc_final_mass_spin(
        m1_i, m2_i, chi1_i, chi2_i, chi1z_i, chi2z_i, phi12_i, fit_formula
    )

    # read data from the ringdown posterior file
    ring_data = np.genfromtxt(ring_post, dtype=None, names=True)

    m1_r, m2_r, chi1_r, chi2_r, chi1z_r, chi2z_r = (
        ring_data[m1_id],
        ring_data[m2_id],
        ring_data[a1_id],
        ring_data[a2_id],
    ...
    # compute the final mass and spin
    Mf_r, chif_r = tgr.calc_final_mass_spin(
        m1_r, m2_r, chi1_r, chi2_r, chi1z_r, chi2z_r, phi12_r, fit_formula
    )

    # read data from the IMR posterior file
    imr_data = np.genfromtxt(imr_post, dtype=None, names=True)

    m1_imr, m2_imr, chi1_imr, chi2_imr, chi1z_imr, chi2z_imr = (
        imr_data[m1_id],
        imr_data[m2_id],
        imr_data[a1_id],
        imr_data[a2_id],
    ...
    # compute the final mass and spin
    Mf_imr, chif_imr = tgr.calc_final_mass_spin(
        m1_imr, m2_imr, chi1_imr, chi2_imr, chi1z_imr, chi2z_imr, phi12_imr, fit_formula
    )
    ...
    Mf_intp = (Mf_bins[:-1] + Mf_bins[1:]) / 2.0
    ...
    if MfafKDE == 1:
        ...
        print(
            "for inspiral kernel",
            kernel.integrate_box([-Mf_lim, -chif_lim], [Mf_lim, chif_lim]),
        )
        P_Mfchif_i = np.vectorize(f_i)(M_i, C_i) / kernel.integrate_box(
            [-Mf_lim, -chif_lim], [Mf_lim, chif_lim]
        )
        ...
        print(
            "for post-inspiral kernel",
            kernel.integrate_box([-Mf_lim, -chif_lim], [Mf_lim, chif_lim]),
        )
        P_Mfchif_r = np.vectorize(f_r)(M_i, C_i) / kernel.integrate_box(
            [-Mf_lim, -chif_lim], [Mf_lim, chif_lim]
        )
        ...
        P_Mfchif_imr = np.vectorize(f_imr)(M_i, C_i) / kernel.integrate_box(
            [-Mf_lim, -chif_lim], [Mf_lim, chif_lim]
        )
        ...
        P_Mfchif_i, Mf_bins, chif_bins = np.histogram2d(
            Mf_i, chif_i, bins=(Mf_bins, chif_bins), density=True
        )
        P_Mfchif_r, Mf_bins, chif_bins = np.histogram2d(
            Mf_r, chif_r, bins=(Mf_bins, chif_bins), density=True
        )
        P_Mfchif_imr, Mf_bins, chif_bins = np.histogram2d(
            Mf_imr, chif_imr, bins=(Mf_bins, chif_bins), density=True
        )
    ...
    if prior_Mfchif_file is not None:
        ...
        # removing nan's
        P_Mfchif_i[np.isnan(P_Mfchif_i)] = 0.0
        P_Mfchif_r[np.isnan(P_Mfchif_r)] = 0.0
        P_Mfchif_imr[np.isnan(P_Mfchif_imr)] = 0.0

        # removing infinities
        P_Mfchif_i[np.isinf(P_Mfchif_i)] = 0.0
        P_Mfchif_r[np.isinf(P_Mfchif_r)] = 0.0
        P_Mfchif_imr[np.isinf(P_Mfchif_imr)] = 0.0
    ...
    P_Mfchif_i_interp_object = scipy.interpolate.interp2d(
        Mf_intp, chif_intp, P_Mfchif_i, fill_value=0.0, bounds_error=False
    )
    P_Mfchif_r_interp_object = scipy.interpolate.interp2d(
        Mf_intp, chif_intp, P_Mfchif_r, fill_value=0.0, bounds_error=False
    )
    ...
    for i, v2 in enumerate(dchifbychif_vec):
        for j, v1 in enumerate(dMfbyMf_vec):
            P_dMfbyMf_dchifbychif[i, j] = tgr.calc_sum(
                Mf_intp,
                chif_intp,
                v1,
                v2,
    ...
    P_dMfbyMf_dchifbychif /= (
        np.sum(P_dMfbyMf_dchifbychif) * diff_dMfbyMf * diff_dchifbychif
    )
    ...
    gr_height = P_dMfbyMf_dchifbychif[
        np.argmin(abs(dMfbyMf_vec)), np.argmin(abs(dchifbychif_vec))
    ]  # taking value closest to (0,0)
    gr_conf_level = conf_v1v2.level_from_height(gr_height)
    print(
        "... no deviation from GR above %.1f%% confidence level"
        % (100.0 * gr_conf_level)
    )
    ...
    np.savetxt(
        "%s/summary_table.txt" % (out_dir),
        np.array(param_table),
        delimiter="\t",
        fmt="%s",
    )
    ...
    P_chi1chi2_i, chi1_bins_i, chi2_bins_i = np.histogram2d(
        chi1_i, chi2_i, bins=50, density=True
    )
    ...
    plt.contour(
        ...
        chi2_bins_i[:-1],
        tgr.gf(P_chi1chi2_i),
        levels=(s2_chi1chi2_i, s1_chi1chi2_i),
        linewidths=(1, 1.5),
    )
    if plot_injection_lines == True:
    ...
    plt.contour(
        ...
        chif_bins[:-1],
        tgr.gf(P_Mfchif_i),
        levels=(s2_Mfchif_i, s1_Mfchif_i),
        linewidths=(1, 1.5),
    )
    if plot_injection_lines == True:
    ...
    P_chi1chi2_r, chi1_bins_r, chi2_bins_r = np.histogram2d(
        chi1_r, chi2_r, bins=50, density=True
    )
    ...
    plt.contour(
        ...
        chi2_bins_r[:-1],
        tgr.gf(P_chi1chi2_r),
        levels=(s2_chi1chi2_r, s1_chi1chi2_r),
        linewidths=(1, 1.5),
    )
    if plot_injection_lines == True:
        plt.axvline(x=chi1_inj, ls="--", color="k")
    ...
    plt.contour(
        ...
        chif_bins[:-1],
        tgr.gf(P_Mfchif_r),
        levels=(s2_Mfchif_r, s1_Mfchif_r),
        linewidths=(1, 1.5),
    )
    if plot_injection_lines == True:
        plt.axvline(x=Mf_inj, ls="--", color="k")
    ...
    # IMR
    P_m1m2_imr, m1_bins_imr, m2_bins_imr = np.histogram2d(
        m1_imr, m2_imr, bins=50, density=True
    )
    P_chi1chi2_imr, chi1_bins_imr, chi2_bins_imr = np.histogram2d(
        chi1_imr, chi2_imr, bins=50, density=True
    )
    ...
    plt.contour(
        ...
        m2_bins_imr[:-1],
        tgr.gf(P_m1m2_imr),
        levels=(s2_m1m2_imr, s1_m1m2_imr),
        linewidths=(1, 1.5),
    )
    if plot_injection_lines == True:
        plt.axvline(x=m1_inj, ls="--", color="k")
    ...
    plt.contour(
        chi1_bins_imr[:-1],
        chi2_bins_imr[:-1],
        tgr.gf(P_chi1chi2_imr),
        levels=(s2_chi1chi2_imr, s1_chi1chi2_imr),
        linewidths=(1, 1.5),
    )
    if plot_injection_lines == True:
        plt.axvline(x=chi1_inj, ls="--", color="k")
    ...
    plt.contour(
        ...
        chif_bins[:-1],
        tgr.gf(P_Mfchif_imr),
        levels=(s2_Mfchif_imr, s1_Mfchif_imr),
        linewidths=(1, 1.5),
    )
    if plot_injection_lines == True:
        plt.axvline(x=Mf_inj, ls="--", color="k")
    ...
    plt.clabel(
        CSi, CSi.levels[::2], inline=True, fmt=fmt_i, fontsize=14, use_clabeltext=True
    )
    plt.clabel(
        CSr, CSr.levels[::2], inline=True, fmt=fmt_r, fontsize=14, use_clabeltext=True
    )
    ...
    plt.contour(
        dMfbyMf_vec,
        dchifbychif_vec,
        tgr.gf(P_dMfbyMf_dchifbychif),
        levels=(s2_v1v2, s1_v1v2),
        linewidths=(1, 1.5),
    )
