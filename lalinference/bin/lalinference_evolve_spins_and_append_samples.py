...
parser = argparse.ArgumentParser(
    description="Evolve the LALInference posterior samples of spins and append to HDF5 file of samples"
)
parser.add_argument(
    "--sample_file", help="path to the HDF5 posterior samples file", required=True
)
parser.add_argument(
    "--vfinal",
    help="final orbital velocity for the evolution (default is the Schwarzschild ISCO velocity 6**-0.5 ~= 0.408)",
    type=str,
    default="ISCO",
)
...
_colname_map = (
    ("tilt_spin1" + label, "tilt1" + label, _identity),
...
if ("mc" in data.dtype.names) and ("q" in data.dtype.names):
    ...
    eta = q / (1.0 + q) / (1.0 + q)
    m1, m2 = mc2ms(
        np.atleast_1d(data["mc"]), eta
    )  # We use the redshifted masses here, since the starting frequency is in the detector frame
else:
    raise ValueError(
        "Chirp mass and mass ratio are not found in %s. There is likely something wrong with this posterior file."
        % hdf_pos_file
    )
if ("a1" in data.dtype.names) and ("a2" in data.dtype.names):
    ...
    raise ValueError(
        "No spin magnitude samples were found in %s. There are no spins to evolve."
        % hdf_pos_file
    )
...
spinfreq_enum = np.array(
    [
        lalsim.SimInspiralGetSpinFreqFromApproximant(int(lal_approx))
        for lal_approx in data["LAL_APPROXIMANT"]
    ]
)
...
f_start = np.where(
    np.array(spinfreq_enum == lalsim.SIM_INSPIRAL_SPINS_FLOW),
    data["flow"],
    data["f_ref"],
)
...
for i, _ in enumerate(m1):
    ...
    if np.logical_or(chi1[i] > 1e-3, chi2[i] > 1e-3):
        ...
        (
            ...
            fStart=f_start[i],
            fEnd=f_final,
            s1x=chi1[i] * np.sin(tilt1[i]),
            s1y=0.0,
            s1z=chi1[i] * np.cos(tilt1[i]),
            s2x=chi2[i] * np.sin(tilt2[i]) * np.cos(phi12[i]),
            s2y=chi2[i] * np.sin(tilt2[i]) * np.sin(phi12[i]),
            s2z=chi2[i] * np.cos(tilt2[i]),
            lnhatx=0.0,
            lnhaty=0.0,
            lnhatz=1.0,
            e1x=1.0,
        ...
        chi1_v = np.array(
            [
                chi1x_v_data.data.data[idx_use],
                chi1y_v_data.data.data[idx_use],
                chi1z_v_data.data.data[idx_use],
            ]
        )
        chi2_v = np.array(
            [
                chi2x_v_data.data.data[idx_use],
                chi2y_v_data.data.data[idx_use],
                chi2z_v_data.data.data[idx_use],
            ]
        )

        Ln_v = np.array(
            [
                Lnx_v_data.data.data[idx_use],
                Lny_v_data.data.data[idx_use],
                Lnz_v_data.data.data[idx_use],
            ]
        )

        tilt1_evol[i], tilt2_evol[i], phi12_evol[i] = (
            tilts_and_phi12_from_Cartesian_spins_and_L(chi1_v, chi2_v, Ln_v)
        )
    else:
...
if v_final != "ISCO":
    vfinal_col = Table(
        [Column(v_final * np.ones_like(tilt1_evol), name="vfinal", meta=meta_f_ref)]
    )
