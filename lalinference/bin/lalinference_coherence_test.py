...
parser.add_option(
    ...
    action="store_true",
    dest="ci",
    help="Perform coherence test of Coherent signal vs incoherent signal",
    default=False,
)
parser.add_option(
    "-n",
    ...
    dest="cin",
    action="store_true",
    help="Perform coherence test of coherent signal vs incoherent signal or noise",
    default=False,
)
parser.add_option(
    "-b",
    ...
    dest="cin_or_n",
    action="store_true",
    help="Perform coherence test of coherent signal vs incoherent signal or noise using the most inclusive hypothesis",
    default=False,
)
parser.add_option(
    "-o",
...
Z_incoherentORnoise = logsumexp(
    [
        sum(p)
        for p in list(
            filter(
                lambda t, seen=set(): (
                    False if t[0] in seen or t[1] in seen else seen.update(t) or True
                ),
                combinations(Z_single_signal + Z_single_noise, len(Z_single_signal)),
            )
        )
    ]
)
