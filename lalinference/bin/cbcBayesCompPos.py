...
oneDMenu = (
    ...
    + bppu.spininducedquadParams
    + bppu.snrParams
    + bppu.spinParams
    + bppu.cosmoParam
    + bppu.calParams
    + bppu.tigerParams
    + bppu.lorentzInvarianceViolationParams
    + bppu.qnmtestParams
)
...
twoDGreedyMenu = [
    ...
    ["chirpmass", "q"],
    ["mc", "asym_massratio"],
    ["mchirp", "asym_massratio"],
    ["chirpmass", "asym_massratio"],
    ["m1", "m2"],
    ["mtotal", "eta"],
    ["distance", "iota"],
    ["dist", "iota"],
...
paramNameLatexMap = {
    ...
    "cos(tilt1)": r"\cos t_1",
    "cos(tilt2)": r"\cos t_2",
    "cos(thetas)": r"\cos \theta_s",
    "cosbeta": r"\cos \beta",
    "phi_orb": r"\phi_{\rm orb}",
    "cos(beta)": r"\cos \beta",
    "cos(iota)": r"\cos \iota",
    "tilt1": r"t_1",
    "tilt2": r"t_2",
    ...
    "dchi2": r"\delta\chi_2",
    "dchi3": r"\delta\chi_3",
    "dchi3S": r"\delta\chi_{3S}",
    "dchi3NS": r"\delta\chi_{3NS}",
    "dchi4": r"\delta\chi_4",
    "dchi4S": r"\delta\chi_{4S}",
    "dchi4NS": r"\delta\chi_{4NS}",
    "dchi5": r"\delta\chi_5",
    "dchi5S": r"\delta\chi_{5S}",
    "dchi5NS": r"\delta\chi_{5NS}",
    "dchi5l": r"\delta\chi_{5l}",
    "dchi5lS": r"\delta\chi_{5lS}",
    "dchi5lNS": r"\delta\chi_{5lNS}",
    "dchi6": r"\delta\chi_6",
    "dchi6S": r"\delta\chi_{6S}",
    "dchi6NS": r"\delta\chi_{6NS}",
    "dchi6l": r"\delta\chi_{6l}",
    "dchi7": r"\delta\chi_7",
    "dchi7S": r"\delta\chi_{7S}",
    "dchi7NS": r"\delta\chi_{7NS}",
    "dbeta2": r"\delta\beta_2",
    "dbeta3": r"\delta\beta_3",
    "dsigma2": r"\delta\sigma_2",
    "dsigma3": r"\delta\sigma_3",
    "dsigma4": r"\delta\sigma_4",
    "dbeta2": r"\delta\beta_2",
    "dbeta3": r"\delta\beta_3",
    "log10lambda_a": r"$\log\lambda_{\mathbb{A}}$",
    "log10lambda_eff": r"$\log\lambda_{eff}$",
    "log10livamp": r"$\log \mathbb{A}$",
    "lambda_a": r"$\lambda_{\mathbb{A}}$",
    "lambda_eff": r"$\lambda_{eff}$",
    "liv_amp": r"$\mathbb{A}$",
    "dquadmons": r"\delta\kappa_s",
    "dchikappaS": r"\delta\chi_{kappa_{S}}",
    "dchikappaA": r"\delta\chi_{\kappa_{A}}",
...
def open_url_curl(url, args=[]):
    ...
    kerberos_args = "--insecure -c /tmp/{0}_cookies -b /tmp/{0}_cookies --negotiate --user : --location-trusted".format(
        getpass.getuser()
    ).split()
...
#
def compare_plots_one_param_line_hist(
    list_of_pos_by_name,
    param,
    cl,
    color_by_name,
    ...
    for name, posterior in list_of_pos_by_name.items():
        ...
        try:
            n, bins = np.histogram(
                posterior[param].samples, bins=posbins, density=True, new=True
            )
        except:
    ...
    for name, posterior in list_of_pos_by_name.items():
        ...
        if cl_intervals[0] is not None and cl_lines_flag:
            try:
                plt.plot(
                    [cl_intervals[0][0], cl_intervals[0][0]],
                    [0, max_y],
                    color=colour,
                    linestyle="--",
                )
                plt.plot(
                    [cl_intervals[0][1], cl_intervals[0][1]],
                    [0, max_y],
                    color=colour,
                    linestyle="--",
                )
...
def compute_ks_pvalue_matrix(list_of_pos_by_name, param):
    ...
    for i in range(N):
        ...
        for j in range(i + 1, N):
            ...
            d, pvalue = ss.ks_2samp(
                pi[param].samples.flatten(), pj[param].samples.flatten()
            )
...
def compare_plots_one_param_line_hist_cum(
    list_of_pos_by_name,
    param,
    cl,
    color_by_name,
...
def compare_bayes(
    ...
    if (
        ...
        and os.path.exists(injection_path)
        and eventnum is not None
    ):
        ...
        if eventnum is not None:
            if len(injections) < eventnum:
                print(
                    "Error: You asked for event %d, but %s contains only %d injections"
                    % (eventnum, injection_path, len(injections))
                )
                sys.exit(1)
            else:
    ...
    for name, pos_folder in names_and_pos_folders:
        ...
        pfu_scheme, pfu_netloc, pfu_path, pfu_params, pfu_query, pfu_fragment = (
            pos_folder_url
        )
        ...
        if "http" in pfu_scheme:
            ...
            pfp_scheme, pfp_netloc, pfp_path, pfp_params, pfp_query, pfp_fragment = (
                pos_folder_parse
            )
            ...
            pos_file_url = urlparse.urlunsplit(
                (
                    pfp_scheme,
                    pfp_netloc,
                    os.path.join(pos_file_part, "posterior_samples.dat"),
                    "",
                    "",
                )
            )
        ...
        try:
            common_output_table_header, common_output_table_raw = peparser.parse(
                open(pos_file, "r")
            )
        except:
        ...
        if "LI_MCMC" in name or "FU_MCMC" in name:
            ...
            try:
                ...
                common_output_table_raw[:, idx] = (
                    np.pi * np.ones(len(common_output_table_raw[:, 0]))
                    - common_output_table_raw[:, idx]
                )
        ...
        if "mass1" in pos_temp.names and "mass2" in pos_temp.names:
            print("Calculating total mass")
            pos_temp.append_mapping(
                "mtotal", lambda m1, m2: m1 + m2, ["mass1", "mass2"]
            )
    ...
    if (
        common_params != [] and common_params is not None
    ):  # If there are common parameters....
        ...
        if len(common_params) > 1:  # If there is more than one parameter...
            ...
            for i, j in all_pairs(
                temp
            ):  # Iterate over all unique pairs in the set of common parameters
                pplst = [i, j]
                rpplst = pplst[:]
                rpplst.reverse()
                ...
                if (
                    pplst_cond or rpplst_cond
                ):  # If this pair of parameters is in the plotting list...
                    ...
                    fig = bppu.plot_two_param_kde_greedy_levels(
                        pos_list,
                        greedy2Params,
                        TwoDconfidenceLevels,
                        color_by_name,
                        figsize=contour_figsize,
                        dpi=contour_dpi,
                        figposition=contour_figposition,
                        legend=ldg,
                        line_styles=slinestyles,
                        hatches_by_name=hatches_by_name,
                        Npixels=Npixels2D,
                    )
                    if fig is None:
                        continue
                    ...
                    fig.savefig(
                        os.path.join(outdir, "%s-%s.png" % (pplst[0], pplst[1])),
                        bbox_inches="tight",
                    )
                    fig.savefig(
                        os.path.join(pdfdir, "%s-%s.pdf" % (pplst[0], pplst[1])),
                        bbox_inches="tight",
                    )
        ...
        for param in common_params:
            ...
            for confidence_level in OneDconfidenceLevels:
                if analyticLikelihood:
                    ...
                    pdf = None
                    cdf = None
                cl_table_header += '<th colspan="2">%i%% (Lower|Upper)</th>' % (
                    int(100 * confidence_level)
                )
                hist_fig, cl_intervals = compare_plots_one_param_line_hist(
                    pos_list,
                    param,
                    confidence_level,
                    color_by_name,
                ...
                hist_fig2, cl_intervals = compare_plots_one_param_line_hist_cum(
                    pos_list,
                    param,
                    confidence_level,
                    color_by_name,
                ...
                for name, pos in pos_list.items():
                    ...
                    confidence_levels[level_index][param].append(
                ...
                confidence_uncertainty[param] = bppu.confidence_interval_uncertainty(
                    confidence_level, cl_bounds, poses
                )

                save_path = ""
                if hist_fig is not None:
                    save_path = os.path.join(
                        outdir, "%s_%i.png" % (param, int(100 * confidence_level))
                    )
                    save_path_pdf = os.path.join(
                        pdfdir, "%s_%i.pdf" % (param, int(100 * confidence_level))
                    )
                    try:
                    ...
                    save_path = os.path.join(
                        outdir, "%s_%i_cum.png" % (param, int(100 * confidence_level))
                    )
                    save_path_pdf = os.path.join(
                        pdfdir, "%s_%i_cum.pdf" % (param, int(100 * confidence_level))
                    )
            ...
            for name, row_contents in cl_table.items():
                cl_table_str += '<tr><td>%s<font color="%s"></font></td>' % (
                    name,
            ...
            cl_uncer_str += (
                "<tr> <td> %g </td> <td> %g </td> <td> %g </td> </tr> </table>"
                % (
                    confidence_uncertainty[param][0],
                    confidence_uncertainty[param][1],
                    confidence_uncertainty[param][2],
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="pos_list",
        action="append",
        help="Path to folders containing output of cbcBayesPostProc.",
    )
    parser.add_option(
        "-n",
        "--name",
        dest="names",
        action="append",
        help="Name of posterior result e.g. followupMCMC 2.5PN (optional)",
    )
    parser.add_option(
        "-i",
        "--inj",
        dest="inj",
        help="Path of injection XML containing SimInspiralTable (optional).",
    )
    parser.add_option(
        "-e",
        "--eventnum",
        dest="eventnum",
        help="Sim ID of injection described in injection XML (optional).",
    )
    parser.add_option(
        "-u",
        dest="username",
        help="User name for https authenticated content (optional).",
    )
    parser.add_option(
        "-x",
        dest="password",
        help="Password for https authenticated content (optional).",
    )
    parser.add_option(
        "--reload",
        dest="reload_flag",
        action="store_true",
        help="Re-download all pos files (optional).",
    )
    parser.add_option(
        "--hide-cl-lines",
        dest="clf",
        action="store_false",
        default=True,
        help="Hide confidence level lines on 1D plots for clarity (optional).",
    )
    parser.add_option(
        "--contour-dpi",
        dest="cdpi",
        default=250,
        help="DPI for contour plot (optional).",
    )
    parser.add_option(
        "--contour-width",
        dest="cw",
        default=7,
        help="Width (in inches) of contour plots (optional).",
    )
    parser.add_option(
        "--contour-height",
        dest="ch",
        default=6,
        help="Height (in inches) of contour plots (optional).",
    )
    parser.add_option(
        "--contour-plot-width",
        dest="cpw",
        default=0.5,
        help="Relative width of plot element 0.15<width<1 (optional).",
    )
    parser.add_option(
        "--contour-plot-height",
        dest="cph",
        default=0.76,
        help="Relative height of plot element 0.15<width<1 (optional).",
    )
    parser.add_option(
        "--no-legend",
        dest="ldg_flag",
    ...
    parser.add_option(
        ...
        default=False,
        action="store_true",
        help="Do not fail when files are missing (optional).",
    )
    parser.add_option(
        "-c",
        "--covarianceMatrix",
    ...
    parser.add_option(
        ...
        type="int",
        default=50,
        help="Number of pixels on a side of the 2D plots (default 50)",
        metavar="N",
    )
    ...
    if opts.outpath is None:
        print(
            "No output directory specified. Output will be saved to PWD : %s"
            % os.getcwd()
        )
        outpath = os.getcwd()
    else:
    ...
    (
        ...
        opts.clf,
        opts.ldg_flag,
        contour_figsize=(float(opts.cw), float(opts.ch)),
        contour_dpi=int(opts.cdpi),
        contour_figposition=[0.15, 0.15, float(opts.cpw), float(opts.cph)],
        fail_on_file_err=not opts.readFileErr,
        covarianceMatrices=opts.covarianceMatrices,
        meanVectors=opts.meanVectors,
        Npixels2D=int(opts.npixels_2d),
    )
    ...
    for (name, logl_max), dic in zip(max_logls, dics):
        param_section_write += (
            '<tr><td><a href="%s">%s</a></td> <td>%g</td> <td>%.1f</td></tr>'
            % (dict(zip(names, pos_list))[name], name, logl_max, dic)
        )
    ...
    if greedy2savepaths:
        ...
        for plot_path in greedy2savepaths:
            ...
            param_section.write(
                '<img src="%s"/>' % str(plotfile)
            )  # str(os.path.relpath(plot_path,outpath)))

    compare_page_footer = compare_page.add_section("")
    compare_page_footer.p(
        "Produced using cbcBayesCompPos.py at " + strftime("%Y-%m-%d %H:%M:%S") + " ."
    )
