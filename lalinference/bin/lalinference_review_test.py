...
parser = argparse.ArgumentParser(
    description="Runs review tests of lalinference using lalinference_pipe_example.ini."
)
...
parser.add_argument(
    "-o", "--output", type=str, nargs="?", default=None, help="output directory."
)
...
if "UNCLEAN" in lalinference.InferenceVCSInfo.vcsId:
    default_outputdir = (
        os.getenv("HOME")
        + "/lalinference_testrun/"
        + lalinference.InferenceVCSInfo.vcsId
        + "_UNCLEAN/"
        + args.engine
        + "/"
    )
...
def set_GraceDB(cp):
    ...
    cp.set(
        "datafind", "types", "{'H1':'H1_HOFT_C00','L1':'L1_HOFT_C00','V1':'V1Online'}"
    )
    cp.set(
        "data",
        "channels",
...
def set_pptest(cp, engine, approximant):
    ...
    cp.set(
        "paths",
        "webdir",
        os.path.join(web_outputdir, "pptest", engine, approximant, "webdir/"),
    )
    cp.set(
        "ppanalysis",
        "webdir",
        os.path.join(web_outputdir, "PPcheck", engine, approximant + "/"),
    )
    cp.set(
        "lalinference",
        "fake-cache",
...
if args.pptest:
    ...
    for engine in args.engine.split(","):
        for approximant in args.pp_approximant.split(","):
            ...
            pptest_ini_file = os.path.join(
                args.output, "pptest", engine, approximant, "pptest.ini"
            )

            cppptest = set_pptest(
                init_ini_file(), engine=engine, approximant=approximant
            )
            ...
            if args.condor_submit:
                condor_submit_dag = [
                    "condor_submit_dag",
                    os.path.join(
                        args.output, "pptest", engine, approximant, "run/priortest.dag"
                    ),
                ]
