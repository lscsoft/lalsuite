...
parser.add_option(
    ...
    action="store",
    type="string",
    help="Path to injection file, will bypass the prior-sampling stage",
    metavar="injections.xml",
)
parser.add_option(
    "-N",
    "--trials",
    ...
    type="int",
    metavar="NUM",
    help="Number of prior samples to analyse",
)
...
if opts.daglog_path is not None:
    prior_cp.set(
        "paths", "daglogdir", os.path.join(os.path.abspath(opts.daglog_path), "prior")
    )
    main_cp.set(
        "paths", "daglogdir", os.path.join(os.path.abspath(opts.daglog_path), "main")
    )
    daglogdir = os.path.abspath(opts.daglog_path)
else:
    prior_cp.set(
        "paths", "daglogdir", os.path.join(os.path.abspath(opts.run_path), "prior")
    )
    main_cp.set(
        "paths", "daglogdir", os.path.join(os.path.abspath(opts.run_path), "main")
    )
...
if prior_cp.get("analysis", "engine") == "lalinferencenest":
    prior_cp.set(
        "engine", "sampleprior", str(20 * opts.trials)
    )  # more samples drawn since not all will end up in posterior
    ...
    prior_cp.set(
        "engine", "neff", str(max(opts.trials, 1000))
    )  # miminum of 1000 effective samples
    prior_cp.set("engine", "zeroLogLike", "")
...
pagenode = filter(
    lambda n: isinstance(n, pipe_utils.ResultsPageNode), priordag.get_nodes()
)[0]
...
if main_cp.has_option("condor", "accounting_group"):
    prior2injjob.add_condor_cmd(
        "accounting_group", main_cp.get("condor", "accounting_group")
    )
