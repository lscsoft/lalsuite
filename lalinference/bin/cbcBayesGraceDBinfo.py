...
def cbcBayesGraceDBinfo(
    ...
    if samples is not None:
        ...
        for i in pars:
            if i in pos.names:
                ...
                if i == "hrss":
                    ...
                    outstr += "<td align=left>%.3e &plusmn; %.3e</td></tr>" % (
                        pos[i].samples[which][0],
                        pos[i].stdev,
                    )
                else:
                    outstr += "<tr><td align=left>%s %s</td>" % (i, units[i])
                    outstr += "<td align=left>%.3f &plusmn; %.3f</td></tr>" % (
                        pos[i].samples[which][0],
                        pos[i].stdev,
                    )
        ...
        if email is not None and bci is not None:
            ...
            SUBJECT = (
                "%s LIB result page is ready at " % pref
                + HOST
                + " for graceID %s!" % (gid)
            )
            TEXT = (
                "LIB run for graceID %s is done on " % gid
                + HOST
                + ".\nThe BCI is %lf\n" % bci
            )
            if bci > 10:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-g", "--gid", dest="gid", help="GraceDB id", metavar="G123456", default=None
    )
    parser.add_option(
        "-s",
        "--samples",
        dest="samples",
        help="posterior_samples.hdf5/dat",
        default=None,
    )
    parser.add_option(
        "--analysis",
        help="Prefix to use for the graceDB entries. Should be the name of the analysis (default LALInference)",
        default="LALInference",
    )
    parser.add_option(
        "--bci", dest="bci", help="coherence test file: bci.dat", default=None
    )
    parser.add_option(
        "--bsn",
        dest="bsn",
        help="evidence file: bsn.dat [Deprecated, now is read from samples.hdf5]",
        default=None,
    )
    ...
    parser.add_option(
        ...
        dest="message",
        type="str",
        help="Message to go with skymap uplaod",
        default=None,
    )
    parser.add_option(
        "--email", dest="email", help="Will email when run is done.", default=None
    )
    parser.add_option(
        "--server",
        dest="server",
        help="GraceDB server to interact with.",
