...
def read_nested_from_hdf5(nested_path_list, strict_versions=True):
    ...
    for path in nested_path_list:
        ...
        run_identifier = extract_metadata(
            path,
            metadata,
            log_noise_evidences,
            log_max_likelihoods,
...
def write_posterior_to_hdf(
    posterior_path, headers, posterior, metadata, run_identifier
):
    ...
    write_samples(
        posterior,
        posterior_path,
        path="/".join(["", "lalinference", run_identifier, posterior_dset_name]),
        metadata=metadata,
        overwrite=True,
    )


def write_posterior_to_ascii(
    posterior_path,
    headers,
    posterior,
    log_bayes_factor,
...
def is_hdf5(path):
    ...
    if ".h5" in extension or ".hdf" in extension:
        ...
        raise ValueError(
            "Extension %r not recognized as HDF5 or '.dat': %r" % (extension, path)
        )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="Nlive",
        default=None,
        help="Number of live points in each chain loaded",
        metavar="NUM",
    )
    parser.add_option(
        "-p",
    ...
    parser.add_option(
        ...
        dest="nonstrict",
        default=False,
        help="Set this flag to allow combining of nested "
    ...
    # Read the input file
    return_values = read_nested_from_hdf5(
        datafiles, strict_versions=(not opts.nonstrict)
    )
    (
        input_arrays,
        log_noise_evidence,
    ...
    if hdf_output:
        write_posterior_to_hdf(opts.pos, headers, posterior, metadata, run_identifier)
    else:
