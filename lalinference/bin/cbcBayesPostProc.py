...
def cbcBayesPostProc(
    ...
    if fm_flag:
        ...
        if os.path.isfile(data[0] + "_header.txt"):
            ...
            shutil.copy2(
                data[0] + "_header.txt", os.path.join(outdir, "nest_headers.txt")
            )
    ...
    html_stats = html.add_collapse_section(
        "Summary statistics", legend=legend, innertable_id=tabid, start_closed=False
    )
    ...
    if not os.path.isdir(wfdir):
        os.makedirs(wfdir)
    try:
        wfpointer = bppu.plot_waveform(
            pos=pos, siminspiral=injfile, event=eventnum, path=wfdir
        )
    ...
    if np.any(["spcal_amp" in param for param in pos.names]) or np.any(
        ["spcal_phase" in param for param in pos.names]
    ):
    ...
    for par_name in oneDMenu:
        ...
        if greedy:
            ...
            _, reses, injstats = bppu.kdtree_bin2Step(
    ...
    if got_any > 0:
        ...
        html_corner += "</table>"
    if html_corner != "":
        html_co = html.add_collapse_section(
        ...
        if twodkdeplots_flag is True:
            if [par1_name, par2_name] in twoDplots or [
                ...
                if myfig:
                    ...
                    print(
                        "Unable to generate 2D kde levels for %s-%s"
                        % (par1_name, par2_name)
                    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--inj",
        dest="injfile",
        help="SimInsipral injection file",
        metavar="INJ.XML",
        default=None,
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Make individual 2-D plots.",
    )
    parser.add_option(
        "--header",
        action="store",
    ...
    parser.add_option(
        "--inj-spin-frame",
        default="OrbitalL",
        help="The reference frame used for the injection (default: OrbitalL)",
    )
    parser.add_option(
        "--downsample",
        action="store",
    ...
    parser.add_option(
        ...
        action="store",
        default=None,
        help="(LALInferenceMCMC) Difference in logL to use for convergence test. (DEPRECATED)",
        type="float",
    )
    parser.add_option(
        "--deltaLogP",
        action="store",
        default=None,
        help="(LALInferenceMCMC) Difference in logpost to use for burnin criteria.",
        type="float",
    )
    parser.add_option(
        "--fixedBurnin",
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="If given, will use 2-step KDE trees to estimate 1-d credible regions [default false: use greedy binning]",
    )
    parser.add_option(
        "--noplot-source-frame",
        action="store_true",
        default=False,
        help="Don't make 1D plots of source-frame masses",
    )
    ...
    oneDMenus["Extrinsic"] = (
        incParams + distParams + polParams + skyParams + phaseParams
    )
