...
parser.add_option(
    "--gid", action="store", type="string", default=None, help="GraceDB ID"
)
parser.add_option(
    "--coinc",
    action="store",
    type="string",
    default=None,
    help="Path to coinc.xml from GraceDB",
)
parser.add_option(
    "--psd",
    action="store",
    type="string",
    default=None,
    help="Path to psd.xml.gz from GraceDB",
)
parser.add_option(
    "--service-url",
    action="store",
    type="string",
    default=None,
    help="GraceDB url from which xml files are downloaded",
)
parser.add_option(
    "-I",
    "--injections",
...
parser.add_option(
    ...
    type="string",
    default=None,
    help="SimBurst table for LIB injections",
    metavar="INJFILE.xml",
)
parser.add_option(
    "-P",
...
def add_variations(cp, section, option, values=None, allowed_values=None):
    ...
    if allowed_values is not None and badvals:
        raise ValueError(
            "Unknown value for {section} . {option} {value}".format(
                section, option, " ".join(badvals)
            )
        )
    if len(vals) > 1:
        ...
        print(
            (
                "Found no variations of [{section}] {option}".format(
...
def check_priors_are_compatible(cp):
    ...
    """
    fixed = {
        key.split("fix-")[1]: float(item)
        for key, item in cp.items("engine")
        if "fix-" in key
    }
    p_min = {
        key.split("-min")[0]: float(item)
        for key, item in cp.items("engine")
        if "-min" in key
    }
    p_max = {
        key.split("-max")[0]: float(item)
        for key, item in cp.items("engine")
        if "-max" in key
    }
    ...
    if condition and condition2:
        ...
        if "q" in fixed.keys():
            ...
            m1 = (q ** (2.0 / 5.0)) * ((1.0 + q) ** (1.0 / 5.0)) * mchirp
            m2 = (q ** (-3.0 / 5.0)) * ((1.0 + q) ** (1.0 / 5.0)) * mchirp

        if "eta" in fixed.keys():
            eta = float(fixed["eta"])

            mtotal = mchirp / (eta ** (3.0 / 5.0))
    ...
    for i in fixed.keys():
        ...
        if condition(i):
            raise Exception(
                ...
                "Either change your prior bounds or change your fixed parameter" % (i)
            )
...
def generate_variations(master_cp, variations):
    ...
    for val in vals:
        ...
        cp.set(
            "paths",
            "basedir",
            os.path.join(cur_basedir, "{val}".format(opt=opt, val=val)),
        )
        cp.set(
            "paths",
            "webdir",
            os.path.join(cur_webdir, "{val}".format(opt=opt, val=val)),
        )
        cp.set(
            "paths",
            "daglogdir",
            os.path.join(cur_dagdir, "{val}".format(opt=opt, val=val)),
        )
...
def setup_roq(cp):
    ...
    if roq_bounds == "chirp_mass_q":
        ...
        mc_priors = {}
        for (roq, m1_prior), (roq2, m2_prior) in zip(
            m1_priors.items(), m2_priors.items()
        ):
    ...
    if roq_mass_freq_scale_factor != 1.0:
        print(
    ...
    if (
        opts.gid is not None
        or (opts.injections is not None or cp.has_option("input", "injection-file"))
        or cp.has_option("lalinference", "trigger_mchirp")
        or cp.has_option("input", "coinc-xml")
    ):
        ...
        if candidate_roq_paths:
            margins = np.array(
                [
                    min(
                        mc_priors[roq][1] * roq_mass_freq_scale_factor - trigger_mchirp,
            ...
            roq_paths = [best_path]
            print(
                "Prior in Mchirp will be [{}, {}] to contain the trigger Mchirp {}.".format(
        ...
        for mc_prior in mc_priors:
            mc_priors[mc_prior] = (
                np.array(mc_priors[mc_prior]) * roq_mass_freq_scale_factor
            )
    ...
    for roq in roq_paths:
        ...
        srate = 2.0 * roq_params[roq]["fhigh"] / roq_mass_freq_scale_factor
        # if srate > 8192:
        #    srate = 8192
        ...
        if roq_bounds == "chirp_mass_q":
            ...
            # params.dat uses the convention q>1 so our q_min is the inverse of their qmax
            q_min = 1.0 / float(roq_params[roq]["qmax"])
            ...
            this_cp.set(
                ...
                str(
                    max(
                        roq_params[roq]["compmin"] * roq_mass_freq_scale_factor,
                        mc_min * pow(1 + q_min, 1.0 / 5.0) * pow(q_min, 2.0 / 5.0),
                    )
                ),
            )
            this_cp.set(
                "engine",
                "comp-max",
                str(mc_max * pow(1 + q_min, 1.0 / 5.0) * pow(q_min, -3.0 / 5.0)),
            )
        elif roq_bounds == "component_mass":
...
outerdaglog = os.path.join(
    daglogdir, "lalinference_multi_" + str(uuid.uuid1()) + ".log"
)
    ...
    if cp.has_option("input", "injection-file"):
        ...
        if os.path.abspath(myinjpath) != os.path.abspath(injpath):
            ...
            if os.path.lexists(myinjpath):
                ...
                if os.path.islink(myinjpath) and os.path.realpath(
                    myinjpath
                ) == os.path.realpath(injpath):
                    cp.set("input", "injection-file", myinjpath)
                else:
                    # Do not over-write the injection file
                    print(
                        (
                            "Error: File {0} exists in run directory, not over-writing with \
                            {1}. Remove the existing file or create a fresh run directory".format(
                                myinjpath, injpath
                            )
                        )
                    )
