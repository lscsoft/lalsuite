...
def DEIM(basis):
    ...
    for i in range(vec_num - 1):
        ...
        c = solve_triangular(
            PTU[:i:, :i:],
            np.array([ei[node] for node in nodes]),
            lower=True,
            check_finite=False,
        )
        # Step (5)
...
flow_in_params, fhigh_in_params, deltaF_in_params = (
    basis_params[0],
    basis_params[1],
    1.0 / basis_params[2],
)
...
params_linear_path = os.path.join(
    options.b_matrix_directory, "selected_params_linear.npy"
)
params_quadratic_path = os.path.join(
    options.b_matrix_directory, "selected_params_quadratic.npy"
)
if (
    os.path.exists(B_linear_path)
    and os.path.exists(B_quadratic_path)
    and os.path.exists(fnodes_linear_path)
    ...
    fnodes_linear, B_linear = construct_nodes(
    ...
    fnodes_quadratic, B_quadratic = construct_nodes(
...
def BuildWeights(data, B, deltaF):
    ...
    """
    weights = np.dot(data, B.conjugate()) * deltaF * 4.0
...
relative_tc_shift = options.seglen - 2.0
...
for ifo in options.IFOs:
    ...
    tcs = np.linspace(
        relative_tc_shift - options.dt - 0.045,
        relative_tc_shift + options.dt + 0.045,
        ceil(2.0 * (options.dt + 0.045) / options.delta_tc),
    )  # array of relative time shifts to be applied to the data

    tc_shifted_data = np.zeros(
        [len(tcs), len(fseries)], dtype=complex
    ...
    for j in range(len(tcs)):
        ...
        tc_shifted_data[j] = data * np.exp(1j * 2.0 * np.pi * fseries * tc)
    ...
    max_elements = int(
        (max_block_gigabytes * 2**30) / 8
    )  # max number of double complex elements

    weights_linear = blockwise_dot(
        tc_shifted_data, B_linear.conjugate(), deltaF, max_elements
    ).T
    ...
    # *************************************************************************** #
    weights_path_quadratic = os.path.join(
        options.outpath, "weights_quadratic_%s.dat" % ifo
    )
    weights_file_quadratic = open(weights_path_quadratic, "wb")
    weights_quadratic = (BuildWeights(1.0 / psd, B_quadratic, deltaF).T).real
    ...
    np.savetxt(
        size_file_path,
        np.array(
            (len(tcs), B_linear.shape[0], B_quadratic.shape[0], B_linear.shape[1])
        ),
        fmt="%u",
    )
