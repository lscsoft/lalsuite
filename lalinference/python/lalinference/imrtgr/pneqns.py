...
def Omega(
    v, m1, m2, S1, S2, Ln
):  # S1, S2 are full spin vector while Ln is a unit vector
    ...
    eta = (m1 * m2) / (m1 + m2) ** 2.0
    return (v**5.0 / m) * (
        (3.0 / 4 + eta / 2.0 - 3.0 * deltam / m / 4.0) * Ln
        + 0.5 * v / m**2.0 * (-3.0 * np.dot((S2 + (m2 / m1) * S1), Ln) * Ln + S2)
        + v**2.0
        * (
            9.0 / 16.0
            + 5.0 * eta / 4.0
...
def denergy_by_flux(v, eta, delta, chiadL, chisdL, chiasqr, chissqr, chisdchia, order):
    ...
    dEbF0 = dEbF2 = dEbF3 = dEbF4 = dEbF5 = dEbF6 = dEbF6L = dEbF7 = 0.0

    if order >= 0:
        dEbF0 = -5.0 / (32.0 * eta)
    if order >= 2:
        dEbF2 = 2.2113095238095237 + (11 * eta) / 4.0
    if order >= 3:
        dEbF3 = (
            (113 * chiadL * delta) / 12.0
            + chisdL * (9.416666666666666 - (19.0 * eta) / 3.0)
            - 4.0 * PI
        )
    if order >= 4:
        dEbF4 = (
            3.010315295099521
            + (233 * chisdchia * delta) / 48.0
            - (719.0 * chiadL * chisdL * delta) / 48.0
            + chiasqr * (2.4270833333333335 - 10.0 * eta)
            + chisdL * chisdL * (-7.489583333333333 - eta / 24.0)
            + chissqr * (2.4270833333333335 + (7.0 * eta) / 24.0)
            + (5429.0 * eta) / 1008.0
            + (617 * eta * eta) / 144.0
            + chiadL * chiadL * (-7.489583333333333 + 30.0 * eta)
        )
    if order >= 5:
        dEbF5 = (
            chiadL * delta * (72.71676587301587 + (7 * eta) / 2.0)
            + chisdL * (72.71676587301587 - (1213 * eta) / 18.0 - 17 * eta * eta / 2.0)
            - (7729 * PI) / 672.0
            + (13 * eta * PI) / 8.0
        )
    if order >= 6:
        dEbF6 = (
            -115.2253249962622
            - 15211 * eta * eta / 6912.0
            + 25565 * eta * eta * eta / 5184.0
            + 32 * PI * PI / 3.0
            + eta * (258.1491854023631 - 451 * PI * PI / 48.0)
            + (1712 * GAMMA) / 105.0
        )
        dEbF6L = 1712.0 / 105.0
    if order >= 7:
        dEbF7 = (
            (-15419335.0 * PI) / 1.016064e6
            - (75703.0 * eta * PI) / 6048.0
            + (14809.0 * eta * eta * PI) / 3024.0
        )
...
def precession_eqns(t, y_vec, m1, m2):
    ...
    Lmag = (eta * m**2.0 / v) * (1.0 + (1.5 + eta / 6.0) * v**2.0)
    ...
    dv_dt = -1.0 / (
        m
        * denergy_by_flux(v, eta, delta, chiadL, chisdL, chiasqr, chissqr, chisdchia, 7)
    )
...
def evolve_spins_dt(
    v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z, Lnx, Lny, Lnz, v_final, dt
):
    ...
    if (chi1x**2 + chi1y**2 + chi1z**2) > 1.0:
        raise ValueError("ERROR: magnitude of spin1 is greater than 1")
    if (chi2x**2 + chi2y**2 + chi2z**2) > 1.0:
    ...
    T_MAX = (
        2.0
        * lalsim.SimInspiralTaylorF2ReducedSpinChirpTime(
            v0**3.0 / (PI * (m1 + m2) * MTSUN_SI),
            m1 * MSUN_SI,
            m2 * MSUN_SI,
            chi_eff,
            0,
    ...
    while (
        solver.successful()
        and solver.t < 2.0 * T_MAX
        and v_current <= v_final
        and dvdt_current > 0.0
        and dvdt_current < dvdt_max
        and abs(Lnorm_current - 1.0) < delta_Lnorm_max
    ):
    ...
    if abs(v_last - v_final) > delta_vf_max:
        if dvdt_current <= 0 or dvdt_current > dvdt_max:
            ...
            print(
                "Warning: Integration stopped at v_max = {0}, more than {1} different from v_final = {2}, because dv/dt became negative or exceeded {3}, with a value of {4}. The final value used is {5}.".format(
                    v_last,
                    delta_vf_max,
                    v_final,
                    dvdt_max,
            ...
            raise ValueError(
                "v_max = {0} is more than {1} different from v_final = {2} and dv/dt is positive and less than {3}".format(
                    v_v[-1], delta_vf, v_final, dvdt_max
                )
            )

    if abs(Lnorm_current - 1.0) >= delta_Lnorm_max:
        raise ValueError(
            "norm of Ln is more than {0:e} different from 1, with distance {1:e}".format(
                delta_Lnorm_max, abs(Lnorm_current - 1.0)
            )
        )
...
def find_tilts_and_phi12_at_freq(
    v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z, Lnx, Lny, Lnz, v_final, dt
):
    """given the spins and ang momentum at a given frequency, find the tilt and in-plane spin angles at a later frequency"""

    print(
        "v0 = %f, m1 = %f, m2 = %f, chi1x = %f, chi1y = %f, chi1z = %f, chi2x = %f, chi2y = %f, chi2z = %f"
        % (v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z)
    )

    # evolve the spins
    v_v, chi1x_v, chi1y_v, chi1z_v, chi2x_v, chi2y_v, chi2z_v, Lnx_v, Lny_v, Lnz_v = (
        evolve_spins_dt(
            v0,
            m1,
            m2,
    ...
    cos_phi12 = np.dot(chi1inplane, chi2inplane) / (
        norm(chi1inplane) * norm(chi2inplane)
    )

    print(
        "cos tilt1 = %f, cos tilt2 = %f, cos phi12 = %f"
        % (cos_tilt1, cos_tilt2, cos_phi12)
    )
