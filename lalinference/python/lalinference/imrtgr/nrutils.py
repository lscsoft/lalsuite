...
class bbh_Kerr_trunc_opts:
    ...
    keep = (
        "KEEP"  # keep fit value, even if superextremal, but still give the info message
    )
...
def _truncate_at_Kerr_limit(chif, behavior, fitname="this"):
    ...
    if behavior != bbh_Kerr_trunc_opts.ign and np.any(abs(chif) > 1.0):
        if isinstance(chif, (list, np.ndarray)):
            ...
            if (
                behavior == bbh_Kerr_trunc_opts.trunc
                or behavior == bbh_Kerr_trunc_opts.trunc_silent
            ):
                chif_trunc = np.sign(chif[idx_over]) * 1.0
                if behavior == bbh_Kerr_trunc_opts.trunc:
                    print(
                        "Truncating %d excessive chif values from %s fit to Kerr limit of +-1.0"
                        % (np.size(idx_over), fitname)
                    )
                chif[idx_over] = chif_trunc
            elif behavior == bbh_Kerr_trunc_opts.keep:
                print(
                    "Note: %s fit predicts %d chif values in excess of the Kerr limit."
                    % (fitname, np.size(idx_over))
                )
            elif behavior == bbh_Kerr_trunc_opts.err:
                raise ValueError(
                    "%s fit predicts %d chif values in excess of the Kerr limit."
                    % (fitname, np.size(idx_over))
                )
        else:
            if (
                ...
                if behavior == bbh_Kerr_trunc_opts.trunc:
                    print(
                        "Truncating excessive chif of %f from %s fit to Kerr limit of %f"
                        % (chif, fitname, chif_trunc)
                    )
                chif = chif_trunc
            elif behavior == bbh_Kerr_trunc_opts.keep:
                print(
                    "Note: %s fit predicts chif of %f in excess of the Kerr limit."
                    % (fitname, chif)
                )
            elif behavior == bbh_Kerr_trunc_opts.err:
                raise ValueError(
                    "%s fit predicts chif of %f in excess of the Kerr limit."
                    % (fitname, chif)
                )
    return chif
...
def bbh_final_spin_non_spinning_Panetal(m1, m2):
    ...
    eta = m1 * m2 / (m1 + m2) ** 2.0
    return np.sqrt(12.0) * eta - 3.871 * (eta**2.0) + 4.028 * (eta**3)
...
def _RIT_setup(m1, m2, chi1, chi2):
    ...
    S = (
        m1 * m1 * chi1 + m2 * m2 * chi2
    ) / m**2  # symmetric spin (dimensionless -- called \tilde{S} in the paper)
    Delta = (
        m2 * chi2 - m1 * chi1
    ) / m  # antisymmetric spin (dimensionless -- called tilde{Delta} in the paper
...
def _RIT_symm_express(eta, delta_m, S, Delta, coeffs):
    ...
    return (
        ...
        * (
            ...
            + coeffs[8] * S2 * S
            + coeffs[9] * S * dm2
            + coeffs[10] * Deltadm * S2
            + coeffs[11] * Delta2 * Deltadm
            + coeffs[12] * Delta2 * Delta2
            + coeffs[13] * S2 * S2
            + coeffs[14] * Delta2 * S2
            + coeffs[15] * dm2 * dm2
            + coeffs[16] * Deltadm * dm2
            + coeffs[17] * Delta2 * dm2
            + coeffs[18] * S2 * dm2
        )
    )
...
def _final_spin_diff_Healyetal(a_f, eta, delta_m, S, Delta, version):
    ...
    J_isco = (3 * np.sqrt(r_isco) - 2 * a_f) * 2.0 / np.sqrt(3 * r_isco)

    # fitting coefficients
    if (
        version == "2014"
    ):  # From Table XI of Healy et al Phys Rev D 90, 104004 (2014) [fourth order fits]
        L0 = 0.686710
        L1 = 0.613247
        L2a = -0.145427
        L2b = -0.115689
        ...
        version == "2016"
    ):  # From Table III of Healy and Lousto arXiv:1610.09713 (values taken from Matlab implementation and thus slightly more precise than the ones in the table)
        L0 = 0.686732132
        L1 = 0.613284976
        L2a = -0.148530075
        L2b = -0.113826318
...
def bbh_final_spin_non_precessing_Healyetal(m1, m2, chi1, chi2, version="2014"):
    ...
    def func(eta, delta_m, S, Delta):
        return so.leastsq(
            _final_spin_diff_Healyetal, 0.0, args=(eta, delta_m, S, Delta, version)
        )[0][0]
...
def bbh_final_mass_non_precessing_Healyetal(
    m1, m2, chi1, chi2, version="2014", chif=None
):
    """
    ...
    E_isco = (1.0 - 2.0 / r_isco + chif / r_isco**1.5) / np.sqrt(
        1.0 - 3.0 / r_isco + 2.0 * chif / r_isco**1.5
    )
...
def bbh_final_mass_projected_spin_Healyetal(
    m1, m2, chi1, chi2, tilt1, tilt2, chif=None
):
    """
    wrapper to bbh_final_mass_projected_spins() for backwards compatibility
    """
    return bbh_final_mass_projected_spins(
        m1, m2, chi1, chi2, tilt1, tilt2, "HLZ2014", chif
    )


def bbh_final_spin_precessing_Healyetal_extension_Minit(
    m1, m2, chi1, chi2, tilt1, tilt2, phi12
):
    """
...
def bbh_final_mass_projected_spins(
    m1, m2, chi1, chi2, tilt1, tilt2, fitname, chif=None
):
    """
    ...
    if fitname == bbh_final_state_fits.pan2011:
        ...
        mf = bbh_final_mass_non_precessing_Healyetal(
            m1, m2, chi1proj, chi2proj, version="2014", chif=chif
        )
    elif fitname == bbh_final_state_fits.hl2016:
        mf = bbh_final_mass_non_precessing_Healyetal(
            m1, m2, chi1proj, chi2proj, version="2016", chif=chif
        )
        ...
        if chif is not None:
            print("Note: Precomputed chif not used by this fit.")
        mf = bbh_final_mass_non_precessing_UIB2016(
            m1, m2, chi1proj, chi2proj, version="v2"
        )
    elif fitname == bbh_final_state_fits.uib2016v1:
        if chif is not None:
            print("Note: Precomputed chif not used by this fit.")
        mf = bbh_final_mass_non_precessing_UIB2016(
            m1, m2, chi1proj, chi2proj, version="v1"
        )
    else:
...
def bbh_final_spin_projected_spins(
    m1, m2, chi1, chi2, tilt1, tilt2, fitname, truncate=bbh_Kerr_trunc_opts.trunc
):
    """
    ...
    if fitname == bbh_final_state_fits.pan2011:
        ...
        chif = bbh_final_spin_non_precessing_Healyetal(
            m1, m2, chi1proj, chi2proj, version="2014"
        )
    elif fitname == bbh_final_state_fits.hl2016:
        chif = bbh_final_spin_non_precessing_Healyetal(
            m1, m2, chi1proj, chi2proj, version="2016"
        )
    elif fitname == bbh_final_state_fits.phenD:
        chif = bbh_final_spin_non_precessing_Husaetal(m1, m2, chi1proj, chi2proj)
    elif fitname == bbh_final_state_fits.uib2016:
        chif = bbh_final_spin_non_precessing_UIB2016(
            m1, m2, chi1proj, chi2proj, version="v2"
        )
    elif fitname == bbh_final_state_fits.uib2016v1:
        chif = bbh_final_spin_non_precessing_UIB2016(
    ...
    chif = _truncate_at_Kerr_limit(
        chif, truncate, fitname
    )  # optionally truncate and/or warn at Kerr limit, depending on user option

    return chif


def bbh_final_spin_precessing(
    m1, m2, chi1, chi2, tilt1, tilt2, phi12, fitname, truncate=bbh_Kerr_trunc_opts.trunc
):
    """
    ...
    if (
        fitname == bbh_final_state_fits.pan2011
        ...
        precfit = True
        chif = bbh_final_spin_precessing_HBR2016(
            m1, m2, chi1, chi2, tilt1, tilt2, phi12
        )
    else:
        raise ValueError("Unrecognized fit name.")
    ...
    if not precfit:
        ...
        chifaligned = bbh_final_spin_projected_spins(
            m1, m2, chi1, chi2, tilt1, tilt2, fitname, truncate
        )
        ...
        Sperpmag2 = (
            S1perpmag * S1perpmag
            + S2perpmag * S2perpmag
            + 2.0 * S1perpmag * S2perpmag * np.cos(phi12)
        )

        # Combine together
        chif = (chifaligned * chifaligned + Sperpmag2 / (m1 + m2) ** 4.0) ** 0.5

    chif = _truncate_at_Kerr_limit(
        chif, truncate, "augmented " + fitname
    )  # optionally truncate and/or warn at Kerr limit, depending on user option
...
def bbh_final_mass_non_precessing_Husaetal(m1, m2, chi1, chi2):
    ...
    S = S1 + S2  # total spin
    Sh = S / (1.0 - 2.0 * eta)  # rescaled total spin
...
def bbh_final_spin_non_precessing_Husaetal(m1, m2, chi1, chi2):
    ...
    chif = (
        ...
        - 4.399247300629289 * eta2
        + 9.397292189321194 * eta3
        - 13.180949901606242 * eta4
        + (1 - 0.0850917821418767 * eta - 5.837029316602263 * eta2) * S
        + (0.1014665242971878 * eta - 2.0967746996832157 * eta2) * Ssq
        + (-1.3546806617824356 * eta + 4.108962025369336 * eta2) * Scu
        + (-0.8676969352555539 * eta + 2.064046835273906 * eta2) * Squ
    )
...
def bbh_final_spin_non_precessing_UIB2016(m1, m2, chi1, chi2, version="v2"):
    ...
    Lorb = (
        (2.0 * sqrt3 * eta + a20 * a2 * eta2 + a30 * a3 * eta3) / (1.0 + a50 * a5 * eta)
        + (
            b10
            * b1
            * Shat
            * (f11 * eta + f12 * eta2 + (64.0 - 16.0 * f11 - 4.0 * f12) * eta3)
            + b20
            * b2
            * Shat2
            * (f21 * eta + f22 * eta2 + (64.0 - 16.0 * f21 - 4.0 * f22) * eta3)
            + b30
            * b3
            * Shat3
            * (f31 * eta + f32 * eta2 + (64.0 - 16.0 * f31 - 4.0 * f32) * eta3)
        )
        / (
            1.0
            + b50
            ...
            * (
                ...
                + f51 * eta
                + f52 * eta2
                + (64.0 - 64.0 * f50 - 16.0 * f51 - 4.0 * f52) * eta3
            )
        )
        + d10 * sqrt1m4eta * eta2 * (1.0 + d11 * eta) * chidiff
        + d30 * Shat * sqrt1m4eta * eta3 * (1.0 + d31 * eta) * chidiff
        + d20 * eta3 * chidiff2
    )
...
def bbh_final_spin_precessing_HBR2016(
    ...
    cos_alpha = (
        ...
        phi12
    ) + cos_beta * cos_gamma  # This rewrites the inner product definition of cos_alpha in terms of cos_beta, cos_gamma, and phi12
    ...
    if np.any(sqrt_arg < 0.0):
        print(
            f"bbh_final_spin_precessing_HBR2016(): The argument of the square root is negative for indexes {np.where(sqrt_arg < 0)[0]}; truncating it to zero."
        )
...
class bbh_Lpeak_fits:
    t1600018 = (
        "T1600018"  # Jimenez Forteza et al. [LIGO-T1600018 (2016)]         (aligned)
    )
    uib2016 = (
        "UIB2016"  # Keitel et al.          [arXiv:1612.09566]             (aligned)
    )
...
def bbh_peak_luminosity_non_precessing_T1600018(m1, m2, chi1, chi2):
    ...
    dm2 = 1.0 - 4.0 * eta  # equivalent to sqrt1m4eta**2

    chi_eff = (
        m1 * chi1 + m2 * chi2
    ) / m  # equivalent to (q_inv*chi1 + chi2)/(1. + q_inv)
    ...
    Lpeak = (
        (
            0.012851338846828302
            + 0.007822265919928252 * chi_eff
            + 0.010221856361035788 * chi_eff2
            + 0.015805535732661396 * chi_eff3
            + 0.0011356206806770043 * chi_eff4
            - 0.009868152529667197 * chi_eff5
        )
        * eta2
        + (
            0.05681786589129071
            - 0.0017473702709303457 * chi_eff
            - 0.10150706091341818 * chi_eff2
            - 0.2349153289253309 * chi_eff3
            + 0.015657737820040145 * chi_eff4
            + 0.19556893194885075 * chi_eff5
        )
        * eta4
        + 0.026161288241420833
        * dm2**0.541825641769908
        * eta**3.1629576945611757
        * chidiff
        + 0.0007771032100485481
        * dm2**0.4499151697918658
        * eta**1.7800346166040835
        * chidiff2
    )
...
def bbh_peak_luminosity_non_precessing_UIB2016(m1, m2, chi1, chi2):
    ...
    Lpeak = (
        ...
        + (
            ...
            * Shat2
            * (f20 + f21 * eta + (16.0 - 16.0 * f20 - 4.0 * f21) * eta2)
            + Shat3 * (f30 + f31 * eta + (-16.0 * f30 - 4.0 * f31) * eta2)
            + Shat4 * (f40 + f41 * eta + (-16.0 * f40 - 4.0 * f41) * eta2)
        )
        / (
            1.0
            - 0.328
            * b4
            * Shat
            * (f60 + f61 * eta + (16.0 - 16.0 * f60 - 4.0 * f61) * eta2)
            + Shat2 * (f70 + f71 * eta + (-16.0 * f70 - 4.0 * f71) * eta2)
        )
        + eta3 * ((d10 + d30 * Shat) * sqrt1m4eta * chidiff + d20 * chidiff2)
    )
...
def bbh_average_fits_precessing(
    m1, m2, chi1, chi2, tilt1, tilt2, phi12, quantity, fits
):
    """
    ...
    def _return_quantity(m1, m2, chi1, chi2, tilt1, tilt2, phi12, fitname):
        if quantity == "Mf":
            return bbh_final_mass_projected_spins(
                m1, m2, chi1, chi2, tilt1, tilt2, fitname
            )
        elif quantity == "af":
            return bbh_final_spin_precessing(
                m1, m2, chi1, chi2, tilt1, tilt2, phi12, fitname
            )
        elif quantity == "afz":
            return bbh_final_spin_projected_spins(
                m1, m2, chi1, chi2, tilt1, tilt2, fitname
            )
        elif quantity == "Lpeak":
            return bbh_peak_luminosity_projected_spins(
                m1, m2, chi1, chi2, tilt1, tilt2, fitname
            )
