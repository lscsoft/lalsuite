...
""" compute the integrant of P(dMf/Mf, daf/chif). """
def P_integrant(chif, Mf, v1, v2, P_dMfdaf_interp_object, P_Mfaf_imr_interp_object):

  Mf_mat, af_mat = np.meshgrid(Mf, chif)
  ...
  P_imr = P_Mfaf_imr_interp_object(Mf, chif)
  return P_imr*P_delta*abs(Mf_mat*af_mat), P_imr, P_delta

""" compute P(dMf/Mf, daf/chif). """
def calc_sum(Mf, chif, v1, v2, P_dMfdaf_interp_object, P_Mfaf_imr_interp_object):

  Pintg, P_imr, P_delta = P_integrant(chif, Mf, v1, v2, P_dMfdaf_interp_object, P_Mfaf_imr_interp_object)
  return np.sum(Pintg)
