...
def calc_final_mass_spin(m1, m2, chi1, chi2, chi1z, chi2z, phi12, fit_formula):
    ...
    if fit_formula == "nospin_Pan2011":
        ...
        chif = nr.bbh_final_spin_non_precessing_Healyetal(
            m1, m2, chi1z, chi2z, version="2014"
        )
        mf = nr.bbh_final_mass_non_precessing_Healyetal(
            m1, m2, chi1z, chi2z, version="2014", chif=chif
        )
        ...
        mf = nr.bbh_average_fits_precessing(
            m1, m2, chi1, chi2, tilt1, tilt2, 0, "Mf", mf_fits
        )
        chif = nr.bbh_average_fits_precessing(
            m1, m2, chi1, chi2, tilt1, tilt2, phi12, "af", chif_fits
        )
    else:
...
def calc_sum(Mf, chif, v1, v2, P_Mfchif_i_interp_object, P_Mfchif_r_interp_object):
    ...
    Pintg, P_i, P_r = P_integrand(
        chif, Mf, v1, v2, P_Mfchif_i_interp_object, P_Mfchif_r_interp_object
    )
    return np.sum(Pintg)
...
def calc_Mfchif_prior_samples(
    ...
    # generate samples of component spins, for uninform spin magnitudes and isotropic spin directions (uniform in cos_tilt_angle)
        ...
        chi1_pr = np.random.uniform(
            comp_spin_min, comp_spin_max, N_sampl
        ) * np.random.uniform(-1.0, 1.0, N_sampl)
        chi2_pr = np.random.uniform(
            comp_spin_min, comp_spin_max, N_sampl
        ) * np.random.uniform(-1.0, 1.0, N_sampl)
...
def calc_Mfchif_prior(
    ...
    # check inputs
    if comp_mass_prior_min < 1.0 or comp_mass_prior_min > 1000.0:
        raise ValueError("comp_mass_prior_min should be in the interval [1, 1000]")
    if comp_mass_prior_max < 1.0 or comp_mass_prior_max > 1000.0:
    ...
    P_Mfchif_pr, Mf_bins, chif_bins = np.histogram2d(
        Mf_pr, chif_pr, bins=(Mf_bins, chif_bins), density=True
    )
