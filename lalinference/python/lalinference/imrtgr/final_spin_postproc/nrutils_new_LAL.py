...
def bbh_final_mass_non_precessing_SEOBNRv2(m1, m2, a1, a2): 
	""" 
	Calculate the mass of the final BH resulting from the 
	merger of two black holes with precessing spins using the fit used in SEOBNRv2--see T1400476-v3. This is a wrapper
        for the implementation in XLALSimIMREOBFinalMassSpin in LALSimBlackHoleRingdown.c.
	...
	"""
        ...
        return np.array([(m1[i]+m2[i])*lalsim.SimIMREOBFinalMassSpin(m1[i], m2[i], [0., 0., a1[i]], [0., 0., a2[i]], lalsim.GetApproximantFromString('SEOBNRv2'))[1] for i in range(len(m1))])

def bbh_final_spin_non_precessing_SEOBNRv2(m1, m2, a1, a2): 
	""" 
	...
	"""
        ...
        return np.array([lalsim.SimIMREOBFinalMassSpin(m1[i], m2[i], [0., 0., a1[i]], [0., 0., a2[i]], lalsim.GetApproximantFromString('SEOBNRv2'))[2] for i in range(len(m1))])

def bbh_final_spin_precessing_SEOBNRv2_extension_Mf(m1, m2, chi1, chi2, tilt1, tilt2, phi12):
    """
    Calculate the spin of the final BH resulting from the merger of two black holes including the in-plane spine by projecting the spins along the angular momentum and using fit used in SEOBNRv2 and then adding in quadrature the in-plane dimensionful spin scaled by the final mass squared.
    ...
    # First compute the final mass and parallel component of the final spin using the aligned components of the initial spins
    chifpara = bbh_final_spin_non_precessing_SEOBNRv2(m1, m2, chi1*np.cos(tilt1), chi2*np.cos(tilt2))
    Mf = bbh_final_mass_non_precessing_SEOBNRv2(m1, m2, chi1*np.cos(tilt1), chi2*np.cos(tilt2))
...
def bbh_final_spin_precessing_SEOBNRv2_extension_Minit(m1, m2, chi1, chi2, tilt1, tilt2, phi12):
    """
    Calculate the spin of the final BH resulting from the merger of two black holes including the in-plane spine by projecting the spins along the angular momentum and using fit used in SEOBNRv2 and then adding in quadrature the in-plane dimensionful spin scaled by the initial mass squared.
...
def bbh_final_mass_precessing_Barausse_and_Rezzolla(m1, m2, a1, a2, tilt1, tilt2, phi12): 
	""" 
	Calculate the mass of the final BH resulting from the 
