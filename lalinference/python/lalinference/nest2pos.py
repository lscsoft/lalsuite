...
def compute_weights(data, Nlive):
    ...
    log_vols_start = cumsum(ones(len(start_data) + 1) * log1p(-1.0 / Nlive)) - log1p(
        -1.0 / Nlive
    )
...
def draw_posterior_many(datas, Nlives, verbose=False):
    ...
    log_evs, log_wts = zip(
        *[compute_weights(data["logL"], Nlive) for data, Nlive in zip(datas, Nlives)]
    )
    if verbose:
    ...
    posts = [
        ...
        for (data, logwt, logZ) in zip(datas, log_wts, log_evs)
    ]
    if verbose:
        print("Number of input samples: %s" % (str([len(x) for x in log_wts])))
    if verbose:
        print(
            "Expected number of samples from each input file %s"
            % (str([int(f * len(p)) for f, p in zip(fracs, posts)]))
        )
    bigpos = []
    ...
    if verbose:
        print("Total number of samples produced: %i" % (len(result)))
    return result
...
def draw_N_posterior_many(datas, Nlives, Npost, verbose=False):
    ...
    posts = [
        draw_N_posterior(data, logwt, N) for (data, logwt, N) in zip(datas, log_wts, Ns)
    ]
...
def draw_posterior_many_ROQ_runs(datas, Nlives, verbose=False):
    ...
    # print 'evidences: %s'%(str(log_evs))
    fracs = [
        exp(log_ev - log_max_evidence) for log_ev in log_evs
    ]  # TODO: add logPriorVol from ROQ run
    if verbose:
        print("Relative weights of input files: %s" % (str(fracs)))
    Ns = [fracs[i] / len(datas[i]) for i in range(len(fracs))]
    Ntot = max(Ns)
