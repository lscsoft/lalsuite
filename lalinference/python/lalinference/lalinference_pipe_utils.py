...
class Event:
    ...
    def __init__(
        ...
        if self.injection is not None:
            self.trig_time = (
                self.injection.geocent_end_time
                + 1.0e-9 * self.injection.geocent_end_time_ns
            )
            if event_id is None:
                self.event_id = self.injection.simulation_id
        if self.burstinjection is not None:
            self.trig_time = (
                self.burstinjection.time_geocent
                + 1.0e-9 * self.burstinjection.time_geocent_ns
            )
            if event_id is None:
                self.event_id = self.burstinjection.simulation_id
        if self.sngltrigger is not None:
            self.trig_time = (
                self.sngltrigger.end_time + 1.0e-9 * self.sngltrigger.end_time_ns
            )
            self.event_id = self.sngltrigger.event_id
        if self.coinctrigger is not None:
            self.trig_time = (
                self.coinctrigger.end_time + 1.0e-9 * self.coinctrigger.end_time_ns
            )
...
def create_events_from_coinc_and_psd(
    ...
    sngl_event_idx = dict(
        (row.event_id, row)
        for row in lsctables.SnglInspiralTable.get_table(coinc_xml_obj)
    )
    ...
    for coinc in coinc_events:
        ...
        for e in these_sngls:
            if roq == False:
                chirplen = SimInspiralChirpTimeBound(
                    flow, e.mass1 * lal.MSUN_SI, e.mass2 * lal.MSUN_SI, 0.0, 0.0
                )
        ...
        if srate:
            if max(srate) < srate_psdfile:
                ...
                if psd_dict is not None and use_gracedbpsd:
                    fhigh = (
                        srate_psdfile / 2.0 * 0.95
                    )  # Because of the drop-off near Nyquist of the PSD from gstlal
        else:
            srate = None
        if dur:
            duration = max(dur)
...
def get_zerolag_lloid(
    ...
    if gpsstart is not None:
        get_coincs = (
            get_coincs
            + " and coinc_inspiral.end_time+coinc_inspiral.end_time_ns*1.0e-9 > %f"
            % (gpsstart)
        )
    if gpsend is not None:
        get_coincs = (
    ...
    for sngl_time, ifo, coinc_id, snr, chisq, cfar in db_out:
        ...
        if not coinc_id in output.keys():
            output[coinc_id] = Event(
                trig_time=sngl_time, timeslide_dict={}, event_id=int(coinc_id)
            )
            extra[coinc_id] = {}
            ...
            for ifo in output[co].ifos:
                fh.write(
                    ...
                    % (
                        ...
                        str(extra[co][ifo]["snr"]),
                        str(extra[co][ifo]["chisq"]),
                        str(extra[co][ifo]["cfar"]),
                    )
                )
        fh.close()
...
def get_timeslides_pipedown(
    database_connection, dumpfile=None, gpsstart=None, gpsend=None, max_cfar=-1
):
    """
    Returns a list of Event objects
    ...
    if gpsstart is not None:
        get_coincs = (
            get_coincs
            + joinstr
            + " coinc_inspiral.end_time+coinc_inspiral.end_time_ns*1e-9 > %f"
            % (gpsstart)
        )
    if gpsend is not None:
        get_coincs = (
    ...
    if max_cfar != -1:
        get_coincs = (
            get_coincs + joinstr + " coinc_inspiral.combined_far < %f" % (max_cfar)
        )
    db_out = database_connection.cursor().execute(get_coincs)
    ...
    for sngl_time, slide, ifo, coinc_id, snr, chisq, cfar in db_out:
        ...
        if not coinc_id in output.keys():
            output[coinc_id] = Event(
                trig_time=slid_time, timeslide_dict={}, event_id=int(coinc_id)
            )
            extra[coinc_id] = {}
...
def get_xml_psds(psdxml, ifos, outpath, end_time=None):
    ...
    for instrument in xmlpsd.keys():
        ...
        combine = np.c_[
            ifodata.f0 + np.arange(ifodata.data.length) * ifodata.deltaF,
            ifodata.data.data,
        ]
...
def get_trigger_chirpmass(coinc_xml_obj):
    ...
    mchirp = (mass1[0] * mass2[0]) ** (3.0 / 5.0) / (
        (mass1[0] + mass2[0]) ** (1.0 / 5.0)
    )

    return mchirp


def get_roq_mchirp_priors(
    path, roq_paths, roq_params, key, coinc_xml_obj=None, sim_inspiral=None
):
    ...
    for roq in roq_paths:
        ...
        mc_priors[roq] = [
            float(roq_params[roq]["chirpmassmin"]),
            float(roq_params[roq]["chirpmassmax"]),
        ]
...
def get_roq_component_mass_priors(
    path, roq_paths, roq_params, key, coinc_xml_obj=None, sim_inspiral=None
):
    ...
    for roq in roq_paths:
        ...
        roq_params[roq] = np.genfromtxt(params, names=True)
        m1_priors[roq] = [
            float(roq_params[roq]["mass1min"]),
            float(roq_params[roq]["mass1max"]),
        ]
        m2_priors[roq] = [
            float(roq_params[roq]["mass2min"]),
            float(roq_params[roq]["mass2max"]),
        ]
...
def get_roq_mass_freq_scale_factor(mc_priors, trigger_mchirp, force_flow=None):
    ...
    if force_flow is None and trigger_mchirp is not None:
        if trigger_mchirp >= mc_max:
            scale_factor = 2.0 ** (floor(trigger_mchirp / mc_max))
        if trigger_mchirp <= mc_min:
            scale_factor = (2.0 / 3.2) ** (ceil(trigger_mchirp / mc_min))
    elif force_flow is not None:
        scale_factor = 20.0 / force_flow
    return scale_factor
...
class LALInferencePipelineDAG(pipeline.CondorDAG):
    def __init__(self, cp):
        ...
        if cp.has_option("paths", "basedir"):
            ...
            print(
                "No basepath specified, using current directory: %s" % (self.basepath)
            )
        ...
        mkdirs(daglogdir)
        self.daglogfile = os.path.join(
            daglogdir, "lalinference_pipeline-" + str(uuid.uuid1()) + ".log"
        )
        ...
        self.datafind_job = pipeline.LSCDataFindJob(
            self.cachepath, self.logpath, self.config
        )
        ...
        if cp.has_option("condor", "accounting_group"):
            self.datafind_job.add_condor_cmd(
                "accounting_group", cp.get("condor", "accounting_group")
            )
        if cp.has_option("condor", "accounting_group_user"):
            self.datafind_job.add_condor_cmd(
                "accounting_group_user", cp.get("condor", "accounting_group_user")
            )
        self.datafind_job.set_sub_file(
            os.path.abspath(os.path.join(self.basepath, "datafind.sub"))
        )
        self.preengine_job = EngineJob(
            self.config,
            os.path.join(self.basepath, "prelalinference.sub"),
            self.logpath,
            engine="lalinferencedatadump",
            ispreengine=True,
            sharedfs=True,
        )
        self.preengine_job.set_universe("vanilla")
        if self.config.getboolean("analysis", "roq"):
            self.computeroqweights_job = ROMJob(
                self.config,
                os.path.join(self.basepath, "computeroqweights.sub"),
                self.logpath,
            )
        if self.config.has_option("condor", "bayesline"):
            self.bayesline_job = BayesLineJob(
                self.config, os.path.join(self.basepath, "bayesline.sub"), self.logpath
            )
        self.bayeswavepsd_job = {}
        self.bayeswavepost_job = {}
        if self.config.has_option("condor", "bayeswave"):
            for ifo in self.ifos:
                self.bayeswavepsd_job[ifo] = BayesWavePSDJob(
                    self.config,
                    os.path.join(self.basepath, "bayeswavepsd_%s.sub" % (ifo)),
                    self.logpath,
                )
                self.bayeswavepost_job[ifo] = BayesWavePostJob(
                    self.config,
                    os.path.join(self.basepath, "bayeswavepost_%s.sub" % (ifo)),
                    self.logpath,
                )
        ...
        for ifos in ifocombos:
            self.engine_jobs[ifos] = EngineJob(
                self.config,
                os.path.join(
                    self.basepath,
                    "engine_%s.sub" % (reduce(lambda x, y: x + y, map(str, ifos))),
                ),
                self.logpath,
                engine=self.engine,
            )
        if "summarypages" in self.config.get("condor", "resultspage"):
            self.results_page_job = PESummaryResultsPageJob(
                self.config,
                os.path.join(self.basepath, "resultspage.sub"),
                self.logpath,
            )
        else:
            self.results_page_job = ResultsPageJob(
                self.config,
                os.path.join(self.basepath, "resultspage.sub"),
                self.logpath,
            )
            self.cotest_results_page_job = ResultsPageJob(
                self.config,
                os.path.join(self.basepath, "resultspagecoherent.sub"),
                self.logpath,
            )
        if self.config.has_section("spin_evol"):
            self.evolve_spins_job = EvolveSamplesJob(
                self.config,
                os.path.join(self.basepath, "evolve_spins.sub"),
                self.logpath,
            )
        if self.engine == "lalinferencemcmc":
            self.combine_job = CombineMCMCJob(
                self.config,
                os.path.join(self.basepath, "combine_files.sub"),
                self.logpath,
            )
            self.merge_job = MergeJob(
                self.config,
                os.path.join(self.basepath, "merge_runs.sub"),
                self.logpath,
                engine="mcmc",
            )
        else:
        ...
        self.coherence_test_job = CoherenceTestJob(
            self.config, os.path.join(self.basepath, "coherence_test.sub"), self.logpath
        )
        self.gracedbjob = GraceDBJob(
            self.config, os.path.join(self.basepath, "gracedb.sub"), self.logpath
        )
        self.mapjob = SkyMapJob(
            cp, os.path.join(self.basepath, "skymap.sub"), self.logpath
        )
        self.plotmapjob = PlotSkyMapJob(
            cp, os.path.join(self.basepath, "plotskymap.sub"), self.logpath
        )
        self.postruninfojob = PostRunInfoJob(
            self.config, os.path.join(self.basepath, "postrungdbinfo.sub"), self.logpath
        )
        ...
        if (
            not (
                self.config.has_option("input", "gps-start-time")
                and self.config.has_option("input", "gps-end-time")
            )
        ...
        if self.config.has_option("paths", "webdir"):
            ...
            with open(
                os.path.join(self.config.get("paths", "webdir"), "config.ini"), "w"
            ) as conffile:
                self.config.write(conffile)
        ...
        if self.config.has_option("analysis", "upload-to-gracedb"):
            ...
            if (
                self.config.has_option("condor", "gdbinfo")
                and self.config.has_option("analysis", "ugid")
                and self.config.getboolean("analysis", "upload-to-gracedb")
            ):
                ...
                self.add_gracedb_info_node(
                    None, event.GID, analysis="LIB", issky=True, server=gdb_srv
                )

        self.dagfilename = "lalinference_%s-%s" % (
            self.config.get("input", "gps-start-time"),
            self.config.get("input", "gps-end-time"),
        )
    ...
    def write_wrapper_script(self, path):
        ...
        os.chmod(
            path,
            stat.S_IXUSR | stat.S_IWUSR | stat.S_IRUSR | stat.S_IXOTH | stat.S_IROTH,
        )
    ...
    def get_required_data(self, times):
        ...
        if self.config.has_option("engine", "seglen") or self.config.has_option(
            "lalinference", "seglen"
        ):
            ...
            try:
                use_gracedbpsd = not self.config.getboolean(
                    "input", "ignore-gracedb-psd"
                )
            ...
            if (
                (
                    use_gracedbpsd
                    and os.path.isfile(os.path.join(self.basepath, "psd.xml.gz"))
                )
                ...
                if self.config.has_option("condor", "bayeswave"):
                    if np.log2(seglen) % 1:
                        seglen = np.power(2.0, np.ceil(np.log2(seglen)))
            else:
                psdlength = 32 * seglen
        else:
    ...
    def setup_from_inputs(self):
        ...
        ReadInputFromList = sum(
            [1 if self.config.has_option("input", name) else 0 for name in inputnames]
        )
        ...
        if (
            self.config.has_option("engine", "correlatedGaussianLikelihood")
            or self.config.has_option("engine", "bimodalGaussianLikelihood")
            or self.config.has_option("engine", "rosenbrockLikelihood")
        ):
            analytic_test = True
        else:
            analytic_test = False
        ...
        if (
            self.config.has_option("input", "analyse-all-time")
            and self.config.getboolean("input", "analyse-all-time") == True
        ):
            print(
                "Setting up for analysis of continuous time stretch %f - %f"
                % (gpsstart, gpsend)
            )
            if self.config.has_option("engine", "seglen"):
        ...
        if self.config.has_option("input", "burst-injection-file"):
            ...
            injTable = lsctables.SimBurstTable.get_table(
                ligolw_utils.load_filename(injfile)
            )
            events = [Event(SimBurst=inj) for inj in injTable]
        # LVAlert CoincInspiral Table
        gid = None
        if self.config.has_option("input", "gid") or self.config.has_option(
            "input", "coinc-xml"
        ):
            flow = 20.0
            if self.config.has_option("lalinference", "flow"):
                flow = min(
                    ast.literal_eval(self.config.get("lalinference", "flow")).values()
                )
            threshold_snr = None
            if not self.config.has_option(
                "engine", "distance-max"
            ) and self.config.has_option("input", "threshold-snr"):
            ...
            if self.config.has_option("input", "gid"):
                ...
                try:
                    ...
                    print(
                        "Failed to download %s psd.xml.gz. lalinference will estimate the psd itself."
                        % gid
                    )
            else:
            ...
            if psd_file_obj is not None:
                ...
                ifos = sorted(
                    lsctables.CoincInspiralTable.get_table(coinc_xml_obj)[0].instruments
                )
                get_xml_psds(
                    os.path.realpath(path_to_psd),
                    ifos,
        ...
        if dbname:
            ...
            if self.config.get("input", "timeslides").lower() == "true":
                events = get_timeslides_pipedown(
                    db_connection,
                    gpsstart=gpsstart,
                    gpsend=gpsend,
                    dumpfile=timeslidedump,
                    max_cfar=maxcfar,
                )
            else:
                events = queryfunc(
    ...
    def spin_evol_checks(self):
        if self.config.has_section("spin_evol"):
            ...
            tidal_run_tests = self.config.has_option(
                "engine", "tidal"
            ) or self.config.has_option("engine", "tidalT")

            nonprecessing_run_tests = self.config.has_option(
                "engine", "disable-spin"
            ) or self.config.has_option("engine", "aligned-spin")

            approx_num = SimInspiralGetApproximantFromString(
                extract_approx(self.config)
            )

            precessing_wf_test = (
                SimInspiralGetSpinSupportFromApproximant(approx_num) == 3
            )  # 3 corresponds to LAL_SIM_INSPIRAL_PRECESSINGSPIN
    ...
    def add_full_analysis_lalinferencenest(self, event):
        ...
        bwpostnodes = {}
        for i in range(Npar):
            n, bwpsdnodes, bwpostnodes = self.add_engine_node(
                event, bwpsd=bwpsdnodes, bwpost=bwpostnodes
            )
            if n is not None:
                if i > 0:
        ...
        mergenode.set_pos_output_file(
            os.path.join(
                self.posteriorpath, "posterior_%s_%s.hdf5" % (myifos, evstring)
            )
        )
        ...
        if self.spin_evol_checks():
            evolve_spins_node = EvolveSamplesNode(
                self.evolve_spins_job,
                posfile=mergenode.get_pos_file(),
                parent=mergenode,
            )
        ...
        if (
            self.config.getboolean("analysis", "coherence-test")
            and len(enginenodes[0].ifos) > 1
        ):
            if "summarypages" not in self.config.get("condor", "resultspage"):
                respagenode = self.add_results_page_node(
                    resjob=self.cotest_results_page_job,
                    outdir=pagedir,
                    parent=respage_parent,
                    ifos=enginenodes[0].ifos,
                )
                ...
                if os.path.exists(self.basepath + "/coinc.xml"):
                    try:
                        ...
                        gid = None
                    respagenode.set_coinc_file(
                        os.path.join(self.basepath, "coinc.xml"), gid
                    )

                par_mergenodes = []
                for ifo in enginenodes[0].ifos:
                    co_merge_job = MergeJob(
                        self.config,
                        os.path.join(self.basepath, "merge_runs_%s.sub" % (ifo)),
                        self.logpath,
                        engine="nest",
                    )
                    cotest_nodes = []
                    for i in range(Npar):
                        cot_node, bwpsdnodes, bwpostnodes = self.add_engine_node(
                            event,
                            bwpsd=bwpsdnodes,
                            bwpost=bwpostnodes,
                            ifos=[ifo],
                    ...
                    for co in cotest_nodes:
                        ...
                        if co != cotest_nodes[0]:
                            ...
                            co.set_psd_files()
                            co.set_snr_file()
                    pmergenode = MergeNode(
                        co_merge_job, parents=cotest_nodes, engine="nest"
                    )
                    pmergenode.set_pos_output_file(
                        os.path.join(
                            self.posteriorpath, "posterior_%s_%s.hdf5" % (ifo, evstring)
                        )
                    )
                    ...
                    subresnode = self.add_results_page_node(
                        outdir=presultsdir, parent=pmergenode, ifos=ifo
                    )
                    ...
                    if os.path.exists(self.basepath + "/coinc.xml"):
                        try:
                            ...
                            gid = None
                        subresnode.set_coinc_file(
                            os.path.join(self.basepath, "coinc.xml"), gid
                        )
                    if (
                        self.config.has_option("input", "injection-file")
                        and event.event_id is not None
                    ):
                        subresnode.set_injection(
                            self.config.get("input", "injection-file"), event.event_id
                        )
                    elif (
                        self.config.has_option("input", "burst-injection-file")
                        and event.event_id is not None
                    ):
                        subresnode.set_injection(
                            self.config.get("input", "burst-injection-file"),
                            event.event_id,
                        )
                coherence_node = CoherenceTestNode(
                    self.coherence_test_job,
                    outfile=os.path.join(
                        self.basepath,
                        "coherence_test",
                        "coherence_test_%s_%s.dat" % (myifos, evstring),
                    ),
                )
                ...
                respagenode = self.add_results_page_node_pesummary(
                    outdir=pagedir,
                    parent=respage_parent,
                    gzip_output=None,
                    ifos=enginenodes[0].ifos,
                ...
                respagenode.set_psd_files(
                    enginenodes[0].ifos, enginenodes[0].get_psd_files()
                )
                try:
                ...
                if cachefiles_option:
                    respagenode.set_cache_files(
                        enginenodes[0].channels, enginenodes[0].cachefiles
                    )
            ...
            if "summarypages" in self.config.get("condor", "resultspage"):
                ...
                if (
                    ...
                    respagenode.set_injection(
                        self.config.get("input", "burst-injection-file"), event.event_id
                    )
        ...
        if self.config.has_option("analysis", "upload-to-gracedb"):
            ...
            if (
                self.config.getboolean("analysis", "upload-to-gracedb")
                and event.GID is not None
            ):
                self.add_gracedb_start_node(
                    event.GID,
                    "LALInference",
                    [
                        sciseg.get_df_node()
                        for sciseg in enginenodes[0].scisegs.values()
                    ],
                    server=gdb_srv,
                )
                ...
                temp_node = self.add_gracedb_info_node(
                    respagenode, ugid, analysis="LIB", email=emailto, server=gdb_srv
                )
        if self.config.has_option(
            "condor", "ligo-skymap-plot"
        ) and self.config.has_option("condor", "ligo-skymap-from-samples"):
            ...
            plotmapnode = PlotSkyMapNode(
                self.plotmapjob,
                parent=mapnode,
                inputfits=mapnode.outfits,
                output=os.path.join(pagedir, "skymap.png"),
            )
    ...
    def add_full_analysis_lalinferencemcmc(self, event):
        ...
        for i in range(Npar):
            combinenodes.append(
                CombineMCMCNode(self.combine_job, parents=[enginenodes[i]])
            )
            input_file = combinenodes[i].get_parent_posfile(enginenodes[i])
            input_file_split_index = input_file.find("lalinferencemcmc-")
            combinenodes[i].set_pos_output_file(
                input_file[:input_file_split_index]
                + "combine_"
                + input_file[input_file_split_index:]
            )
        ...
        if self.config.has_option("resultspage", "deltaLogP"):
            mergenode.add_var_arg(
                "--deltaLogP " + str(self.config.getfloat("resultspage", "deltaLogP"))
            )
        if self.config.has_option("resultspage", "downsample"):
            mergenode.add_var_arg(
                "--downsample " + str(self.config.getint("resultspage", "downsample"))
            )
        if self.config.has_option("resultspage", "fixedBurnin"):
            mergenode.add_var_arg(
                "--fixedBurnin " + str(self.config.getint("resultspage", "fixedBurnin"))
            )
        self.add_node(mergenode)
        ...
        if "summarypages" in self.config.get("condor", "resultspage"):
            respagenode = self.add_results_page_node_pesummary(
                ...
                ifos=enginenodes[0].ifos,
                evstring=evstring,
                coherence=self.config.getboolean("analysis", "coherence-test"),
            )
            respagenode.set_psd_files(
                enginenodes[0].ifos, enginenodes[0].get_psd_files()
            )
    ...
    def add_science_segments(self):
        ...
        if self.config.has_option("input", "ignore-state-vector"):
            if self.config.getboolean("input", "ignore-state-vector"):
                ...
                for ifo in self.ifos:
                    ...
                    df_node = self.get_datafind_node(
                        ifo, self.frtypes[ifo], int(sciseg.start()), int(sciseg.end())
                    )
        ...
        for ifo in self.ifos:
            print(
                "Read state vector for {0} data between {1} and {2}".format(
                    ifo, start, end
                )
            )
            segs = findSegmentsToAnalyze(
                ...
                self.frtypes[ifo],
                ast.literal_eval(
                    self.config.get("statevector", "state-vector-channel")
                )[ifo],
                ast.literal_eval(self.config.get("statevector", "bits")),
                start,
                end,
            )
            segs.coalesce()
            for seg in segs:
                sciseg = pipeline.ScienceSegment(
                    (segs.index(seg), seg[0], seg[1], seg[1] - seg[0])
                )
                df_node = self.get_datafind_node(
                    ifo, self.frtypes[ifo], int(sciseg.start()), int(sciseg.end())
                )
    ...
    def add_engine_node(
        self, event, bwpsd={}, bwpost={}, ifos=None, co_test=False, extra_options=None
    ):
        """
        ...
        if use_lalinference_psd and self.config.has_option(
            "input", "minimum_realizations_number"
        ):
            psdstart = (
                segstart
                - self.config.getint("input", "padding")
        ...
        roqeventpath = os.path.join(
            self.preengine_job.roqpath, str(event.event_id) + "/"
        )
        if (
            self.config.has_option("condor", "bayesline")
            or self.config.has_option("condor", "bayeswave")
            or self.config.getboolean("analysis", "roq")
        ):
            mkdirs(roqeventpath)
        ...
        if np.log2(original_srate) % 1:
            print(
                "The srate given," + str(original_srate) + " Hz, is not a power of 2."
            )
            print(
                "For data handling purposes, the srate used will be "
                + str(np.power(2.0, np.ceil(np.log2(original_srate))))
                + " Hz"
            )
            print(
                "The inner products will still however be integrated up to "
                + str(original_srate / 2.0)
                + " Hz"
            )
            srate = np.power(2.0, np.ceil(np.log2(original_srate)))
        else:
        ...
        if original_srate != 0:
            for ifo in ifos:
                node.fhighs[ifo] = str(original_srate / 2.0 - 1.0 / seglen)
                prenode.fhighs[ifo] = str(original_srate / 2.0 - 1.0 / seglen)
                ...
                if segstart >= seg.start() and segend <= seg.end():
                    if not self.config.has_option("lalinference", "fake-cache"):
                        if self.config.has_option(
                            ...
                            prenode.add_ifo_data(
                                ifo, seg, self.channels[ifo], timeslide=slide
                            )
                        gotdata += node.add_ifo_data(
                            ifo, seg, self.channels[ifo], timeslide=slide
                        )
                    else:
                        fakecachefiles = ast.literal_eval(
                            self.config.get("lalinference", "fake-cache")
                        )
                        if self.config.has_option(
                            "condor", "bayesline"
                        ) or self.config.getboolean("analysis", "roq"):
                            prenode.add_fake_ifo_data(
                                ifo,
                                seg,
                                fakecachefiles[ifo],
                                self.channels[ifo],
                        ...
                        gotdata += node.add_fake_ifo_data(
                            ifo,
                            seg,
                            fakecachefiles[ifo],
                            self.channels[ifo],
        ...
        if self.config.has_option("lalinference", "psd-xmlfile"):
            ...
            node.psds = get_xml_psds(
                psdpath, ifos, os.path.join(self.basepath, "PSDs"), end_time=end_time
            )
            prenode.psds = get_xml_psds(
                psdpath, ifos, os.path.join(self.basepath, "PSDs"), end_time=end_time
            )
            if len(ifos) == 0:
        ...
        if use_gracedbpsd:
            if os.path.isfile(os.path.join(self.basepath, "psd.xml.gz")):
                ...
                node.psds = get_xml_psds(
                    psdpath, ifos, os.path.join(self.basepath, "PSDs"), end_time=None
                )
                prenode.psds = get_xml_psds(
                    psdpath, ifos, os.path.join(self.basepath, "PSDs"), end_time=None
                )
        ...
        prenode.add_file_opt(
            "outfile", roqeventpath + "data-dump", file_is_output_file=True
        )
        ...
        if self.config.has_option("condor", "bayeswave"):
            ...
            if np.log2(bw_seglen) % 1:
                ...
                print(
                    ...
                    + str(np.power(2.0, np.ceil(np.log2(bw_seglen))))
                ...
                bw_seglen = np.power(2.0, np.ceil(np.log2(bw_seglen)))
        if self.config.has_option("condor", "bayeswave"):
            if self.config.has_option("bayeswave", "bw_srate"):
                ...
                print(
                    "Instead, a srate of "
                    + str(np.power(2.0, np.ceil(np.log2(srate))))
                ...
                bw_srate = np.power(2.0, np.ceil(np.log2(srate)))
            else:
                bw_srate = srate
        ...
        for ifokey, seg in node.scisegs.items():
            ...
            if 1 == 1:
                if self.config.has_option("condor", "bayeswave") and not co_test:
                    for ifo in ifos:
                        bwPSDpath = os.path.join(
                            roqeventpath, "BayesWave_PSD_" + ifo + "/"
                        )
                        ...
                        if ifo not in bayeswavepsdnode:
                            ...
                            for seg in self.segments[ifo]:
                                if segstart >= seg.start() and segend <= seg.end():
                                    if not self.config.has_option(
                                        "lalinference", "fake-cache"
                                    ):
                                        bayeswavepsdnode[ifo].add_ifo_data(
                                            ifo,
                                            seg,
                                            self.channels[ifo],
                                            timeslide=slide,
                                        ...
                                        bayeswavepsdnode[ifo].add_fake_ifo_data(
                                            ifo,
                                            seg,
                                            fakecachefiles[ifo],
                                            self.channels[ifo],
                            ...
                            if self.config.has_option("lalinference", "flow"):
                                bayeswavepsdnode[ifo].flows[ifo] = np.power(
                                    2,
                                    np.floor(
                                        np.log2(
                                            ast.literal_eval(
                                                self.config.get("lalinference", "flow")
                                            )[ifo]
                                        )
                                    ),
                                )
                                print(
                                    "BayesWave requires f_low being a power of 2, therefore f_low for "
                                    + ifo
                            ...
                            bayeswavepsdnode[ifo].set_chainseed(
                                self.randomseed + event.event_id
                            )
                            if self.dataseed:
                                bayeswavepsdnode[ifo].set_dataseed(
                                    self.dataseed + event.event_id
                                )
                            else:
                                bayeswavepsdnode[ifo].set_dataseed(
                                    self.randomseed + event.event_id
                        ...
                        if ifo not in bayeswavepostnode:
                            bayeswavepostnode[ifo] = self.add_bayeswavepost_node(
                                ifo, parent=bayeswavepsdnode[ifo]
                            )
                            ...
                            bayeswavepost_fakecache = (
                                "interp:" + bwPSDpath + ifo + "_fairdraw_asd.dat"
                            )
                            ...
                            for seg in self.segments[ifo]:
                                if segstart >= seg.start() and segend <= seg.end():
                                    ...
                                    bayeswavepostnode[ifo].add_fake_ifo_data(
                                        ifo,
                                        seg,
                                        bayeswavepost_fakecache,
                                        self.channels[ifo],
                            ...
                            if self.config.has_option("lalinference", "flow"):
                                bayeswavepostnode[ifo].flows[ifo] = np.power(
                                    2,
                                    np.floor(
                                        np.log2(
                                            ast.literal_eval(
                            ...
                            bayeswavepostnode[ifo].set_chainseed(
                                self.randomseed + event.event_id
                            )
                            if self.dataseed:
                                bayeswavepostnode[ifo].set_dataseed(
                                    self.dataseed + event.event_id
                                )
                            else:
                                bayeswavepostnode[ifo].set_dataseed(
                ...
                if self.config.has_option(
                    ...
                    if gotdata and event.event_id not in self.prenodes.keys():
                        if prenode not in self.get_nodes():
                            ...
                            for ifo in ifos:
                                if self.config.getboolean("analysis", "roq"):
                                    computeroqweightsnode[ifo] = (
                                        self.add_rom_weights_node(ifo, prenode)
                                    )
                                # self.add_node(computeroqweightsnode[ifo])
                                if self.config.has_option("input", "injection-file"):
                                    freqDataFile = os.path.join(
                                        roqeventpath,
                                        "data-dump"
                                        + ifo
                                    ...
                                    freqDataFile = os.path.join(
                                        roqeventpath,
                                        "data-dump" + ifo + "-freqData.dat",
                                    )
                                prenode.add_output_file(freqDataFile)
                                prenode.add_output_file(
                                    os.path.join(
                                        roqeventpath, "data-dump" + ifo + "-PSD.dat"
                                    )
                                )
                                if self.config.has_option("condor", "bayesline"):
                                    bayeslinenode[ifo] = self.add_bayesline_node(
                                        ifo, prenode
                                    )
                                    bayeslinenode[ifo].add_var_arg("-i " + freqDataFile)
                                    bayeslinenode[ifo].add_input_file(freqDataFile)
                                    bayeslinenode[ifo].add_var_arg(
                                        "-o "
                                        + os.path.join(
                                            roqeventpath,
                                            "BayesLine_PSD_" + ifo + ".dat",
                                        )
                                    )
                                    bayeslinenode[ifo].add_output_file(
                                        os.path.join(
                                            roqeventpath,
                                            "BayesLine_PSD_" + ifo + ".dat",
                                        )
                                ...
                                if self.config.getboolean("analysis", "roq"):
                                    computeroqweightsnode[ifo].add_var_arg(
                                        "--fHigh " + str(prenode.fhighs[ifo])
                                    )
                                    computeroqweightsnode[ifo].add_file_opt(
                                        "data", freqDataFile
                                    )
                                    computeroqweightsnode[ifo].add_file_opt(
                                        "psd",
                                    ...
                                    computeroqweightsnode[ifo].add_file_opt(
                                        "out", roqeventpath, file_is_output_file=True
                                    )
                                    computeroqweightsnode[ifo].add_output_file(
                                        os.path.join(
                                            roqeventpath,
                                            "weights_quadratic_" + ifo + ".dat",
                                        )
                                    )
                            ...
                            if self.config.getboolean("analysis", "roq"):
                                self.prenodes[event.event_id] = (
                                    prenode,
                                    computeroqweightsnode,
                                )

            if self.config.has_option("condor", "bayesline") or self.config.getboolean(
                "analysis", "roq"
            ):
        ...
        if self.config.has_option("input", "injection-file"):
            ...
            prenode.set_injection(
                self.config.get("input", "injection-file"), event.event_id
            )
            if (
                self.config.has_option("condor", "bayeswave")
                and bayeswavepsdnode
                and bayeswavepostnode
            ):
                for ifo in ifos:
                    bayeswavepsdnode[ifo].set_injection(
                        self.config.get("input", "injection-file"), event.event_id
                    )
                    bayeswavepostnode[ifo].set_injection(
                        self.config.get("input", "injection-file"), event.event_id
                    )
        if self.config.has_option("input", "burst-injection-file"):
            node.set_injection(
        ...
        if self.config.has_option("input", "psd-length"):
            ...
            if (
                ...
                for ifo in ifos:
                    bayeswavepsdnode[ifo].set_psdlength(
                        self.config.getint("input", "psd-length")
                    )
                    bayeswavepostnode[ifo].set_psdlength(
                        self.config.getint("input", "psd-length")
                    )
        if self.config.has_option("input", "psd-start-time"):
            ...
            if (
                ...
                for ifo in ifos:
                    bayeswavepsdnode[ifo].set_psdstart(
                        self.config.getfloat("input", "psd-start-time")
                    )
                    bayeswavepostnode[ifo].set_psdstart(
                        self.config.getfloat("input", "psd-start-time")
                    )
        ...
        if (
            ...
            for ifo in ifos:
                bayeswavepsdnode[ifo].set_max_psdlength(
                    self.config.getint("input", "max-psd-length")
                )
                bayeswavepsdnode[ifo].set_padding(
                    self.config.getint("input", "padding")
                )
                bayeswavepostnode[ifo].set_max_psdlength(
                    self.config.getint("input", "max-psd-length")
                )
                bayeswavepostnode[ifo].set_padding(
                    self.config.getint("input", "padding")
                )
        out_dir = os.path.join(self.basepath, "engine")
        mkdirs(out_dir)
        node.set_output_file(
            os.path.join(
                out_dir,
                node.engine
                + "-"
        ...
        if self.config.getboolean("analysis", "roq"):
            for ifo in ifos:
                node.add_file_opt(
                    ifo + "-roqweightsLinear",
                    os.path.join(roqeventpath, "weights_linear_" + ifo + ".dat"),
                )
                node.add_file_opt(
                    ifo + "-roqweightsQuadratic",
                    os.path.join(roqeventpath, "weights_quadratic_" + ifo + ".dat"),
                )
            node.add_file_opt(
                "roqtime_steps", os.path.join(roqeventpath, "roq_sizes.dat")
            )
            node.add_file_opt("roq-times", os.path.join(roqeventpath, "tcs.dat"))
            node.add_file_opt(
                "roqnodesLinear", os.path.join(roqeventpath, "fnodes_linear.dat")
            )
            node.add_file_opt(
                "roqnodesQuadratic", os.path.join(roqeventpath, "fnodes_quadratic.dat")
            )
        if self.config.has_option("condor", "bayesline"):
            for ifo in ifos:
                node.psds[ifo] = os.path.join(
                    roqeventpath, "BayesLine_PSD_" + ifo + ".dat"
                )
                node.add_input_file(
                    os.path.join(roqeventpath, "BayesLine_PSD_" + ifo + ".dat")
                )
                prenode.psds[ifo] = os.path.join(
                    roqeventpath, "BayesLine_PSD_" + ifo + ".dat"
                )
                prenode.add_input_file(
                    os.path.join(roqeventpath, "BayesLine_PSD_" + ifo + ".dat")
                )
        if self.config.has_option("condor", "bayeswave") and bayeswavepostnode:
            for ifo in ifos:
                node.psds[ifo] = os.path.join(
                    ...
                    "BayesWave_PSD_"
                    + ifo
                    + "/post/clean/glitch_median_PSD_forLI_"
                    + ifo
                    + ".dat",
                )
                node.add_input_file(
        ...
        if self.config.has_option("condor", "bayeswave") and self.engine not in [
            "bayeswave",
            "bayeswavepost",
        ]:
            for ifo in ifos:
    ...
    def add_results_page_node_pesummary(
        self,
        resjob=None,
        outdir=None,
        parent=None,
        ...
        if parent is not None:
            ...
            for ifo in ifos:
                try:
                    calibration_envelope = self.config.get(
                        "engine", "%s-spcal-envelope" % (ifo)
                    )
    ...
    def add_gracedb_log_node(self, respagenode, gid, burst=False, server=None):
        ...
        resurl = respagenode.webpath.replace(
            self.gracedbjob.basepath, self.gracedbjob.baseurl
        )
        # node.set_message('online parameter estimation results:  '+resurl+'/posplots.html')
        node.set_message(
            "LALInference online parameter estimation finished. <a href="
            + resurl
        ...
        if burst is False:
            node = GraceDBNode(
                self.gracedbjob, parent=respagenode, gid=gid, command="upload", tag=tag
            )
            ...
            node = GraceDBNode(
                self.gracedbjob, parent=respagenode, gid=gid, command="upload", tag="pe"
            )
            ...
            if self.config.has_option("analysis", "upload-to-gracedb"):
                if self.config.getboolean("analysis", "upload-to-gracedb"):
                    ...
                    for sk in skynodes:
                        if len(sk.ifos) > 1:
                            node = GraceDBNode(
                                self.gracedbjob, parent=sk, gid=gid, tag=tag
                            )
    ...
    def add_gracedb_info_node(
        self,
        respagenode,
        gid,
        analysis="LALInference",
        issky=False,
        prefix="LIB",
        email=None,
        server=None,
        ...
        if respagenode is not None:
            ...
            hdf5samples = respagenode.get_in_files()[
                0
            ]  # This should only  be called by LIB, which only uses one file
        else:
            # Try to find it
            resnodes = filter(
        ...
        if issky is False:
            ...
            for sk in skynodes:
                ...
                node = PostRunInfoNode(
                    self.postruninfojob, parent=sk, gid=gid, samples=None, server=server
                )
    ...
    def add_rom_weights_node(self, ifo, parent=None):
        ...
        # except KeyError:
        node = ROMNode(
            self.computeroqweights_job, ifo, parent.seglen, parent.flows[ifo]
        )
...
class LALInferenceDAGJob(pipeline.CondorDAGJob):
    ...
    def write_sub_file(self):
        ...
        if self.requires_frames and self.osg:
            ...
            self.set_x509path(
                os.path.join(self.get_config("paths", "basedir"), "x509proxy")
            )
        ...
        if self.requirements:
            self.add_condor_cmd(
                "requirements", "&&".join("({0})".format(r) for r in self.requirements)
            )
...
class LALInferenceDAGNode(pipeline.CondorDAGNode):
    ...
    def add_output_file(self, filename):
        ...
        if self.job().transfer_files:
            fname = os.path.basename(
                os.path.relpath(fname, start=self.job().get_config("paths", "basedir"))
            )
    ...
    def add_input_file(self, filename):
        filename = os.path.relpath(
            filename, start=self.job().get_config("paths", "basedir")
        )
    ...
    def finalize(self):
        # Set up file mapping
        outputremaps = ";".join(
            ["{0}={1}".format(os.path.basename(f), f) for f in self.get_output_files()]
        )
        if self.job().transfer_files:
            self.add_macro("macrooutputremaps", outputremaps)
            self.add_input_file(
                os.path.join(
                    self.job().get_config("paths", "basedir"), "lalinf_touch_output"
                )
            )
        ...
        **kwargs,
        ...
        if ispreengine is True:
            ...
            if self.engine == "lalinferencemcmc":
                ...
                self.engine == "lalinferencenest" or self.engine == "lalinferenceburst"
            ):
                exe = cp.get("condor", self.engine)
            else:
        ...
        LALInferenceDAGJob.__init__(
            self, cp, sharedfs=sharedfs, requires_frames=requires_frames
        )
        ...
        if self.engine == "lalinferencemcmc":
            ...
            if cp.has_section("mpi"):
                ...
                self.machine_count = str(1)
                self.machine_memory = str(
                    1024
                )  # default value if the user did not specify something
                self.mpi_task_count = self.machine_count
            ...
            self.add_condor_cmd(
                "environment",
                "OMP_NUM_THREADS="
                + str(int(ceil(int(self.machine_count) / int(self.mpi_task_count)))),
            )
            self.add_condor_cmd(
                "request_memory",
                str(float(self.machine_count) * float(self.machine_memory)),
            )
        ...
        self.set_stdout_file(
            os.path.join(logdir, "lalinference-$(cluster)-$(process)-$(node).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "lalinference-$(cluster)-$(process)-$(node).err")
        )
...
class EngineNode(LALInferenceDAGNode):
    ...
    def get_trig_time(self):
        return self.__trigtime

    def add_fake_ifo_data(
        self, ifo, sciseg, fake_cache_name, fake_channel_name, timeslide=0
    ):
        """
    ...
    def _finalize_ifo_data(self):
        ...
        offset = maxLength + self.seglen - 2.0 + self.padding
        ...
        while (
            (self.GPSstart + length >= trig_time)
            or any(
                [
                    self.GPSstart < seg.start() + self.padding
                    for seg in self.scisegs.values()
                ]
            )
        ) and length > 0:
            self.GPSstart += dt
        ...
        while (
            any(
                [
                    self.GPSstart + length >= seg.end() - self.padding
                    for seg in self.scisegs.values()
                ]
            )
            and length > 0
        ...
        if length < maxLength:
            print(
                "Trimmed analysis data for trigger {}. Analysing {} -- {} ({}s of data)".format(
                    trig_time, self.GPSstart, self.GPSstart + length, length
                )
            )
...
class BayesWavePSDJob(
    LALInferenceDAGSharedFSJob, pipeline.CondorDAGJob, pipeline.AnalysisJob
):
    """
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.set_stdout_file(
            os.path.join(logdir, "bayeswavepsd-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "bayeswavepsd-$(cluster)-$(process).err")
        )
        # Bayeswave actually runs on node filesystem via these commands
        self.add_condor_cmd("transfer_executable", "False")
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd(
            "request_memory", cp.get("condor", "bayeswave_request_memory")
        )
        ...
        self.add_condor_cmd("when_to_transfer_output", "ON_EXIT_OR_EVICT")
        self.add_condor_cmd(
            "transfer_input_files", "$(macroinput),/usr/bin/mkdir,caches"
        )
        self.add_condor_cmd("transfer_output_files", "$(macrooutput)")
...
class BayesWavePSDNode(EngineNode):
    ...
    def set_output_dir(self, dirname):
        path = os.path.relpath(dirname, start=self.job().get_config("paths", "basedir"))
...
class BayesWavePostJob(
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.add_condor_cmd("transfer_executable", "False")
        self.set_stdout_file(
            os.path.join(logdir, "bayeswavepost-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "bayeswavepost-$(cluster)-$(process).err")
        )
        self.add_condor_cmd("getenv", "True")
        self.add_condor_cmd(
            "request_memory", cp.get("condor", "bayeswavepost_request_memory")
        )
        self.add_condor_cmd("stream_output", True)
        self.add_condor_cmd("stream_error", True)
        self.add_condor_cmd("+SuccessCheckpointExitCode", 77)
...
class PESummaryResultsPageJob(LALInferenceDAGSharedFSJob, pipeline.AnalysisJob):
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.set_sub_file(os.path.abspath(submitFile))
        self.set_stdout_file(
            os.path.join(logdir, "resultspage-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "resultspage-$(cluster)-$(process).err")
        )
        self.add_condor_cmd("getenv", "True")
        try:
            self.add_condor_cmd(
                "request_memory", cp.get("condor", "resultspage_memory")
            )
...
class CoherenceTestJob(LALInferenceDAGJob, pipeline.CondorDAGJob, pipeline.AnalysisJob):
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.add_condor_cmd("getenv", "True")
        self.set_stdout_file(
            os.path.join(logdir, "coherencetest-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "coherencetest-$(cluster)-$(process).err")
        )
        self.set_sub_file(os.path.abspath(submitFile))
...
class ROMJob(LALInferenceDAGSharedFSJob, pipeline.CondorDAGJob, pipeline.AnalysisJob):
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.set_sub_file(submitFile)
        self.set_stdout_file(
            os.path.join(logdir, "computeroqweights-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "computeroqweights-$(cluster)-$(process).err")
        )
        self.add_condor_cmd("getenv", "True")
        ...
        if cp.has_option("condor", "computeroqweights_memory"):
            ...
            if os.path.exists(os.path.join(roq_dir, "B_linear.npy")) and os.path.exists(
                os.path.join(roq_dir, "B_quadratic.npy")
            ):
                linear_basis_size = os.path.getsize(
                    os.path.join(roq_dir, "B_linear.npy")
                )
                quadratic_basis_size = os.path.getsize(
                    os.path.join(roq_dir, "B_quadratic.npy")
                )
            elif os.path.exists(
                os.path.join(roq_dir, "selected_params_linear.npy")
            ) and os.path.exists(
                os.path.join(roq_dir, "selected_params_quadratic.npy")
            ):
                linear_basis_size = (
                    32
                    * (fhigh - flow)
                ...
                quadratic_basis_size = (
                    ...
                    * seglen
                    * len(
                        np.load(os.path.join(roq_dir, "selected_params_quadratic.npy"))
                    )
                )
            roq_weights_size = (
                3
                * ((fhigh - flow) * seglen)
                * (float(dt + 0.05) * 2 / float(time_step))
                * 2
                * 8
            )
...
class BayesLineJob(
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.set_sub_file(submitFile)
        self.set_stdout_file(
            os.path.join(logdir, "bayesline-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "bayesline-$(cluster)-$(process).err")
        )
        self.add_condor_cmd("getenv", "True")
...
class SkyMapJob(
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.set_sub_file(submitFile)
        self.set_stdout_file(
            os.path.join(logdir, "samples2map-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "samples2map-$(cluster)-$(process).err")
        )
        ...
        if cp.has_option("engine", "margdist") or cp.has_option(
            "engine", "margdist-comoving"
        ):
...
class PlotSkyMapJob(
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.set_sub_file(submitFile)
        self.set_stdout_file(
            os.path.join(logdir, "plotskymap-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "plotskymap-$(cluster)-$(process).err")
        )
        # The user environment PYTHONPATH may be set to python2.7 version of lalsuite, so this is disabled
        # self.add_condor_cmd('getenv','True')
        # Add user-specified options from ini file
...
class PostRunInfoNode(LALInferenceDAGNode):
    def __init__(
        self, post_run_info_job, gid=None, parent=None, samples=None, server=None
    ):
...
class EvolveSamplesJob(pipeline.CondorDAGJob, pipeline.AnalysisJob):
    ...
    def __init__(self, cp, submitFile, logdir):
        ...
        self.set_sub_file(submitFile)
        self.set_stdout_file(
            os.path.join(logdir, "evolve_spins-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "evolve_spins-$(cluster)-$(process).err")
        )
        self.add_condor_cmd("getenv", "True")
