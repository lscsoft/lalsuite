...
clusters = {
    "atlas": "titan2.atlas.aei.uni-hannover.de",
...
def main():
    ...
    if py_version < (2, 7):
        ...
        parser.add_option(
            ...
            dest="preprocessfile",
            type=str,
            help="preprocess data before sending to main script (NB: NOT WORKING!)",
        )
    ...
    if (
        (args.configfile is None)
        and (args.examplecfg is None)
        and (args.preprocessfile is None)
    ):
...
def TigerPostProcess(configfile):
    ...
    if len(tigerruns) > 1:
        ...
        html_data_ks = [
            [TigerKSandPvalue(tigerruns[0], j, N=ns) for ns in N_sources]
            for j in tigerruns[1:]
            if len(tigerruns) > 1
        ]
    # for ns in N_sources:
    ...
    if plot_hist == True:
        ...
        for ns in N_sources:
            ...
            for typ in types:
                html_files_hist.append(
                    path.join(
                        localdest, "hist_" + runid + "_" + str(ns) + "sources." + typ
                    )
                )
    ...
    if plot_snrVSodds == True:
        ...
        for ext in types:
            html_files_snrvsodds.append(
                path.join(localdest, "snrVSodds_" + runid + "." + ext)
            )
    ...
    if plot_cumfreq == True:
        ...
        for i in range(len(tigerruns)):
            ...
            for ext in types:
                html_files_cumfreq[i].append(
                    path.join(localdest, "cumfreq_" + tigerruns[i].label + "." + ext)
                )
        ...
        for i in range(len(tigerruns)):
            ...
            for j in range(len(N_sources_cats)):
                ...
                for k in range(tigerruns[i].nsources / N_sources_cats[j]):
                    ...
                    for ext in types:
                        html_files_cumbayes[i][j].append(
                            path.join(
                                localdest,
                                "cumbayes_"
                                + tigerruns[i].label
    ...
    for i in range(len(tigerruns)):
        ...
        for j in range(len(html_files_cumbayes[i])):
            indhtmltxt += (
                "<h2>" + str(N_sources_cats[j]) + " sources per catalog" + "</h2>"
            )
...
class TigerRun:
    ...
    def searchsources(self):
        ...
        if self.engine == "lalnest":
            self.detectors = array(
                [k.split("_")[2].split(".")[0] for k in self.bayesfiles]
            )
        elif self.engine == "lalinference":
        ...
        # GET POSTERIOR FILES
        if self.engine == "lalnest":
            self.posteriorfiles = array(
                [k.replace("_B.txt", "") for k in self.bayesfiles]
            )
        elif self.engine == "lalinference":
            self.posteriorfiles = array(
                [k.replace("_B.txt", "") for k in self.bayesfiles]
        ...
        self.snrfiles = array(
            [
                k.replace("_", "-")
                .replace("-B.txt", "_snr.txt")
                .replace(
                    "posterior", "lalinferencenest-%s" % (k.split("-")[1].split(".")[0])
                )
                .replace(
                    "%s" % (k.split("-")[0].split("_")[-1]),
                    "%.1f" % (float(k.split("-")[0].split("_")[-1])),
                )
                for k in self.bayesfiles
            ]
        )
        ...
        stdout.write(
            "... searching sources: %s\t%s - %d sources\n"
    ...
    def pullbayes(self):
        ...
        """
        stdout.write(
            "... pulling Bayes factors: %s\t%s\r" % (self.cluster, self.directory)
        )
        stdout.flush()
        if self.nsources == 0:
            stdout.write(
            ...
            for fsplit in array_split(files.split(), nsplit):
                command = (
                    "%s %s '%s %s'"
                    % ("gsissh -C", clusters[self.cluster], "cat", " ".join(fsplit))
                    if self.cluster in clusters.keys()
                    else "%s %s" % ("cat", " ".join(fsplit))
                )
                p = Popen(command, stdout=PIPE, stderr=PIPE, shell=True)
        ...
        stdout.write(
            "... pulling Bayes factors: %s\t%s - %d x %d dimension\n"
        ...
        if self.nsources == 0:
            ...
            for k in range(len(self.gps)):
                ...
                if ndet == 1:
                    self.snr.append(
                        float(
                            snrrawdata[count + ndet - 1]
                            .strip("%s:" % (self.detectors[k]))
                            .strip()
                        )
                    )
                    count += 1
                    ...
                    self.snr.append(
                        float(snrrawdata[count + ndet].strip("Network:").strip())
                    )
                    count += ndet + 1
...
class TigerSet:
    ...
    def __init__(self, locations, label, latexlabel, testcoef, engine):
        ...
        if engine in [
            "lalinference",
            "lalnest",
        ]:  # engine: either lalnest or lalinference
            self.engine = engine
        else:
        ...
        # CREATE LOCATIONS
        self.locations = [
            TigerRun(loc[0].lower(), loc[1], self.engine, self.subhyp)
            for loc in locations
            if loc[0].lower() in clusters.keys() + ["local"]
        ]
    ...
    def odds(self, N=1):
        ...
        for i in range(self.nsources / N):
            odds = append(
                odds,
                OddsFromBayes(self.bayes[i * N : (i + 1) * N, :], len(self.testcoef)),
            )
        return odds
    ...
    def savetoascii(self, filename):
        ...
        savedata = empty(
            (0, 4 + len(self.subhyp))
        )  # cluster, directory, gps, bayes, ...., netsnr
        for loc in self.locations:
            if loc.nsources > 0:
                locsdata = column_stack(
                    ([loc.cluster] * loc.nsources, [loc.directory] * loc.nsources)
                )
                savedatatmp = column_stack((locsdata, loc.gps, loc.bayes, loc.snr))
                savedata = vstack((savedata, savedatatmp))
        header = array(
            concatenate((["#cluster", "folder", "gps"], self.subhyp, ["netsnr"]))
        )
...
def TigerCreateHistogram(list_tigerruns, axis, N=1, xlim=None, bins=50):
    ...
    for i in range(len(list_tigerruns)):
        if N == 1:
            # PLOT HISTOGRAMS
            axis.hist(
                list_tigerruns[i].odds(N),
                facecolor="none",
                label=list_tigerruns[i].latexlabel
                + r"\n$\mathrm{("
                + str(size(list_tigerruns[i].odds(N)))
                + r"\; sources)}$",
                histtype="stepfilled",
                linewidth=1,
                range=xlim,
...
def TigerCreateSNRvsOdds(list_tigerruns, axis):
    ...
    for i in range(len(list_tigerruns)):
        axis.scatter(
            list_tigerruns[i].snr,
            list_tigerruns[i].odds(),
            color=colors[i],
            marker=markers[i],
            label=list_tigerruns[i].latexlabel
            + r"\n$\mathrm{("
...
def TigerCreateCumFreq(tigerrun, axis):
    ...
    """
    xlim = [5.0, 100.0]
    snrrange = arange(xlim[0], xlim[1], 5.0)
    ...
    if sum(freqs[-1, :]) != tigerrun.nsources:
        print(
            r"Warning, some sources maybe out of the SNR range and are not plotted: SNR \in [%.1f:%.1f], %i sources missing"
            % (xlim[0], xlim[1], tigerrun.nsources - sum(freqs[-1, :]))
        )
    ...
    for i in range(tigerrun.nsubhyp):
        if tigerrun.subhyp[i].split("_")[-1] == "GR":
            ...
            if tigerrun.engine == "lalnest":
                curlabel = (
                    "$H_{"
                    + "".join(tigerrun.subhyp[i].split("_")[-1].split("dphi"))
                    + "}$"
                )
            elif tigerrun.engine == "lalinference":
                curlabel = (
                    "$H_{"
                    + "".join(tigerrun.subhyp[i].split("_")[-1].split("dchi"))
                    + "}$"
                )
            axis.plot(
                snrrange,
...
def TigerCreateCumBayes(tigerrun, axis, nthcat, nsourcespercat):
    ...
    bayescat = tigerrun.bayes[
        nthcat * nsourcespercat : (nthcat + 1) * nsourcespercat, :
    ]
    ...
    cumodds = array(
        [
            ...
            for i in range(nsourcespercat)
        ]
    )
    cumbayes = array(
        [
            sum(
                subtract(bayescatsorted[: i + 1, 1:].T, bayescatsorted[: i + 1, 0]).T,
                axis=0,
            )
            for i in range(nsourcespercat)
        ]
    ...
    for i in range(tigerrun.nsubhyp - 1):
        ...
        if tigerrun.engine == "lalnest":
            curlabel = (
                "$H_{"
                + "".join(tigerrun.subhyp[i + 1].split("_")[-1].split("dphi"))
                + "}$"
            )
        elif tigerrun.engine == "lalinference":
            curlabel = (
                "$H_{"
                + "".join(tigerrun.subhyp[i + 1].split("_")[-1].split("dchi"))
                + "}$"
            )
