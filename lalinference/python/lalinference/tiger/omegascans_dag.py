...
DefaultFrameTypes = {
    "L1": "L1_RDS_R_L1",  # Includes auxiliary channels
    "H1": "H1_RDS_R_L1",  # Includes auxiliary channels
    "V1": "T1300121_V1_EARLY_RECOLORED_V2",
}

DefaultConfigFiles = {
    "L1": "/archive/home/omega/configurations/S6/S6b/L0L1-RDS_R_L1-selected.txt",
...
ExampleSourceFile = """trigtime H1 L1 V1 logodds
...
def get_framedir(trigtime, timeslide, ifo, frametype):
    ...
    command = (
        "/usr/bin/gw_data_find --observatory=%s --url-type=file --type=%s --gps-start-time=%.3f --gps-end-time=%.3f"
        % (ifo[0], frametype, scantime, scantime)
    )
...
class OmegaScansDAG(pipeline.CondorDAG):
    def __init__(self, cp):
        ...
        self.daglogfile = os.path.join(
            self.basepath, "omegascans-" + str(uuid.uuid1()) + ".log"
        )
        ...
        if cp.has_option("analysis", "configfiles"):
            ...
            if cp.has_option("analysis", "basicscan"):
                ...
                for ifo in self.ifos:
                    self.configfiles[ifo] = os.path.join(
                        self.basepath, "basic_config_%s.txt" % ifo
                    )
                    with open(
                        os.path.join(self.basepath, self.configfiles[ifo]), "w"
                    ) as f:
                        f.write(ConfigsRecolored[ifo])
            else:
                if "V1" in self.ifos:
                    # For V1 there is no other file available on non-virgo clusters
                    DefaultConfigFiles["V1"] = os.path.join(
                        self.basepath, "basic_config_V1.txt"
                    )
                    with open(
                        os.path.join(self.basepath, DefaultConfigFiles["V1"]), "w"
                    ) as f:
        ...
        for n in range(Nsources):
            ...
            if self.oddslimit_set:
                if (
                    info["logodds"][n] > self.oddslimit["low"]
                    and info["logodds"][n] < self.oddslimit["high"]
                ):
                    for ifo in self.ifos:
                        node = OmegaScanNode(
                            info["trigtime"][n],
                    ...
                    self.add_table_entry(
                        info["trigtime"][n], timeslides, self.ifos, info["logodds"][n]
                    )
            else:
                for ifo in self.ifos:
                    node = OmegaScanNode(
    ...
    def write_table(self):
        ...
        for e in entries_to_write:
            ...
            for ifo in self.ifos:
                ...
                if webdir:
                    link = os.path.join(
                        webdir, "scan_%d" % e["trigtime"], "%s_%.2f" % (ifo, time)
                    )
    ...
    def ReadInfoFromFile(self, ifos):
        ...
        with open(self.sourcefile, "r") as f:
            ...
            for ifo in ifos:
                if not headerdict.has_key(
                    "timeslide_" + ifo
                ) and not headerdict.has_key(ifo):
                    sys.exit(
                        'ERROR: Not all ifos from config file are present in "'
                        + self.sourcefile
                        + '"'
                    )
            ...
            if oddslimit_set:
                ...
                if not headerdict.has_key("logodds") and not headerdict.has_key("logO"):
                    sys.exit(
                        'ERROR: An odds limit was set, but "'
                        + self.sourcefile
...
class OmegaScanJob(pipeline.CondorDAGJob):
    def __init__(self, cp, submitFile, logdir):
        ...
        pipeline.CondorDAGJob.__init__(
            self, "vanilla", cp.get("omegapipe", "executable")
        )
        ...
        self.set_stdout_file(
            os.path.join(logdir, "omegascans-$(cluster)-$(process)-$(node).out")
        )
        self.set_stderr_file(
            os.path.join(logdir, "omegascans-$(cluster)-$(process)-$(node).err")
        )
...
class OmegaScanNode(pipeline.CondorDAGNode):
    ...
    def __init__(
        self, trigtime, timeslide, ifo, frametype, configfile, outdir, job, logodds=None
    ):
        ...
        lockfile = os.path.join(
            outdir, "scan_%.0f" % trigtime, "%s_%.2f" % (ifo, scantime), "lock.txt"
        )
        ...
        self.add_var_opt(
            "outdir",
            os.path.join(outdir, "scan_%.0f" % trigtime, "%s_%.2f" % (ifo, scantime)),
        )
