...
class IFO:
        ...
        for seg in self._unvetoed._seglist:
            ...
            if whereInj == "often":
                ...
                while t + self._minlen <= seg[2]:
                    if (t + self._minlen // 2 >= seg[1] + lmargin) and (
                        t + self._minlen // 2 + rmargin <= seg[2]
                    ):
    ...
    def plotCumulativeDurations(self, outfile, maxdur=None):
        ...
        ax.hist(
            ...
            histtype="stepfilled",
            alpha=0.3,
            label="All segments (" + str(self._segments.length()) + ")",
        )
        ax.hist(
            udur,
            bins=sort(udur),
            ...
            histtype="stepfilled",
            alpha=0.3,
            label="Unvetoed segments (" + str(self._unvetoed.length()) + ")",
        )
        ax.legend(loc=4)
        fig.savefig(outfile)

    def plotSegments(
        self, outfile=None, lenperline=200000, segcolorlist=["b", "r", "g"], title=None
    ):
        ...
        for k, segs in zip([0, 1, 2], [self._segments, self._vetoes, self._unvetoed]):
            ...
            for i in arange(n):
                ...
                ax.tick_params(
                    axis="x", which="both", top="on", bottom="off", labelbottom="off"
                )
                ax.set_ylim(0, 2)
...
class segmentData:
    ...
    def unionSegments(self, other):
        ...
        united = (
            self.notSegments(start, end)
            .intersectSegments(other.notSegments(start, end))
            .notSegments(start, end)
        )
        return united
...
def getDoubles(ifo1, ifo2, unvetoed=False):
    ...
    if unvetoed:
        seg12 = unv1.intersectSegments(
            unv2
        )  # only uses unvetoed long segment list for each IFO
    else:
        seg12 = seg1.intersectSegments(
            seg2
        )  # uses full segment lists including small&vetoed segments
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Reads segment files and outputs injection times in veto-free segments."
    )
    ...
    parser.add_argument(
        ...
        metavar="IFO",
        dest="ifos",
        help="List of IFOs to be considered (currently takes 3)",
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        dest="segf",
        help="path to the segment files for IFOS in the order entered above",
    )
    parser.add_argument(
        "-v",
        "--vetofiles",
        ...
        metavar="FILE",
        dest="vetof",
        help="path to the veto files for IFOS in the order entered above",
    )
    parser.add_argument(
        "-o",
        "--outfolder",
    ...
    parser.add_argument(
        ...
        metavar="INT",
        dest="length",
        help="length of signal segments in seconds",
        default=45.0,
    )
    parser.add_argument(
        "-P",
        ...
        metavar="INT",
        dest="psdlength",
        help="minimum length for calculating PSD in seconds",
        default=1024,
    )
    parser.add_argument(
        "-N", "--Ninj", type=int, dest="Ninj", help="# injections", default=1000
    )
    parser.add_argument(
        "-I",
        "--interval",
        ...
        metavar="INT",
        dest="interval",
        help="minimum space between injections in seconds (defaults to injecting only in the middle of segments)",
        default=None,
    )
    parser.add_argument(
        "-S",
        ...
        action="store_true",
        dest="singles",
        help="process singles (default for timeslides)",
        default=False,
    )
    parser.add_argument(
        "-D",
        ...
        action="store_true",
        dest="doubles",
        help="process doubles (N/A for timeslides)",
        default=False,
    )
    parser.add_argument(
        "-T",
        ...
        action="store_true",
        dest="triples",
        help="process triples (N/A for timeslides)",
        default=False,
    )
    parser.add_argument(
        "-t",
        ...
        action="store_true",
        dest="timeslides",
        help="Enable timeslides. This automatically deactivates --triples and --doubles.",
        default=False,
    )
    parser.add_argument(
        "-p",
        ...
        action="store_true",
        dest="plotsegdist",
        help="plot cumulative segment length distribution",
        default=False,
    )
    parser.add_argument(
        "-c",
        ...
        action="store_true",
        dest="check",
        help="verbose check that all timeslid times are in unvetoed segments",
        default=False,
    )
    parser.add_argument(
        "-u",
        ...
        action="store_true",
        dest="dumpunvetoed",
        help="dump list of unvetoed segments to file and plot segments",
        default=False,
    )
    ...
    for i in arange(nifos):
        ...
        if timeslides:
            if plotsegdist:
                IFOlist[-1].plotCumulativeDurations(
                    os.path.join(outfolder, "singleseg_" + IFOlist[-1]._name + ".png"),
                    maxplot,
                )
            timesdict[ifos[i]] = IFOlist[-1].getTrigTimes(
                whereInj=whereInj, interval=interval, lmargin=seglen
            )
    ...
    if timeslides:
        ...
        generateTimeslides(
            timesdict, Ninj, ref="H1", outfolder=os.path.join(outfolder, "timeslides")
        )
        singles = True
        doubles = False
        triples = False
    ...
    if singles:
        for single in IFOlist:
            if plotsegdist:
                single.plotCumulativeDurations(
                    os.path.join(outfolder, "singleseg_" + single._name + ".png"),
                    maxplot,
                )
            if not timeslides:
                single.getTrigTimes(
                    ...
                    outfile=os.path.join(
                        outfolder,
                        "injtimes_" + single._name + "_" + str(single._minlen) + ".dat",
                    ),
                )
                ...
                if check:
                    ...
                    for time in single.getTrigTimes(
                        whereInj=whereInj, n=Ninj, interval=interval, lmargin=seglen
                    ):
                        dist = single._unvetoed.timeIn(time)
                        print(dist, dist > 0)
            # Dump unvetoed segment list to file and plot segments to file
            if dumpunvetoed:
                single._unvetoed.printToFile(
                    os.path.join(
                        outfolder, "segments", single._name + "_unvetoed_segs.dat"
                    )
                )
                figseg = single._unvetoed.plotSegments(
                    os.path.join(
                        outfolder, "segments", single._name + "_unvetoed_segs.pdf"
                    ),
                    title=single._name + " unvetoed segments",
                    segcolor=colordict[single._name],
                )
    ...
    if doubles:
        ...
        )  # , unvetoed=not plotsegdist) )  #FIXME: test it with plots!
        doubleIFOs.append(
            getDoubles(IFOlist[1], IFOlist[2])
        )  # , unvetoed=not plotsegdist) )  # speedup is ~3s for full triples set
        doubleIFOs.append(
            getDoubles(IFOlist[0], IFOlist[2])
        )  # , unvetoed=not plotsegdist) )  # not worth overall?
        ...
        for double in doubleIFOs:
            double.getTrigTimes(
                ...
                outfile=os.path.join(
                    outfolder,
                    "injtimes_" + double._name + "_" + str(double._minlen) + ".dat",
                ),
            )
            if plotsegdist:
                double.plotCumulativeDurations(
                    os.path.join(outfolder, "doubleseg_" + double._name + ".png"),
                    maxplot,
                )
            # Check unvetoedness and prints the distance of trigger times to edges of unvetoed segments
            if check:
                print("checking double " + double._name)
                for time in double.getTrigTimes(
                    whereInj=whereInj, n=Ninj, interval=interval, lmargin=seglen
                ):
            ...
            # Dump unvetoed segment list to file and plot segments to file
            if dumpunvetoed:
                double._unvetoed.printToFile(
                    os.path.join(
                        outfolder, "segments", double._name + "_unvetoed_segs.dat"
                    )
                )
                figseg = double._unvetoed.plotSegments(
                    os.path.join(
                        outfolder, "segments", double._name + "_unvetoed_segs.pdf"
                    ),
                    title=double._name + " unvetoed segments",
                )
    ...
    if triples:
        ...
        if doubles:
            tripleIFO = getDoubles(
                doubleIFOs[0], IFOlist[2]
            )  # , unvetoed=not plotsegdist)
        else:
            tripleIFO = getTriples(
                IFOlist[0], IFOlist[1], IFOlist[2]
            )  # , unvetoed=not plotsegdist)

        # Generate output for doubles
        tripleIFO.getTrigTimes(
            whereInj=whereInj,
            n=Ninj,
            interval=interval,
            lmargin=seglen,
            outfile=os.path.join(
                outfolder,
                "injtimes_" + tripleIFO._name + "_" + str(tripleIFO._minlen) + ".dat",
            ),
        )
        if plotsegdist:
            tripleIFO.plotCumulativeDurations(
                os.path.join(outfolder, "tripleseg_" + tripleIFO._name + ".png"),
                maxplot,
            )
            # Check unvetoedness and prints the distance of trigger times to edges of unvetoed segments
        if check:
            print("checking triple " + tripleIFO._name)
            for time in tripleIFO.getTrigTimes(
                whereInj=whereInj, n=Ninj, interval=interval, lmargin=seglen
            ):
        ...
        # Dump unvetoed segment list to file and plot segments to file
        if dumpunvetoed:
            tripleIFO._unvetoed.printToFile(
                os.path.join(
                    outfolder, "segments", tripleIFO._name + "_unvetoed_segs.dat"
                )
            )
            figseg = tripleIFO._unvetoed.plotSegments(
                os.path.join(
                    outfolder, "segments", tripleIFO._name + "_unvetoed_segs.pdf"
                ),
                title=tripleIFO._name + " unvetoed segments",
            )
