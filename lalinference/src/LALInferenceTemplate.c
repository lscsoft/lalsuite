...
/*************************************************************************************************************************/
{
  ...
  if(model->domain == LAL_SIM_DOMAIN_FREQUENCY) {
	...
	XLAL_TRY(ret=XLALSimInspiralChooseFDWaveformFromCache(&hptilde, &hctilde, phi0,
            ...
            spin2x, spin2y, spin2z, f_start, f_max, f_ref, distance, inclination,lambda1, lambda2, model->waveFlags, nonGRparams, amporder, order, approximant,model->waveformCache, NULL), errnum);
