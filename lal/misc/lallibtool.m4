...
if test "x$enable_dlopen" != xyes; then
  ...
  *)
  ...
  dlopen)
    ...
    save_LDFLAGS="$LDFLAGS"
    wl=$lt_cv_prog_compiler_wl eval LDFLAGS=\"\$LDFLAGS $export_dynamic_flag_spec\"
    ...
    if test "x$lt_cv_dlopen_self" = xyes; then
      wl=$lt_cv_prog_compiler_wl eval LDFLAGS=\"\$LDFLAGS $lt_cv_prog_compiler_static\"
...
if test "$GCC" = yes; then
  ...
  AC_LIBTOOL_COMPILER_OPTION([if $compiler supports -fno-rtti -fno-exceptions],
    ...
    [_LT_AC_TAGVAR(lt_cv_prog_compiler_no_builtin_flag, $1)="$_LT_AC_TAGVAR(lt_prog_compiler_no_builtin_flag, $1) -fno-rtti -fno-exceptions"])
fi
...
case $host_os in
  ...
  *djgpp*)
    ...
    ;;
  *)
    _LT_AC_TAGVAR(lt_cv_prog_compiler_pic, $1)="$_LT_AC_TAGVAR(lt_prog_compiler_pic, $1)ifelse([$1],[],[ -DPIC],[ifelse([$1],[CXX],[ -DPIC],[])])"
    ;;
esac
...
#
wl=$_LT_AC_TAGVAR(lt_cv_prog_compiler_wl, $1) eval lt_tmp_static_flag=\"$_LT_AC_TAGVAR(lt_prog_compiler_static, $1)\"
...
_LT_EOF
    ...
    *)
    ...
    aix3*)
      ...
      _LT_AC_TAGVAR(hardcode_minus_L, $1)=yes
      if test "$GCC" = yes && test -z "$lt_cv_prog_compiler_static"; then
...
x|xyes)
  ...
  if test "$enable_shared" = yes && test "$GCC" = yes; then
    ...
    *'~'*)
	...
	pic_flag=$_LT_AC_TAGVAR(lt_cv_prog_compiler_pic, $1)
        compiler_flags=-v
        linker_flags=-v
        verstring=
        output_objdir=.
