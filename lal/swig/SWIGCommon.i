...
///
/// Public macros (i.e. those used in headers) are contained inside the \b
/// SWIGLAL() macro, so that they can be easily removed from the proprocessing
/// interface. The \b SWIGLAL_CLEAR() macro is used to clear any effects of a
/// public macro. Calls to \b SWIGLAL_CLEAR() are generated from the
...
///
/// Define a SWIG preprocessor symbol \c NAME1_NAME2.
///
...
///
/// The macro \b %swiglal_map() maps a one-argument \c MACRO(X) onto a list of
/// arguments (which may be empty). Based on SWIG's \c %formacro().
///
...
///
/// The macro \b %swiglal_map_a() maps a two-argument \c MACRO(A,X) onto a list
/// of arguments (which may be empty), with a common first argument \c A.
///
...
///
/// The macro \b %swiglal_map_ab() maps a three-argument \c MACRO(A,B,X) onto a
/// list of arguments (which may be empty), with common first arguments \c A and
/// \c B.
///
...
///
/// The macro \b %swiglal_map_abc() maps a four-argument \c MACRO(A,B,C,X) onto
/// a list of arguments (which may be empty), with common first arguments \c A,
/// \c B, and \c C.
///
...
///
/// The macros \b %swiglal_apply() and \b %swiglal_clear() apply and clear SWIG
/// typemaps.
///
...
///
/// The macros \b %swiglal_feature and \b %swiglal_feature_nspace apply SWIG
/// features.
///
...
///
/// The macros \b %swiglal_warnfilter and \b %swiglal_warnfilter_nspace suppress
/// SWIG warnings.
///
...
///
/// These macros allocate and/or copy new instances and arrays.  They are
/// analogous to similarly-named SWIG macros but use XLALMalloc().
///
...
///
/// Include SWIG configuration header generated from \c config.h, but don't
/// generate wrappers for any of its symbols
///
...
///
/// Include LAL standard definitions header LALStddef.h: this contains macro
...
/// <dl>
///    <dt>XLAL</dt><dd>Before performing any action, clear the XLAL error
///    number.  Check it after the action is performed; if it is non-zero, raise
///    a native scripting-language exception with the appropriate XLAL error
///    message.</dd>
///    <dt>LAL</dt><dd>For any function which has a LALStatus* argument, create
///    a blank LALStatus struct and pass its pointer to the LAL function.  After
///    the function is called, check the LALStatus statusCode; if it is
///    non-zero, raise a generic XLAL error (to set the XLAL error number), then
///    a native scripting-language exception. The \c swiglal_check_LALStatus (C)
///    preprocessor symbol determines if the LAL error handling code is invoked;
///    by default this symbol should be undefined, i.e. functions are XLAL
///    functions by default.</dd>
/// </dl>
///
%header %{
...
///
/// Wrappers around SWIG's pointer to/from SWIG-wrapped scripting language
/// object functions.  \b swiglal_from_<i>SWIGTYPE</i>() simply returns a
/// SWIG-wrapped object containing the input pointer.  \b
/// swiglal_from_<i>SWIGTYPE</i>() extracts a pointer from a SWIG-wrapped
/// object, then struct-copies the pointer to the supplied output pointer.
///
...
///
/// Call the destructor \c DTORFUNC, but first call \b swiglal_release_parent()
/// to check whether \c PTR own its own memory (and release any parents).
///
...
/// If this is an opaque struct, create an empty struct to represent the opaque
/// struct, so that SWIG has something to attach the destructor to.  No
/// constructors are generated, since it is assumed that the struct will have a
/// creator function. Otherwise, if this is not an opaque struct, generate a
/// basic constructor, using XLALCalloc() to allocate memory.
#if OPAQUE
struct TAGNAME {
};
#else
...
/// Create destructor, using either the destructor function \c DTORFUNC, or else
/// XLALFree() if \c DTORFUNC is undefined, to destroy memory.
#if #DTORFUNC == ""
%extend TAGNAME {
  ~TAGNAME() {
...
/// e.g. <tt>$typemap(swiglal_fixarr_ltype, const int[][])</tt> returns \c int.
...
/// <ul>
///    <li>\b %swiglal_array_copyin...() copies a scripting-language array into
///    a C array.</li>
///    <li>\b %swiglal_array_copyout...() copies a C array into a
///    scripting-language array.</li>
///    <li>\b %swiglal_array_viewin...() tries to view a scripting-language
///    array as a C array.</li>
///    <li>\b %swiglal_array_viewout...() wraps a C array inside a
///    scripting-language array, if this is supported by the target scripting
///    language.</li>
/// </ul>
///
...
///    <li><tt>SWIG_Object parent</tt>: SWIG-wrapped object containing parent
///    struct.</li>
///    <li><tt>void* ptr</tt>: pointer to C array.</li>
///    <li><tt>const size_t esize</tt>: size of one C array element, in
///    bytes.</li>
...
///    <li><tt>const size_t strides[]</tt>: strides of C array dimension, in
///    number of elements.</li>
///    <li><tt>swig_type_info *tinfo</tt>: SWIG type info of the C array element
///    datatype.</li>
...
///    <li>\b %swiglal_array_copyout...(): output scripting-language array as a
///    \c SWIG_Object.</li>
///    <li>\b %swiglal_array_viewin...(): SWIG error code as an \c int.</li>
///    <li>\b %swiglal_array_viewout...(): output scripting-language array view
///    as a \c SWIG_Object.</li>
/// </ul>
///
...
///
/// The \b %swiglal_array_type() macro maps <tt>TYPE</tt>s of C arrays to an \c
/// ACFTYPE of the appropriate array conversion functions, using a special
/// typemap. The typemap also ensures that fragments containing the required
...
/// <ul>
///    <li>output-only arguments: <b>SWIGLAL(OUTPUT_ARRAY_<i>n</i>D(TYPE,
///    ...))</b></li>
...
%swiglal_map_a(%swiglal_clear, TYPE, __VA_ARGS__);
%enddef
///    <li>input-output arguments: <b>SWIGLAL(INOUT_ARRAY_<i>n</i>D(TYPE,
///    ...))</b></li>
...
///
/// Get the correct descriptor for a dynamic array element.  Always return a
/// pointer description, even for non-pointer types, and determine whether array
/// is an array of pointers or of data blocks.
///
...
///
/// Create immutable members for accessing the array's dimensions.  \c NI is the
/// name of the dimension member, and \c SIZET is its type.
///
...
///
/// The \b %swiglal_array_dynamic_<i>n</i>D() macros create typemaps which
/// convert <i>n</i>D dynamically-allocated arrays in structs \c NAME. The
/// macros must be added inside the definition of the struct, before the struct
/// members comprising the array are defined. The \c DATA and \c N{I,J} members
/// give the array data and dimensions, \c TYPE and \c SIZET give their
/// respective types. The \c S{I,J} give the strides of the array, in number of
/// elements.  If the sizes or strides are members of the struct,
/// <tt>arg1-\></tt> should be used to access the struct itself.
///
/// <ul><li>
/// 1-D arrays:
...
/// </li><li>
/// a 1-D array of pointers to 1-D arrays; \c NI refer to the array of pointers,
/// \c NJ/SJ refer to the arrays pointed to:
%define %swiglal_array_dynamic_1d_ptr_1d(NAME, TYPE, SIZET, DATA, NI, NJ, SJ)
/// <ul><li>

/// Typemaps which convert from the dynamically-allocated array, indexed by \c
/// arg2
%typemap(out, noblock=1) TYPE* DATA {
  if (arg1) {
...
///
/// The \b %swiglal_array_struct_<i>n</i>D() macros create typemaps which
/// attempt to view a scripting-language array as a C array <tt>struct
/// NAME</tt>. If the input is not already a SWIG-wrapped object wrapping a
/// <tt>struct NAME</tt>, an input view is attempted using \b
/// %swiglal_array_viewin...().
///
/// The main concern with passing scripting-language memory to C code is that it
/// might try to re-allocate or free the memory, which would certainly lead to
/// undefined behaviour. Therefore, by default a view is attempted only for
/// pointers to <tt>const NAME*</tt>, since it is reasonable to assume the
/// called C code will not try to re-allocate or free constant memory. When it
/// is known that the called C function will not try to re-allocate or free a
/// particular argument, the <b>SWIGLAL(VIEWIN_STRUCTS(NAME, ...))</b> macro can
/// be used to apply the typemap to pointers to (non-<tt>const</tt>) \c NAME*.
///
/// <ul><li>
/// 1-D arrays:
...
///
/// The <b>SWIGLAL(VIEWIN_STRUCTS(NAME, ...))</b> macro can be used to apply the
/// above input view typemaps to pointers to (non-<tt>const</tt>) \c NAME*.
///
...
///
/// The <b>SWIGLAL(ARRAY_<i>n</i>D...(NAME, ...))</b> macros should be called
/// from within the definitions of LAL structs NAME containing
...
/// </li><li>
/// a 1-D array of pointers to 1-D arrays, e.g. <tt>SIZET NI; SIZET NJ; ATYPE*
/// DATA[(NI members)];</tt>
...
/// </li><li>
/// 2-D arrays of fixed-length arrays, e.g <tt>typedef ETYPE[NJ] ATYPE; SIZET
/// NI; ATYPE* DATA;</tt>:
...
/// other output arguments (via \c argout typemaps), the \c int return value is
/// ignored. This is because \c int is very commonly used to return an XLAL
...
/// but do return a useful \c int can use <b>SWIGLAL(RETURN_VALUE(int, ...))</b>
/// to disable this behaviour.
///
/// For functions, since \c %feature("new") is set, the \c out typemap will have
/// <tt>$owner=1</tt>, and the \c newfree typemap is also applied. The \c out
/// typemap ignores the \c int return value by setting the output argument list
/// to \c VOID_Object; the wrapping function them proceeds to add other output
/// arguments to the list, if any. After this, the \c newfree typemap is
/// triggered, which appends the \c int return if the output argument list is
/// empty, using the scripting-language-specific macro \b
/// swiglal_append_output_if_empty(). For structs, <tt>$owner=0</tt>, so the int
/// return is set straight away, and the \c newfree typemap is never applied.
///
...
///
/// Typemaps for empty arguments. These typemaps are useful when no input from
/// the scripting language is required, and an empty struct needs to be supplied
/// to the C function. The <b>SWIGLAL(EMPTY_ARGUMENT(TYPE, ...))</b> macro
/// applies the typemap which supplies a static struct, while the
/// <b>SWIGLAL(NEW_EMPTY_ARGUMENT(TYPE, ...))</b> macro applies the typemap
/// which supplies a dynamically-allocated struct. These typemaps may cause
/// there to be no SWIG-wrapped object for the first argument; if so, \c
...
///
/// SWIG conversion functions for C99 integer types.  These are mapped to the
/// corresponding basic C types, conversion functions for which are supplied by
/// SWIG.
///
...
/// Appropriate typemaps are then generated by \c %typemaps_asvalfromn().
///
...
///
/// Fragments and typemaps for LAL strings, which should be (de)allocated using
/// XLALMalloc() and XLALFree(). The fragments re-use existing scriping-language
/// conversion functions for ordinary \c char* strings. Appropriate typemaps are
/// then generated by \c %typemaps_string_alloc(), with custom memory
/// allocators.
///
...
///
/// Typemaps for string pointers.  By default, treat arguments of type \c char**
...
/// an \c INOUT typemap for input-output arguments, which allows a
/// scripting-language input string to be supplied. The \c INOUT typemaps can be
/// applied as needed using the <b>SWIGLAL(INOUT_STRINGS(...))</b> macro.
///
...
///
/// Input typemap for pointer-to-<tt>const</tt> <tt>SWIGTYPE</tt>s. This typemap
/// is identical to the standard \c SWIGTYPE pointer typemap, except
/// <tt>$disown</tt> is commented out. This prevents SWIG transferring ownership
/// of SWIG-wrapped objects when assigning to pointer-to-<tt>const</tt> members
/// of structs. In this case, it is assumed that the struct does not want to
/// take ownership of the pointer, since it cannot free it (since it is a
...
///
/// Typemaps for output <tt>SWIGTYPE</tt>s. This typemaps will match either the
/// SWIG-wrapped return argument from functions (which will have the \c
/// SWIG_POINTER_OWN bit set in <tt>$owner</tt>) or return a member of a struct
/// through a \c get functions (in which case \c SWIG_POINTER_OWN will not be
/// set). They require the following macros:
///
/// The macro \b %swiglal_store_parent() is called to store a reference to the
/// struct containing the member being accessed, in order to prevent it from
/// being destroyed as long as the SWIG-wrapped member object is in scope. The
/// return object is then always created with \c SWIG_POINTER_OWN, so that its
...
%enddef
///
/// The macro \b %swiglal_set_output() sets the output of the wrapping
/// function. If the (pointer) return type of the function is the same as its
/// first argument, then <tt>swiglal_return_1starg_\#\#<i>NAME</i></tt> is
/// defined. Unless \c swiglal_no_1starg is defined (in which case the first
/// argument is being handled by e.g. the \c EMPTY_ARGUMENT typemap), the macro
/// compares the pointer of the return value (\c result) to that of the first
/// argument (\c arg1). If they're equal, the SWIG-wrapped function will return
/// a \e reference to the SWIG object wrapping the first argument, i.e. the same
/// object with its reference count incremented. That way, if the return value
/// is assigned to a different scripting-language variable than the first
/// argument, the underlying C struct will not be destroyed until both
/// scripting-language variables are cleared.  If the pointers are not equal, or
/// one pointer is \c NULL, the macro return a SWIG object wrapping the new C
/// struct.
///
...
///
/// The INOUT typemaps can be applied instead using the macro
/// <b>SWIGLAL(INOUT_SCALARS(TYPE, ...))</b>.
///
...
%enddef
///
/// The INPUT typemaps can be applied instead using the macro
/// <b>SWIGLAL(INPUT_SCALARS(TYPE, ...))</b>.
///
...
///
/// Typemaps for double pointers. By default, treat arguments of type \c TYPE**
/// as output-only arguments, which do not require a scripting-language input
/// argument, and return their results in the output argument list. Also supply
/// an \c INOUT typemap for input-output arguments, which allows a
/// scripting-language input argument to be supplied. The INOUT typemaps can be
/// applied as needed using the <b>SWIGLAL(INOUT_STRUCTS(TYPE, ...))</b> macro.
///
/// The typemaps apply the following convention for \c NULL pointers. For an
/// argument \c arg of type \c TYPE**, where <b>SWIGLAL(INOUT_STRUCTS(TYPE,
/// arg))</b> has been applied:
/// <ul>
///    <li>if \c arg is passed a scripting language \c empty value (e.g. <tt>[]</tt>
///    in Octave, or <tt>None</tt> in Python), this is interpreted as a \c NULL
///    pointer of type \c TYPE**, e.g.  <tt>TYPE** arg = NULL;</tt>.</li>
///    <li>if \c arg is passed the integer value \c 0, this is interpreted as a
///    valid pointer of type TYPE** to a NULL pointer of type TYPE*, e.g.
///    <tt>TYPE* ptr = NULL; TYPE** arg = \&ptr;</tt>.</li>
/// </ul>
///
...
///
/// Make the wrapping of <tt>printf()</tt>-style LAL functions a little safer,
/// as suggested in the SWIG 2.0 documentation (section 13.5). These functions
/// should now be safely able to print any string, so long as the format string
/// is named \c format or \c fmt.
///
...
///
/// Specialised input typemap for C file pointers. Generally it is not possible
/// to convert scripting-language file objects into \c FILE*, since the
/// scripting language may not provide access to the \c FILE*, or even be using
/// \c FILE* internally for I/O.  The \c FILE* will therefore have to be
/// supplied from another SWIG-wrapped C function.  For convenience, however, we
/// allow the user to pass integers 0, 1, or 2 in place of a \c FILE*, as an
/// instruction to use standard input, output, or error respectively.
///
...
///
/// Specialised input typemaps for a given <tt>struct TAGNAME</tt>. If
/// conversion from a SWIG-wrapped scripting language object \c INPUT fails,
/// call the function defined in \c FRAGMENT,
/// <b>swiglal_specialised_<i>TAGNAME</i>(INPUT, OUTPUT)</b>, to try to convert
/// \c INPUT from some other value, and if successful store it in <tt>struct
/// TAGNAME OUTPUT</tt>.  Otherwise, raise a SWIG error. Separate typemaps are
/// needed for <tt>struct TAGNAME</tt> and pointers to <tt>struct TAGNAME</tt>.
///
...
///
/// The <b>SWIGLAL(VARIABLE_ARGUMENT_LIST(...))</b> macro supports functions
/// which require a variable-length list of arguments of type \c TYPE, i.e. a
/// list of strings. It generates SWIG \e compact default arguments, i.e. only
/// one wrapping function where all missing arguments are assigned \e ENDVALUE,
/// generates 11 additional optional arguments of type \e TYPE, and creates a
/// contract ensuring that the last argument is always \c ENDVALUE, so that the
/// argument list is terminated by \c ENDVALUE.
///
...
///
/// The <b>SWIGLAL(EXTERNAL_STRUCT(...))</b> macro can be used to support
/// structs which are not declared in LALSuite. It treats the struct as opaque,
