...
#define %swiglal_new_copy_LALCHAR(PTR, SIZE, TYPE...) \
  %reinterpret_cast(memcpy(XLALMalloc(sizeof(TYPE)*(SIZE)), PTR, sizeof(TYPE)*(SIZE)), TYPE*)
  ...
  SWIGINTERN int SWIG_AsLALCHARPtrAndSize(SWIG_Object obj, CHAR **pstr, size_t *psize, int *alloc) {
    ...
    if (pstr) {
      ...
      if (alloc) {
        ...
        *pstr = %swiglal_new_copy_LALCHAR(slstr, slsize, CHAR);
        *alloc = SWIG_NEWOBJ;
      }
      else {
        return SWIG_TypeError;
...
%typemaps_string_alloc(%checkcode(STRING), %checkcode(CHAR), CHAR, LALCHAR,
                       ...
                       strlen, %swiglal_new_copy_LALCHAR, XLALFree,
...
%typemap(out,noblock=1) SWIGTYPE *, SWIGTYPE &, SWIGTYPE[] {
  ...
  %set_output(SWIG_NewPointerObj(%as_voidptr($1), $descriptor, SWIG_POINTER_OWN | $owner | %newpointer_flags));
}
...
%typemap(out, noblock=1) SWIGTYPE (void* copy = NULL) {
  copy = %new_copy($1, $ltype);
  %swiglal_store_parent(copy, SWIG_POINTER_OWN);
  %set_output(SWIG_NewPointerObj(copy, $&descriptor, SWIG_POINTER_OWN | %newpointer_flags));
}
