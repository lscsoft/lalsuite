...
static UINT8 hash_elem( const void *x )
{
  ...
  return XLALComputeHash( &ex->key, sizeof( ex->key ) );
}
...
int main( void )
{
  ...
  /* Test XLALComputeHash() */
  {
    int n = 0, nc = 0;
    for ( int i = 0; i < 2048; ++i ) {
      const UINT8 hi = XLALComputeHash( &i, sizeof( i ) );
      for ( int j = 0; j < i; ++j ) {
        const UINT8 hj = XLALComputeHash( &j, sizeof( j ) );
        ++n;
        if ( hi == hj ) {
          ++nc;
        }
    ...
    printf( "XLALComputeHash() collisions in %i integer pairs: %i\n", n, nc );
    XLAL_CHECK_MAIN( nc == 0, XLAL_EFAILED );
  }
