...
This section describes how to perform a basic install of LAL.  This is not
quite a minimal install---support for the (optional) frame library is
included---but it is probably the most useful basic installation.  It is
assumed that you are obtaining LAL from CVS rather than from a distribution
tarball.  If you are using a distribution tarball then the installation will
be somewhat more straightforward.

This section assumes you are using a Unix system with standard GNU tools
in place, e.g., gcc, GNU m4, cvs, etc.  The instructions are written for use
...
The part of the instructions in this document are extracted from the shell
scripts \verb|lal-preinstall-x.sh| and \verb|lal-install-x.sh| in the
\verb+doc+ subdirectory in LAL.  Also extracted are plain-text
\verb|lal-preinstall.txt| and \verb|lal-install.txt|, and nicely-formated shell
scripts \verb|lal-preinstall.sh| and \verb|lal-install.sh|.  The shell scripts
are a superset of the instructions here.  They also set up some useful
configuration files.  Rather than copying the instructions in this document,
you can simply edit the scripts \verb|lal-preinstall.sh| and
\verb|lal-install.sh| (to specifiy the install locations, etc.) and run these
scripts instead.  (Run \verb|lal-preinstall.sh| first and \verb|lal-install.sh|
second.)

\part{Pre-Installation}

This pre-installation is required to obtain the necessary tools to build LAL
from a CVS checkout.  If you are building LAL from a distributed tarball then
you will not need to install \verb+automake+ or \verb+autoconf+, but you will
still need \verb+fftw3+ and \verb+gsl+.  The \verb+frame+ software is optional
and \verb+metaio+ software is optional in LAL, but it is very useful so these
instructions assume that you will want to build LAL to interface with these
libraries.

\chapter{Obtaining required software}

\input{lal-preinstall}

\chapter{Obtaining LAL from CVS}

This describes how to obtain a distribution of LAL from CVS, and to prepare
LAL to be built.  Make sure that you are in the directory where you want to
download LAL.

{\itshape \noindent {\small\color{blue}\verb|LSCSOFT_LOCATION|} must be set --- 
make sure this software is available:}
\begin{color}{blue}
\small
\begin{alltt}
...
{\itshape \noindent set {\small\color{blue}\verb|LSCSOFT_CVSUSER|} to your usern
ame and make sure you run {\small\color{blue}\verb|cvs login|}!}
\begin{color}{blue}
\small
\begin{alltt}
  LSCSOFT_CVSUSER="anonymous"
  LSCSOFT_CVSROOT=":pserver:$LSCSOFT_CVSUSER@gravity.phys.uwm.edu:2402/usr/local
/cvs/lscsoft"
\end{alltt}
\end{color}

{\itshape \noindent get LAL and LALApps from CVS}
\begin{color}{blue}
\small
\begin{alltt}
cvs -d$LSCSOFT_CVSROOT checkout -d lal-current lal 
\end{alltt}
\end{color}

{\itshape \noindent now prepare LAL to be configured}
\begin{color}{blue}
\small
\begin{alltt}
cd lal-current
...
LAL is now ready to be configured, built, and installed.
