...
\tracingstats=1
\usepackage{lal,array,fancyhdr,geometry,graphicx,color,hyperref,makeidx,wrapfig}
\includeonly{%
  std,%
  support,%
  tools,%
...
\author{Jolien Creighton (Librarian), Alan Wiseman (Coordinator) 
and Members of the LSC}
...
contained a lot of instructions specifying the layout of the
documentation and code; however many of these rules have migrated
to the LLAL-Spec itself, and thus there is less need for the such
long list of rules.  Furthermore, now there is a much larger body of
example code  to help guide people in how to follow the LAL
rules.  Therefore Part I of this new edition of the 
{{\textcolor{red}{\bf L}} {\textcolor{green}{\bf S}} {\textcolor{blue}{\bf D}} }
...
{\bf Documentation for a single package:} In the documentation
directory within each package there will be a file called {\tt
main.tex}. The contents of which are essentially identical in each
package. For example, in a package called {\tt samplepackage} the
following file ({\tt main.tex}) must be in the directory {\tt
/lal/packages/samplepackage/doc/}
\begin{verbatim}
     \documentclass[oneside]{book}
     ...
     % This is the only line that will change from package to package.
...
The guts of the documentation for this package should reside in the
file that is \verb@\include{}@ed  into  this file (e.g. {\tt
/lal/packages/samplepackage/doc/samplepackage.tex}).  This file may
...
largely left up to their judgment and patience. However, there are a
few things that must be auto-documented.
...
\item[(2)] In {\tt /junk} run the command ``\verb@./laldoc LalDocDemo.h Errors.out@'' 
