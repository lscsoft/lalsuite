...
related to $\Psi_4$ by two time derivatives\footnote{We define
  ...
  spatial slice as $e_t^a$ and the promotions of
  $\{\rhat,\iotahat,\phihat\}$ to the full spacetime as
  $\{e_r^a,e_\iota^a,e_\phi^a\}$, then the null tetrad adapted to the
  constant $r$ spheres is $\{\ell^a,n^a,m^a,\bar{m}^a\}$ where $\ell^a
  = (e_t^a + e_r^a)/\sqrt{2}$, $n^a = (e_t^a - e_r^a)/\sqrt{2}$, $m^a
  = (e_\iota^a + ie_\phi^a)/\sqrt{2}$, and $\bar{m}^a$ is the complex
  conjugate of $m^a$.}
\begin{equation}
  \label{eq:3}
