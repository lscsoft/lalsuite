...
\index{\texttt{LALInspiralParameterCalc()}}
...
It also calculates the Newtonian chirp time $\tau_{N}$, the first post--Newtonian chirp time
$\tau_{P^{1}N}$, the 1.5 order post--Newtonian chirp time $\tau_{P^{1.5}N}$, the second order
post--Newtonian chirptime $\tau_{P^{2}N}$ and the chirp
