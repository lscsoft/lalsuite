...
/* Number of templates is nlist */
  ...
  LALInspiralCreateBCVBank(&status, &tempParams, &nlist, coarseIn);
    
  fprintf(stderr, "#nlist right after=%d\n", nlist);
...

  {
    ...
    RectIn.dx = sqrt(2.0 * (1. - coarseIn.mmCoarse)/tempParams[0].metric.g00 );
    RectIn.dy = sqrt(2.0 * (1. - coarseIn.mmCoarse)/tempParams[0].metric.g11 );
    RectIn.theta = tempParams[0].metric.theta;
	...
	*/
	RectIn.x0 = (REAL8) tempParams[j].params.psi0;
	RectIn.y0 = (REAL8) tempParams[j].params.psi3;
	/*
