...
\texttt{b2} &=& \Big (\texttt{I1} - \frac{\texttt{I53}^2}{\texttt{I73}}
	\Big )^{-1/2} \\
\texttt{b1} &=& -\frac{\texttt{I53}}{\texttt{I73}} \: \texttt{b2}. \\
\end{eqnarray}
...
\begin{eqnarray}
q_j &=&\sum_{k=0}^{N/2}e^{2 \pi i j k/N} \frac{(dR \tilde{v}_k^{\ast}) 
	\: \texttt{a1} \: k^{-7/6} e^{i \psi'} }{ |dR|^2 S_v(|f_k|) } \\
q^{{}_{BCV}}_j &=& \sum_{k=0}^{N/2}e^{ 2 \pi i j k/N} \frac{(dR \tilde{v}_k 
^{\ast})( \texttt{b1} \:k^{-7/6} + \texttt{b2}\: k^{-1/2} ) e^{i \psi'}}
	{|dR|^2 S_v(|f_k|)} \\
\end{eqnarray}
up to the appropriate normalization factors, namely $(\texttt{tmpltNorm}) 
^{1/2}( \Delta t/N)$
