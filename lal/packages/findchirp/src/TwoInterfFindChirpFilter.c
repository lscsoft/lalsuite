  ...
  for ( j = startIndex1; j < endIndex1; ++j ) 
      ...
      for ( k = ((j < startIndex2+maxLagPts) ? startIndex2 : (j-maxLagPts)) ; k < (( (j + maxLagPts+1) < endIndex2) ? (j + maxLagPts+1) : endIndex2) ; ++k )
	{
	  ...
	  if ( unresolvedEvent ) {
		...
		/*XXX compute sigmasq (and not sigma!); store it as sigma XXX*/
		...
		thisEvent->eventIn1->effDist = 
		  (input->filterInput[0].fcTmplt->tmpltNorm * 
		   input->filterInput[0].segment->segNorm * 
		   input->filterInput[0].segment->segNorm) * 
		  norm1 / thisEvent->eventIn1->snrsq;
		thisEvent->eventIn1->effDist 
		  = sqrt( thisEvent->eventIn1->effDist );
		
		thisEvent->eventIn2->effDist = 
		  (input->filterInput[1].fcTmplt->tmpltNorm * 
		   input->filterInput[1].segment->segNorm * 
		   input->filterInput[1].segment->segNorm) * 
		  norm2/ thisEvent->eventIn2->snrsq;
		thisEvent->eventIn2->effDist 
		  = sqrt( thisEvent->eventIn2->effDist );
		...
		/*XXX sigma here is really sigmasq XXX*/
		thisEvent->sigma =0.5*(thisEvent->eventIn1->sigma 
				       +thisEvent->eventIn2->sigma);
		thisEvent->effDist =0.5*(thisEvent->eventIn1->effDist
					 +thisEvent->eventIn2->effDist);
    ...
    )
{
  ...
  for ( n=0; n<2; n++) {
    numChisqBins[n] = input->filterInput[n].segment->chisqBinVec->length - 1;
  }  
  ...
  for ( n = 0 ; n < 2 ; ++n )
	...
	memset( params->paramsVec->filterParams[n].rhosqVec->data->data, 0, 
		numPoints * sizeof( REAL4 ) );
      mismatch[n] = 1.0 - input->filterInput[n].tmplt->minMatch;
    }/*end loop over detectors*/
  
  /* normalization */
  params->paramsVec->filterParams[0].norm = 
    norm1 = 4.0 * (deltaT / (REAL4)numPoints) / input->filterInput[0].segment->segNorm;
  params->paramsVec->filterParams[1].norm = 
    norm2 = 4.0 * (deltaT / (REAL4)numPoints) / input->filterInput[1].segment->segNorm;
  ...
  chisqThreshFac[0] = norm1 * mismatch[0] * mismatch[0] / (REAL4) numChisqBins[0];
  chisqThreshFac[1] = norm2 * mismatch[1] * mismatch[1] / (REAL4) numChisqBins[1];
  ...
  /* look for events in the filter output */
  for ( j = startIndex1; j < endIndex1; ++j ) 
    {
      for ( k = ((j < startIndex2+maxLagPts) ? startIndex2 : (j-maxLagPts)) ; k < (( (j + maxLagPts+1) < endIndex2) ? (j + maxLagPts+1) : endIndex2) ; ++k )
	{
	  ...
	  if ( rhosq > twoInterfRhosqThresh
	      ...
	      if ( ! haveChisq1   && input->filterInput[0].segment->chisqBinVec->length )
		{
		  ...
		  /* XXX this should be passed in from the bank XXX 
		     params->paramsVec->filterParams[0].chisqParams->bankMatch=0.97;*/;
#if 0
		  params->paramsVec->filterParams[0].chisqParams->bankMatch=
		    input->filterInput[0].tmplt->minMatch;
#endif	  
		  /* compute the chisq threshold for detector 1 */
	      ...
	      if ( ! haveChisq2   && input->filterInput[1].segment->chisqBinVec->length )
		{
		  ...
		  */
#if 0
		  params->paramsVec->filterParams[1].chisqParams->bankMatch=
		    input->filterInput[1].tmplt->minMatch;
#endif		  
		  /* compute the chisq threshold for detector 2 */
		 ...
		 threshold, then start processing events */
	      if ( (! input->filterInput[0].segment->chisqBinVec->length ||
		    params->paramsVec->filterParams[0].chisqVec->data[j] 
		    < ( params->paramsVec->filterParams[0].chisqThresh
			* ( 1.0 + modqsq1 * chisqThreshFac[0] )) 
		   && (! input->filterInput[1].segment->chisqBinVec->length ||
		       params->paramsVec->filterParams[1].chisqVec->data[k] 
		       < params->paramsVec->filterParams[1].chisqThresh 
		       * ( 1.0 + modqsq2 * chisqThreshFac[1] ) ) ))
		{
		  
		  if ( unresolvedEvent ) {
		    /*  current rhosq peak falls within boundaries of 
		     *  last chirp, so it is treated as being
		     *  the same event 
                     */
		    ...
		    if (rhosq > thisEvent->snrsq) {
		      ...
		      cohEvent->end_time.gpsSeconds = j;		      
		      cohEvent->snr = rho; 
		    }
		    /* if the rhosq value was less than previous values for this event, it is ignored */
		  }
		    ...
		    if ( ! *eventList ) {
		      /* if this is the first event, start the list */
		      thisEvent =  (TwoInterfInspiralEvent *) 
			LALCalloc( 1, sizeof(TwoInterfInspiralEvent) );
		      
		      /*if this is the first index, start the coherent list*/
		      eventStartIdx = j;
		      cohEvent = *eventList = (MultiInspiralTable *) 
			LALCalloc( 1, sizeof(MultiInspiralTable) );
		      if ( ! (thisEvent && cohEvent) )
			{
			  ABORT( status, TWOINTERFFINDCHIRPH_EALOC, 
				 TWOINTERFFINDCHIRPH_MSGEALOC );
			}
		      ...
		      thisEvent->next = (TwoInterfInspiralEvent *) 
			LALCalloc( 1, sizeof(TwoInterfInspiralEvent) );
		      cohEvent->next = (MultiInspiralTable *) 
			LALCalloc( 1, sizeof(MultiInspiralTable) );
		      
		      if ( ! (thisEvent->next && cohEvent->next) )
			{
			  ABORT( status, TWOINTERFFINDCHIRPH_EALOC, 
				 TWOINTERFFINDCHIRPH_MSGEALOC );
			}
		    ...
		    /*XXX sigma is really sigmasq below XXX*/
		    thisEvent->eventIn1->sigma= norm1*
		      input->filterInput[0].segment->segNorm*
		      input->filterInput[0].segment->segNorm*
		      input->filterInput[0].fcTmplt->tmpltNorm;
		    ...
		    */
		    ...
		    cohEvent->end_time.gpsSeconds = j;
		    cohEvent->ifo1_eff_distance = thisEvent->eventIn1->effDist;
		    cohEvent->ifo2_eff_distance = thisEvent->eventIn2->effDist;
		    cohEvent->eff_distance = thisEvent->effDist;

		    cohEvent->ifo1_snr = sqrt(thisEvent->eventIn1->snrsq); 
		    cohEvent->ifo2_snr = sqrt(thisEvent->eventIn2->snrsq); 
		    cohEvent->snr = rho;
		...
		} /* done processing event which passed chisq test */
	  ...
	  if ( unresolvedEvent ) {
		...
		/*set the event LIGO GPS time of event for the network
		  to be = that in detector 1 (chosen as reference detector) */
		cohEvent->end_time.gpsSeconds = (INT4) (timeNS1/1000000000L);
		cohEvent->end_time.gpsNanoSeconds = (INT4) (timeNS1%1000000000L);
		LALGPStoGMST1( status->statusPtr, &(cohEvent->end_time_gmst),
			       &(cohEvent->end_time), &gmstUnits );
		CHECKSTATUSPTR( status );
		...
		/* record the ifo and channel names for the network event
		   strncpy( cohEvent->ifo, input->filterInput[0].segment->data->name, 
		   2 * sizeof(CHAR) );
		   
		   strncpy( thisEvent->channel, input->segment->data->name + 3,
		   (LALNameLength - 3) * sizeof(CHAR) );
		*/
		cohEvent->impulse_time = cohEvent->end_time;
		...
		/* copy ifo1's template into the network event */
		cohEvent->mass1  = (REAL4) input->filterInput[0].tmplt->mass1;
		cohEvent->mass2  = (REAL4) input->filterInput[0].tmplt->mass2;
		cohEvent->mchirp = (REAL4) input->filterInput[0].tmplt->chirpMass;
		cohEvent->eta    = (REAL4) input->filterInput[0].tmplt->eta;
		cohEvent->tau0   = (REAL4) input->filterInput[0].tmplt->t0;
		cohEvent->tau2   = (REAL4) input->filterInput[0].tmplt->t2;
		cohEvent->tau3   = (REAL4) input->filterInput[0].tmplt->t3;
		cohEvent->tau4   = (REAL4) input->filterInput[0].tmplt->t4;
		cohEvent->tau5   = (REAL4) input->filterInput[0].tmplt->t5;
		cohEvent->ttotal = (REAL4) input->filterInput[0].tmplt->tC;

		/* set the type of the template used in the analysis */
		LALSnprintf( cohEvent->search, LIGOMETA_SEARCH_MAX * sizeof(CHAR),
			     "FindChirpSPtwoPN" );
		...
		/*XXX Earlier, sigma and eff_distance was given below XXX*/
		
		/*XXX for now, a simple-minded average of the two detectors; 
		  note that the event->sigmas are really sigmasqs XXX*/
		cohEvent->sigmasq = thisEvent->sigma;
		cohEvent->ifo1_eff_distance = thisEvent->eventIn1->effDist;
		cohEvent->ifo2_eff_distance = thisEvent->eventIn2->effDist;
		...
		/*we store chisq distributed with 2p - 2 degrees of 
		  freedom in the database. params->chisqVec->data = r^2 = 
		  chisq / p so we multiply r^2 by p here to get chisq */
		cohEvent->chisq = 0.5*(thisEvent->eventIn1->chisq+thisEvent->eventIn1->chisq);/* XXX for now, a simple-minded average XXX*/
		cohEvent->chisq_dof =thisEvent->eventIn1->numChisqBins;
		cohEvent->snr = sqrt(thisEvent->snrsq);
		...
		cohEvent->ligo_axis_ra = thisEvent->twoInterfAxisRa; 
		cohEvent->ligo_axis_dec= thisEvent->twoInterfAxisDec;
	...
	
		{
		  ...
		  thisEvent->twoInterfAngle   = acos(LAL_C_SI * delay / distance);
		  cohEvent->ligo_angle = thisEvent->twoInterfAngle; 

		  /*XXX this needs to be computed correctly */
		  cohEvent->ligo_angle_sig = 1.0;
		}
