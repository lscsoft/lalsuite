...
This program generates the amplitude and phase of a post-Newtonian
inspiral waveform as functions of time.  The following option flags
are accepted:
\begin{itemize}
...
If the \verb@-w@ \emph{and} \verb@-o@ options are given, the amplitude
and phase are generated as above, but are then resampled at intervals
\verb@deltat@ to generate actual wave output.
...
main(int argc, char **argv)
{
  ...
  /* Fixed parameters. */
  params.ra = params.dec = params.psi = 0.0;
  params.lengthIn = 0;
  ...
  I8ToLIGOTimeGPS( &(phi.epoch), EPOCH );
  a.deltaT = phi.deltaT = dt;
  ...
  sprintf( message, "%d: %s", params.termCode,
	   params.termDescription );
  INFO( message );

  /* Print coalescence phase. */
  sprintf( message, "Waveform ends %.3f cycles before coalescence",
