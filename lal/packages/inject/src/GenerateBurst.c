...
)
{
	...
	for(; sim_burst; sim_burst = sim_burst->next) {
		...
		if(!strcmp(sim_burst->waveform, "BTLWNB")) {
			/* E_{GW} is in M_{sun}, so we multiply by (M_{sun}
			 * c^2) to convert to energy, which is then
			 * multiplied by (4 G / c^3) to convert to a value
			 * of \int \dot{h}^{2} \diff t. */
			double int_hdot_squared_dt = sim_burst->egw_over_rsquared * LAL_MSUN_SI * 4 * LAL_G_SI / LAL_C_SI;

			/* the waveform number is interpreted as the seed
			 * for GSL's Mersenne twister random number
			 * generator */
			...
			if(XLALGenerateBandAndTimeLimitedWhiteNoiseBurst(&injection_hplus, &injection_hcross, sim_burst->duration, sim_burst->frequency, sim_burst->bandwidth, int_hdot_squared_dt, series->deltaT, rng)) {
				gsl_rng_free(rng);
				XLAL_ERROR(func, XLAL_EFUNC);
			}
			gsl_rng_free(rng);
		} else if(!strcmp(sim_burst->waveform, "StringCusp")) {
			if(XLALGenerateStringCusp(&injection_hplus, &injection_hcross, sim_burst->amplitude, sim_burst->frequency, series->deltaT))
				XLAL_ERROR(func, XLAL_EFUNC);
		} else if(!strcmp(sim_burst->waveform, "SineGaussian")) {
			if(XLALSimBurstSineGaussian(&injection_hplus, &injection_hcross, sim_burst->q, sim_burst->frequency, sim_burst->hrss, sim_burst->pol_ellipse_e, sim_burst->pol_ellipse_angle, series->deltaT))
				XLAL_ERROR(func, XLAL_EFUNC);
		} else
