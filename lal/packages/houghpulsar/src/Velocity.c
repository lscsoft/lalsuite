...
given time. The output is of the form \verb@REAL8  v[3]@, and the input is a
time \verb@LIGOTimeGPS  *time@ and a structure  \verb@AvgVelPar  *in@ containing
inforamation about the detector and the ephemeris data.
...
{ /*-------------------------------------------------</lalVerbatim> */
  ...
  vTol = in->vTol;
  t0gps = in->startTime;
  avgVelParam.detector = in->detector;
  avgVelParam.edat = in->edat;
  ...
  TRY( LALDetectorVel( status->statusPtr, tempVel, &t0gps, &avgVelParam), status);
  for (n=0; n<3; n++) trapSum[n] = 0.5 * tempVel[n]; 

  /*calculate velocity at finish time */
  
  TRY( LALDetectorVel( status->statusPtr, tempVel, &tgps, &avgVelParam), status);
