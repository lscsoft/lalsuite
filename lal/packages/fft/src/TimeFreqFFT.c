 ...
 * $\Delta f$.
 * 
 * The routine \verb+LALREAL4AverageSpectrum()+ computes the average power
 * spectrum of the time series stored in the input structure \verb+tSeries+
 * and returns it in the output structure \verb+fSeries+. On entry the
 * parameter structure \verb+params+ must contain a valid \verb+REAL4Window+
 * generated by \verb+LALCreateREAL4Window()+, an integer that determines the
 * overlap as described below and a forward FFT plan for transforming data
 * of the specified window length into the time domain. The parameter
 * structure also detemines the method used to compte the average sprctrum.
 *
 * The routines breaks the input time series into smaller time series that are
 * the same length as the window function in the parameter structure. These
 * smaller time series are overlapped by a specified number of points and a
 * power spectrum comuted for each one. The average of these power spectra is
 * the computed and returned.
 *
 * If the length of the window is $N$, the overlap is $o$ and the length of
 * the input time series is $M$, the the time series is broken up into $i$
 * where
 * \begin{equation}
 * i = \frac{M-o}{N-o}.
 * \end{equation}
 * The power spectrum returned will be of length $N/2-1$ points and be the
 * average (mean or median) of the $i$ power spectra also of length $N/2+1$.
 * 
