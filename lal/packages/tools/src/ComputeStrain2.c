...
/*  } */
 for (p=0; p<(int)output->hR.data->length-(2*N_FIR_LP); p++){
   output->hR.data->data[p]=output->hR.data->data[p+(2*N_FIR_LP)];
 }

 /* Low pass filter twice to smooth time series */
/*  XLALFIRFilter(&uphR,&LPFIR);  */
/*  XLALFIRFilter(&uphR,&LPFIR);  */
 XLALFIRFilter(&(output->hR),&LPFIR); 
 XLALFIRFilter(&(output->hR),&LPFIR); 
/*  LALFFTFIRFilter(status->statusPtr,&uphR,&(LPFIR)); */
/*  CHECKSTATUSPTR( status ); */
/*  LALFFTFIRFilter(status->statusPtr,&uphR,&(LPFIR)); */
/*  CHECKSTATUSPTR( status ); */
...
/*  LALFFTFIRFilter(status->statusPtr,&uphR,input->Cinv); */
/*  CHECKSTATUSPTR( status ); */
