...
#include "ldasfircoefs.h"
    ...
    CHECKSTATUSPTR( status );
      ...
      params->filterType == GDSfirLSOne ||
      params->filterType == GDSfirLSTwo ||
      params->filterType == GDSfirLSThree ||
      params->filterType == GDSfirPMOne )
  {
#include "gdsfircoefs.h"
    REAL4 *filt_coeff;
    REAL8  mantissa;
    REAL4  sum;
    REAL4  delay;
    ...
    if ( params->filterType == GDSfirLSOne )
    {
      filt_coeff = gdsFIRls1;
      filt_length = 11;
    }
    else if ( params->filterType == GDSfirLSTwo )
    {
      filt_coeff = gdsFIRls2;
      filt_length = 6;
    }
    else if ( params->filterType == GDSfirLSThree )
    {
      filt_coeff = gdsFIRls3;
      filt_length = 21;
    }
    else if ( params->filterType == GDSfirPMOne )
    {
      filt_coeff = gdsFIRpm1;
      filt_length = 11;
    }
    else
    {
    ...
    if ((mantissa = frexp((double) resampleFactor, &ndec)) != 0.5)
    {
      ABORT( status, RESAMPLETIMESERIESH_ELOG2, RESAMPLETIMESERIESH_MSGELOG2 );
    }
    --ndec;			       /* this makes 2^(ndec) = dec_factor   */
    npt_y = (ts->data->length/resampleFactor);/* # pts in the output array   */
    ...
    /* use philips trick of changing the start time to cope with the delay */
    LALGPStoINT8( status->statusPtr, &startTimeNS, &(ts->epoch) );
    CHECKSTATUSPTR( status );
    startTimeNS -= (INT8) floor( ts->deltaT * delay * 1.0e9 + 0.5 );
    LALINT8toGPS( status->statusPtr, &(ts->epoch), &startTimeNS );
    CHECKSTATUSPTR( status );
  }
