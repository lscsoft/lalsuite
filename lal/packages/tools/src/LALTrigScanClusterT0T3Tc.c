/*-----------------------------------------------------------------------------
 *
 * File Name: LALTrigScanClusterT0T3Tc.c
 *
...
#if 0
<lalVerbatim file="LALTrigScanClusterT0T3TcCV">
Author: Sengupta, Anand. S. and Gupchup, Jayant A.
$Id$"
</lalVerbatim>
#endif
...
NRCSID (LALTRIGSCANCLUSTERT0T3TCC,
        "$Id$"
        );
        ...
        )
{
    ...
    INITSTATUS (status, "LALTrigScanClusterT0T3Tc.c", LALTRIGSCANCLUSTERT0T3TCC);
    ...
    nstepX = (REAL8 *) LALMalloc( n * sizeof(REAL8));
    nstepY = (REAL8 *) LALMalloc( n * sizeof(REAL8));
    nalpha = (REAL8 *) LALMalloc( n * sizeof(REAL8));

    /*-- Calculate the step size now --*/
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (masterList[i].clusterID == TRIGSCAN_UNCLASSIFIED)
        {
            ...
            list = (trigScanInputPoint *) LALMalloc (sizeof(trigScanInputPoint));
            ...
            if (!(XLALTrigSCanExpandCluster (list, masterList, expandClusterIn)))
            {
        ...
        )
{
    ...
    INITSTATUS (status, "LALTrigScanFindStepT0T3", LALTRIGSCANCLUSTERT0T3TCC);
    ATTATCHSTATUSPTR(status);
