...
#define DEFINE_VECTORMATH_SS2uU(NAME, AVX_PRED)                            \
  DEFINE_VECTORMATH_ANY( XLALVectorMath_SS2uU_GEN, NAME ## REAL4, ( UINT4* count, UINT4 *out, const REAL4 *in1, const REAL4 *in2, const UINT4 len ), ( (count != NULL) && (out != NULL) && (in1 != NULL) && (in2 != NULL) ), ( count, out, in1, in2, len, AVX_PRED ) )
...
#define DEFINE_VECTORMATH_sS2uU(NAME, AVX_PRED)                            \
  DEFINE_VECTORMATH_ANY( XLALVectorMath_sS2uU_GEN, NAME ## REAL4, ( UINT4* count, UINT4 *out, REAL4 scalar, const REAL4 *in, const UINT4 len ), ( (count != NULL) && (out != NULL) && (in != NULL) ), ( count, out, scalar, in, len, AVX_PRED ) )
