    ...
    def write_sub_file(self):
        ...
        if self.__universe == "grid":
            if self.__grid_type == "gt2":
                subfile.write(
                    "grid_resource = %s %s\n" % (self.__grid_type, self.__grid_server)
                )
            if self.__grid_type == "gt4":
                subfile.write(
                    "grid_resource = %s %s %s\n"
                    % (self.__grid_type, self.__grid_server, self.__grid_scheduler)
                )
        ...
        if (
            list(self.__options.keys())
            or list(self.__short_options.keys())
            or self.__arguments
        ):
...
class CondorDAGNode(object):
    ...
    def __init__(self, job):
        ...
        """
        if not isinstance(job, CondorDAGJob) and not isinstance(job, CondorDAGManJob):
    ...
    def write_job(self, fh):
        ...
        if isinstance(self.job(), CondorDAGManJob):
            ...
            fh.write(
                " ".join(["SUBDAG EXTERNAL", self.__name, self.__job.get_sub_file()])
            )
    ...
    def write_parents(self, fh):
        ...
        if len(self.__parents) > 0:
            fh.write(
                "PARENT "
                + " ".join((str(p) for p in self.__parents))
                + " CHILD "
                + str(self)
                + "\n"
            )
    ...
    def add_parent(self, node):
        ...
        if not isinstance(node, (CondorDAGNode, CondorDAGManNode)):
            raise CondorDAGNodeError(
                "Parent must be a CondorDAGNode or a CondorDAGManNode"
            )
    ...
    def write_script(self):
        ...
        for node in self.__nodes:
            ...
            if isinstance(node, CondorDAGManNode):
                ...
                outfile.write(
                    "%s %s\n\n" % (node.job().get_executable(), node.get_cmd_line())
                )
        outfile.close()
...
class AnalysisNode(object):
    ...
    def set_cache(self, filename):
        ...
        if isinstance(filename, str):
            ...
            for lfn in filename:
                ...
                t_end = int(c) + int(d)
                if t_start <= (
                    self.get_data_end() + self.get_pad_data() + int(d) + 1
                ) and t_end >= (
                    self.get_data_start() - self.get_pad_data() - int(d) - 1
                ):
    ...
    def calibration_cache_path(self):
        ...
        if self.__ifo and self.__start > 0:
            ...
            if (
                self.__LHO2k.match(self.__ifo)
                and (self.__start >= 729273613)
                and (self.__start <= 734367613)
            ):
                if self.__start < int(
...
class ScienceData(object):
    ...
    def make_chunks_from_unused(
        self, length, trig_overlap, play=0, min_length=0, sl=0, excl_play=0, pad_data=0
    ):
        """
        ...
        for seg in self:
            ...
            if start > ostop:
                ...
                if ostop >= 0:
                    ...
                    x = ScienceSegment(
                        tuple([id, ostart, ostop, ostop - ostart])
                    )  # noqa: F821
                    outlist.append(x)
    ...
    def play(self):
        ...
        for seg in self:
            ...
            play_start = begin_s2 + play_space * (
                1 + int((start - begin_s2 - play_len) / play_space)
            )
    ...
    def parse(self, type_regex=None):
        ...
        for line in f:
            ...
            if key in gwfDict[site][frameType]:
                msg = "The combination %s is not unique in the frame cache file" % str(
                    key
                )
...
class LSCDataFindJob(CondorDAGJob, AnalysisJob):
    ...
    def __init__(
        ...
        log_dir,
        config_file,
        lsync_cache_file=None,
        lsync_type_regex=None,
    ):
        """
        ...
        if config_file.has_option("condor", "accounting_group"):
            self.add_condor_cmd(
                "accounting_group", config_file.get("condor", "accounting_group")
            )
        if lsync_cache_file:
...
class LigolwAddJob(CondorDAGJob, AnalysisJob):
    ...
    def __init__(self, log_dir, cp):
        ...
        self.set_stdout_file(
            os.path.join(log_dir, "ligolw_add-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(log_dir, "ligolw_add-$(cluster)-$(process).err")
        )
...
class LigolwCutJob(CondorDAGJob, AnalysisJob):
    ...
    def __init__(self, log_dir, cp):
        ...
        self.set_stdout_file(
            os.path.join(log_dir, "ligolw_cut-$(cluster)-$(process).out")
        )
        self.set_stderr_file(
            os.path.join(log_dir, "ligolw_cut-$(cluster)-$(process).err")
        )
