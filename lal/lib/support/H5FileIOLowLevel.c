...
int XLALH5AttributeQueryStringValue(char UNUSED *value, size_t UNUSED size, const LALH5Generic UNUSED object, const char UNUSED *key)
{
	...
	if (dtype_id < 0) {
        ...
        H5T_cset_t dtype_cset = threadsafe_H5Tget_cset(dtype_id); /* threadsafe not needed as dtype_id local to this function */
	threadsafe_H5Tclose(dtype_id);
	...
	if (memtype_id < 0 || threadsafe_H5Tset_size(memtype_id, H5T_VARIABLE) || threadsafe_H5Tset_cset(memtype_id, dtype_cset)) {
		threadsafe_H5Sclose(space_id);
		threadsafe_H5Aclose(attr_id);
		XLAL_ERROR(XLAL_EIO);
	}
