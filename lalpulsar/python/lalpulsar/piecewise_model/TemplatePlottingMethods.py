...
def metric_ellipse(g11, mutilde, tol=0.01):
	...
	x = np.cos(zz[-1])
	y = np.sin(zz[-1])
	rr = [np.sqrt(mutilde / (g11[0, 0]*x**2 + 2*g11[0, 1]*x*y + g11[1, 1]*y**2))]
	...
	while zz[-1] < 2*np.pi:
		...
		x = np.cos(z)
		y = np.sin(z)
		r = np.sqrt(mutilde / (g11[0, 0]*x**2 + 2*g11[0, 1]*x*y + g11[1, 1]*y**2))
		e = abs(r - rr[-1]) / rr[-1]
...
# Plots all the ellipses in the given parameter space for 2 and 3 dimensions
def plottemplatesspecifyaxis(temps, x_cross_section, dim1, dim2, metric, max_mismatch, axis, show=False):
...
# Plots all the ellipses in the given parameter space for 2 and 3 dimensions
def plottemplatesalldims(temps, x_cross_section, segment, metric, max_mismatch, show=True):
	...
	axislist = [ax00_01, ax00_02, ax01_02, ax00_10, ax01_10, ax02_10, ax00_11, ax01_11, ax02_11, ax10_11, ax00_12, ax01_12, ax02_12, ax10_12, ax11_12]
	dimslist = [ [0, 1],  [0, 2],  [1, 2],  [0, 3],  [1, 3],  [2, 3],  [0, 4],  [1, 4],  [2, 4],  [3, 4],  [0, 5],  [1, 5],  [2, 5],  [3, 5],  [4, 5]]
	...
	for i, axis in enumerate(axislist):
		...
		plottemplatesspecifyaxis(temps, x_cross_section, segment * 3 + dimslist[i][0], segment * 3 + dimslist[i][1], metric, max_mismatch, axis, show=False)
