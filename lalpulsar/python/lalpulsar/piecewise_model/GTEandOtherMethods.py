...
def gtederivs(t, f0, ngte, kgte, deriv):
    if deriv == 0:
        ...
        return (
            -kgte
            * (f0**ngte)
            * (1 + (ngte - 1) * kgte * (f0 ** (ngte - 1)) * t) ** (1 / (1 - ngte) - 1)
        )
    elif deriv == 2:
        return (
            ngte
            * (kgte**2)
            * (f0 ** (2 * ngte - 1))
            * (1 + (ngte - 1) * kgte * (f0 ** (ngte - 1)) * t) ** (1 / (1 - ngte) - 2)
        )
    ...
    # The below expression may not correct
        ...
        return (
            prefactor
            * factorialfactor
            * (1 + (ngte - 1) * f0 ** (ngte - 1) * kgte * t) ** (1 / (ngte - 1) - deriv)
        )
...
def knotmatrixdash(p0orig, p1orig):
    ...
    kmd = [
        [
            -10 * p0**2 * p1**3 + 5 * p0 * p1**4 - p1**5,
            4 * p0**2 * p1**3 - p0 * p1**4,
            -(p0**2 * p1**3) / 2.0,
            p0**5 - 5 * p0**4 * p1 + 10 * p0**3 * p1**2,
            -(p0**4 * p1) + 4 * p0**3 * p1**2,
            (p0**3 * p1**2) / 2.0,
        ],
        [
            30 * p0**2 * p1**2,
            -12 * p0**2 * p1**2 - 4 * p0 * p1**3 + p1**4,
            (3 * p0**2 * p1**2) / 2.0 + p0 * p1**3,
            -30 * p0**2 * p1**2,
            p0**4 - 4 * p0**3 * p1 - 12 * p0**2 * p1**2,
            -(p0**3 * p1) - (3 * p0**2 * p1**2) / 2.0,
        ],
        [
            -60 * p0**2 * p1 - 60 * p0 * p1**2,
            24 * p0**2 * p1 + 36 * p0 * p1**2,
            -3 * p0**2 * p1 - 6 * p0 * p1**2 - p1**3,
            60 * p0**2 * p1 + 60 * p0 * p1**2,
            36 * p0**2 * p1 + 24 * p0 * p1**2,
            p0**3 + 6 * p0**2 * p1 + 3 * p0 * p1**2,
        ],
        [
            60 * p0**2 + 240 * p0 * p1 + 60 * p1**2,
            -24 * p0**2 - 120 * p0 * p1 - 36 * p1**2,
            3 * p0**2 + 18 * p0 * p1 + 9 * p1**2,
            -60 * p0**2 - 240 * p0 * p1 - 60 * p1**2,
            -36 * p0**2 - 120 * p0 * p1 - 24 * p1**2,
            -9 * p0**2 - 18 * p0 * p1 - 3 * p1**2,
        ],
        [
            -360 * p0 - 360 * p1,
            168 * p0 + 192 * p1,
...
def TExpParamsToTExpRefParams(texpparams, tref):
    refmatinv = [
        ...
        [0, 1, tref, (tref**2) / 2.0, (tref**3) / 6.0, (tref**4) / 24.0],
        [0, 0, 1, tref, (tref**2) / 2.0, (tref**3) / 6.0],
        [0, 0, 0, 1, tref, (tref**2) / 2.0],
        [0, 0, 0, 0, 1, tref],
        [0, 0, 0, 0, 0, 1],
    ]
    ...
    elems = [
        ...
        ** 0.25
        / abs(
            2
            + abs(dref) ** 2
            + abs(((-8640 * dref) / dt**9 + 8640 / dt**8) * dt**9) ** 2
            / 7.46496e7
        )
        ** 0.25,
        abs(
        ...
        ** 0.25,
        abs(
            abs((-360 * dref**2) / dt**5 + (360 * dref) / dt**4 - 60 / dt**3)
            ** 2
            + abs((360 * dref**2) / dt**5 - (360 * dref) / dt**4 + 60 / dt**3)
            ** 2
            + abs((-180 * dref**2) / dt**4 + (192 * dref) / dt**3 - 36 / dt**2)
            ** 2
            + abs((-180 * dref**2) / dt**4 + (168 * dref) / dt**3 - 24 / dt**2)
            ** 2
            + abs((-30 * dref**2) / dt**3 + (36 * dref) / dt**2 - 9 / dt) ** 2
            + abs((30 * dref**2) / dt**3 - (24 * dref) / dt**2 + 3 / dt) ** 2
        )
        ** 0.25
        / abs(
            abs(dref) ** 2
        ...
        ** 0.25,
        abs(
            abs((720 * dref) / dt**5 - 360 / dt**4) ** 2
            + abs((-720 * dref) / dt**5 + 360 / dt**4) ** 2
            + abs((-360 * dref) / dt**4 + 168 / dt**3) ** 2
            + abs((-360 * dref) / dt**4 + 192 / dt**3) ** 2
            + abs((60 * dref) / dt**3 - 24 / dt**2) ** 2
            + abs((-60 * dref) / dt**3 + 36 / dt**2) ** 2
        )
        ** 0.25
        / abs(
            abs(dref) ** 4 / 4.0
        ...
        ** 0.25,
        abs(1036800 / abs(dt) ** 10 + 259200 / abs(dt) ** 8 + 7200 / abs(dt) ** 6)
        ** 0.25
        / abs(
            abs(dref) ** 6 / 36.0
            + abs(dref) ** 8 / 576.0
...
def PlotPWModel(pwparams, show=True, label="", linewidth=2):
    ...
    for t in times:
        ...
        for spindown in range(s):
            value += pwparams[segment * s + spindown] * bf.basisfunctionvalue(
                t, segment, 0, spindown, basiscoeffs
            ) + pwparams[(segment + 1) * s + spindown] * bf.basisfunctionvalue(
                t, segment, 1, spindown, basiscoeffs
            )
