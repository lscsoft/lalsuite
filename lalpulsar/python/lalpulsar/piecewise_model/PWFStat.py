...
def fstatinput(SFTfmin, SFTfmax, sft_catalog, finputdata, inject_params, timeint=[-1, -2], trefsegfrac=0., sig_params=None):
        ...
        with open(filename, "a+") as reader:
                ...
                for elem in fstat_res_heap:
                        ...
                        for i in range(numdets):
                                two_F_on_segs = [fstat_res.twoFPerDet(i)[0] for fstat_res in fstat_res_array]
...
def semifstatcatalogue(SFTfmin, SFTfmax, tbank, finputdata, signalparams, filename, out_directory=".", SFTdirectory=".", trefsegfrac=0., rtnsum=False, SFTFiles=False, tempsperfile=1000, step_size=0.01, Fstat_mismatch=False, fstat_hist=False, mode="search"):
        ...
        for seg in range(segs):
                ...
                inject_params   = gom.PWParamstoTrefParams(signalparamsseg, p0 - tstart, p1 - tstart, tref - tstart, s)

                transformmatrices.append(gom.ParamTransformationMatrix(p0, p1, tref, s))
                
                if SFTFiles or finputdata.inject_data:
                        finput = fstatinputfromSFTFiles(SFTfmin, SFTfmax, finputdata, timeint=[p0, p1], sft_path=SFTdirectory, detectors=finputdata.detectors.data, inject_params=inject_params)
                else:
                        sft_catalog = SFTCatalog(p0, p1 - p0, finputdata)
                        finput = fstatinput(SFTfmin, SFTfmax, sft_catalog, finputdata, inject_params, timeint=[p0, p1], trefsegfrac=trefsegfrac)
                print()
        ...
        fstat_res_heap_to_file(out_directory + "/" + filename, sig_fstat_heap, fstats_first=True)
        ...
        fstat_res_heap_to_file(out_directory + "/" + mismatchfilename, sig_fstat_heap, fstats_first=False)
        ...
        for counter in trange(tempcount, file=tqdm_file):
                ...
                if counter < tempsperfile:
                        ...
                        hq.heappush(mismatch_fstat_res_heap, (-this_mismatch, twoF, counter, thistemp, fstat))
                else:
                        hq.heappushpop(         fstat_res_heap, (twoF, this_mismatch, counter, thistemp, fstat))
                        hq.heappushpop(mismatch_fstat_res_heap, (-this_mismatch, twoF, counter, thistemp, fstat))
        ...
        fstat_res_heap_to_file(out_directory + "/" +         filename,          fstat_res_heap, fstats_first=True)
        fstat_res_heap_to_file(out_directory + "/" + mismatchfilename, mismatch_fstat_res_heap, fstats_first=False)
...
def pw_fstat_search_catalogue(SFTfmin, SFTfmax, tbank, finputdata, filename, tbankdirectory=".", SFTdirectory=".", trefsegfrac=0., rtnsum=False, tempsperfile=1000):
        ...
        for seg in range(segs):
                ...
                finput = fstatinputfromSFTFiles(SFTfmin, SFTfmax, finputdata, timeint=[p0, p1], sft_path=sft_path, detectors=tbank.detectors)
