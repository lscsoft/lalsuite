...
class TBank:
        ...
        def SetTBankParams(self, args):
                ...
                if args.s                        : self.s             = args.s
                if args.fmin                     : self.fmin          = args.fmin
                if args.fmax                     : self.fmax          = args.fmax
                if args.nmin                     : self.nmin          = args.nmin
                if args.nmax                     : self.nmax          = args.nmax
                if args.Izz                      : self.Izz           = args.Izz
                if args.ellip                    : self.ellip         = args.ellip
                if args.radius                   : self.radius        = args.radius
                
                self.kmin          = args.kmin if args.kmin is not None else gom.kforGWsource(Izz=self.Izz, ellip=self.ellip, radius=self.radius) * 0.1
                self.kmax          = args.kmax if args.kmax is not None else gom.kforGWsource(Izz=self.Izz, ellip=self.ellip, radius=self.radius)
                
                if args.dur                      : self.dur           = args.dur
                if args.maxmismatch              : self.maxmismatch   = args.maxmismatch
                if args.knotnum                  : self.knotnum       = args.knotnum
                
                if args.flags_bbox    is not   []: self.flags_bbox    = args.flags_bbox
                if args.flags_intbox  is not   []: self.flags_intbox  = args.flags_intbox
                
                if args.tstart                   : self.tstart        = args.tstart
                
                if args.cosi                     : self.cosi          = args.cosi
                if args.psi                      : self.psi           = args.psi
                if args.phi0                     : self.phi0          = args.phi0
                if args.Alpha                    : self.Alpha         = args.Alpha
                if args.Delta                    : self.Delta         = args.Delta
                
                if not (args.detectors == [])    : self.detectors     = args.detectors
                
                if args.noise_path               : self.pc_noise_path     = args.noise_path
                if args.noise_path               : self.ozstar_noise_path = args.noise_path
                
                if args.sft_path                 : self.pc_sft_path     = args.sft_path
                if args.sft_path                 : self.ozstar_sft_path = args.sft_path
        ...
        def SetDefaultGW170817(self):
                ...
                self.kmin          = gom.kforGWsource(Izz=self.Izz, ellip=self.ellip, radius=self.radius) * 0.1
                ...
                self.ozstar_noise_path = "/fred/oz162/bgrace/post-merger-signal/manuscript/data_release/"
        
                self.pc_sft_path       = "/home/ben/PhD/SFT_Data_16kHz_Frame/GW170817_SFTs/"
                self.ozstar_sft_path   = "/fred/oz162/data/GW170817_SFTs/"
                
                self.pc_psd_path       = "/home/ben/PhD/post-merger-signal/manuscript/data_release/GW170817_PSDs/"
        ...
        def SetDefaultGW190425(self):
                ...
                self.pc_psd_path       = "/home/ben/PhD/post-merger-signal/manuscript/data_release/GW190425_PSDs/"
        ...
        def toStringFull(self):
                ...
                skip_variables = ['knots', 'flags_bbox', 'flags_intbox', 'pc_noise_path', 'ozstar_noise_path', 'pc_sft_path', 'ozstar_noise_path']
        ...
        def toString(self):
                ...
                skip_variables = ['name', 'knots', 'flags_bbox', 'flags_intbox', 'pc_noise_path', 'ozstar_noise_path', 'pc_sft_path', 'ozstar_noise_path']
