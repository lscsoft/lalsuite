...
def buildSFTs(Tdata, tbank, finputdata, trefsegfrac=0., parentdirectory=".", rand_seed=0):
        ...
        for i, detector in enumerate(finputdata.detectors.data):
                ...
                S = simulateCW.CWSimulator(tref, bf.knotslist[0], Tdata, wf, finputdata.dt_wf, finputdata.phi0, finputdata.psi, finputdata.Alpha, finputdata.Delta, detector, tref_at_det=True)
                ...
                path_to_detector_sfts = os.path.join(parentdirectory, f"SFTs_h0-{finputdata.h0:.2e}_f0-{f0:.3f}_f1-{f1:.2e}_f2-{f2:.2e}_dur-{tbank.dur}_tstart-{finputdata.tstart}/")
                ...
                for file, i, N in S.write_sft_files(fmax=tbank.fmax + 20, Tsft=finputdata.Tsft, noise_sqrt_Sh=finputdata.noise_sqrt_Sh[i], comment="PWModPhase", out_dir=directoryname):
