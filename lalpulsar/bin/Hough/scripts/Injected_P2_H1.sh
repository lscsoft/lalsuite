...
./DriveHoughMulti -d 0 --f0=$freq --fSearchBand=$freqband --timeStampsFile=./ts-inject-S4_H1.txt  --skyfile=./skypulsar.2 --weighAM=1 --weighNoise=1 --printMaps=1 --printStats=1 --earthEphemeris=./earth05-09.dat --sunEphemeris=./sun05-09.dat --sftDir=$sftdir --dirnameOut=${tempoutdir}/H1.P2 --fbasenameOut=freq_${freq}_  --linefiles=./S4lines_H1_xavi.txt.v2,./S4lines_L1_xavi.txt.v2,./S4lines_H2.txt --printLog

echo finished running driver

# now copy the results to the home directory
mkdir -p ${workdir}/H1.P2
rsync -a ${tempoutdir}/H1.P2 $workdir
