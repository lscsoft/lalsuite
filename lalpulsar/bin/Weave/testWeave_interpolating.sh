...
set -x
lalpulsar_WeaveSetup --ephem-earth=earth00-40-DE405.dat.gz --ephem-sun=sun00-40-DE405.dat.gz --first-segment=1122332211/90000 --segment-count=3 --segment-gap=11130000 --detectors=H1,L1 --output-file=WeaveSetup.fits
...
lalpulsar_Weave --output-file=WeaveOut.fits \
    ...
    --segment-info --time-search --setup-file=WeaveSetup.fits --sft-files='*.sft' \
...
env LAL_DEBUG_LEVEL="${LAL_DEBUG_LEVEL},info" lalpulsar_WeaveCompare --setup-file=WeaveSetup.fits --result-file-1=WeaveOut.fits --result-file-2=RefWeaveOut.fits \
    --round-param-to-dp=9 --round-param-to-sf=3 --unmatched-item-tol=0.5
set +x
echo
