...
parser.add_argument("--j",              type=int,       help="The job array number on OzSTAR, to ensure unique random seeds, j < 10,000", default=0)
...
parser.add_argument("--noise_sqrt_Sh",  type=float,     help="Noise level to be added to data", nargs='+', default=[]) 
...
parser.add_argument("--UseLocal",                       help="If selected all files will be written to local storage and deleted once job finished except return values of pwf.semifstatcatalogue", action='store_true')
parser.add_argument("--BaseSeed",       type=int,       help="BaseSeed * 10e7 + job number * 10e3 + iterations is the seed for generating random signal params", default=0)

# Parameters to be used when creating the necessary elements to use lp.ComputeFStat
parser.add_argument("--Tsft",           type=int,       help="The length of each SFT to be built",                       default=10)
parser.add_argument("--h0",             type=float,     help="Strain of the injected signal",                            default=0)
parser.add_argument("--cosi",           type=float,     help="Default injection cos-iota",                               default=None)
...
parser.add_argument("--Alpha",          type=float,     help="Right Ascension to sky position of source",                default=None)
parser.add_argument("--Delta",          type=float,     help="Declination to sky position of source",                    default=None)
parser.add_argument("--detectors",      type=str,       help="Set detector being used. Options H1, V1, etc.", nargs='+', default=[])
...
parser.add_argument("--Izz",            type=float,     help="Inertia of the NS. Used to calculate default k values",                   nargs='?', const=1, default=None)
parser.add_argument("--ellip",          type=float,     help="Ellipticity of NS. Used to calculate default k values",                   nargs='?', const=1, default=None)
parser.add_argument("--radius",         type=float,     help="Radius of the  NS. Used to calculate default k values",                   nargs='?', const=1, default=None)
parser.add_argument("--kmin",           type=float,     help="Set the kmin parameter. Overrides Izz, ellip and radius values",          nargs='?', const=1, default=None)
parser.add_argument("--kmax",           type=float,     help="Set the kmax parameter. Overrides Izz, ellip and radius values",          nargs='?', const=1, default=None)
...
parser.add_argument("--knotnum",        type=int,       help="Use a knotnum number of knots generated by algorithm",                    nargs='?', const=1, default=None)
parser.add_argument("--knot_alg_dur",   type=float,     help="Uses knots generated by the algorithm extending to this duration",        nargs='?', const=1, default=None)
parser.add_argument("--flags_bbox",     type=float,     help="Fractional bounding box padding to use on each dimension",                nargs='+',          default=[])
parser.add_argument("--flags_intbox",   type=int,       help="Integer points to use for padding on each dimension",                     nargs='+',          default=[])

# Different modes for the code to run
parser.add_argument("--det_prob",                       help="If selected, does an iterations number of searches, with signal h0 varying from min_h0 to max_h0. Also includes h0 = 0 for computing 2F* simultaneously",             action='store_true')
parser.add_argument("--threshold_2F",                   help="Does an iterations number of searches with no injected signal. Useful for calculating 2F* without calculating det_prob",                    action='store_true')
parser.add_argument("--min_h0",         type=float,     help="Minimum h0 for calculating detection probabilities, det_prob must be true",                                    default=1e-30)
parser.add_argument("--max_h0",         type=float,     help="Maximum h0 for calculating detection probabilities, det_prob must be true",                                    default=1e-10)
parser.add_argument("--template_count",                 help="Returns the template count for the parameter space. Does not compute anything else",                           action='store_true')
parser.add_argument("--mis_hist",                       help="If selected with Fstat_mismatch, only returns the lowest mis_match",                                           action='store_true')
parser.add_argument("--fake_data",                      help="If selected, carries out a search on fake data with an injected signal",                                       action='store_true')
parser.add_argument("--inject_data",                    help="If selected, injects a randomly generated signal into the SFTs located at the path set by . Can be selected alongside other modes", action='store_true')

# Other options available
parser.add_argument("--Fstat_mismatch",                 help="If selected, determines mismatches using the 2F definition, otherwise uses the metric",       action='store_true')
parser.add_argument("--signal_as_template",             help="If selected, the random injected signal will coincide with a template",                       action='store_true')
parser.add_argument("--iterations",     type=int,       help="Number of loops for the code to perform. For unique random seeds, iterations < 1000",         default=1)
parser.add_argument("--freq_bands",     type=float,     help="Defines frequency bands for search to cover. Frequency bands chosen as fmin=freq_bands[j], fmax=freq_bands[j+1]", nargs='+', default=[])
parser.add_argument("--fstat_hist",                     help="If selected, returns data on the distribution of 2Fs as a histogram",                         action='store_true')
parser.add_argument("--data_psd",                       help="Use simulated data with a noise level equal to the PSD of data",                              action='store_true')
...
parser.add_argument("--noise_path",     type=str,       help="Path to noise data. Default option harded coded in tbank object", default=None)
parser.add_argument("--sft_path",       type=str,       help="Path to sfts. Default option hard coded in tbank object",         default=None)
...
parser.add_argument("--machine",        type=str,       help="Specify the machine you are working on to find the path to noise curves", default='OzSTAR')
...
finputdata.noise_sqrt_Sh = args.noise_sqrt_Sh if args.noise_sqrt_Sh != [] else noise_curve_data
...
if UseLocal:
        ...
        tbankdirectory = os.path.join(local_disk_directory, tbank.toString()) + append_string
else:
...
for i in range(iterations + plus_one):
        ...
        mismatchtempname = "Mismatch_Temps_For_" + tbank.toString() + iteration_string + sumstr + ".txt"
        ...
        if SFTFiles:
                signalparams, SFTdirectory = pwsim.buildSFTs(tbank.dur, tbank, finputdata, trefsegfrac=trefsegfrac, parentdirectory=tbankdirectory, rand_seed=this_seed)
                ...
                if args.signal_as_template:
                        ...
                        lp.NearestLatticeTilingPoint(locator, signal_params_vec, nearest_point, nearest_index)
                ...
                SFTdirectory = os.path.join(tbankdirectory, f"SFTs_h0-{finputdata.h0:.2e}_f0-{f0:.3f}_f1-{f1:.2e}_f2-{f2:.2e}_dur-{tbank.dur}_tstart-{tbank.tstart}/")
        ...
        if mode == "search" and not args.fstat_hist:
            lowest_mismatch_metric, lowest_mismatch_Fstat, fstat_max, template_count = pwf.pw_fstat_search_catalogue(SFTfmin, SFTfmax, tbank, finputdata, tempsfilename, tbankdirectory=tbankdirectory, SFTdirectory=SFTdirectory, trefsegfrac=trefsegfrac, rtnsum=rtnsum, tempsperfile=tempsperfile)
        else:
            lowest_mismatch_metric, lowest_mismatch_Fstat, fstat_max, template_count = pwf.semifstatcatalogue(SFTfmin, SFTfmax, tbank, finputdata, signalparams, tempsfilename, out_directory=tbankdirectory, SFTdirectory=SFTdirectory, trefsegfrac=trefsegfrac, rtnsum=rtnsum, SFTFiles=SFTFiles, tempsperfile=tempsperfile, Fstat_mismatch=args.Fstat_mismatch, fstat_hist=args.fstat_hist, mode=mode)
        ...
        if mode != "search":
        	...
        	if lowest_mismatch_metric != 0:
        	    ...
        	    with open(mismatch_directory + "/" + mismatch_file, "a+") as reader:
        	        line = str(lowest_mismatch_metric) + "        " + str(lowest_mismatch_Fstat) + "\n"
        	        reader.write(line)
