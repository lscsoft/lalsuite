...
// NOTE: this must be kept in sync with src/SuperskyMetrics.c
...
const double ssky_data_align_sky_ref[3][3] = {
...
const double ssky_data_sky_offsets_ref[2][3] = {
...
int main( void )
{
  ...
  gsl_matrix *rssky_metric = NULL, *ussky_metric = NULL;
  SuperskyTransformData *ssky_data = NULL;
  XLAL_CHECK_MAIN( XLALComputeSuperskyMetrics( &rssky_metric, &ussky_metric, &ssky_data, 1, &ref_time, &segments, 100.0, &detectors, NULL, DETMOTION_SPIN | DETMOTION_PTOLEORBIT, edat ) == XLAL_SUCCESS, XLAL_EFUNC );
  ...
  }
  {
    ...
    for( size_t i = 0; i < 3; ++i ) {
      for( size_t j = 0; j < 3; ++j ) {
        const double align_sky_ij = ssky_data->align_sky[i][j];
        const double align_sky_ref_ij = ssky_data_align_sky_ref[i][j];
        const double err_ij = fabs( ( align_sky_ij - align_sky_ref_ij ) / align_sky_ref_ij );
        if( err_ij > max_err ) {
          max_err = err_ij;
        }
      }
    ...
    for( size_t i = 0; i < 2; ++i ) {
      for( size_t j = 0; j < 3; ++j ) {
        const double sky_offsets_ij = ssky_data->sky_offsets[i][j];
        const double sky_offsets_ref_ij = ssky_data_sky_offsets_ref[i][j];
        const double err_ij = fabs( ( sky_offsets_ij - sky_offsets_ref_ij ) / sky_offsets_ref_ij );
        if( err_ij > max_err ) {
          max_err = err_ij;
        }
      }
    ...
    XLAL_CHECK_MAIN( max_err <= err_tol, XLAL_ETOL, "'ssky_data' check failed: max(err) = %0.3e > %0.3e = err_tol", max_err, err_tol );
  }
  ...
  // Check round-trip conversions of each test point
  {
    ...
    for( size_t i = 0; i < NUM_POINTS; ++i ) {
      ...
      XLAL_CHECK_MAIN( XLALConvertPhysicalToSupersky( SC_USSKY, ussky_point, &phys_points[i], ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
      XLAL_CHECK_MAIN( XLALConvertSuperskyToPhysical( &point, SC_USSKY, ussky_point, ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
  ...
  }
  {
    ...
    for( size_t i = 0; i < NUM_POINTS; ++i ) {
      PulsarDopplerParams XLAL_INIT_DECL(point);
      XLAL_CHECK_MAIN( XLALConvertPhysicalToSupersky( SC_RSSKY, rssky_point, &phys_points[i], ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
      XLAL_CHECK_MAIN( XLALConvertSuperskyToPhysical( &point, SC_RSSKY, rssky_point, ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
      XLAL_CHECK_MAIN( CompareDoppler( &phys_points[i], &point ) == EXIT_SUCCESS, XLAL_EFUNC );
    }
    GFVEC( rssky_point );
  }
  ...
  // Check mismatches between pairs of points
  {
    ...
    for( size_t i = 0; i < NUM_POINTS; ++i ) {
      XLAL_CHECK_MAIN( XLALConvertPhysicalToSupersky( SC_USSKY, ussky_point_i, &phys_points[i], ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
      for( size_t j = 0; j < NUM_POINTS; ++j ) {
        XLAL_CHECK_MAIN( XLALConvertPhysicalToSupersky( SC_USSKY, ussky_point_j, &phys_points[j], ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
  ...
  }
  {
    ...
    for( size_t i = 0; i < NUM_POINTS; ++i ) {
      XLAL_CHECK_MAIN( XLALConvertPhysicalToSupersky( SC_RSSKY, rssky_point_i, &phys_points[i], ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
      for( size_t j = 0; j < NUM_POINTS; ++j ) {
        XLAL_CHECK_MAIN( XLALConvertPhysicalToSupersky( SC_RSSKY, rssky_point_j, &phys_points[j], ssky_data ) == XLAL_SUCCESS, XLAL_EFUNC );
  ...
  // Cleanup
  GFMAT( rssky_metric, ussky_metric );
  XLALFree( ssky_data );
  LALCheckMemoryLeaks();
