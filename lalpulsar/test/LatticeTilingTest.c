  ...
  )
{
  ...
  for( size_t i = 0; i < 3; ++i ) {
    ...
    XLAL_CHECK( XLALSetLatticeTilingConstantBound( space, i, fndot[i], fndot[i] + fndotband[i] ) == XLAL_SUCCESS, XLAL_EFUNC );
  }
  ...
  )
{
  ...
  XLAL_CHECK( XLALSetLatticeTilingReducedSuperSkyBounds( space ) == XLAL_SUCCESS, XLAL_EFUNC );
  XLAL_CHECK( XLALSetLatticeTilingPhysicalSpinBound( space, rssky_transf, 0, freq, freq + freqband ) == XLAL_SUCCESS, XLAL_EFUNC );
  GFMAT( rssky_transf );

  // Perform mismatch test
  XLAL_CHECK( MismatchTest( space, rssky_metric, max_mismatch, lattice, total_ref, mism_hist_ref ) == XLAL_SUCCESS, XLAL_EFUNC );
